...
# `wrangler.request.production.toml` by `pnpm cf:render:production`, which
# substitutes `${...}` placeholders with outputs from the
# `fog-cf-resources/production` stack.
#
# Source of truth: this `.tpl` file (committed) + Pulumi state.
# The rendered `wrangler.request.production.toml` is git-ignored — do not edit it
...
#   3. `wrangler secret put <NAME> --config wrangler.request.production.toml`
#      (request side: SESSION_SECRET / AI_CLIENT_TOKEN_SECRET /
#       DIRECTORY_ROUTING_SECRET — see `apps/web/.dev.vars.example`)
#   4. `pnpm deploy:production` (state Worker first, then this one — the DO
