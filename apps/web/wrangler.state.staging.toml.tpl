# Staging deploy config TEMPLATE for the **state Worker** (the Durable Object
# host) — rendered to `wrangler.state.staging.toml` by `pnpm cf:render:staging`.
#
# Source of truth: this `.tpl` file (committed) + Pulumi state.
# The rendered `wrangler.state.staging.toml` is git-ignored — do not edit it
...
#   3. `wrangler secret put <NAME> --config wrangler.state.staging.toml`
...
#       be set here)
#   4. `pnpm deploy:state:staging` — this Worker deploys **before** the request
#      Worker, whose DO bindings name this script
# =========================================================================
