...
describe("guardStreamedRender", () => {
  ...
  // the boundary unclassified": sharing `toClientError` is no reason to believe
  // the guard redacts and logs (see the brand-stripped cases below), so the
  // unclassified path needs its own pin here too.
  ...
  // pin here for the same reason: the middleware's BRAND_STRIPPED_CASES
  // exercise the `system` row for the middleware alone, and a change that made
  // this guard read the remnant some other way would leave it untested.
