...
import { z } from "zod";
import { renderErrorMessage } from "../errorDisplay";
import {
...
describe("validateInput", () => {
  ...
  // A top-level type mismatch has an empty `issue.path`, which the
  // dotted-path join flattens to the `""` key — the marker
  // `errorDisplay.formatFieldErrors` substitutes its generic wording for,
  // since the message under it is zod's raw English default rather than a
  // schema-authored one. Both halves of that contract are held here: the
  // flatten keeps producing `""`, and rendering it never shows the raw
  // message.
  it("files a top-level type mismatch under the empty-string key and renders it generically", () => {
    const serialized = extractSerializedError(
      captureFrom(null),
    ) as SerializedValidationError;

    expect(Object.keys(serialized.fieldErrors ?? {})).toEqual([""]);
    const rawMessage = serialized.fieldErrors?.[""]?.[0];
    expect(rawMessage).toBeDefined();

    const rendered = renderErrorMessage(serialized);
    expect(rendered).toBe("入力内容が正しくありません");
    expect(rendered).not.toContain(rawMessage as string);
  });
