...
本 Issue の成果物はドキュメントのみだが、書くべきドキュメントの内容そのものが設計判断の集合である。以下はその判断の記録であり、AD-1〜AD-62 のうちプロジェクト全体に効くものが `.adr/013` へ昇格する。**設計判断の記録先は本ファイルだけである** — plan.md / steps.md は結論を参照するだけで、判断の根拠を重複させない。
...
Proposed（**レビュー7周目・impl B-001 への決定**）

### Context

AD-58 が置いた正本（`spec/database/index.md`「Alarm の多重化」の張り直しの項）は、射程を「`enqueueJob` / `enqueueEvent` を含むトランザクションと `requeue-quarantined-event` の**3経路が全数**」という閉じた列挙で宣言した。ところが同じファイルは、`reindex` / `migrate-bulk` を「**ユースケースからは投入せず、migration ゲート（アダプター）が投入する**」と明示している。ゲートは `alarm()` だけでなく**全 RPC エントリの先頭**でも走るので、RPC 経由の投入は3経路のどれにも当たらない。

具体的な破れ: ゴミ箱が空で保留ジョブを持たない User Data DO は実行可能集合が空なので定義上 `deleteAlarm()` 済みである（大半の利用者の定常状態）。tokenizer / 正規化規則を変える migration を配備したあと、その利用者の最初のリクエストでゲートが `reindex` を `pending`（`next_run_at = now`）で1行書いて終わる。**Alarm は張られないので、次に別のジョブが投入されるまで `reindex` は一度も走らず、検索インデックスが旧規則のまま残る。** これは AD-58 自身が塞ごうとした停止そのものである。

さらに `rotate-encryption`（operator 専用 maintenance 経路からの起動）も `enqueueJob` を通るとはどこにも書かれていない。**「取りこぼしが1件」ではなく「閉じた列挙という形そのものが取りこぼしを生む」**のが問題の本体である — `finalize-withdrawal` のように投入点の一部が別 Issue（#45 / #51）に委ねられている `kind` もあり、今日書ける経路の列挙はその時点で既に不完全でありうる。

**加えて位置の問題がある** — `setAlarm()` は非同期 API であり、ゲートは「`schema_version` の読み取りから全 DDL ステップの適用まで `await` を1つも挟まない」同期関数である。ゲートの中では呼べないので、「そのトランザクションのあと」がどこを指すのかを決めないと `enqueueJob` / `enqueueEvent` と同じ形にならない。

### Decision

**(1) 全数宣言の形を変える。** 「実行可能な行を増やした主体は、**例外なく**、その書き込みのあとに4本の min の合成で `setAlarm` を張り直す」という規則だけを残し、**射程を経路の閉じた列挙では宣言しない。** 既知の経路（`enqueueJob` / `enqueueEvent` を含むトランザクション / `requeue-quarantined-event` / migration ゲートによる `reindex` / `migrate-bulk` の投入 / operator 経路による `rotate-encryption` の起動）は**例示であって全数ではない**と明記する。**新しい投入経路を足す者は経路の側で例外を主張できない。** claim / 終端 / prune が射程外であることは、行を増やさないという理由で引き続き成立する。

**(2) 張る位置を確定する。** 「その書き込みが確定したあと、**最初に来る非同期の文脈**」とし、経路ごとの実体を3つ書く — ユースケース / operator RPC は `UnitOfWorkProvider.run` が戻ったあと応答を返す前、**migration ゲートはゲートが同期関数として戻った直後・RPC 本体に入る前**、**`alarm()` の中でゲートが投入した場合は末尾 (4) が兼ねるので追加の呼び出しは要らない**。ゲート経路を本体の**前**に置くのは、本体が throw してもゲートが書いた行はゲート自身のトランザクションで既に確定しているからである（起床を本体の成否に依存させない）。**ゲート経路の射程は「実際にジョブ行を投入した起動」だけ**とし、何も適用しなかったゲート通過では張らない（規則の言い換えであって例外ではない。全 RPC が毎回1回の非同期ストレージ操作を負う形も避ける）。

**(3) `TC-outboxDelivery-028` を「投入経路ごとに実行する」形へ広げる**（`TC-001` / `-002` の「DO クラスごとに実行する」と同じ既存の書き方）— (a) `enqueueEvent` / (b) `enqueueJob` / (c) migration ゲートによる投入。**テストケース件数は 876 のまま動かない。**

### 検討した代替案

**列挙に2項（migration ゲート / operator 経路）を足して4〜5経路にする**（レビュアーの提案 (a)）— 差分が小さい。採らなかった理由は、**同じ型の破れを次のラウンドへ持ち越す**こと。投入点の一部が別 Issue へ委ねられている `kind`（`finalize-withdrawal` の #45 / #51）がある以上、今日書ける列挙はその時点で既に不完全でありうる。AD-58 の取りこぼしは「1項の書き忘れ」ではなく「規約を経路の集合として宣言した」ことの帰結であり、項を足す修正はその形を温存する。

**新しいテストケースを1件足す**（`TC-outboxDelivery-029`）— (c) を独立ケースにできる。採らなかった理由は、測っている契約が `TC-028` と同一（張られるか否か）で、件数の同期先を3系統動かす代償に見合わないこと。同ファイルには「経路 / DO クラスごとに実行する」1ケースの先例が既に2件ある。

**ゲート経路の `setAlarm` を RPC 本体の完了後に1回だけ張る**（本体が書いた行と合わせて1回で済む）— 採らなかった理由は、**本体が throw した経路でゲートの行が取り残される**こと。ゲートの投入は本体と別のトランザクションで確定するので、起床を本体の成否に依存させる理由が無い。

### Consequences

- 良い点: 規約が「行を増やした」という事実に掛かるので、**将来の投入経路が自動的に射程に入る。** 全数宣言の維持義務が1つ減る。
- 良い点: 非同期 API と同期ゲートの境界が spec の側で決着し、#51 が実装で判断し直さない。
- トレードオフ: **閉じた数え上げが1つ減る** — 「射程は3経路で全数」という機械検査可能な形を失い、代わりに「例外を作らない」という規則の遵守を実装レビューで見ることになる。`TC-outboxDelivery-028` の (c) がその最低限の網である。

---

## AD-61: 「写像鍵は DO の中に無い」の射程を「鍵であって HMAC の値ではない」と明示する

### Status

Proposed（**レビュー7周目・design W-001 への決定**）

### Context

本 PR は「canonical を全長 HMAC へ写す**写像鍵**は request Worker 側（stub 選択アダプター）にあり、DO の中には無い」を3箇所（`spec/async/index.md` の衛生規則 / `.adr/013` の影響 / `spec/database/index.md`「窓キーの導出」）へ逐語で置き、しかもそれを「窓キーの導出を DO 側へ寄せる案（論点11 B）への導線を断つ」**能動的な禁止則**として使っている。

一方、`requestPasswordReset` の手順3 は `claimWindow` が `true` を返した場合に **Identity Directory DO の中**で `CredentialMappingRepository.findByEmail(email: Email)` を呼ぶ。同期契約なので `transactionSync` の中で走り、`credential_mappings` の PK `(kind, hmac)` を引くには canonical の全長 HMAC が要る。**禁止則を字義どおり読むと、DO は `email` から `hmac` を導けないので `findByEmail` が実装できない。**

ポートの signature（`spec/domains/identity.md`）と `ADP-identity-004` は `main` から無改変なので本 PR が作った破れではないが、**写像鍵の在り処を確定させたのは本 PR**であり、その確定によって張力が初めて可読になった。

### Decision

**「DO が持たないのは鍵であって HMAC の値ではない」を明示する。** 正本は `spec/database/index.md`「窓キーの導出」で、そこへ「**facade が受け取る全長 HMAC は、同じ DO の中で `CredentialMappingRepository.findByEmail` の PK 引きにも同じ値が使われる。DO 側は HMAC を導出せず、facade から受け取った値を使う**」を足す。`spec/domains/identity.md` の `PasswordResetThrottlePort`「導出主体」に相互参照を1本足して導線を閉じる。

**ポートの signature は動かさない**（`findByEmail(email: Email)` のまま）。**HMAC を返すドメインポートも、canonical から HMAC を導く DO 内の経路も足さない。** `spec/async/index.md` / `.adr/013` の側は既に「正本は `spec/database/index.md`『窓キーの導出』」と書いているので、そこから辿れる — **同じ文を3箇所へ複製しない**（複製すると片方だけが伸びて割れる）。

### 検討した代替案

**`findByEmail` の signature を HMAC 受け取りへ変える** — 断言と実装が字義でも揃う。採らなかった理由は、`main` から無改変の箇所（ポート契約と `ADP-identity-004`）を本 PR が書き換えることになり、差分の射程が「Outbox 配送の設計訂正」から外れること。

**禁止則を緩めて「HMAC 導出鍵は DO の中から出ない」を無限定に書き直す** — 採らなかった理由は、本 PR がその無限定形を明示的に却下していること（論点11 B への導線 / `.dev.vars.example` の帰属漏れ）。**限定そのものは正しく、足りないのは限定の射程の明示だけである。**

### Consequences

- 良い点: 禁止則（DO は導出しない）と既存ポート（DO の中で PK を引く）が両立し、`ADP-identity-004` の「HMAC で bucket を決めて PK を引く」という**2つの主体にまたがった書き方**の読み方が1つに定まる。
- 良い点: ポートも `Promise` 例外2件も閉じた数え上げも1つも動かない。
- トレードオフ: `requestPasswordReset` の DO facade が「全長 HMAC をプリミティブで受け取る」ことに、窓キーの導出**以外**の用途が1つ増える。#51 は facade の引数の説明をそのように書く。

---

## AD-62: 列挙オラクルの残差 (2) から「宛先の復号」を落とし、復号の在り処を明示する

### Status

Proposed（**レビュー8周目・design W-001 への決定**）

### Context

`requestPasswordReset` の列挙オラクル分析は、測定可能な等価性の対象4つの**外側に残る差**を2つ挙げている。そのうち (2) を「`CredentialMappingRepository.findByEmail` のヒット / ミスと、**行が在るときの宛先の復号**」と書き、直後に「この残差は依頼のトランザクションの内側の処理（(1) は追加の書き込み、(2) は読みと復号）」と射程を明示していた。**同じ文が4箇所**（`spec/usecases/identity.md` 処理フロー4 / `spec/testcases/identity/requestPasswordReset.md` / `spec/inventory/usecase.md` `UC-identity-005` / `spec/inventory/test.md` `TC-requestPasswordReset-004` の要点欄）にある。

**その設計要素は本 PR の他の記述が3方向から否定している。**

1. `ADP-identity-004`（`spec/inventory/adapter.md`）— `findByEmail` は「canonical 化済み email の HMAC で bucket を決めて PK (`kind`, `hmac`) を引き、`CredentialMapping` または null を返す」。復号は登場しない。
2. `CredentialMapping` のフィールド全数（`spec/inventory/domain.md` `DOM-identity-036`）に宛先メールアドレスが**含まれない**。原本の暗号文は `credential_mappings.encrypted_canonical` にあるが、それを平文へ戻す経路は「本人が自分のアドレスを見る経路」と送信材料 RPC の中に限定されている（`spec/domains/identity.md`）。
3. 復号は非同期 API でしか実装できない（`spec/domains/index.md` の Promise 例外の判定基準そのもの）。`claimWindow` → `findByEmail` → `issue` → `enqueueEvent` はすべて同じ `transactionSync` の中なので、**同期契約の下でそこに復号は置けない。**

残差を**過大に**宣言しているのでセキュリティの主張としては安全側だが、実害が2つある — (a) 「残差の出所は1つではない」という数え上げの根拠が1つ実体を欠く、(b) #51 の実装者が「`findByEmail` は宛先を復号する」と読み、同期契約を破る実装へ向かう導線が残る。

### Decision

**残差 (2) を「`findByEmail` のヒット / ミス」へ狭め、「宛先の復号」を落とす。狭めるだけでは (b) が閉じないので、同じ括弧の中に否定と復号の在り処を書く。** 4箇所すべてに同一の節を置く（縫い目を作らない）:

> 行が在るときだけ `credential_mappings` の行を1件読み出す差であり、**宛先の復号はこの経路では起きない** — `CredentialMapping` は宛先の原本を持たず、復号が起きるのは送信材料 RPC の中だけで、そちらは依頼の応答時間に現れない。同期契約の下では `transactionSync` の中に復号を置けない

あわせて `spec/testcases/identity/requestPasswordReset.md` の射程の1文を「(2) は読みと復号」→「**(2) は行の読み出し**」へ直す。

**残差の数は2のまま動かない。** 差は消えていない — `findByEmail` のヒット / ミスは実在し、行が在る側だけが `credential_mappings` の行を1件読む。**削るのは「復号」という**実体の無い**内訳だけである**（残差の全数を過小に書かない、が列挙オラクル分析の要件）。測定対象4つ・4ケースの一様性・緩和の射程（(1)(2) とも窓あたり1サンプル）はいずれも触らない。

### 検討した代替案

**「宛先の復号」をそのまま残す** — 過大宣言は安全側だから、という理由づけは成り立つ。採らなかったのは (b) の導線が閉じないため。**過大な残差宣言は、それが実装の要求として読み戻されうる限り安全側ではない。**

**復号を残差から落とし、送信材料 RPC の側の残差として書き直す** — 復号は実在するので場所を移して数え直す案。採らなかった理由は、送信材料 RPC は**依頼の応答時間に現れない**ので、そもそもこの列挙オラクルの観測点にならないこと。観測点でないものを残差の表に載せると、測定対象4つの射程が曖昧になる。**在り処の明示だけを (2) の括弧の中で行う**形に留めた。

### Consequences

- 良い点: 設計要素の実在と記述が一致し、#51 が「トランザクション内で復号する」実装へ向かう導線が消える。
- 良い点: 残差の数（2）・測定対象（4）・4ケース・スロットルの射程は1つも動かない。閉じた数え上げを開かない。
- トレードオフ: 4箇所に同じ括弧を複製することになる（AD-61 が「同じ文を3箇所へ複製しない」と決めたのと逆向きに見える）。**この4箇所は既に同じ文を複製している**（残差の全数宣言は正本を1つに寄せられない — ユースケース・テストケース・台帳2件がそれぞれ自己完結して読まれる）ので、複製の数は増えていない。増やさないことだけを守った。
