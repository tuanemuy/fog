...
- **(I-4) E-3 / E-4 が名指しするテーブルと列は E-1 に実在する。逆向きも同じ強さで見る — 第6〜9章の本文が要求する列が E-1 に無ければ違反である。** 直す向きは E-1 の側である（第4.1.1節のタイブレーク規則）。**逆向きは目視に落とさない** — 認証・saga・ジョブ系テーブルの列を `テーブル:列` の一覧（61件）として検査8 に持ち、E-1 の当該行に実在するかを機械で見る。**本文が新しい列を導入したら、E-1 と同時に検査8 の一覧にも足す。**
...
- **検査8（I-4 の逆向き）** — 第6〜9章の本文が要求する列が E-1 に実在する。**テーブル名だけを見る検査6 では拾えない。**
  ...
  COLS
  # 期待: 出力なし（一覧は61件。件数は `awk '/<<.COLS./{f=1;next} /^COLS$/{f=0} f'` で数え直せる）
  ```
...
| Identity Directory DO | `password_reset_tokens` | `tokenId`（**暗号論的乱数由来の128ビット以上の不透明値**。bucket 内で採番する。**連番・rowid・時刻由来の値を使わない** — 理由は第6.1節 (d)）/ `tokenHash` / `expiresAt` / `usedAt` / **対象 credential のキー `credentialId`**（第6.1.2節）/ **`changeAuthToken`**（`consume-reset-token` が消費時に採番する**128ビットの暗号論的乱数**。`begin-credential-change` の**起点 B の必須ガード**であり、束縛の実体はこの列だけである。**成功時に同じ `transactionSync` で `NULL` へ戻す**（`値 → NULL` の CAS）ので一回性である。第5.1節・第6.5.1節。第5.2節 (c) の非露出対象）/ **`consumedByOperationId`**（`consume-reset-token` が消費時に記録する。**監査用の記録であって束縛材料ではない** — 第5.2節 (c) が未認証経路のログへ出すことを許している値なので capability にできない。第5.1節 (3-a)）/ **`tokenKeyGeneration`**（`IDENTITY_RESET_TOKEN_KEY` の世代。**routing 世代とは別の番号体系である**。第6.1節 (d)）。credential キー `credentialId` に索引を張り、credential 変更 / unlink / 退会が未使用トークンを一括削除できるようにする。**キーを `(kind, hmac)` にしない理由は第6.1.2節にある** — `hmac` は世代依存なので、ローテーション中に発行されたトークンが世代の違う削除要求から漏れる。**生トークンは保存しない**（`tokenId` から導出する） | 第6.1節 (d)・第6.1.2節 |
...
- 再 HMAC が必要なローテーション（第6.8節）は、operator 専用の maintenance 経路（公開ルートを持たない）が request Worker 側で起動する。**ただし再 HMAC の計算そのものは bucket の中で行い、平文 canonical を Worker 境界の外へ出さない。** maintenance 経路は鍵を RPC 引数として bucket へ**一時的に注入**し、bucket は受け取った鍵を SQLite にもインスタンスフィールドにも書かずに、その呼び出しの中だけで使って破棄する。**この2点（再 HMAC の場所と鍵を永続化しないこと）は本 Issue の決定である。**
...
| 鍵ローテーションの再写像（第6.8節） | 同じ credential の行が新旧2世代に並存する / `credential_locators` に2世代の行が並存する / 移送をスキップされた行 | **operator の maintenance 経路が駆動する `rotate-remap` チャンク**（Alarm ジョブではない。第6.8節） | **常に前進**（並存する中間状態でも login は通る）。**中間状態の全数と回収規則の設計は #44** であり、本表はこの操作が cross-DO 操作の1つであることの記録として行を残す（第6.8節の論点1・論点5） |
...
3. **Directory bucket** の mapping 行（`encryptedCanonical` を含む）を物理削除する。**削除対象は `credential_locators` に記録された `credentialId` ごとの全世代分**である — 鍵ローテーション中（第6.8節）は active 世代と previous 世代の両方に行が存在しうるので、**両世代の bucket に対して削除を発行し、「無ければ成功」の冪等操作**にする（第6.1.1節 (R3)。unlink 手順3 と同じ規則である）。RPC は `delete-mapping` で、**引数に `credentialId` と `account.callerToken` を載せる**（第5.1節 (3-b)）。この手順は `credential_locators` を読んで locator を得るので、**その行がまだ存在している必要がある**。**同じ `transactionSync` で、その `credentialId` の `password_reset_tokens` 行も全削除する**（第6.1節 (d)）。トークン行を残すと第6.2.1節 (d) の「bucket 側には何も残さない」が守られない。
   - **削除対象集合は `credential_locators` だけでは閉じない。未完了の unlink も引き取る、と決め切る。** 具体は「**`operations` に `kind = 'unlink'` かつ `phase != 'done'` の行があれば、その行の `targetLocators` が持つ全世代の locator へも `delete-mapping` を発行し、成功したらその unlink 行を `phase = 'done'` にする**」（`updateOperation`。第4.1.1節）。**理由は unlink 手順2 が `credential_locators` から対象行を先に消すことである** — その credential の locator は `operations.targetLocators` にしか残っていないので、`credential_locators` だけを見る本手順の削除対象には**入らない**。
4. **User Data DO** が `credential_locators` を消し、`account.callerToken` を消し、`account.status = 'deleted'` にする。残すのは**非 PII の tombstone だけ** — 不透明なアカウントキー（= `userId`）、status、epoch、完了時刻。
   - **手順4 の前提条件を1つ置く。「`operations` に未完了の unlink 行が1件も無いこと」を満たすまで手順4 へ進まない**（満たさないなら `finalize-withdrawal` は手順3 を再試行して次の起動へ回す。第7.4節 (1-B)）。**`account.callerToken` は `delete-mapping` の唯一の束縛材料なので、これを消すと未完了 unlink の mapping を誰も消せなくなる。** その状態は3つの破れを同時に生む — (i) `encryptedCanonical` を持つ mapping が bucket に残って第6.2.1節 (d) の「bucket 側には何も残さない」が破れる、(ii) その canonical が**恒久的に再登録不能**になる（次の signup が `EMAIL_ALREADY_REGISTERED` で敗北する）、(iii) `sweep-orphan-mapping` の残件条件（`operations` の未完了 unlink 行）が**永久に偽にならず**、分類 (B) の残件が単調減少しない（第7.4節 (1-B)）ので tombstone だけになった DO が最小再開間隔で起き続ける。**手順3 の引き取りと手順4 の前提条件は対で入れる** — 前者だけでは順序の保証が無く、後者だけでは引き取る主体が居ない。
...
3. **鍵は世代（`generation`）を持つ keyring として配布する。** エントリは `{ generation, key, bucketCount }` で、active 1件 + previous 0〜1件である（第5.2.3節）。**`bucketCount` を locator 名（`dir:g{generation}:b{index}`）と keyring エントリの両方に含める** — 名前に入るのは世代と index で、bucket 数は keyring が持つ（第6.2節）。**bucket 数の変更も世代の変更として表現する。**
4. **再 HMAC は bucket の中で行い、平文 canonical を Worker 境界の外へ出さない。鍵は RPC 引数として一時注入し、SQLite にもインスタンスフィールドにも書かない**（第5.2.3節・第6.2.1節 (c) 2）。
5. **`rotate-remap` は Alarm ジョブではない。** 鍵は「呼び出しのスコープを出たら破棄する」ので Alarm 起動時には存在しない。Alarm ジョブにすると鍵を bucket の SQLite かインスタンスフィールドへ置く実装に倒れ、第3.2節の非重複配布が壊れる。**したがって第7.4節の `kind` 全数表と第4.1.1節の Directory `jobs` 行に `rotate-remap` は無い**（第1.4節 I-3 がこれを検査する）。**`rotate-encryption` はこの制約を受けない** — `IDENTITY_MAIL_ENCRYPTION_KEY` は state Worker の常設バインディングなので Alarm 起動時にも鍵が手元にある。実行主体が2つのローテーションで違うのはこの非対称による。
...
| 2 | **移送（`rotate-remap`）の実行主体・ガード・RPC エントリとしての位置づけ。** 移送先 bucket への書き込みと巻き戻しの破棄を含む（**RPC 全数表に無くガード未定義。R8 do-boundary B-002**） | 第5.1節の RPC エントリ表から外し、外した理由と全数の射程の限定を同表に書いた |
...
| **退会が未完了の unlink を引き取らず、退会後に孤児 mapping が残る**（第6.6節 unlink 手順2 × 第6.7節 手順3〜4）。unlink 手順2 は対象 `credentialId` を `credential_locators` から消して locator を `operations.targetLocators` へ退避するので、**退会 手順3 が `credential_locators` から引く削除対象にはその credential が入っていない**。手順4 が `account.callerToken` を消すと `delete-mapping` の束縛材料も失われ、`sweep-orphan-mapping` は残件（`operations` の未完了 unlink 行）を**永久に減らせない**。帰結は (i) `encryptedCanonical` を持つ mapping が bucket に残って第6.2.1節 (d) が破れる、(ii) そのメールアドレス / SSO 主体が**再登録不能**になる、(iii) 分類 (B) の残件が単調減少しないので tombstone の DO が最小再開間隔で永久に起き続ける | **退会 手順3 の削除対象集合に「`operations` の `kind = unlink` かつ `phase != done` の行が持つ `targetLocators` の全世代分」を加え、削除が成功したその unlink 行を `phase = done` にする。手順4（`credential_locators` と `account.callerToken` の削除）は「未完了 unlink 行が0件になったあと」に限る**（第6.7節 手順3〜4・第7.4節 (1-B)）。これで退会経路でも残件が単調減少し、`sweep-orphan-mapping` と `finalize-withdrawal` のどちらが先に走っても同じ結果に収束する |
...
**本設計が新しく導入した列が変更対象一覧に与える影響。** 下の12行はいずれも新設テーブルの列なので DDL 側の追加作業は増えないが、**テーブル定義の外に作業を1つずつ生む**。
...
| **`password_reset_tokens.changeAuthToken`** / `consumedByOperationId` | Directory `password_reset_tokens` | **`changeAuthToken` は `consume-reset-token` が採番する128ビットの暗号論的乱数で、`begin-credential-change` の起点 B（未認証）の唯一の束縛材料である**（定数時間比較を必須にし、成功時に同じ `transactionSync` で `値 → NULL` の CAS で消す。第5.1節 (3-d)・第6.5.1節 起点 B）。第5.2節 (c) の非露出対象に含めるので、RPC の引数・戻り値ロギングを有効化しない。**`consumedByOperationId` は監査用の記録に格下げした** — `operationId` は未認証経路のログへ出してよい値なので capability にできず、**それを束縛にしていた初版は乗っ取り原始関数を残していた**（第5.1節 (3-a)(3-d)）。**#37 は2列とも実装するが、認可の判定に使うのは `changeAuthToken` だけである** |
