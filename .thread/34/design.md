...
- **検査5（I-6）** — クラス (3) の行数と4群の割り当て数が一致する。
  ...
  ```bash
  tbl '| クラス | エントリ | 所属 DO |' | grep -c '^| (3) |'   # 期待: 12
...
- **検査8（I-4 の逆向き）** — 第6〜9章の本文が要求する列が E-1 に実在する。**テーブル名だけを見る検査6 では拾えない。**
  ...
  COLS
  # 期待: 出力なし（`MISSING column in 4.1.1:` が1行も出ない）
  # 一覧の件数は次で数え直す。開始パターンをヒアドキュメントの開き行だけに限定し、
  # 終端で exit するので、この注記行自身に再ヒットして走り続けることがない
  awk '/^ *done <<.COLS.$/{f=1;next} f&&/^ *COLS$/{exit} f' "$D" | grep -c ':'
  ```

  **件数の期待値を注記に書かない**（I-8）。上のコマンドが出す数と、`while` ループが読んだ行数は同じヒアドキュメントを源にするので構造的に一致する。**本文が新しい列を導入したら一覧に1行足すだけでよく、注記を直す作業は発生しない。** R9 で問題になった旧注記（`awk '/<<.COLS./{f=1;next} /^COLS$/{f=0} f'`）は、(i) 終端パターンが実ファイルの2スペース字下げ `  COLS` に一致せず、(ii) 注記行自身が `<<.COLS.` に一致して再点火するため、期待 61 に対し実測 2,242 行を出していた。**検査本体（`MISSING column` の検出）はそのときも正しく動いていた。**

- **検査9（I-5b）** — E-2 の「書き込み箇所（全数）」欄が、本文で同じ行に現れる書き手（E-3 の `kind`）を取りこぼしていない。

  ```bash
  cat > /tmp/e2skip.txt <<'SKIP'
  operations:rotate-encryption
...
**そのうえで「(3) のガードは呼び出し元が DO であることに一切依存していない」を全エントリについて成り立たせる。ただし守り方は一様ではないので、エントリを4群に分けて明示する。初版は「守っているのは CAS と phase 条件である」と一括で正当化していたが、それが当てはまらないエントリが実際には9本あった。**
...
| (3-c) | 到達制御と監査、＋ bucket が自分で検証できる材料 | `check-previous-generation` / `purge-user-mappings` / `rotate-encryption` の起動（3本） |
...
- **(3-c) 到達制御と監査、＋ bucket が自分で検証できる材料が守る群** — `check-previous-generation` / `purge-user-mappings` / `rotate-encryption` の起動の3本。前2本は追加の束縛を持てず、`rotate-encryption` の起動は**bucket が自分の手元の材料だけで検証できるガードを1つ持つ**（引数の `encryptionGeneration` が state Worker の keyring の active 世代と一致すること。暗号化 keyring は常設バインディングなので手元にある）。
...
- **(d) パスワード認証・パスワードリセットで必要な認証情報の所有境界。** **Directory bucket が持つ。**
  ...
  - **発行済みトークンを無効化する手段を持つ。`password_reset_tokens` 行は対象 credential のキー `credentialId` を持ち、それに対する索引を定義する。** 現行 spec（`spec/database/index.md:77-101`）は `user_id` 列と `prt_user_idx`（用途欄に「ユーザーの既存トークン**無効化**・整理」と明記）を持っており、これを落とすと既存設計からの後退になる。bucket 内では `userId` より credential 単位のほうが自然なので、キーは credential 側に置く。**キーを `(kind, hmac)` にしない理由は第6.1.2節にある** — `hmac` は世代依存の値なので、ローテーション中に previous 世代の bucket で発行されたトークン行を active 世代側の削除要求が取りこぼす。`credentialId` は世代非依存なので、どの bucket にある行も同じキーで引ける。
    - **無効化を発行する箇所は3つで、いずれも同じ `transactionSync` の中で行う。** 第6.5.1節 phase 1（credential 変更の起点）/ 第6.6節 unlink 手順3 / 第6.7節 手順3（退会）。
...
1. **予約 TTL は「saga の再開間隔 × 再試行上限 + マージン」より必ず長く取る。** 予約行の `reservedUntil`（絶対時刻。第4.1.1節）は phase 1a / 1b で `now + TTL` として書く。具体値は #38（第11.3節）だが、**この不等式そのものは設計の制約として固定する**。**同じ不等式は SSO link 手順2 の予約にも掛かる**（第6.6節。あちらの再開間隔と再試行上限は `resume-link` のものである）。
...
5. **`rotate-remap` は Alarm ジョブではない。** 根拠は方針4 の制約 (ii) である — **routing 鍵を bucket の SQLite にもインスタンスフィールドにも永続化しない**ので、Alarm が起動した時点で鍵は手元に無い。Alarm ジョブにすると鍵を永続化する実装に倒れ、その制約と第3.2節の非重複配布が同時に壊れる。**この結論は #44 がどの実行主体を選んでも変わらない**（制約 (ii) が入力だからである）。**したがって第7.4節の `kind` 全数表と第4.1.1節の Directory `jobs` 行に `rotate-remap` は無い**（第1.4節 I-3 がこれを検査する）。**`rotate-encryption` はこの制約を受けない** — `IDENTITY_MAIL_ENCRYPTION_KEY` は state Worker の常設バインディングなので Alarm 起動時にも鍵が手元にある。実行主体が2つのローテーションで違うのはこの非対称による。
...
| SSO link の部分失敗で mapping が残り、その主体が永久ロックになる（第6.6節）。**落下点によって残るものが違うので両方を塞ぐ** — 手順3→4 では `active` な孤児 mapping、手順2〜3 では `reserved` 行である | `resume-link` で常に前進させる。`operations` 行に `targetLocators` を持たせて前進を可能にする。**`reserved` 行の側は、手順2 が `reservedUntil` を書き、同じ `transactionSync` で対象 bucket へ `sweep-reservations` を投入することで回収される**（第6.6節 手順2・第7.4節 E-3 の投入点欄）。**どちらか一方を落とすと閉じない** — `reservedUntil` が `NULL` だと作業述語から永久に外れ、投入点が無いと定常状態で `done` に落ちた `sweep-reservations` が二度と `pending` へ戻らない（第7.4節 (7)）。`resume-link` は分類 (C) なのでバックオフ上限で `poison` に落ち、それ以降は誰も予約行を見ない |
...
- **通常の DO 入力（RPC）での再設定は、DO facade のラッパーが発行する。実行主体をここで名指しする。射程は `run()` を呼ぶ RPC に限らず、全 RPC エントリである。** `enqueueJob` は `UnitOfWorkContext` の同期メソッドでトランザクション内に閉じる（第8.2節）一方、`setAlarm` はトランザクションの外なので、誰がいつ呼ぶかを決めないと #37 に落ちる。**規約は次の4点である。**
