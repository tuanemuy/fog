...
3. bucket は `{ userId, credentialId, passwordVerifier, status, credentialVersion, usedLocator }` を値として返す。`usedLocator` は実際にヒットした行の locator（世代 + bucket index + 全長 HMAC）であり、`credentialId` はその行が持つ**世代非依存の credential 同一性**である（第6.1.2節）。**見つからない場合もダミーの検証材料を返す**（request Worker 側の定数でもよい）。**mapping 行が credential 変更中（`changeState = 'pending'`。第6.5.1節）なら、行が存在しても未登録と同じダミー材料を返す** — 旧検証材料での照合を成立させないためである。**同じく `status != 'active'` の行（signup phase 1a / link 手順2 が書いた予約行）もダミー材料へ倒す** — signup の予約行は phase 1a の時点で `passwordVerifier` を持つので（第6.3節 phase 0・phase 1a）、`status` を見ないと未活性の signup の検証材料が未認証経路へ出る。
...
1. **予約 TTL は「バックオフ上限に達するまでの総経過時間 + マージン」より必ず長く取る。** 予約行の `reservedUntil`（絶対時刻。第4.1.1節）は phase 1a / 1b で `now + TTL` として書く。具体値は #38（第11.3節）だが、**この不等式そのものは設計の制約として固定する**。**同じ不等式は SSO link 手順2 の予約にも掛かる**（第6.6節。あちらのバックオフ列は `resume-link` のものである）。
...
3. **それでも前進できなかった場合の終端を決める。発火条件は「前進不能が確定した場合」であり、`ConflictError` に限らない**（第7.4節「分類 (C) の cross-DO saga 前進ジョブが `poison` に達したときの終端規則」の適用である）。具体的には **(a) 再開が `ConflictError` で確定した場合**（印を書く前に落ち、かつ TTL 経過後に別の利用者へ canonical を取られた場合）と、**(b) バックオフ上限に達した場合**（phase 3 / phase 4 が `.overloaded` や `SQLITE_FULL` で失敗し続けた場合）の2つである。そのとき `resume-signup` は**次の2つをこの順で行う**。同時に `terminalReason` を残して poison 化し、**運用へエスカレーションする**（#38）。「黙って到達不能アカウントを残す」だけは選ばない。
   ...
   - **(3-i) 先に、自分が作った mapping をすべて回収する。ただしコーディネーター自身の行は最後まで残す。** コーディネーター予約行の `locators[]` を辿り、**自分以外の bucket** へ `cancel-reservation` を発行して**自分の `operationId` を持つ行を `status` を問わず削除する**（第5.1節）。**`status` を問わないのが要点である** — phase 3 は逐次なので部分成功しうる（コーディネーターが自分の行を昇格した後、`activate-reservation` の途中で落ちる）。
...
- **リクエスト跨ぎの冪等性は `changeState = 'pending'` + `operationId` が担う。** 同じ `operationId` の再開は phase 2 / 3 を何度実行しても同じ結果に収束する。**別の `operationId` による新しい変更依頼**が `pending` 中に来た場合は、後勝ちで `pendingVerifier` と `operationId` を差し替える（利用者が「リセットをもう一度やり直した」ケースが自然に通る）。差し替えても phase 2 の epoch 前進は `operations` 行で一度だけに保たれる。**「新しい依頼」であることが要件である** — 起点 A は `callerToken`、起点 B は一回性の `changeAuthToken` を必須ガードにするので、**同じ依頼の材料を後から再提示して差し替えることはできない**（起点 B は成功時に `changeAuthToken` を消す。上記）。
- **`resume-credential-change` がバックオフ上限に達したときの終端を決める。巻き戻す。** 分類 (C) なので `poison` に落ちると投入点（phase 1 = `begin-credential-change`）からの再投入だけが `pending` へ戻せるが（第7.4節）、**その投入点を叩くには 起点 A（セッション + 旧パスワード照合）か 起点 B（新しいリセットトークン）が要る。ところが `changeState = 'pending'` のあいだ脱出経路は3つとも閉じている** — 旧パスワードは `lookup-credential` がダミーへ倒すので login 不能（第5.1節）、新パスワードは phase 3 未完了なので未昇格、リセット依頼も同じダミー化で解決されないためトークンが発行されない。**残るのは既存セッション（TTL 7日）からの 起点 A 再実行だけで、TTL が切れた時点で復旧手段がゼロになる**（`purge-user-mappings` は mapping を消すだけなので復旧ではなく恒久ロックの確定である）。
  ...
  - **判定材料を `changeState` の値域に持たせる。`null` / `'pending'` / `'advanced'` の3値にする。** phase 2 が成功した直後、bucket は phase 3 へ進む前に**自 bucket のローカル書き込みで `changeState` を `'advanced'` へ倒す**（phase 3 が `null` へ戻す）。**判定は「行が `'pending'` なら phase 2 未適用 → (i)、`'advanced'` なら phase 2 適用済み → (ii)」である。**
    ...
    - **`'advanced'` を書く前にエビクションが起きた場合も安全側に倒れる。** そのとき行は `'pending'` のままなので判定は (i) になるが、**phase 2 は冪等なので次の実行が再発行して成功すれば `'advanced'` へ進む。** (i) に倒れるのは phase 2 が**上限まで成功しなかった**場合だけであり、その場合 User Data 側は前進していない。**「1回成功したが記録前に落ちた」窓は次の実行が閉じる** — 閉じないまま上限へ達するのは phase 2 が失敗し続けている場合であり、それは phase 2 が適用されていないことと等価である。
...
| **複数クレデンシャル signup の phase 3 部分成功で `active` な孤児 mapping が残り、その credential が恒久的に再登録不能になる**（第6.4節 3） | 終端規則を「`cancel-reservation` で自分の `operationId` の行を `status` を問わず全 bucket から回収してから `abandon-account`」の2段にする（第6.4節 3）。**発火条件は `ConflictError` に限らず「前進不能が確定した場合」である** — バックオフ上限到達（`.overloaded` / `SQLITE_FULL` の連続失敗）も含める。限定すると `overloaded` 系の経路で終端規則が1つも走らず、同じ状態が残る（第7.4節「分類 (C) の…終端規則」） |
...
- **後始末そのものが失敗したときの規則を置く。これは上の「backoff と poison」の段落に対する明示的な例外である。** 同段落は「上限回数を超えたら `status='poison'` にする」と無条件に書いているが、**分類 (C) の cross-DO saga については `poison` を後始末の成功と同じ `transactionSync` でしか書かない。** 理由は、`resume-link` / `resume-signup` では**発火の契機と後始末の失敗の契機が同一である**ことである（どちらも対象 bucket の `.overloaded` であり、それは攻撃者が誘導できる。第6.2.2節）。後始末が失敗したら `poison` を書かず、`attempt` を進めてバックオフで次の起動へ回す（`status` は実行可能集合に留まるので `deleteAlarm()` も発火しない）。
  - **後始末の再試行を無限にはしない。上限は既存の `attempt` で数え、`attempt` が前進上限の2倍に達したら (ii) へ落とす。** 新しい列は置かない — `attempt` は既に `jobs` の12列にあり（第4.1.1節）、`terminalReason` に「後始末が完了しなかった」ことと**回収できなかった locator（`operations.targetLocators` の内容）**を載せる。具体値（前進上限そのもの）は #38 だが、**2倍という関係と「後始末側にも上限がある」ことは設計の制約として固定する。**
  - **(ii) の受け口は `purge-user-mappings` の operator 経路である**（第6.7節「最後の砦」。孤児 mapping は当該アカウントのものなので、`userId` を引数に取るこのエントリで消せる）。**したがって `resume-link` / `resume-signup` にも (ii) 側の受け口が割り当たっている。**
  ...
  - **終端は不可逆である。終端規則を1段でも実行したジョブは、以後 saga を前進させない、と決め切る。** 後始末が失敗して次の起動へ回った実行は、**残りの後始末を続けるだけで、phase の前進を再試行しない。** 決めておかないと `resume-signup` が「3-i で消した予約を phase 1b が作り直す」ことになり、回収と前進が同じジョブの中で打ち消し合う（`reserve-credential` は冪等なので黙って成功し、孤児が復活する）。**判定材料は既に行にある** — `resume-signup` はコーディネーター予約行が消えているかどうか（3-i の進捗）、`resume-link` は `operations.phase = 'done'` と `terminalReason` の有無で終端に入ったことが読める。**`resume-credential-change` は巻き戻しが1つの `transactionSync` で完結するので、この状態を持たない。**
...
- **`kind` ごとの割り当てが全数である。分類 (C) の7種すべてに規則が付いている。**
  - **`resume-signup`** → (i)。第6.4節 3 の終端規則（`cancel-reservation` で全 bucket の予約と mapping を回収してから `abandon-account`）。後始末は DO 間 RPC なので上の射程の後者に当たる。
