...
| (2) | `lookup-credential`（login step 3 / リセット依頼の解決） | Directory bucket | request Worker（未認証） | 無条件に応答し、**中身を均一化する** — 未登録 / **`status != 'active'`（＝ signup / link の予約行）** / **`changeState` が `null` でない**（`'pending'` / `'advanced'` のどちらも。第6.5.1節。2値を区別しない — 区別するのは終端規則の (i)/(ii) 判定だけである）/ `nextAttemptAllowedAt` 未到達はすべてダミー検証材料へ倒す（第5.3節・第6.2.2節）。**`status` の条件が要るのは、signup phase 1a の予約行が `passwordVerifier` を持つからである**（第6.3節）— 落とすと、まだ活性化していない signup の検証材料が未認証経路へ出る |
...
| (3) | `abandon-account`（第6.4節 3 の終端規則） | User Data DO | コーディネーター bucket | `operations` 行の `operationId` 一致 + `kind = 'signup'` + `phase` が `done` でないこと **かつ `callerToken` の定数時間比較**。`account.status` を `deleting` へ倒して `finalize-withdrawal` を投入する（第7.4節 E-3 の投入点欄の第2の投入点である）。**`done` に達した saga は倒せない**。**`callerToken` を課す理由は `cancel-reservation` と同じである** — 同じ終端規則の2手順のうち片方だけを束縛する形は自己矛盾であり、`phase != 'done'` は「秘密」ではなく「状態」なので必要な知識は依然 `operationId` だけになる（下記 (3-a) の但し書き） |
...
- **(3-a) 既存の saga 状態を前提とするエントリ — CAS と phase 条件が守る群** — `activate-reservation` / `promote-verifier` / `propagate-saga-committed` / `cancel-reservation` / `abandon-account`。いずれも**呼び出し前に別の主体が書いた行（予約行 / `operations` 行 / `changeState = 'pending'` の mapping 行）の存在を条件にする**ので、存在しない saga を前進させることも、記録されていない `operationId` で phase を飛ばすこともできない。locator の材料も呼び出し側が永続化済みの値に限られる（第5.5節 1）。
  ...
  - **`abandon-account` に `callerToken` を課す。`phase != 'done'` は束縛ではないからである。** この条件は「秘密」ではなく「状態」であり、**必要な知識は依然 `operationId` だけである**。`abandon-account` は `account.status` を `deleting` へ倒して `finalize-withdrawal` を投入するので破壊的という性質は `cancel-reservation` と同じで、しかも第6.4節 3-i / 3-ii として**同じ1つの終端規則を構成している**。**同じ規則の2手順のうち片方だけを束縛する形は設計の自己矛盾である。** 材料は増えない — 呼び出し元のコーディネーター bucket は自分の予約行に `callerToken` を持ち（phase 1a / 1b。第6.3節）、`account.callerToken` は phase 2 の `initialize-account` が書いている。
    - **Blocker ではなく本項の規則として扱う理由も書いておく。** `phase = 'done'` が phase 4 の2つの作用（`operations.phase = 'done'` と `credential_locators` の記録）と**同じ `transactionSync` で原子的に書かれる**限り、射程は「進行中の signup（データが空のアカウント）」に留まり利用者は再登録で回復できる。**逆に phase 4 を2本の RPC に分けて実装すると**、`credential_locators` が書かれて login が通るのに `phase` が `activating` のままという状態が生じ、`resume-signup` が `poison` に落ちればそれが安定状態になる — その口座は `abandon-account` で破壊できるので、**非原子実装では Blocker 相当になる**。原子性の要求は第6.3節 phase 4 と第6.6節 link 手順4 の両方に明記してある。
...
| SSO link（第6.6節） | 手順2〜3 のあいだは **`reserved` 行**（`reservedUntil` 付き。第6.6節 手順2）。手順3 まで進んで手順4 で落ちると、`credential_locators` に対応行を持たない **`active` な孤児 mapping** | User Data DO の `resume-link`（前進）と、予約段の取り残しを回収する対象 bucket の `sweep-reservations`（第6.6節 手順2 が投入する）。**退会と競合した場合の回収は `finalize-withdrawal` が引き取る**（第6.7節 手順3） | **常に前進**（手順1 で `operations` に記録した時点で利用者の意図が確定しているため）。**ただし対象アカウントが `deleting` / `deleted` に入ったら前進をやめ、退会側の回収へ倒す**（第6.6節・第6.7節 手順3） |
...
1. **予約 TTL は「バックオフ上限に達するまでの総経過時間 + マージン」より必ず長く取る。ここでいうバックオフ上限は**前進上限 + 後始末上限**（＝ `attempt` が前進上限の2倍に達するまで）である**（第7.4節の終端規則）。 予約行の `reservedUntil`（絶対時刻。第4.1.1節）は phase 1a / 1b で `now + TTL` として書く。具体値は #38（第11.3節）だが、**この不等式そのものは設計の制約として固定する**。**同じ不等式は SSO link 手順2 の予約にも掛かる**（第6.6節。あちらのバックオフ列は `resume-link` のものである）。
   - **「再開間隔 × 再試行上限」と書いてはならない。再試行は指数バックオフである**（第7.4節「backoff と poison」）。等間隔で見積もると総経過時間を大きく下回るので、**TTL 掃除が下の 3 の終端規則より先に走る**。そのとき掃除されるのはコーディネーター予約行であり、失われるのは `locators[]` / `candidateUserId` / **`callerToken`** の3つで、**`callerToken` は `abandon-account`（3-ii）の必須ガードなので `jobs.payload` に退避できない**（第7.4節の `payload` 制約が「PII **および再利用可能な秘密**を入れない」と決めており、`callerToken` は第5.2節 (c) の非露出対象である）。
   - **不等式を守れば、掃除と終端規則の順序は2つの場合に分かれてどちらも閉じる。** **(a) phase 2 が成功済みなら**コーディネーター予約行は `sagaCommitted` 印を持つので（下の 2 が同じ `transactionSync` で書く）**掃除の対象から外れ、終端規則の材料は残る**。**(b) phase 2 が成功していないなら**予約行は掃除されうるが、**その掃除そのものが正しい巻き戻しである**（`active` へ昇格した行は `status` 条件で掃除されないので、残る孤児 mapping は無い）。したがって (b) では 3 の終端規則が空振りしてよく、`resume-signup` は `terminalReason` を残して `poison` にするだけでよい。**この2場合分けが成立する条件が上の不等式である** — 破れると (a) の側で材料だけが消える。
...
3. **それでも前進できなかった場合の終端を決める。発火条件は「前進不能が確定した場合」であり、`ConflictError` に限らない**（第7.4節「分類 (C) の cross-DO saga 前進ジョブが `poison` に達したときの終端規則」の適用である）。具体的には **(a) 再開が `ConflictError` で確定した場合**（印を書く前に落ち、かつ TTL 経過後に別の利用者へ canonical を取られた場合）と、**(b) バックオフ上限に達した場合**（phase 3 / phase 4 が `.overloaded` や `SQLITE_FULL` で失敗し続けた場合）の2つである。そのとき `resume-signup` は**まず `terminalReason` を書いてコミットし**（終端モードの印。第7.4節。以後の起動は前進を1段も実行しない）、**続けて次の3つをこの順で行う**。最後に `poison` 化し、**運用へエスカレーションする**（#38）。「黙って到達不能アカウントを残す」だけは選ばない。**3段のうち1つでも失敗したら `poison` を書かず、`attempt` を進めて次の起動で残りを続ける**（第7.4節の「後始末そのものが失敗したときの規則」）。
   - **(b) を条件に含めないと、同節が「恒久的に再登録不能」と書いた状態がそのまま残る。** 初版は発火条件を `ConflictError` 限定で書いていたので、`overloaded` 系の連続失敗では 3-i（`cancel-reservation` による回収）も 3-ii（`abandon-account`）も走らなかった。**`overloaded` は攻撃者が誘導できる**（第6.2.2節。任意の1 bucket を soft limit へ追い込める）ので例外事象ではなく、**SSO signup は常に2 bucket を跨ぐ**（第6.3節）ので構成としても標準経路である。**手順（3-i → 3-ii）はそのまま流用でき、新しい RPC も列も要らない。**
   - **(3-i) 先に、自分が作った mapping をすべて回収する。ただしコーディネーター行は最後まで残す。除外の単位は「bucket」ではなく「行」である。** コーディネーター予約行の `locators[]` を辿り、**コーディネーター行1行を除く全 locator** について**自分の `operationId` を持つ行を `status` を問わず削除する**（他 bucket は `cancel-reservation`、自 bucket 内はローカル削除。第5.1節）。**コーディネーター行の識別は「`locators[]` を持つ行」である**（非コーディネーター行は代わりに `coordinatorLocator` を持つ。第6.3節 phase 1a / 1b・第4.1.1節）。
     - **除外を bucket 単位にすると、コーディネーター bucket に同居した別 credential の行が回収されない。** SSO signup は2 canonical を持ち、**その2つが同じ bucket に落ちることは 256 分割なら約 1/256 の確率で起きる**（第5.2.5節が bucket の衝突を必然として扱っている）。同居した2行目は 3-i でも 3-iii（コーディネーター行1行だけを消す）でも消えず、**`sagaCommitted` 印により `sweep-reservations` の作業述語からも駆動源からも構造的に外れる**（第7.4節 (1-A)）。帰結は上と同じ「恒久的に再登録不能」である。**除外は行単位でなければならない。****`status` を問わないのが要点である** — phase 3 は逐次なので部分成功しうる（コーディネーターが自分の行を昇格した後、`activate-reservation` の途中で落ちる）。
     - **コーディネーター自身の予約行を先に消してはならない。3-ii を発行できなくなるからである。** その行は `locators[]` / `candidateUserId` / **`callerToken`** の唯一の置き場であり、**`callerToken` は `abandon-account`（3-ii）の必須ガードである**（第5.1節 (3-a)）。しかも `jobs.payload` へ退避できない（第7.4節の `payload` 制約が再利用可能な秘密を禁じており、`callerToken` は第5.2節 (c) の非露出対象である）。**したがって「3-i で自分の行も消す」と読むと、3-i が成功して 3-ii が失敗した次の起動で 3-ii を永久に発行できず、ログイン手段を1件も持たない `active` アカウントが恒久残留する**（第7.4節の終端規則が (ii) の受け口として割り当てた `purge-user-mappings` は mapping を消すエントリなので、この残渣には効かない）。
     - **順序は「他 bucket の回収 → 3-ii → 自分の行の削除 + `poison`」である。最後の2つは同じ `transactionSync` に入れる**（どちらもコーディネーター bucket のローカル書き込みである。第7.4節の終端規則の射程の前者に当たる）。**これで材料は必要な瞬間まで必ず生きている。**
   - **(3-ii) その後で User Data DO の `account.status` を `deleting` へ倒す**（`abandon-account`）。到達不能アカウントは利用者から見て存在しないので消してよい — signup が完走していない以上セッションも発行されておらず、利用者データも空である。回収は退会と同じ経路（第6.7節）に乗る。**ここで提示する `callerToken` は、まだ消していないコーディネーター予約行から読む。**
   - **(3-iii) 最後に、コーディネーター自身の予約行を削除し、同じ `transactionSync` で `poison` と `terminalReason` を書く。** **3-i / 3-ii のいずれかが失敗していたら 3-iii を実行しない** — `poison` を書かず `attempt` を進めて次の起動へ回す（第7.4節の終端規則の「後始末そのものが失敗したときの規則」）。**次の起動でも材料は生きている**（予約行を消していないため）。
...
- **リクエスト跨ぎの冪等性は `changeState`（`null` でない値）+ `operationId` が担う。** 同じ `operationId` の再開は phase 2 / 3 を何度実行しても同じ結果に収束する。**別の `operationId` による新しい変更依頼**が来た場合の扱いは、**行の値で分かれる。決め切る。**
  ...
  - **`'advanced'` に倒れている窓は phase 3 のローカル書き込み1回ぶんなので短い。** 拒否された利用者は phase 3 の完了後（`changeState` が `null` に戻る）に依頼し直せば通る。**恒久的に `'advanced'` のまま留まるのは phase 3 が恒久的に失敗している場合だけで、それは終端規則の (ii) として既に operator 経路へ割り当ててある**（第7.4節・下記）。**したがって「拒否」が新しい恒久ロックアウトを作ることはない。****「新しい依頼」であることが要件である** — 起点 A は `callerToken`、起点 B は一回性の `changeAuthToken` を必須ガードにするので、**同じ依頼の材料を後から再提示して差し替えることはできない**（起点 B は成功時に `changeAuthToken` を消す。上記）。
- **`resume-credential-change` がバックオフ上限に達したときの終端を決める。巻き戻す。** 分類 (C) なので `poison` に落ちると投入点（phase 1 = `begin-credential-change`）からの再投入だけが `pending` へ戻せるが（第7.4節）、**その投入点を叩くには 起点 A（セッション + 旧パスワード照合）か 起点 B（新しいリセットトークン）が要る。ところが `changeState = 'pending'` のあいだ脱出経路は3つとも閉じている** — 旧パスワードは `lookup-credential` がダミーへ倒すので login 不能（第5.1節）、新パスワードは phase 3 未完了なので未昇格、リセット依頼も同じダミー化で解決されないためトークンが発行されない。**残るのは既存セッション（TTL 7日）からの 起点 A 再実行だけで、TTL が切れた時点で復旧手段がゼロになる**（`purge-user-mappings` は mapping を消すだけなので復旧ではなく恒久ロックの確定である）。
  ...
  - **規則は次のとおりである。`resume-credential-change` が前進不能を確定した時点で、`poison` へ落とすのと同じ `transactionSync` で mapping 行の `changeState` / `changeOrigin` / `pendingVerifier` / `operationId` を `null` へ戻す。** `passwordVerifier` は phase 3 まで一切触られないので**旧パスワードが復活する**。**認可が開く方向へは倒れない** — 変更が「効かなかった」だけであり、起点 B の材料（`changeAuthToken`）は phase 1 が既に失効させている（下記）。
  - **「自力で再試行できる」は起点 A に限る。起点 B（`changeOrigin = 'reset'`）では (ii) の受け口も併せて割り当てる。** 起点 B に来る利用者は**旧パスワードを知らない**（それがリセットを依頼した理由である）ので、巻き戻して旧 `passwordVerifier` が正本に戻っても login できない。しかも本規則の発火契機として本節が名指ししている `SQLITE_FULL` は**書き込みだけが恒久的に失敗する**半死状態なので、**リセットを何度やり直しても phase 2 が毎回失敗して巻き戻る**。第6.2.2節 (a) の脱出経路は (i) リセット完走が上記で塞がり、(ii) SSO はパスワード単独アカウントには存在せず、第4.6節の逼迫時導線はログインを要求する。**したがってパスワード単独アカウントの失念利用者は恒久的に締め出される。** 塞ぎ方は**巻き戻しに加えて `terminalReason` を残し運用エスカレーションへ回すこと**である（#38。`terminalReason` に「起点が `'reset'` である」ことを載せ、operator が容量解放を先に行えるようにする。第11.3節の `poison` 検知項目と容量監視の項目が対で受ける）。**巻き戻し自体はやめない** — やめると `changeState = 'pending'` が恒久化して W-006 の経路に戻り、そちらは認可も可用性も同時に閉じるので明確に悪い。
  - **phase 2 が成功した後（`changeState = 'advanced'`）に上限へ達した場合は巻き戻さない。** そのとき User Data 側は `sessionEpoch` と `credentialVersion` を進めており、Directory 側だけを戻すと第5.3節 step 5 (iii) が不一致になる（第6.9節が登録している締め出し経路そのものである）。**したがって (ii) 側**（`terminalReason` を残して運用エスカレーション。#38）**に倒す。**
    - **「恒久的に」不一致とは書かない。射程を正確に書く。** 不一致は後続のリセット / パスワード変更が1回完走すれば解消する（phase 2 が User Data 側を n+2 へ進め、phase 3 が Directory 側を同じ値へ揃えるので、`credentialVersion` は再び一致する。上の phase 3 の「揃える」がこれを保証している）。**恒久化するのは、完走を妨げている原因が恒久的な場合だけである** — 具体的には本規則の発火契機として名指ししている `SQLITE_FULL`（当該 DO の書き込みが恒久的に失敗する半死状態。第4.7節）である。**したがって受け口は運用側で正しい** — operator が容量を解放すれば次のリセットが完走して解消する。この射程を書かないと、(ii) 側が「復旧不能」に見えて #38 が手順を書けなくなる。
  - **判定材料を `changeState` の値域に持たせる。`null` / `'pending'` / `'advanced'` の3値にする。** phase 2 が成功した直後、bucket は phase 3 へ進む前に**自 bucket のローカル書き込みで `changeState` を `'advanced'` へ倒す**（phase 3 が `null` へ戻す）。**判定は「行が `'pending'` なら phase 2 未適用 → (i)、`'advanced'` なら phase 2 適用済み → (ii)」である。**
    - **`operations` 行の有無では判定できない。** その行は **User Data DO 側**にあり、それを読む RPC を第5.1節の全数表に持たない。
    - **「上限に達したその実行の中で `advance-credential-change` が成功したか」でも判定できない。これは失われうる信号である。** phase 2 が成功した直後・`'advanced'` を書く前にエビクションが起きると（第2.1節 F-4）、以後の実行は phase 2 を冪等に再発行するが、その再発行が `.overloaded` で失敗し続けて上限に達しうる。そのとき「その実行では失敗した」を根拠に巻き戻すと、**User Data 側だけが前進した状態で Directory を戻す**ことになり、下の「phase 2 の成功後は巻き戻さない」に正面から反する。**本書は「commit 後に応答が失われる」事象クラスを phase 2 の冪等性の根拠としてすでに認めている**（第6.4節 phase 2 の no-op 規定・第6.5.1節 phase 1 の `changeAuthToken` 温存）ので、同じ事象クラスを判定の側で無視してはならない。
    - **記録は phase 2 の成功「後」に書く。前に書かない。fail-safe の向きをここで決め切る。** 先に書くと「phase 2 が失敗したのに適用済みと記録される」ので**巻き戻しが一度も発火せず**、第6.9節に登録した恒久ロックアウト（`changeState` が `null` でないまま固定される）がそのまま戻る。後に書くと「成功したのに記録前に落ちる」窓が残るが、**その帰結は `credentialVersion` の不一致であって認可は開かない**（巻き戻しは `pendingVerifier` を捨てるだけで `passwordVerifier` に触らない）。**恒久ロックアウトと、後続の完走1回で解消する不一致の二択なので、後者を選ぶ。**
    - **その窓の射程を正確に書く。** 「phase 2 が成功し、`'advanced'` を書く前にエビクションが起き、**以後 User Data DO が上限まで到達不能であり続ける**」の3つが同時に成立したときだけ (i) へ誤判定する。**次の実行が再発行して成功すれば `'advanced'` へ進む**ので、通常のエビクションでは閉じる。誤判定が残るのは User Data DO が恒久的に到達不能な場合であり、**そのときは step 5 が同じ DO を叩くので login はどのみち成立しない。** 到達可能に戻った後は不一致が残るが、後続のリセット / パスワード変更が1回完走すれば phase 3 が両側を揃える（上の phase 3 の「揃える」）。**この残余は #38 の運用検知（`poison` 行と容量監視）で拾う。**
    - **値域を1つ広げるだけで、`null` でない値の扱いは変えない。** `lookup-credential` は **`changeState` が `null` でない行**をダミー材料へ倒す（`'pending'` と `'advanced'` を区別しない。第5.1節）。`promote-verifier`（phase 3）のガードは `'advanced'` かつ `operationId` 一致である。**どちらも fail closed の方向にしか動かない**ので、第6.9節が宣言した「どの中間状態でも認可は開かない」は保たれる。
...
- **`resume-link` がバックオフ上限に達したときの終端を決める。前進が原則だが、終端では巻き戻す。** `resume-link` は分類 (C) なので、`.overloaded`（第4.7節。リトライ禁止）や `SQLITE_FULL` で失敗し続ければ `poison` に落ちる。**そのとき手順3→4 の落下点で残る `active` な孤児 mapping は、回収経路を1つも持たない** — `sweep-reservations` は `status: 'reserved'` しか見ず、`sweep-orphan-mapping` の残件条件は `kind = 'unlink'` 限定（第7.4節 (1-B)）、`finalize-withdrawal` は `deleting` 前提である。**利用者の再試行も効かない** — 再 link は request Worker が新しい `credentialId` を採番する（第6.1.2節 (C1)）ので、手順2 の `reserve-credential` は同じ `(kind, hmac)` に既存の `active` 行がある状態で走って敗北する。`resume-link` の `operationKey` は対象 `operationId` を含むので、`poison` 行の復活規則（第7.4節）も掛からない。**したがって、これは前進ジョブを新設して塞いだはずの状態そのものが単一世代で再生する形である。**
  - **規則は次のとおりである。`resume-link` が前進不能を確定した時点（`ConflictError` またはバックオフ上限到達）で、`poison` へ落とすのと同じ実行の中で巻き戻す。** **(i) `operations.targetLocators` が持つ全世代の locator へ `delete-mapping`（引数は `credentialId` + `account.callerToken`。「無ければ成功」の冪等操作）を発行して mapping を回収する。(ii) `operations` 行を `terminalReason` 付きで `phase = 'done'` にし、同じ `transactionSync` でジョブを `poison` にする。** 束縛材料は揃っている — 退会していない限り `account.callerToken` は生きている（第6.7節 手順4 が消すのは退会完走時である）。
    - **(i) が失敗したら (ii) を実行しない。`poison` を書かず `attempt` を進めて次の起動へ回す**（第7.4節の終端規則の「後始末そのものが失敗したときの規則」。この規則は正本の側にあり、本項はその適用である）。**発火の契機と (i) の失敗の契機が同一なので**（どちらも対象 bucket の `.overloaded`。攻撃者が誘導できる）、1回試して `poison` にする形では塞いだことにならない。**後始末の再試行上限（`attempt` が前進上限の2倍）に達したら (ii) 側へ落とし、`terminalReason` に回収できなかった locator を載せる。受け口は `purge-user-mappings` の operator 経路である。**
    - **`account.callerToken` は `operations` 行と同じ DO にあるので、(i) の途中で材料を失うことはない。** signup の終端規則が `callerToken` の置き場（コーディネーター予約行）を自分で消しうるのに対し（第6.4節 3-i の但し書き）、link は User Data DO 側の `account` 行から読むので同じ問題を持たない。
  - **link は「常に前進」が原則だが、終端では巻き戻しが正しい。** 利用者から見て「連携に失敗した」であり、canonical が解放されるので**同じ SSO 主体で再 link すれば回復する**。これは手順2 が敗北したときに既に採っている扱いと同じである。**前進側に倒す代替（operator 経路を新設して locator を手で書く）は採らない** — 認証状態を operator が書ける経路を1本増やすことになり、第5.1節 (3-d) が `record-credential-locator` について塞いだ「任意アカウントへクレデンシャルを注入する原始関数」と同じ形の口が開く。
  - **退会と競合している場合は本規則より退会側の引き取りが先である。** `resume-link` は `account.status != 'active'` を見たら `operations` に触れずにジョブだけを `done` にする（下記）ので、そもそも `poison` には落ちない。**回収は退会 手順3 が行う**（第6.7節）。
...
| SSO link の部分失敗で mapping が残り、その主体が永久ロックになる（第6.6節）。**落下点によって残るものが違うので両方を塞ぐ** — 手順3→4 では `active` な孤児 mapping、手順2〜3 では `reserved` 行である | `resume-link` で前進させる。`operations` 行に `targetLocators` を持たせて前進を可能にする。**ただし前進不能が確定したとき（`ConflictError` またはバックオフ上限到達）は終端規則で巻き戻す** — `targetLocators` の全世代へ `delete-mapping` を発行して mapping を回収し、`operations` 行を `terminalReason` 付きで `phase = 'done'` にしてから `poison` にする（第6.6節 link の終端規則・第7.4節「分類 (C) の…終端規則」）。**終端で巻き戻すのが正しい** — 利用者から見て「連携に失敗した」であり、canonical が解放されるので再 link で回復できる。**`reserved` 行の側は、手順2 が `reservedUntil` を書き、同じ `transactionSync` で対象 bucket へ `sweep-reservations` を投入することで回収される**（第6.6節 手順2・第7.4節 E-3 の投入点欄）。**どちらか一方を落とすと閉じない** — `reservedUntil` が `NULL` だと作業述語から永久に外れ、投入点が無いと定常状態で `done` に落ちた `sweep-reservations` が二度と `pending` へ戻らない（第7.4節 (7)）。**`resume-link` が分類 (C) なので `poison` に落ちうるという認識を、`reserved` 行の側だけでなく `active` な孤児 mapping の側にも適用したのが上の終端規則である** — 適用しないと、この行が塞いだはずの永久ロックが単一世代のまま再生する |
...
| **複数クレデンシャル signup の phase 3 部分成功で `active` な孤児 mapping が残り、その credential が恒久的に再登録不能になる**（第6.4節 3） | 終端規則を3段にする（第6.4節 3）— **(3-i) コーディネーター行1行を除く全 locator から `operationId` 一致の行を `status` を問わず回収 →(3-ii) `abandon-account` →(3-iii) コーディネーター行の削除と `poison` を同じ `transactionSync`**。**除外は bucket ではなく行の単位である**（同 bucket に同居した2行目が取り残されるため）。**終端に入る前に `terminalReason` を書いてコミットし、以後の起動は前進を1段も実行しない**（第7.4節）。**発火条件は `ConflictError` に限らず「前進不能が確定した場合」である** — バックオフ上限到達（`.overloaded` / `SQLITE_FULL` の連続失敗）も含める。限定すると `overloaded` 系の経路で終端規則が1つも走らず、同じ状態が残る（第7.4節「分類 (C) の…終端規則」） |
| **`resume-credential-change` が `poison` に落ちて `changeState` が `null` でない値（`'pending'` / `'advanced'`）のまま恒久化し、脱出経路が3つとも閉じる**（第6.5.1節 × 第7.4節）。旧パスワードは `lookup-credential` のダミー化で login 不能、新パスワードは phase 3 未完了で未昇格、リセット依頼も同じダミー化で解決されずトークンが発行されない。分類 (C) なので投入点（起点 A / 起点 B）からの再投入だけが `pending` へ戻せるが、その投入点を叩く材料が上の3つと同じものである。**残る手段は既存セッション（TTL 7日）からの 起点 A 再実行だけで、TTL が切れた時点で復旧手段がゼロになる。** 契機は設計内に実在する — 10 GB に達した User Data DO では `SQLITE_FULL`（書き込みだけが恒久的に失敗し `retryable` false。第4.7節）で phase 2 が毎回失敗し、逼迫時の導線（第4.6節）はログインを要求するので到達できない | **前進不能が確定した時点で、`poison` へ落とすのと同じ `transactionSync` で `changeState` / `changeOrigin` / `pendingVerifier` / `operationId` を `null` へ戻す**（第6.5.1節の終端規則）。`passwordVerifier` は phase 3 まで触られないので旧パスワードが復活する。認可が開く方向へは倒れない（変更が効かなかっただけで、起点 B の材料は phase 1 が既に失効させている）。**塞ぎ方は起点で2つに分かれる** — **起点 A（`changeOrigin = 'password-change'`）は巻き戻しだけで閉じる**（旧パスワードを知っているので自力で再試行できる）。**起点 B（`'reset'`）は巻き戻しに加えて `terminalReason` + 運用エスカレーション（#38）を併せて課す** — 旧パスワードを知らない利用者は巻き戻っても login できず、発火契機の `SQLITE_FULL` は書き込みだけが恒久的に失敗するのでリセットをやり直しても同じ結果になり、SSO も逼迫時導線（ログインを要求する）も使えないため**自力の脱出経路が1本も無い**。**phase 2 の成功後は巻き戻さず (ii) だけに倒す** — Directory 側だけを戻すと step 5 (iii) が恒久的に不一致になり、同じ表の別の行になる。**判定は `changeState` の値で行う**（`'pending'` = phase 2 未適用 → (i) / `'advanced'` = 適用済み → (ii)）。`operations` 行は User Data DO 側にあり読む RPC が無く、「その実行で phase 2 が成功したか」は commit 後に応答が失われる事象クラスで壊れるので、**判定材料を行の値として持つ**（第6.5.1節 phase 2・第4.1.1節） |
...
- **意図的に可用性を落とす中間状態は上の一覧に含めない。ただし除外の根拠は「短時間だけ」ではなく「前進または巻き戻しが必ず終端する」である。** credential 変更の `changeState` が `null` でない期間（`'pending'` / `'advanced'`）（旧新どちらのパスワードも通らない。第6.5.1節）と退会の `deleting` は、**利用者の操作の結果として生じ、前進ジョブが完走するか、前進不能が確定して終端規則が発火するかのどちらかで必ず抜ける**ので締め出しではない。
  - **「短時間だけ閉じる」を根拠にしてはならない。** 前進ジョブは分類 (C) なのでバックオフ上限で `poison` に落ちうるので、実行時間の上界は設計として保証されていない。**根拠を保証されていない前提に置いていた初版は、`resume-credential-change` の `poison` が恒久ロックアウトであるという反例を抱えていた**（上の表に行として足した）。**終端規則（第7.4節「分類 (C) の…終端規則」）が全 `kind` について割り当てられていることが、この除外条項が成立する条件である。**
...
| `finalize-withdrawal` | User Data DO | 第6.7節 手順1（`account.status = 'deleting'` と同じ `transactionSync`）**および第6.4節 3-ii（signup の終端規則で `abandon-account` が投入する。エントリの定義は第5.1節）**。**投入点は2つあり、これが全数である**（I-1） | **(C) 一回性** | 退会の手順3〜4 を前進させる（第6.7節）。signup の終端規則による回収も同じ経路に乗る（第6.4節 3-ii） |
...
**分類 (C) の cross-DO saga 前進ジョブが `poison` に達したときの終端規則を1つ置く。正本は本項である。** 分類 (C) の `done` は復活させない（上の `status` 別の規則）ので、**`poison` は「その saga がもう誰にも前進させられない」ことを意味する**。ところが saga の中間状態は残る。**したがって前進ジョブの `kind` ごとに「前進不能が確定したときに何を行うか」を持たせ、`poison` へ落とすのと同じジョブ実行の中で（＝別の Alarm 起動へ持ち越さずに）順に完了させる。**

- **「同じ実行の中で」であって「同じ `transactionSync` で」ではない、と射程を割る。** 後始末が**自 DO のローカル書き込みだけで済む** `kind`（`resume-credential-change` の巻き戻し）は `poison` を書くのと**同じ `transactionSync`** に入れる。**後始末が DO 間 RPC を伴う** `kind`（`resume-signup` の `cancel-reservation` + `abandon-account`、`resume-link` の `delete-mapping`）は `transactionSync` の中に置けない — コールバックは完全同期で `await` を書けず（第2.1節 F-7）、第6.9節 制約1 が「DO 間 RPC は必ずトランザクションの外で行う」と禁じている。**この場合は RPC を先に完走させ、その後で `operations` の終端更新と `poison` を1つの `transactionSync` で書く。** 第6.4節 3（「次の2つをこの順で行う」）と第6.6節 link の終端規則（「同じ実行の中で」）は既にこの形で書いてあり、本項だけが実装不能な形だった。
- **規則の形は2つで、排他ではない。** **(i) 巻き戻せる saga は巻き戻す**（材料が解放されるので**起点を自分で叩き直せる利用者は**自力で再試行できる）。**(ii) 自力で回復できない saga は operator 経路へ渡す**（`terminalReason` を残して運用エスカレーション。#38）。**同じ `kind` でも起点によって自力回復の可否が変わる場合は (i) と (ii) を併せて適用する**（`resume-credential-change` の `changeOrigin = 'reset'` がそれである。第6.5.1節）。**「黙って中間状態を残す」は選ばない** — 第6.4節が signup について既にこの基準を置いており、他の `kind` にも同じ基準が掛かる。
- **後始末そのものが失敗したときの規則を置く。これは上の「backoff と poison」の段落に対する明示的な例外である。** 同段落は「上限回数を超えたら `status='poison'` にする」と無条件に書いているが、**分類 (C) の cross-DO saga については `poison` を後始末の成功と同じ `transactionSync` でしか書かない。** 理由は、`resume-link` / `resume-signup` では**発火の契機と後始末の失敗の契機が同一である**ことである（どちらも対象 bucket の `.overloaded` であり、それは攻撃者が誘導できる。第6.2.2節）。後始末が失敗したら `poison` を書かず、`attempt` を進めてバックオフで次の起動へ回す（`status` は実行可能集合に留まるので `deleteAlarm()` も発火しない）。
  - **後始末の再試行を無限にはしない。上限は既存の `attempt` で数え、`attempt` が前進上限の2倍に達したら (ii) へ落とす。** 新しい列は置かない — `attempt` は既に `jobs` の12列にあり（第4.1.1節）、`terminalReason` に「後始末が完了しなかった」ことと**回収できなかった locator**を載せる。具体値（前進上限そのもの）は #38 だが、**2倍という関係と「後始末側にも上限がある」ことは設計の制約として固定する。**
    - **この2倍キャップは上の「`poison` を後始末の成功と同じ `transactionSync` でしか書かない」に対する唯一の出口であり、両者は矛盾しない。** 前者は「後始末が失敗している**あいだ**は `poison` を書かない」、後者は「**上限に達したら**後始末の未完了を `terminalReason` に記録して `poison` を書く」であり、**適用される局面が違う。** 書き分けないと `resume-signup` / `resume-link` は字義どおり読むと永久に終わらないジョブになる（第11.3節の #38 引き継ぎはこの出口があることを前提に手順を書いている）。**出口が発火したときに残るものを `terminalReason` に全部書くのが (ii) の実体である。**
  - **(ii) の受け口を `kind` と残渣の種類で書き分ける。「`purge-user-mappings` に送る」だけでは届かない残渣がある。**
    - **`resume-link` と、`resume-signup` のうち `active` へ昇格済みの mapping** → `purge-user-mappings` の operator 経路（第6.7節「最後の砦」）。どちらも当該アカウントの `userId` で引ける。
    - **`resume-signup` のうち `reserved` のまま残った行** → **`purge-user-mappings` では届かない。** 予約行が持つのは `candidateUserId` であって `userId` 列ではなく（第4.1.1節）、しかも `sagaCommitted` 印のある行は `sweep-reservations` の作業述語からも駆動源からも構造的に外れる（第7.4節 (1-A)）。**したがって受け口は `cancel-reservation` の operator 実行である** — 同エントリは `operationId` 一致 + `callerToken` で `status` を問わず削除する冪等操作なので（第5.1節）、**`terminalReason` に `operationId` と対象 bucket の一覧を載せておけば operator がそのまま再発行できる。** `callerToken` は `terminalReason` に載せない（第5.2節 (c) の非露出対象）ので、**operator は `account.callerToken` を第6.5.1節 phase 0 の A-1 相当の maintenance 経路から取る**。**この経路の到達制御と監査は `purge-user-mappings` と同じ扱いで #38 へ送る**（第11.3節）。
    - **`terminalReason` に載せる内容をここで確定する** — **(a) 終端の理由**（`ConflictError` / バックオフ上限 / 後始末上限）、**(b) `operationId`**、**(c) 回収できなかった locator の一覧**（`operations.targetLocators` またはコーディネーター予約行の `locators[]` の内容。世代と bucket index を含む）。**PII と再利用可能な秘密は載せない**（`jobs.payload` と同じ制約。第7.4節）。
  - **この2段が無いと、B-001 が塞いだはずの `active` な孤児 mapping が「後始末に失敗した」経路から同じ形で残る。** 攻撃者から見れば、狙う bucket を overload し続けるという1つの手段で発火と後始末の両方を止められるので、**後始末を1回試して諦める設計は塞いだことにならない。**
  - **終端は不可逆である。終端規則を1段でも実行したジョブは、以後 saga を前進させない、と決め切る。** 後始末が失敗して次の起動へ回った実行は、**残りの後始末を続けるだけで、phase の前進を再試行しない。** 決めておかないと `resume-signup` が「3-i で消した予約を phase 1b が作り直す」ことになり、回収と前進が同じジョブの中で打ち消し合う（`reserve-credential` は冪等なので黙って成功し、孤児が復活する）。
    - **判定材料は `jobs.terminalReason` である。終端に入った時点で、後始末の1段目を実行する**前**に、`terminalReason` を書いてコミットする**（`status` は `pending` / `running` のまま。`poison` は最後まで書かない）。**以後の起動は `terminalReason` が入っている行を見て「終端モード」と判断し、前進を1段も実行しない。** 列は増えない — `terminalReason` は既に `jobs` の12列にあり（第4.1.1節）、**書く契機を「`poison` へ落とすとき」から「終端に入るとき」へ前倒しするだけである**（`poison` へ落とすときに上書きして最終的な理由に差し替えてよい）。
    - **「コーディネーター予約行が消えているか」を判定材料にしてはならない。** その削除は第6.4節 3-iii（`poison` と同じ `transactionSync`）に置いたので、**ジョブがまだ動ける全ての起動でその行は存在し、消えているのは既に `poison` のときだけである。** つまりこの述語は終端モードを一度も返さず、**上で塞いだはずの孤児復活がそのまま生きる。** 同様に `attempt` も使えない（2倍キャップの判定にしか使っておらず、発火条件 (a)（`ConflictError`）では上限未満のままである）。**Directory bucket には `operations` 表が無い**（第4.1.1節 E-1）ので、`resume-link` が使える `operations.phase` に相当する印も `resume-signup` には存在しない。**`terminalReason` が3つの `kind` すべてに共通で使える唯一の印である。**
    - **`resume-credential-change` は巻き戻しが1つの `transactionSync` で完結するので、この状態を持たない**（終端に入った実行がそのまま `poison` まで到達する）。それでも規則の一様性のために `terminalReason` を先に書く形に揃える — **`kind` ごとに終端の入り方を変えない。**
  - **予約 TTL の下限はこの後始末の上限まで含めて取る。** 第6.4節 1 の不等式が覆うべき総経過時間は「前進上限まで」ではなく「**前進上限 + 後始末上限**（＝ `attempt` が前進上限の2倍に達するまで）」である。**指数バックオフなので2倍は総経過時間で見ると大きく伸びる** — ここを取り違えると、後始末の再試行中に TTL 掃除が走ってコーディネーター予約行（`callerToken` の唯一の置き場）が消える。**第6.4節 1 と第11.3節の #38 への引き継ぎも同じ形に揃えてある。**
- **`kind` ごとの割り当てが全数である。分類 (C) の7種すべてに規則が付いている。**
  - **`resume-signup`** → (i)。第6.4節 3 の終端規則（3段。コーディネーター行1行を除く全 locator から回収 → `abandon-account` → コーディネーター行の削除と `poison`）。後始末は DO 間 RPC なので上の射程の後者に当たる。
  - **`resume-link`** → (i)。第6.6節 link の終端規則（`operations.targetLocators` の全世代へ `delete-mapping` を発行して mapping を回収し、`operations` 行を `terminalReason` 付きで `phase = 'done'` にする）。同じく DO 間 RPC を伴う。
  - **`resume-credential-change`** → phase 2 の成功前は (i)、成功後は (ii)。**起点が `'reset'` の場合は (i) に加えて (ii) も併せて適用する**（巻き戻しても旧パスワードを知らない利用者は自力で再試行できないため。第6.5.1節の終端規則）。後始末は自 bucket のローカル書き込みだけなので `poison` と同じ `transactionSync` に入る。
  - **`finalize-withdrawal`** → (ii)。受け口は operator 経路の `purge-user-mappings` である（第6.7節「最後の砦」）。
  - **`send-mail`** → 巻き戻す状態を持たない。窓が変われば新しい `operationKey` になるので投入点から回復する（第7.6節）。
  - **`reindex`** / **`migrate-bulk`** → (ii)。投入点が migration ゲートで、`schema_version` が進んだ後は冪等にスキップされるので二度と投入されない（第9.2節）。**受け口は #38 の運用検知である。**
- **発火条件を `ConflictError` に限定してはならない。** 第6.4節 3 は初版で「再開が `ConflictError` で確定した場合」だけを条件にしていたが、**`.overloaded`（第4.7節。リトライ禁止）・`SQLITE_FULL`・連続エビクション（第2.1節 F-4）で失敗し続けてバックオフ上限に達する経路では `ConflictError` が出ない**ので、終端規則が1つも走らないまま `poison` になっていた。**しかも `overloaded` は攻撃者が誘導できる** — 第6.2.2節が「任意の1 bucket を選んで 1,000 req/s の soft limit へ追い込める」と断定しているので、`resume-*` が Directory RPC を通る瞬間を狙い続ければバックオフ上限まで押し切れる。**したがって条件は「前進不能が確定した場合（`ConflictError` またはバックオフ上限到達）」である。**
- **`poison` 行そのものの検知と手動再投入は運用側の項目として #38 へ送る**（第11.3節）。**本項が決めるのは「`poison` に落ちる瞬間に設計側が何を後始末するか」であって、落ちた行の運用ではない。**
...
- **trigram / `bm25` の再確認 spike。** 第2.1.1節の手順を着手時に1回走らせる。#37 の Issue が要求する tokenizer 実環境検証と同じ作業である。**あわせて第11.4節の表のうち「#37 / 着手時の spike」を含む行すべてを同じ着手時にまとめて走らせる。第11.4節は10行あり、うち9行が #37 の着手時 spike を含む**（うち2行は #37 が根拠値を出して #38 が運用値を確定する2段の分担である。**残り1行は鍵ローテーションの手順で、決める主体が #44 なので spike の対象ではない**）。trigram / `bm25` の再確認以外の**残り8件**は、`snippet()` / `highlight()` の可用性 / `transactionSync` のネスト可否 / Alarm・RPC の CPU リセット契機 / 単一 SQL クエリの結果セット合計サイズ上限 / チェックポイント予算の中間・内側の初期値 / `SELECT changes()` の意味論 / `transaction()` コールバックの `async` 可否 / `sql.exec()` が `Date.now()` を進めるか、である。**件数を書き換えるときは第11.4節の表を数え直す。**
...
- **分類 (C) の終端規則**（第7.4節の正本）— `kind` ごとの後始末、**後始末が失敗したら `poison` を書かず `attempt` を進める**規則、後始末の再試行上限（前進上限の2倍）、`terminalReason` への locator の載せ方。**signup は「他 bucket の回収 → `abandon-account` → 自分の予約行の削除 + `poison`」の順序**（第6.4節 3）。
...
- **`jobs.poison` 行の検知と手動再投入** — 本 Issue が決めたのは「`poison` へ落ちる瞬間に設計側が何を後始末するか」（第7.4節「分類 (C) の cross-DO saga 前進ジョブが `poison` に達したときの終端規則」）であって、落ちた行の運用ではない。**#38 が書くのは3つである** — (i) `poison` 行の検知手段（`status = 'poison'` の存在をメトリクスに出す。`terminalReason` を添える）、(ii) 終端規則が **(ii) 側**（`terminalReason` を残して運用エスカレーション）が割り当たっている `kind` — `finalize-withdrawal` / `reindex` / `migrate-bulk` / **`changeState = 'advanced'` の `resume-credential-change`**（巻き戻さない）/ **起点が `'reset'` の `resume-credential-change`**（巻き戻したうえで併せて (ii)）/ **後始末の再試行上限に達した `resume-link` / `resume-signup`**（`terminalReason` に回収できなかった locator が載る）— についての手動再投入手順（第7.4節の `status = 'poison'` への再投入規則が `pending` へ戻すので、operator 経路は「同じ `operationKey` で `enqueueJob` を1回発行する」で足りる）、(iii) `SQLITE_FULL` に起因する `poison` の切り分け（第4.7節。容量監視の項目と対で読む）、**(iv) `poison` 行としては現れない終端の検知**。**(iv) が要る理由を明示する** — 終端規則は `poison` を後始末の成功と同じ `transactionSync` でしか書かないので（第7.4節）、**その書き込み自体が恒久的に失敗する状態（当該 DO の `SQLITE_FULL`）では `poison` も `terminalReason` も残らず、`poison` 行の存在を見る検知が発火しない。** 代わりに見るのは **(a) 容量監視（下の項目）と (b) 同一 `operationKey` の claim が繰り返され続けること**（`jobs` の `attempt` が伸び続けるが `status` が実行可能集合から出ない）の2つである。**#38 はこの2つを `poison` 行の検知と並べて書く。**
...
- **容量監視** — 「本体 + FTS インデックスの合計で 10 GB」の監視閾値、逼迫時の利用者向け導線（第4.6節）。**あわせて「逼迫時の導線がログインを要求する」ことの帰結を書く** — 10 GB に達した利用者がパスワード変更を試みると phase 2 が `SQLITE_FULL` で失敗し続け、`resume-credential-change` の終端規則で巻き戻る（第6.5.1節）。**利用者から見て「パスワードが変わらない」という症状になるので、問い合わせの切り分けに含める。**
...
- **予約 TTL の具体値** — 「**バックオフ上限（前進上限 + 後始末上限。第7.4節の終端規則により前進上限の2倍）に達するまでの総経過時間** + マージン」より長く取るという不等式は設計側で固定済みなので、値だけを決める（第6.4節）。到達不能アカウントの終端（poison + エスカレーション）の運用受け口も含む。
...
本 Issue で結論を出さなかったのは下表の10件だけである。いずれも**誰がいつ決めるかを割り当ててある**。**種類は4つに分かれる** — (i) 未確認の事実で、設計がそれに依存しないよう組んであるもの、(ii) 結論は出ているが依拠する事実が実測1件なので再確認が要るもの（`.adr/003-sqlite-fts5-only-search.md` が「再確認が覆れば本決定そのものが成立しない」と明示している）、(iii) 決定は済んでいて値だけが2段階で決まるもの、**(iv) 設計そのものを別 Issue へ切り出したもの（1件。鍵ローテーションの手順 → #44）**。**「本設計への影響」欄でどれかが分かる。**

| 論点 | 決める主体 | いつ | 本設計への影響 |
|---|---|---|---|
| **鍵ローテーションの手順**（移送の phase・ガード・巻き戻し / 移送中の2世代並存の規則 = 第6.1.1節 (R6)(R7)(R9) / 2つのローテーションの関係と退役条件 / 漏えい起因のローテーションでの失効の成立性）。**唯一「設計そのものを切り出した」種類の行である** | **#44** | #44 の設計時 | **#37 の着手には影響しない。** 初期デプロイでは keyring の世代が1つだけで2世代並存の状態が存在しないので、**#37 は「ローテーション未実行」を前提に着手できる**（第6.8節に #37 / #44 の境界表を置いた）。**方針側（世代付き keyring・`userId` 由来 locator と credential 由来 locator の分離・`credentialId` の世代非依存性・AAD の束縛範囲・`rotationKind` 列）は本 Issue で決着済みであり、#44 はそれを入力として手順だけを設計する** |
