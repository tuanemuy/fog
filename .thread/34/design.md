...
- **#38（運用ドキュメント）は第10章と第11.3節を起点に、そこの各行が指す節をすべて辿る。** ここも件数は書かない（上と同じ理由）。必ず入力として要るのは第6.2.2節（レート制限の具体値）・第6.4節（予約 TTL の不等式）・第6.8節（鍵ローテーションの方針と、手順が #44 へ委譲されていること）・第9.4節（fail-closed の検知）・第10.1節（PITR の設計上の帰結）である。**鍵ローテーションの運用手順は #44 が手順を決着させたあとにしか書けないので、第11.3節の該当項目は「#44 待ち」として区別してある。**
...
分割元 Issue #19 のブランチには DO 実装一式と ADR 10件（`.thread/19/adr.md` の **ADR-001〜ADR-010**。`git show origin/issue/19/cloudflare-do-fts:.thread/19/adr.md` で実取得して数え直した）が残っている。#19 はクローズ済みで #34〜#38 に分割されたので、**これは採用済みの決定ではなく、本 Issue が引き受けるか棄却するかを判断すべき先行案**である。**ADR 10件すべて + レビュー指摘1件 + ADR 由来でない先行実装2件に採否を出した。保留はゼロ** — すべて採用 / 部分採用 / 棄却のいずれかに倒してある。

**表の行数と ADR の件数は一致しない。対応関係を先に書く**（書かないと「10件」と「表の行数」が食い違って見える。第1.4節 I-8）。**下の表は15行**で、内訳は **ADR 由来12行 + ADR 由来でない先行実装2行（行2 = request / state Worker 分割、行4 = 検索 API の詳細。どちらも先行ブランチの実装にあるが `.thread/19/adr.md` に対応する ADR が無い）+ レビュー指摘1行（行13 = B-IDDS6-001）**である。ADR 由来が10行ではなく12行になるのは、**ADR-003 が行5 と行7 に、ADR-006 が行6 と行11 に、それぞれ2行へ分かれている**ためで、**採否が分割の前後で違うので1行にまとめない**（ADR-003 は行5 が採用・行7 が部分採用、ADR-006 は行6 が棄却・行11 が採用）。**行14（最小 DO command harness）はレビュー指摘ではなく ADR-007 である。**
...
| **#19 のスコープを「既存 identity の移行と将来 primitive の確立」までに限定し、password change / reset・SSO link / unlink・export の完成 usecase と UI は #11 / #12 / #15 へ引き継ぐ**（`.thread/19/adr.md` ADR-009） | **棄却（対象消滅）** | **#19 のクローズと #34〜#38 への分割そのものがこのスコープ宣言を置き換えたので、引き受ける対象が無い。** 先行案15行のうち唯一「Issue のスコープ宣言」であって設計判断ではない。本書は「どの usecase をどこまで実装するか」を決めない — 実装範囲は #37 の Issue 本文、spec 側の範囲は #35 が持つ。**なお ADR-009 が #11 / #12 / #15 へ送っていた対象のうち、本書が設計として引き受けたのは「再開可能な saga primitive と非公開 RPC contract」だけである**（第6.3〜6.8節）。完成 usecase / UI は引き続き後続 Issue の領分である |
...
本書には「全数」を名乗る表が7つあり、互いを参照し合っている。**片方を直すともう片方が取り残される**という同じ形の破れが、レビューの R3・R5・R6・R7 の4ラウンドで繰り返し検出された（`jobs` の列の欠落 / 駆動源表の行の欠落 / 投入点の不在 / RPC エントリの分類漏れ / UoW 書き込み口が7ストア中3つ / 秘密の個数の不一致 / **`rotation_checkpoints` に区別列が無く2つのローテーションが同じ行を潰し合う** / **`reindex` のカーソルにどの表にも置き場が無い** / **第1.3節の数え上げが表と合わない**）。**個別に直すのをやめ、表どうしが満たすべき不変条件を明文化し、機械で検査できる形にする。本節がその正本である。**

**R7 の3件は、検査そのものに穴があったからすり抜けた。** 穴は3つで、いずれも本節で塞いである — (i) **I-4 を「表 → 本文」の向きだけで検査していた**（テーブル**名**の実在しか見ず、本文が要求する**列**が E-1 にあるかを見なかった）→ **検査8** を足した。(ii) **E-2 の「書き込み箇所」欄が本文と一致するかを検査していなかった** → **検査9** を足した。(iii) **検査7 の grep が5パターン決め打ちで、散文中の数え上げ（第1.3節が先行案の件数を数えている箇所）を拾わなかった** → **検査7b** を足し、対応する表の行数を同じ手順で数えて突き合わせる形にした。あわせて **I-2 の重複検出**（同じ `kind` が2つの分類表に現れないこと）を検査2 に足した。

**R8 では検査7b 自身が I-8 を破っていた。** 注記に書いた期待値（`-> 10`）が実測（12）と食い違い、しかも**本節が `body` の走査対象外なので機械では検出できなかった**。したがって **検査7b から「注記に数を書く」形そのものを廃止し、左辺（本文の数）と右辺（表の行数）を両方コマンドで出して `ok` 関数で突き合わせる形へ変えた。** これで注記が陳腐化する余地が構造的に無くなり、食い違いは `NG` 行として機械検出される。
...
- **(I-1) E-3 の各行の投入点欄は、そのジョブ行を最初に書く箇所を全数で名指しする。** 「そのジョブ行を最初に書く箇所」が本書の節として特定でき、投入点を持たない `kind` は存在しない。**投入点が2つ以上ある `kind` は、その全部を欄に列挙する** — 「ある投入点」しか名指ししていない欄は違反である。**欄が書き手を取りこぼすと、そのジョブを投入しているもう一方の機構が設計の視界から外れる**（I-5b が `rotation_checkpoints` について特定したのとまったく同じ破れ方である。R10 の `finalize-withdrawal` の欄が第5.1節の `abandon-account` エントリを取りこぼしていたのがこの形で、**「欄が空でないこと」しか見ていなかった検査2 では構造的にすり抜けた**）。**空欄は検査2 の前半が、取りこぼしは検査2 の後半（逆向き）が見る。**
- **(I-2) E-3 の各行は再武装分類 (A) / (B) / (C) のちょうど1つに属する。** **(A) 時刻駆動の周期ジョブ**は E-4 の (1-A) 表にちょうど1行を持つ。**(B) 残件駆動の反復ジョブ**は E-4 の (1-B) 表にちょうど1行を持つ。**(C) 一回性ジョブ**はどちらの表にも行を持たない。**「ちょうど1つ」は両向きで見る** — E-3 の分類欄が2つの分類を同時に名乗る行が無いこと、および (1-A) と (1-B) の両方に現れる `kind` が無いことの2つを検査する（分類欄だけを数えると、後者はすり抜ける）。
- **(I-3) E-3 の `kind` 集合と、E-1 の `jobs` 2行が列挙する `kind` 集合は一致する。** どちらにも `rotate-remap` は現れない（Alarm ジョブではないため。第7.4節）。
- **(I-4) E-3 / E-4 が名指しするテーブルと列は E-1 に実在する。逆向きも同じ強さで見る — 第6〜9章の本文が要求する列が E-1 に無ければ違反である。** 直す向きは E-1 の側である（第4.1.1節のタイブレーク規則）。**逆向きは目視に落とさない** — 認証・saga・ジョブ系テーブルの列を `テーブル:列` の一覧として検査8 に持ち、E-1 の当該行に実在するかを機械で見る。**一覧の件数は注記に書かず、検査8 に併記した再カウントコマンドが出す**（I-8 の「表を持たない列挙には数を書かない」に揃える。R9 では注記の `61` に添えた再カウントコマンドが自己参照で終端せず 2,242 行を出す状態だった）。**本文が新しい列を導入したら、E-1 と同時に検査8 の一覧にも足す。**
- **(I-5) E-2 の7ストアはそれぞれ、E-6 が列挙する口のいずれかにちょうど1回対応する。** ただし `_meta` だけは「UoW からの口を持たない（アダプター専用）」という値を取る。**この1件が唯一の例外である。**
  - **(I-5b) E-2 の「書き込み箇所（全数）」欄は、本文が「そのストアへ書く」と述べている書き手をすべて名指しする。** 例外は `jobs` の1件だけで、こちらは投入点の全数を E-3 の投入点欄へ**委譲する**（同じ列挙を2箇所に持たない）。**欄が書き手を取りこぼしていると、そのストアを共有している別の機構の存在が設計の視界から外れる** — R7 の `rotation_checkpoints`（`rotate-encryption` が欄から漏れており、2つのローテーションが同じ表を共有している事実が見えていなかった）がこの形の破れである。
...
- **(I-8) 本書が「N つの秘密」「N 種」「N 件」「N 行」「N 経路」と数える箇所は、対応する表の行数と一致する。** 数を書くのは表を持つ箇所だけにし、表を持たない列挙には数を書かない（第1.1節が読者導線について既に置いている規則を、全数表へ広げたものである）。
  - **射程は E-1〜E-7 に限らない。本書のどこであれ、表を持つ数え上げはすべて対象である。** R7 の第1.3節（先行案の数え上げが、直下の表の行数とも先行ブランチの実体とも食い違っていた）は E-1〜E-7 の外側で起きた。したがって検査7 は**表の行数を計算して突き合わせる側**（検査7b）を持つ。
  - **数え上げの単位が表の行数と一致しない場合は、対応関係を同じ段落に書く。** 第1.3節が「ADR 10件」と「表15行」の両方を持つのがその形である（1つの ADR が採否の違いで2行に分かれる／ADR 由来でない行がある）。**対応関係を書かずに片方だけを載せてはならない。**

#### 検査手順

リポジトリルートで実行する。**すべて「出力が期待どおりであること」で判定し、目視の読み下しに依存しない。** 表の行はヘッダ行の先頭語で選ぶ。**検査は1から9まで上から順に実行する** — 検査1 が作る `/tmp/e3.txt`（`kind` の全数）を検査4 と検査9 が、検査6 が作る `/tmp/e1tbl.txt` を検査8 が入力にするためである。

**各コードブロックは実ファイルから逐語でコピーして貼れる。字下げを手で剥がす前処理は要らない。** どのブロックも箇条書きの中にあるので実ファイル上は2スペース字下げされているが、字下げが値に混入しうる2箇所（検査8 の `COLS` と検査9 の `SKIP` のヒアドキュメント）は**読む側で剥がす処理を検査自身が持っている**。`tbl()` が `sub(/^[ \t]+/,"",l)` を持つのと同じ前提である。**この前処理を落とすと検査8 は62行、検査9 は4行の偽陽性を出す**（R10 で実際に「E-1 に列が1つも無い」という見え方の誤検出が報告された）。
...
- **検査2（I-1 / I-2）** — 投入点欄が空の行が無く、分類 (A)(B)(C) の割り当てが E-4 の2表と一致する。
  ...
  # 重複検出（I-2 の「ちょうど1つ」の逆向き）— 分類欄を数えるだけでは拾えない
  tbl '| `kind` | 所有者 |' | awk -F'|' '{n=0
      if ($5 ~ /\(A\) 時刻駆動/) n++; if ($5 ~ /\(B\) 残件駆動/) n++; if ($5 ~ /\(C\) 一回性/) n++
      if (n != 1) print "分類がちょうど1つでない:", $2, "(" n ")"}'   # 期待: 出力なし
  cat /tmp/e4a.txt /tmp/e4b.txt | sort > /tmp/e4all.txt
  diff <(sort -u /tmp/e4all.txt) /tmp/e4all.txt && echo 'I-2 重複なし OK'   # (1-A)/(1-B) の両方に出る kind が無い
  # I-1 の逆向き（投入点欄の取りこぼし）— 「欄が空でない」だけでは2つ目の投入点の欠落がすり抜ける
  cell() { tbl '| `kind` | 所有者 |' | grep -F "| \`$1\` |" | awk -F'|' '{print $4}'; }
  # (a) 本文の `enqueueJob('<kind>'` の出現節が、その kind の投入点欄に現れる
  awk -v q="'" '/^#{3,4} [0-9]/{s=$2}
      match($0, "enqueueJob\\(" q "[a-z-]+" q) {print s, substr($0, RSTART+12, RLENGTH-13)}' "$D" \
    | sort -u | while read -r s k; do
        printf '%s' "$(cell "$k")" | grep -qF "第${s}節" \
          || echo "投入点欄に無い enqueueJob: $k <- 第${s}節"
      done                                            # 期待: 出力なし
  # (b) E-7（第5.1節の RPC エントリ表）が「その kind を投入する」と書いている行があれば、欄が第5.1節を名指しする
  tbl '| クラス | エントリ | 所属 DO |' | grep -F 'を投入' | grep -oE '`[a-z-]+`' | tr -d '`' \
    | grep -Fx -f /tmp/e3.txt | sort -u | while read -r k; do
        printf '%s' "$(cell "$k")" | grep -qF '第5.1節' \
          || echo "投入点欄に無い RPC 由来の投入点: $k <- 第5.1節"
      done                                            # 期待: 出力なし
  ```

  **逆向きを `enqueueJob('<kind>'` の literal と E-7 の行に限る理由。** 「本文に `` `<kind>` `` と『投入』が同居する箇所」を素朴に拾うと、**他の節の投入点を参照している文**（第6.6節の「link 手順1 が `resume-link` を、退会 手順1 が `finalize-withdrawal` を…投入するのとまったく同じ形である」、第6.9節の塞ぎ方欄）と**定義文そのもの**（本節・第7.4節・第8.2節の `enqueueJob(kind: JobKind, ...)`）が混ざり、検査が常に非空になって無力化する。**この2つの形に限ると、どちらも「その節が実際にジョブ行を書く」ことの機械的な印になる。**
...
- **検査5（I-6）** — クラス (3) の行数と4群の割り当て数が一致する。
  ...
  ```bash
  tbl '| クラス | エントリ | 所属 DO |' | grep -c '^| (3) |'   # 期待: 14
...
- **検査7a（I-8 — 表と1対1で対応する数え上げ）** — 数を書いている箇所を洗い出し、対応する表の行数と突き合わせる。

  ```bash
  grep -nE '新設する[0-9]+つの秘密|新設[0-9]+秘密|第7\.4節の[0-9]+列|同じ[0-9]+列|[0-9]+種である' "$D"
  ```

  現在の期待値は次の5種で、**grep が返す主張はこれで全数である**（ヒット行の主張が下の5種に収まらなくなったら、その主張を本段落に足す）。**(1) 新設する秘密は4つ**（request 側2 = `AI_CLIENT_TOKEN_SECRET` / `DIRECTORY_ROUTING_SECRET`、state 側2 = `IDENTITY_MAIL_ENCRYPTION_KEY` / `IDENTITY_RESET_TOKEN_KEY`。第3.2節の表の state 側3行目はバインディングであって秘密ではない）。**(2) `jobs` は12列**（第4.1.1節・第7.4節）。**(3) `kind` は各クラス6種**（E-1 の `jobs` 2行）**・合計12種**。**(4) 再武装分類 (A)(B) は合計5種**（第7.4節の再武装規則の射程）。**(5) 永続カーソルを持つのは2種**（`migrate-bulk` / `reindex`。第7.4節 (iii)）。

- **検査7b（I-8 — 散文中の数え上げ）** — 決め打ちの5パターンでは拾えない数え上げを洗い出し、**対応する表の行数を同じ手順で数えて突き合わせる。突き合わせもコマンドで行い、期待値を注記として書かない。** R8 でこの検査自身が I-8 を破っていた（注記の `-> 10` が実測12と食い違い、しかも本節が走査対象外なので機械検出できなかった）ので、**注記に数を書く形そのものを廃止する。** 左辺（本文の数）も右辺（表の行数）も両方コマンドが出し、`ok` が `NG` を返すことで検出する。

  ```bash
  # 本節（第1.4節）自身は検査の定義文なので走査対象から外す。第1.3節は `body` の外なので別に取る
  body() { awk '/^## 2\. /{f=1} f' "$D"; }
  sec13() { awk '/^### 1\.3 /{f=1} /^### 1\.4 /{f=0} f' "$D"; }
  # 数え上げの本体は「N行 / N件 / N経路」の形なので、節番号（第11.4節 など）を拾わないよう単位ごと切り出す
  ...
  P='全[0-9]+件|レビュー指摘[0-9]+件|下の[0-9]+行|実行数[0-9]+行|次の[0-9]+経路|下表の[0-9]+件|第11\.4節は[0-9]+行|表は[0-9]+行'
  { sec13; body; } | grep -cE "$P"     # 期待: 下の ok の呼び出し回数（8）と一致する
  ok '第1.3節 表の行数'     "$(sec13 | num '表は[0-9]+行')"           "$(rows '| 先行案 | 採否 |')"
  ok '第1.3節 レビュー指摘' "$(sec13 | num 'レビュー指摘[0-9]+件')"   "$(tbl '| 先行案 | 採否 |' | grep -c 'レビュー指摘')"
  ok '第3.2節 秘密配布表'   "$(body  | num '表は[0-9]+行')"           "$(rows '| Worker | 持つ秘密 |')"
  ok '第4.3節 帰属表'       "$(body  | num '実行数[0-9]+行')"         "$(rows '| # | カテゴリ | 箇所 |')"
  ok '第6.9節 締め出し経路' "$(body  | num '次の[0-9]+経路')"         "$(rows '| 締め出す経路 | 塞ぎ方 |')"
  ok '第11.2節 新設列表'    "$(body  | num '下の[0-9]+行')"           "$(rows '| 列 | テーブル | テーブル定義の外に生む作業 |')"
  ok '第11.4節 未決事項(a)' "$(body  | num '第11\.4節は[0-9]+行')"    "$(rows '| 論点 | 決める主体 |')"
  ok '第11.4節 未決事項(b)' "$(body  | num '下表の[0-9]+件')"         "$(rows '| 論点 | 決める主体 |')"
  ```

  **期待は「`NG` が1行も出ないこと」と「grep のヒット数が `ok` の呼び出し回数（8）と一致すること」の2つだけである。** ヒット数が増えたら、その数え上げに対応する `ok` を1本足す（足せない＝対応する表が無い数え上げは、I-8 により本文から数を落とす）。**`ADP-*` の件数（第4.3節の「53件」「85件」「32件」）だけは表の行数ではなく `spec/inventory/adapter.md` の実測なので、同節が持つ再走査コマンドで確認する**（`grep -o 'ADP-[A-Za-z0-9-]*' spec/inventory/adapter.md | sort -u | wc -l`）。`.thread/34/testing.md` の確認項目11 が別途この照合を持つ。

- **検査8（I-4 の逆向き）** — 第6〜9章の本文が要求する列が E-1 に実在する。**テーブル名だけを見る検査6 では拾えない。**

  ```bash
  tbl '| DO クラス | テーブル |' > /tmp/e1tbl.txt
  # 一覧はヒアドキュメントではなく変数に持つ。ヒアドキュメントにすると、実ファイル上は
  # このブロックが箇条書きの中にあって終端行が `  COLS` と字下げされるため、逐語で貼ると
  # 終端に一致せず heredoc が閉じない（レンダリング後の表示からコピーしたときだけ動く）
  COLS='account:status
  account:sessionEpoch
  account:callerToken
  account:version
  account:resetVersion
  user_settings:version
  credential_locators:credentialId
  credential_locators:hmac
  credential_locators:generation
  credential_locators:bucketIndex
  credential_locators:credentialVersion
  credential_locators:usableForLogin
  credential_locators:label
  ai_client_connections:version
  ai_client_connections:createdAtResetVersion
  jobs:operationKey
  jobs:payloadDigest
  jobs:attempt
  jobs:nextRunAt
  jobs:leaseUntil
  jobs:ownerToken
  jobs:providerIdempotencyKey
  jobs:terminalReason
  jobs:completedAt
  operations:operationId
  operations:phase
  operations:targetLocators
  migration_progress:targetVersion
  migration_progress:step
  migration_progress:cursor
  credential_mappings:credentialId
  credential_mappings:passwordVerifier
  credential_mappings:pendingVerifier
  credential_mappings:changeState
  credential_mappings:changeOrigin
  credential_mappings:credentialVersion
  credential_mappings:encryptedCanonical
  credential_mappings:encryptionGeneration
  credential_mappings:encryptionNonce
  credential_mappings:failedAttempts
  credential_mappings:nextAttemptAllowedAt
  credential_mappings:lastResetRequestedAt
  credential_mappings:reservedUntil
  credential_mappings:sagaCommitted
  credential_mappings:coordinatorLocator
  credential_mappings:callerToken
  password_reset_tokens:tokenId
  password_reset_tokens:tokenHash
  password_reset_tokens:expiresAt
  password_reset_tokens:usedAt
  password_reset_tokens:credentialId
  password_reset_tokens:consumedByOperationId
  password_reset_tokens:changeAuthToken
  password_reset_tokens:tokenKeyGeneration
  rotation_checkpoints:rotationKind
  rotation_checkpoints:bucketIndex
  rotation_checkpoints:previousCount
  rotation_checkpoints:scannedAt
  rotation_checkpoints:conflictCount
  rotation_checkpoints:lastConflictAt
  rotation_checkpoints:lastConflictCredentialId
  _meta:schema_version'
  # 変数の各行は実ファイル上では字下げされているので、読む側で剥がす（`IFS=:` は
  # 先頭の空白を落とさないため、剥がさないとテーブル名が `  account` になって全行 MISSING になる）
  cols() { printf '%s\n' "$COLS" | sed -E 's/^[[:space:]]+//'; }
  cols | while IFS=: read -r t c; do
    grep -E "^\| (User Data DO|Identity Directory DO) \| .*\`$t\`" /tmp/e1tbl.txt \
      | grep -qF "\`$c\`" || echo "MISSING column in 4.1.1: $t.$c"
  done                      # 期待: 出力なし（`MISSING column in 4.1.1:` が1行も出ない）
  cols | grep -c ':'        # 一覧の件数。ループと同じ変数を源にするので構造的に一致する
  ```

  **件数の期待値を注記に書かない**（I-8）。上の `cols | grep -c ':'` が出す数と `while` ループが読んだ行数は**同じ変数を源にする**ので構造的に一致する。**本文が新しい列を導入したら一覧に1行足すだけでよく、注記を直す作業は発生しない。**

  **一覧をヒアドキュメントで持たず変数で持つ理由は、逐語実行できるようにするためである。** このコードブロックは箇条書きの中にあるので実ファイル上は2スペース字下げされており、**ヒアドキュメントだと終端行が `  COLS` になって `<<'COLS'` に一致せず、貼った時点で heredoc が閉じない**（Markdown レンダラは箇条書き内のフェンスを字下げなしで表示するので、レンダリング後の表示からコピーしたときだけ動いていた）。**変数なら閉じクォートが字下げされていてもよく、字下げは値に混ざるだけなので読む側の `sed` で剥がせる。** R8 / R9 / R10 の3ラウンド連続でこの検査自身が壊れていた（R8: 注記の期待値 `-> 10` が実測12と食い違い。R9: 再カウント awk が `<<.COLS.` に自己ヒットして 2,242 行を出す。R10: 字下げ混入で62行の偽陽性）ので、**注記にも `"$D"` の自己走査にも依存しない形へ変える。検査本体（`MISSING column` の検出）はどのラウンドでも正しく動いていた。**

- **検査9（I-5b）** — E-2 の「書き込み箇所（全数）」欄が、本文で同じ行に現れる書き手（E-3 の `kind`）を取りこぼしていない。

  ```bash
  # 除外リストも検査8 と同じ理由で変数に持ち、読む側で字下げを剥がす
  SKIP='operations:rotate-encryption
  operations:sweep-reservations
  credential_locators:sweep-orphan-mapping
  credential_locators:sweep-reservations'
  printf '%s\n' "$SKIP" | sed -E 's/^[[:space:]]+//' > /tmp/e2skip.txt
  # `jobs` は投入点の全数を E-3 へ委譲しているので対象外（空欄は検査2 が見る）
  # 本節（第1.4節）自身は検査の定義文なので走査対象から外す
  body() { awk '/^## 2\. /{f=1} f' "$D"; }
  for s in operations migration_progress credential_locators password_reset_tokens rotation_checkpoints; do
    cell=$(tbl '| 非集約ストア | 所属 |' | grep -F "| \`$s\`" | awk -F'|' '{print $5}')
    body | grep -v '^|' | grep -F "\`$s\`" | grep -oE '`[a-z-]+`' | tr -d '`' | sort -u \
      | grep -Fx -f /tmp/e3.txt | while read -r w; do
          printf '%s' "$cell" | grep -qF -- "$w" && continue
          grep -qx "$s:$w" /tmp/e2skip.txt || echo "E-2 の欄に無い書き手候補: $s <- $w"
        done
  done   # 期待: 出力なし
  ```

  **`/tmp/e2skip.txt` は「同じ行に名前が出るが、そのストアへは書かない `kind`」の除外リストである。** 現在の4件はいずれも本文で書き手でないことを確認済み — `rotate-encryption` / `sweep-reservations` は `operations` 行を読みも書きもせず（前者は `credential_mappings` の再暗号化、後者は予約行の掃除）、`sweep-orphan-mapping` / `sweep-reservations` は Directory 側の行だけを触るので `credential_locators`（User Data DO）へは書かない。**除外リストへ足してよいのは、本文を読んで「書かない」ことを確認できた組だけである。確認せずに足すと、この検査は R7 の `rotation_checkpoints` と同じ形で無力化する。**
...
**配布は非重複である。** request Worker 側の**3つの秘密**（`SESSION_SECRET` / `AI_CLIENT_TOKEN_SECRET` / `DIRECTORY_ROUTING_SECRET`）を state Worker に置かず、state Worker 側の**2つの秘密**（`IDENTITY_MAIL_ENCRYPTION_KEY` / `IDENTITY_RESET_TOKEN_KEY`）とメール送信プロバイダのバインディングを request Worker に置かない。**上の表は6行あるが、秘密は5つでありメール送信バインディングは秘密ではない** — バインディングにはブランド型も最小長も keyring 検査も適用できないので、数えるときに混ぜない（第1.4節 I-8）。**鍵ローテーションの移送だけは、この配布境界の上でどう鍵を扱うかが未決である。** 本 Issue が決着させたのは制約2点（**平文 canonical を Worker 境界の外へ bulk で出さない / routing 鍵を bucket の SQLite・インスタンスフィールドへ永続化しない**）だけで、**実行主体と鍵の渡し方（一時注入という形そのものを含む）と、渡された鍵を bucket が検証する手段は #44 が決める**（第5.2.3節・第6.8節の方針4・論点2）。**「bucket が routing secret をバインディングとして保持しない」という本節の境界を引き直すかどうかも、#44 の論点2 に含まれる。** #44 が取りうる解として第5.2.3節・第6.8節が挙げている2つ（**bucket に routing keyring を配る** / 専用の maintenance Worker を立てて境界を引き直す）のうち前者は、意味するところが**常設のバインディング**以外にありえないので、まさに本節の非重複配布そのものの見直しである。**引き直す場合は、本節の非重複配布と第6.8節の方針5（`rotate-remap` を Alarm ジョブにしない）を同時に見直す判断が要り、その判断は #44 が下す。** 本 Issue が確定させたのは**方針4 の制約2点**（平文 canonical を Worker 境界の外へ bulk で出さない / routing 鍵を bucket の SQLite・インスタンスフィールドへ**永続化**しない）だけである。**制約 (ii) は永続化の禁止であってバインディングの禁止ではない**（第5.2.3節）ので、それだけでは前者の解を排除しない — **排除しているのは本節の非重複配布であり、それが #44 の論点だという以上、本節の側で「どの形を選んでも変わらない」と断定してはならない。** 初版はそう断定しており、第5.2.3節・第6.8節・#44 の Issue 本文の3箇所と食い違っていた。
...
|---|---|---|---|
| User Data DO | `account` | `status`（`active` / `deleting` / `deleted`）/ `sessionEpoch` / `deletedAt` / **`callerToken`**（DO 間 RPC の呼び出し元束縛。第5.1節）/ **`resetVersion`**（**パスワードリセット完了だけで進む単調増加カウンタ。** 初期値0。`credentialVersion` とは別の列であり、通常のパスワード変更・SSO link / unlink では進まない。AI クライアント接続の自動失効の基準。第5.4節 (i)・第6.5.1節 phase 2）/ **`version`**（OCC。第8.4節）。非 PII tombstone は退会後もこの行として残る（`callerToken` は退会完了時に消す） | 第3.1節・第5.1節・第5.4節・第6.7節 |
...
| User Data DO | `ai_client_connections` | `id` / `clientName` / `scope` / `status` / `connectedAt` / `lastUsedAt` / `revokedAt` / **`createdAtResetVersion`**（作成時点の `account.resetVersion`。リセット完了時の自動失効に使う。**`credentialVersion` を基準にしない** — 侵害と復旧のあいだにパスワード変更が1回でも挟まると失効対象が0件になるからである。第5.4節 (i)）/ **`version`**（OCC。第8.4節。**設定画面からの二重解除操作の競合を検出する対象そのものである**）。ただし `recordUsage` による `lastUsedAt` の単独更新は `version` を進めない後勝ち更新にする（`spec/database/index.md` の現行規約を維持する） | 第5.4節・第6.5.1節 phase 2・第8.4節 |
...
| User Data DO | `migration_progress` | `targetVersion` / `step` / `cursor` / `updatedAt`。**主キーは `(targetVersion, step)`。** **`migrate-bulk` と `reindex` の2種が共有する**（同じ `targetVersion` から両方が投入されても行が衝突しない。第7.4節「カーソルの永続先」・第9.2節） | 第7.4節・第9.2節・第9.3節 |
| User Data DO | `_meta` | `schema_version` / 自 locator（`userId`。`ctx.id.name` のフォールバック） | 第6.3節・第9.2節 |
| Identity Directory DO | `credential_mappings` | **識別**: **`credentialId`**（世代非依存の credential 同一性。第6.1.2節）/ `kind`（`email` / `sso`）/ `hmac`（全長64 hex）/ `generation`。一意性は `(kind, hmac)` で取る（第6.1節 (b)）。**`credentialId` にも bucket 内 UNIQUE を張る。** 根拠は Directory DO の命名規則である — DO 名は `dir:g{generation}:b{index}` で**世代を含む**ので（第6.1節 (d)・第6.2節）、世代が違えば bucket index が一致しても別の DO インスタンスであり、**1つの Directory DO の `credential_mappings` に載る行は常に同一世代である**。したがって bucket 内では `(credentialId, generation)` UNIQUE と `credentialId` UNIQUE が等価になる。`(credentialId, generation)` の UNIQUE も残してよいが、`generation` 列は DO 名と冗長なので実効は変わらない。**初版は「新旧2世代の locator が同じ bucket index に落ちると同じ bucket に2行並ぶ」を根拠に非一意と書いていた。これは DO 名が世代を含むという自分の決定と矛盾しており、事実として誤りである** — 2世代が同居するのは `credential_locators`（User Data DO 側。1 DO に両世代が載る）であって `credential_mappings` ではない。`consume-reset-token` / `delete-mapping` / トークン一括削除はこの UNIQUE 索引から引く。**写像**: `userId` / `status`（`reserved` / `active`）。**認証材料**: `passwordVerifier`（**書き込み点は2つだけである** — signup phase 1a の予約行（`reserve-credential` の引数。第6.3節）と credential 変更 saga の phase 3（`pendingVerifier` からの昇格。第6.5.1節）。**SSO signup がメール一意性のために置く行には載せない**ので、その行の `usableForLogin` は偽になる）/ `pendingVerifier` / `changeState`（**`null` / `pending` / `advanced` の3値**。`pending` は phase 1 が書き、`advanced` は phase 2 の成功を bucket 側に記録する値で phase 2 の直後に書く。phase 3 が `null` へ戻す。**`null` でない値はどちらも `lookup-credential` でダミー材料へ倒す** — 2値を区別するのは **(1) `promote-verifier`（phase 3）のガード**（第5.1節）**と (2) 別の `operationId` による新しい依頼の後勝ち規則**（第6.5.1節）の2つであり、**(3) #45 が設計する自動回収も同じ値を「phase 2 が適用済みか」の判定材料に使う**（第7.4節の前方互換点4））/ **`changeOrigin`**（`null` / `'password-change'` / `'reset'`。**credential 変更 saga の起点を永続化する列である。** phase 1 が `changeState = 'pending'` と同じ `transactionSync` で書き、phase 3 が `changeState` を解除するのと同じ `transactionSync` で `null` へ戻す。**この列が無いと phase 2 が `resetVersion` を進めるかどうかを Alarm 再開経路で決められない** — `resume-credential-change` は mapping 行だけを入力に駆動するので、起点が行に無ければ復元できない。第6.5.1節 phase 1・phase 2）/ `credentialVersion`。**PII**: `encryptedCanonical` / `encryptionGeneration` / **`encryptionNonce`**（AES-256-GCM の96ビット nonce。**独立列に持ち、暗号文に連結しない** — AAD が `(kind, credentialId, encryptionGeneration)` を束縛するので、nonce の切り出し規則を暗号文の形式に埋め込むと再暗号化ジョブが世代ごとに別の切り出しを持つことになる。行ごと・書き込みごとに再生成し、使い回さない。第6.2.1節 (b-2)）。**濫用抑止**: `failedAttempts` / `nextAttemptAllowedAt` / `lastResetRequestedAt`。**saga コーディネーター状態**: `operationId` / `candidateUserId` / **`reservedUntil`**（予約 TTL の絶対時刻。**NOT NULL であり、予約行を書く3箇所すべてが値を書く** — signup phase 1a / 1b と SSO link 手順2。第6.3節・第6.6節）/ `sagaCommitted` / `locators[]`（コーディネーター行が持つ全 credential の locator 一覧。要素は `credentialId` + `kind` + 全長 HMAC + 世代 + bucket index）/ `coordinatorLocator`（非コーディネーター行が持つ）。**呼び出し元束縛**: **`callerToken`**（この mapping の所有アカウントが提示すべき不透明値。User Data DO 側の `account.callerToken` と同じ値。第5.1節） | 第6.1節・第6.1.2節・第6.2.1節・第6.2.2節・第6.3節・第6.4節・第6.5.1節 |
| Identity Directory DO | `password_reset_tokens` | `tokenId`（**暗号論的乱数由来の128ビット以上の不透明値**。bucket 内で採番する。**連番・rowid・時刻由来の値を使わない** — 理由は第6.1節 (d)）/ `tokenHash` / `expiresAt` / `usedAt` / **対象 credential のキー `credentialId`**（第6.1.2節）/ **`changeAuthToken`**（`consume-reset-token` が消費時に採番する**128ビットの暗号論的乱数**。`begin-credential-change` の**起点 B の必須ガード**であり、束縛の実体はこの列だけである。**成功時に同じ `transactionSync` で `NULL` へ戻す**（`値 → NULL` の CAS）ので一回性である。**一回性は失効ではないので、失効経路を別に持つ** — credential 変更 saga の phase 1 が、対象 `credentialId` の**残る全行**（＝消費済みで未使用の `changeAuthToken` を持つ行）についてこの列を `NULL` にする（第6.5.1節 phase 1・第6.1節 (d)）。**列が `NULL` の行は照合で常に不一致とする**（下記の共通規則。第5.1節 (3-b)）。第5.1節・第6.5.1節。第5.2節 (c) の非露出対象）/ **`consumedByOperationId`**（`consume-reset-token` が消費時に記録する。**監査用の記録であって束縛材料ではない** — 第5.2節 (c) が未認証経路のログへ出すことを許している値なので capability にできない。第5.1節 (3-a)）/ **`tokenKeyGeneration`**（`IDENTITY_RESET_TOKEN_KEY` の世代。**routing 世代とは別の番号体系である**。第6.1節 (d)）。credential キー `credentialId` に索引を張り、credential 変更 / unlink / 退会がトークン行を一括処理できるようにする。**削除の射程は経路ごとに違う。credential 変更 phase 1 は「未使用行を全削除し、残る全行の `changeAuthToken` を `NULL` にする」の2段、unlink 手順3 と退会 手順3 はその `credentialId` の行を `usedAt` の有無を問わず全削除である**（第6.1節 (d)・第6.5.1節 phase 1・第6.6節 unlink 手順3・第6.7節 手順3）。**キーを `(kind, hmac)` にしない理由は第6.1.2節にある** — `hmac` は世代依存なので、ローテーション中に発行されたトークンが世代の違う削除要求から漏れる。**生トークンは保存しない**（`tokenId` から導出する） | 第6.1節 (d)・第6.1.2節 |
| Identity Directory DO | `jobs` | User Data DO 側と同じ12列（`completedAt` を含む）。`kind` は `send-mail` / `resume-signup` / `resume-credential-change` / `sweep-reservations` / `sweep-reset-tokens` / `rotate-encryption` の6種である。**`rotate-remap` は Alarm ジョブではない** — routing key を Alarm 起動時に保持できないためである（第6.8節。移送の実行主体そのものは #44 が決める） | 第7.4節 |
| Identity Directory DO | `rotation_checkpoints` | **`rotationKind`**（`'remap'`（routing 鍵のローテーション）/ `'encryption'`（メール暗号鍵のローテーション）の2値。**この列が無いと2つのローテーションの記録が同じ行を潰し合う** — 詳細と置換キーは第6.8節「「旧世代 locator が0件である」ことの証明の枠組み」）/ `bucketIndex` / `generation`（**`rotationKind` によって意味が違う。** `'remap'` では退役させる routing 世代、`'encryption'` では退役させる `encryptionGeneration` であり、**2つは独立した番号体系である**。第6.2.1節 (b-1)）/ `previousCount` / `scannedAt`（**置換**で記録する。**置換キーは `(rotationKind, bucketIndex, generation)` である**）/ **`conflictCount`** / **`lastConflictAt`** / **`lastConflictCredentialId`**（移送先に `userId` の違う行があって移送を見送った件数と、直近の発生時刻・対象。**`rotationKind = 'remap'` の行だけが使う**（`rotate-encryption` は行を移送しないので衝突が発生しない）。移送先に `userId` の違う行があって移送を見送った場合の記録先である。**移送は `jobs` 行も `operations` 行も持たないので `poison` / `terminalReason` を記録先にできない** — この3列がその代わりである。**記録の契機と読み方は #44 が決める**（第6.8節の論点1）） | 第6.2.1節 (b-1)・第6.8節 |
...
- **`credential_mappings` は OCC の `version` を持たない、と断定する。** Directory 側の書き込みはすべて `operationId` / `changeState` / `status` / `sagaCommitted` を条件に含む CAS で直列化されており（第6.3節・第6.5.1節）、**同じ行に対する「読んで判断して書く」がリクエストを跨がない**。分裂した認証情報側に汎用の OCC を重ねると、CAS 条件と `version` のどちらが権威かが二重になる。**したがって `UserRepository` を割った2つのポートのうち、Directory 側だけは `ExpectedVersion` を取らない。**
...
**「書き込み箇所（全数）」欄は書き手を名指しする**（第1.4節 I-5b）。**唯一の例外が `jobs` で、こちらは投入点の全数を第7.4節の `kind` 全数表へ委譲する** — 同じ列挙を2箇所に持つと必ず片方が取り残されるからである。**欄が書き手を取りこぼすと、そのストアを共有している別の機構が設計の視界から外れる**（`rotation_checkpoints` の欄から `rotate-encryption` が漏れていたために、2つのローテーションが同じ表を共有している事実が見えていなかった。第6.8節）。

| 非集約ストア | 所属 | UoW からの書き込み口 | 書き込み箇所（全数） |
|---|---|---|---|
| `jobs` | 両クラス | `enqueueJob`（副作用登録メソッド） | 全ジョブの投入点（第7.4節の `kind` 全数表の「投入点」欄へ**委譲する。本欄が唯一の委譲である**）。claim / 完了 / backoff / prune はジョブランナー（アダプター）が同じ行に対して行う |
| `operations` | User Data DO | `recordOperation` / `updateOperation` | **`recordOperation`**: signup phase 2（`initialize-account`）/ link 手順1 / unlink 手順2 / credential 変更 phase 2（`advance-credential-change`）（第6.3節・第6.6節・第6.5.1節）。**`updateOperation`**: `resume-signup` / `resume-link` / `resume-credential-change` / `sweep-orphan-mapping` / `finalize-withdrawal` の phase 前進と終端（第6.4節・第6.6節・第6.5.1節・第6.7節） |
| `migration_progress` | User Data DO | `setMigrationCursor` | `migrate-bulk` と **`reindex`** のカーソル前進（第9.3節・第7.4節 (iii)）。**この2種が永続カーソルを持つ全数である** |
| `credential_locators` | User Data DO | `credentialLocatorStore` ポート | `record-credential-locator` の upsert（signup phase 4 = `resume-signup` / link 手順4 = `resume-link`。第5.1節）/ unlink 手順2 と退会 手順4（`finalize-withdrawal`）の削除（第6.6節・第6.7節）/ credential 変更 phase 2（`resume-credential-change` → `advance-credential-change`）の全世代更新（第6.5.1節）。**移送による追加と旧世代行の削除も同じ口を使うが、移送側の呼び出し規約は #44 が定める**（第6.8節） |
| `password_reset_tokens` | Identity Directory DO | `resetTokenStore` ポート | リセット依頼時の発行と**同じ `transactionSync` での未使用行の全削除**（第6.1節 (d) の無効化点の1つ目）/ `consume-reset-token` の消費記録（第6.1節 (d)・第5.1節）/ credential 変更 phase 1・unlink 手順3・退会 手順3 の一括削除（第6.1節 (d)）/ `sweep-reset-tokens` の期限切れ掃除（第7.4節）。**無効化を発行する箇所は4つで、その全数は第6.1節 (d) が持つ** |
| `rotation_checkpoints` | Identity Directory DO | `rotationCheckpointStore` ポート | **`rotate-remap`**: 移送の進捗と競合の記録。**`rotate-encryption`**: 再暗号化の進捗の記録。**置換キーは両者とも `(rotationKind, bucketIndex, generation)` であり、これは本 Issue の決定である**（第4.1.1節・第6.8節）。**それぞれが「いつ・どの値を」書くかの規則は #44 が決める**（第6.8節の論点3。R8 do-boundary B-003 が、初版の2つの規則が互いに矛盾していたことを特定している）。したがって本欄は書き手の全数だけを名指しし、記録の契機は名指ししない |
...
- **列は NOT NULL であり、予約行を書く箇所は3つとも値を書く、と決め切る。** signup phase 1a / 1b（第6.3節）と **SSO link 手順2**（第6.6節）である。**link 手順2 を落とすと二次障害が出る** — 作業述語は `reservedUntil < ?` なので `NULL` 行は永久に掃除対象にならない一方、第7.4節 (1-A) の駆動源クエリ（`WHERE status = 'reserved' AND sagaCommitted IS NULL` の `min(reservedUntil)`）の集合には入るので、「残件はあるが `min(...)` が `NULL`」という状態が作れる。そのとき (1-A) は `nextRunAt = NULL` を書くことになり、完了時に `nextRunAt` を `NULL` にする規定（第7.4節「backoff と poison」）と衝突して、`deleteAlarm()` の述語も最早 `nextRunAt` の読み直しも未定義の値を掴む。**NOT NULL にすることでこの状態が構造的に作れなくなる。**
...
|---|---|---|---|---|
| `.overloaded` が真のエラー（1オブジェクト 1,000 req/s の soft limit 超過） | **呼び出し側（request Worker / 他 DO）の stub アダプター** | `SystemError(ServiceOverloaded)` | **false** | 超過分は DO へ配送されないので、エラーを受け取るのは stub を叩いた側である。公式がリトライ禁止と明記している（第2.1節 F-19）。**`ConflictError("OPTIMISTIC_LOCK_FAILURE")` のような `kind: "conflict"`（HTTP 409）系へ写してはいけない** — 409 はクライアントに再送を促す意味を持つからである。**`retryable` フラグの話ではない**（`ConflictError` は `CodedError.retryable` の既定 `false` を返す。`packages/core/src/lib/error.ts:35-37`。同表 行4 も false である） |
...
| `ctx.abort()` / DO のリセット | **呼び出し側の stub アダプター** | `SystemError(DatabaseError)` | **false** | DO が消滅した結果として RPC の promise が reject する形でしか観測できない。公式も output gate について「書き込みに失敗するとシステムは Object をリセットし、**送出待ちのメッセージをすべて破棄し**、クライアントにはエラーを返す」としており、DO 側に catch する場所が残らない。接続断と同種で、次のリクエストで DO が再構築される。**`retryable` を false にするのは、この欄が `SystemError.retryable` の実値だからである** — 実装の `RETRYABLE_SYSTEM_CODES` は `NetworkError` / `ExternalApiError` の2値だけで `DatabaseError` を含まない（`packages/core/src/application/errors.ts:206-210`）。**`DatabaseError` を `RETRYABLE_SYSTEM_CODES` へ足す変更は行わない** — 足すと既存の全 D1 経路（現在は D1、移行後は DO 内アダプター）の `retryable` が一斉に反転するので、この1行のために払う代償として釣り合わない。「次のリクエストで DO が再構築されるので上位でのリトライには意味がある」ことは事実だが、それは**シリアライズ契約の `retryable` フラグではなく運用上の性質**であり、この欄には書かない |
...
**`ServiceOverloaded` / `StorageCapacityExceeded` は新規コードである。** 現行の `packages/core/src/application/errors.ts` の `SystemErrorCode` は `DatabaseError` / `DataIntegrityError` / `CryptoError` / `SessionError` / `NetworkError` / `ExternalApiError` の6値で、この2つは存在しない。**#37 は同ファイルへ2値を追加する**（`RETRYABLE_SYSTEM_CODES` には**入れない** — どちらも retryable false）。**`RETRYABLE_SYSTEM_CODES` の中身そのものは変えない、と断定する**（`NetworkError` / `ExternalApiError` の2値のまま。`packages/core/src/application/errors.ts:206-210` で実確認済み）。第11.2節の変更対象一覧に行を足してある。**上の表の `retryable` 欄は、この集合から導出される実値である** — 表と実装が食い違ったら表を直す（表は要約であり、値を決める理由を持っているのは実装側の集合定義だからである）。
...
- **(3) 内部エントリ** — 呼び出し元が DO（saga の前進・Alarm）または operator 専用の maintenance 経路。**既定のガードは binding 到達性 + `operationId` / `payloadDigest` の CAS + phase 条件**である。
  ...
  - **既定のガードが当てはまらないエントリが11本ある。** `read-own-canonical` / `delete-mapping` / `lookup-credential-by-locator` は `callerToken` が守り（下記 (3-b)）、`advance-credential-change` / ローテーション経路の `record-credential-locator` は**`operations` 行を新規に作る側なので phase 条件が原理的に効かず**、同じく `callerToken` が守る（下記 (3-d)。同じ理由でクラス (2) の `begin-credential-change` も (3-d) に含める）。`check-previous-generation` / `purge-user-mappings` / **`list-bucket-user-ids` / `read-schema-version`** / `rotate-encryption` の起動は到達制御と監査（＋自分で検証できる材料）だけが守る（下記 (3-c)）。**とくに `check-previous-generation` は signup 起点では未認証の request Worker から呼ばれるので、呼び出し元だけを見れば (2) に見える。** それでも (3) に置くのは、**返す情報が真偽1ビットで副作用が無く、locator の算出に `DIRECTORY_ROUTING_SECRET` を要する**ため、(2) が必要とする「応答の均一化」や「レート制限」に相当する個別ガードを持たないからである。**#37 はこの1本について「(3) だから DO からしか来ない」と仮定しない。**
...
| (2) | `lookup-credential`（login step 3 / リセット依頼の解決） | Directory bucket | request Worker（未認証） | 無条件に応答し、**中身を均一化する** — 未登録 / **`status != 'active'`（＝ signup / link の予約行）** / **`changeState` が `null` でない**（`'pending'` / `'advanced'` のどちらも。第6.5.1節。2値を区別しない — 区別するのは同表の `promote-verifier` のガードと、第6.5.1節の後勝ち規則、および #45 が設計する自動回収の判定である）/ `nextAttemptAllowedAt` 未到達はすべてダミー検証材料へ倒す（第5.3節・第6.2.2節）。**`status` の条件が要るのは、signup phase 1a の予約行が `passwordVerifier` を持つからである**（第6.3節）— 落とすと、まだ活性化していない signup の検証材料が未認証経路へ出る |
...
| (2) | `reserve-credential`（signup phase 1a / 1b / link 手順2） | Directory bucket | request Worker（未認証）**または** コーディネーター bucket（`resume-signup` の phase 1b。第6.4節）**または** User Data DO（link 手順2 / `resume-link`。第6.6節） | 既存 active 行との一意制約 + `operationId` / `payloadDigest` の CAS。**予約行には `credentialId` / `callerToken` / `reservedUntil` を載せる** — signup 経路は phase 0 が採番した値、link 経路は request Worker が採番した `credentialId` と `account.callerToken` から読んだ値である（第6.1.2節・第6.6節 手順1〜2）。**`kind = 'email'` の signup 予約には `passwordVerifier` も引数で載せる**（phase 0 が request Worker で計算した値。第4.8節・第6.3節 phase 0）。**SSO signup がメール一意性のために置く予約と SSO link の予約には載せない** — 前者は `usableForLogin = false` が正しく出るための条件であり（第6.3節 phase 4・第7.6節）、後者は `kind = 'sso'` なのでパスワード検証材料を持たない |
...
| (2) | `lookup-credential-by-locator`（パスワード変更 phase 0 の旧検証材料取得。第6.5.1節） | Directory bucket | request Worker（セッション認証済み） | 引数は canonical ではなく **`(credentialId, generation, bucketIndex)`**（呼び出し元が `credential_locators` から得た永続化済みの locator。第6.1.1節 (R5)）+ `callerToken` の定数時間比較。**加えて `nextAttemptAllowedAt` 未到達なら検証材料を返さず明示的に拒否する**（ダミー材料へ倒すのではない。下記）。**評価順序を固定する — `callerToken` の照合を先に行い、不一致ならロックアウト状態を区別しない同一の拒否を返す。** 逆順に実装すると、`credentialId`（第6.1.2節 (C5) が UI へ出す非秘密）と `bucketIndex`（0..255 の総当たり）だけで「その credential がロックアウト中か」を**認証なしに1ビット引ける** — login 経路でわざわざダミー化して隠した性質そのものが、認証済み向けのエントリから漏れる。**ここに条件が無いと「数える前に検証材料が出る」ので、第6.2.2節 (a) の天井・減衰・非加算の3規則が変更経路では意味を持たない。** 返すのは `passwordVerifier` / `credentialVersion` / `changeState` だけで、`encryptedCanonical` も復号結果も返さない。**ダミー化にしない理由** — ダミーは `lookup-credential`（未認証、無条件応答）が**列挙オラクルを作らないため**に置いている手段であり、本エントリは `callerToken` で認証済み本人に束縛されているのでその根拠が当てはまらない。倒すと (a) ロックアウト中の正規利用者に「旧パスワードが違う」と誤った理由が返り、(b) A-3 の PBKDF2 と A-4 の報告 RPC が無駄に走って本節が挙げる `overloaded` 圧力を自分で上げる。**拒否なら A-3 / A-4 を実行せずに済み、UI に「試行が制限されている」を正しく出せる**（#35 の画面文言） |
| (2) | `report-verify-result`（パスワード変更 phase 0 の A-4。第6.5.1節） | Directory bucket | request Worker（セッション認証済み） | **`callerToken` の定数時間比較**（材料は A-1 が既に返している）+ 対象 `credentialId` の mapping 行が自 bucket に実在すること。実在しなければカウンタを更新せず成功を返す。**成功の報告で `failedAttempts` を0にし `nextAttemptAllowedAt` を過去へ戻し、失敗の報告で `failedAttempts` を前進させて `nextAttemptAllowedAt` を先送りする** — `report-login-result` とまったく同じ更新であり、天井・減衰・非加算の3規則も同じものが掛かる（第6.2.2節 (a)）。**`report-login-result` と同じく「応答を返す前に完了させる」を課す**（接続を切ってカウンタを回避させない）。カウンタの更新以外の副作用を持たない |
| (2) | `request-password-reset` | Directory bucket | request Worker（未認証） | レート制限と応答均一化のみ（第6.2.2節 (b)・第7.6節） |
| (2) | `begin-credential-change`（第6.5.1節 phase 1） | Directory bucket | request Worker | **起点ごとに DO 側の束縛を1つ持つ**（これが無いと「検証材料は読めないが差し替えはできる」状態になる。下記 (3-d)）。**起点 A（パスワード変更）は `callerToken` の定数時間比較**（呼び出し元は phase 0 の A-1 で `account.callerToken` を取得済みである）。**起点 B（リセット完了）は、引数の `changeAuthToken` が同 bucket の `password_reset_tokens` 行の `changeAuthToken` と定数時間比較で一致し、その行の `credentialId` が対象と一致すること。成功時は同じ `transactionSync` でその列を `NULL` へ戻す（`値 → NULL` の CAS）ので、この束縛は一回性である**（`consume-reset-token` が採番して返した値を呼び出し元がそのまま提示する。照合材料は同じ bucket にある）。**一回性は失効ではないので、phase 1 に失効の2段（未使用行の全削除 + 残る全行の `changeAuthToken` の `NULL` 化）を課す**（第6.5.1節 phase 1）。**どちらの起点も引数に `changeOrigin` を載せ、phase 1 が mapping 行へ書く**（起点 A は `'password-change'`、起点 B は `'reset'`。第4.1.1節・第6.5.1節 phase 1）。**`operationId` / `consumedByOperationId` の一致を束縛に使ってはならない** — 前者は第5.2節 (c) がログ出力を許した値、後者はその写しなので、どちらも capability になりえない（下記 (3-a) の但し書きと同じ根である）。認可の判定そのものは呼ぶ前に request Worker が済ませる — パスワード変更はセッション + 旧パスワード照合（材料の取得経路は第6.5.1節 phase 0 の起点 A）、リセット完了はトークンの消費。**Directory は epoch を持たないので、ここで認可を再判定しない**。対象 locator は呼び出し元が第6.1.1節 (R5) で解決した値であり、canonical から導出し直さない |
| (2) | `consume-reset-token` | Directory bucket | request Worker（未認証） | トークンハッシュの一致 + 未使用 + 未期限 + **同 bucket にトークン行の `credentialId` と一致する mapping 行が存在すること**。消費時に **128ビットの暗号論的乱数 `changeAuthToken` を採番して行に保存し、戻り値で返す**（`begin-credential-change` の起点 B の束縛材料はこれ**だけ**である。bucket が自分で採番して自分で照合するので `callerToken` と同じ形であり、新しい秘密も配布経路も増えない）。**あわせて `consumedByOperationId` へ引数の `operationId` を記録するが、これは監査用であって束縛材料ではない。** 戻り値は `userId` / `credentialId` / `credentialVersion` / `changeAuthToken`。**`generation` / `bucketIndex` の範囲検査は DO を叩く前に request Worker の transport 境界で行う**（第6.1節 (d)） |
...
| (3) | `promote-verifier`（第6.5.1節 phase 3） | Directory bucket | 自 bucket の Alarm | **`changeState = 'advanced'`** かつ `operationId` 一致。**`'pending'` では通さない** — phase 2 の成功が bucket 側に記録される前に昇格すると、User Data 側が前進していないまま新しい検証材料が正本になる（第6.5.1節 phase 2） |
...
| (3) | `cancel-reservation`（signup 敗北時の敗者補償 / 終端後の operator 経路 / #45 が設計する自動回収） | Directory bucket | コーディネーター bucket（operator 実行もありうる） | 行の `operationId` 一致 **かつ `callerToken` の定数時間比較**。**`status` は `reserved` / `active` を問わない** — phase 3 で `active` へ昇格済みの行も回収の対象になりうるため（第6.4節 3）。`operationId` が一致しない行には触れない。「無ければ成功」の冪等操作。**`callerToken` を課す理由は下記 (3-a) の但し書きにある** — `status` 非依存の破壊的削除を `operationId` の知識だけに束ねると、第5.2節 (c) が未認証経路のログへ出すことを許している値が capability になる |
| (3) | `abandon-account`（到達不能な signup の破棄。**発行主体は #45 が設計する自動回収であり、本 Issue の一様な終端は発行しない**。第6.4節 3・第7.4節） | User Data DO | コーディネーター bucket | `operations` 行の `operationId` 一致 + `kind = 'signup'` + `phase` が `done` でないこと **かつ `callerToken` の定数時間比較**。`account.status` を `deleting` へ倒して `finalize-withdrawal` を投入する（第7.4節 E-3 の投入点欄の第2の投入点である）。**`done` に達した saga は倒せない**。**エントリと守りは本 Issue の決定として残す** — 発行主体が決まっていないことは束縛の設計に影響しない（移送のチャンク駆動を本表から外したのとは扱いが違う。あちらは守りそのものが未定義である。第6.8節の論点2）。**`callerToken` を課す理由は `cancel-reservation` と同じである** — 同じ回収の2手順のうち片方だけを束縛する形は自己矛盾であり、`phase != 'done'` は「秘密」ではなく「状態」なので必要な知識は依然 `operationId` だけになる（下記 (3-a) の但し書き） |
...
| (3) | `list-bucket-user-ids`（PITR / 不良 migration の**対象特定**。第9.5節・第11.3節） | Directory bucket | **operator 専用 maintenance 経路**（request Worker 内） | **CAS も phase 条件も持たない。** 性質上 `callerToken` で束縛できない（`purge-user-mappings` と同じ理由 — **対象を特定する前に呼ぶ**ので、束縛材料である `callerToken` をまだ知らない）。したがって守りは maintenance 経路の到達制御（公開ルートを持たない。#38）+ 実行の監査ログである。**加えて2つを課す** — **(i) 返す `userId` をログ・トレースに出さない**（第5.2節 (c) が `userId` の扱いを揃えた値の bulk 取得だからである）、**(ii) 1回の呼び出しで返す件数に上限を置き、カーソルで刻む**（第6.2.1節 (c) が復号経路に課しているチェックポイント制約と同じ形。第7.4節の (iii-a) と同じ上限を使う）。**migration ゲート（第9.2節）・fail-closed（第9.4節）・第7.4節 (4) の arming を通さない**（第9.2節「ゲートを通さない2本」） |
| (3) | `read-schema-version`（同上。対象 User Data DO の `_meta.schema_version` を読む） | User Data DO | operator 専用 maintenance 経路（request Worker 内） | **CAS も phase 条件も持たない。** 読み取り専用で、返すのは `schema_version` の整数1つだけである（`account` の中身も PII も返さない）。守りは maintenance 経路の到達制御 + 実行の監査ログ。**`callerToken` は要求しない** — 呼び出し元は `list-bucket-user-ids` が返した `userId` を持つだけで、その `account.callerToken` を知らない（知るには第6.5.1節 phase 0 の A-1 が要り、それはセッションを前提とする）。**引数の `userId` は `list-bucket-user-ids` の戻り値に限る**（第5.5節 1 の (iii)。したがって第6.2節の判断軸 (iv) に触れない）。**migration ゲート（第9.2節）・fail-closed（第9.4節）・第7.4節 (4) の arming を通さない**（第9.2節「ゲートを通さない2本」） |
| (3) | `rotate-encryption` の起動（自 bucket の job table へジョブを投入する） | Directory bucket | operator 専用 maintenance 経路（request Worker 内） | maintenance 経路そのものの到達制御（#38）+ **引数の `encryptionGeneration` が state Worker の keyring の active 世代と一致すること**（bucket 側で検証できる — 暗号化 keyring は state Worker の常設バインディングなので bucket の手元にある。第6.2.1節 (b-1)）+ 実行の監査ログ。**鍵の注入を伴わない**（暗号化鍵は常設バインディングである）ので、移送のように「注入された鍵が本物か」を bucket が検証する必要が無い。**このエントリのガードは本 Issue で決着している**（下記 (3-c)） |
...
**全数の射程を1点だけ限定する。鍵ローテーションの移送（`rotate-remap`）が必要とする RPC エントリは本表に置かない。設計は #44 が行う**（第6.8節）。**したがって #37 は移送経路の RPC を実装しない。** 初版は「`rotate-remap` の1チャンク駆動」だけを1行として持っていたが、移送は最低3つの書き込み口（チャンクの駆動 / **移送先 bucket への active 行の書き込み** / 巻き戻し時の移送先行の破棄）を持ち、**後2つは表に無くガードが未定義だった**（#34 R8 do-boundary B-002）。しかも移送先への書き込みは (3-d) が `callerToken` で塞いだ「任意アカウントへクレデンシャルを注入する原始関数」より強い（予約と昇格を経由せずに `active` な mapping を直接書けるため）ので、**半端なガードのまま表に残すほうが危険である。** #44 が移送の実行主体を決めたうえで、本表と同じ形式でエントリ・呼び出し元・ガードを全数で列挙する。**本表のその他の行は移送の有無に依存しないので、#44 の結論を待たずに #37 が実装できる。**
...
**そのうえで「(3) のガードは呼び出し元が DO であることに一切依存していない」を全エントリについて成り立たせる。ただし守り方は一様ではないので、エントリを4群に分けて明示する。初版は「守っているのは CAS と phase 条件である」と一括で正当化していたが、それが当てはまらないエントリが実際には11本あった。**

**割り当ての全数を表で持つ。** クラス (3) の行数と、下の4群に割り当てられたエントリ数の合計が一致することが不変条件である（第1.4節 I-6）。**表に行を足したら、下の表の該当群の本数も同時に直す。**

| 群 | 守り | 該当エントリ（クラス (3)） |
|---|---|---|
| (3-a) | CAS と phase 条件（**破壊的な作用を持つ `cancel-reservation` / `abandon-account` の2本は `callerToken` を追加で課す**） | `activate-reservation` / `promote-verifier` / `propagate-saga-committed`（前進のみ・非破壊の3本）/ `cancel-reservation` / `abandon-account`（破壊的な2本）（5本） |
| (3-b) | `callerToken`（既存状態の読みと削除） | `read-own-canonical` / `delete-mapping`（2本） |
| (3-c) | 到達制御と監査、＋ bucket が自分で検証できる材料 | `check-previous-generation` / `purge-user-mappings` / `list-bucket-user-ids` / `read-schema-version` / `rotate-encryption` の起動（5本） |
...
- **(3-a) 既存の saga 状態を前提とするエントリ — CAS と phase 条件が守る群** — `activate-reservation` / `promote-verifier` / `propagate-saga-committed` / `cancel-reservation` / `abandon-account`。いずれも**呼び出し前に別の主体が書いた行（予約行 / `operations` 行 / `changeState = 'pending'` の mapping 行）の存在を条件にする**ので、存在しない saga を前進させることも、記録されていない `operationId` で phase を飛ばすこともできない。locator の材料も呼び出し側が永続化済みの値に限られる（第5.5節 1）。
  ...
  - **群の中に規則を1つ置く — 破壊的な作用を持つエントリは `operationId` の知識だけに束ねない。** (3-a) の正当化文が届くのは**前進のみで破壊的でない3本**（`activate-reservation` / `promote-verifier` / `propagate-saga-committed`。いずれも予約行や mapping 行の状態を1段進めるだけで、行を消さず `account.status` も倒さない）であり、**破壊的な2本（`cancel-reservation` / `abandon-account`）には届かない。** 破壊的かどうかで線を引いていることを群の説明に明示する — 初版は「CAS と phase 条件」という一括の説明しか持たず、線がどこにあるかが本文に現れていなかった。**したがってこの2本には `callerToken` の定数時間比較を追加で課す。**
  - **`abandon-account` に `callerToken` を課す。`phase != 'done'` は束縛ではないからである。** この条件は「秘密」ではなく「状態」であり、**必要な知識は依然 `operationId` だけである**。`abandon-account` は `account.status` を `deleting` へ倒して `finalize-withdrawal` を投入するので破壊的という性質は `cancel-reservation` と同じで、しかも第6.4節 3 が残す中間状態の回収において**同じ1つの手順を構成する**（回収の段の設計は #45 だが、束縛の設計はそれに依存しない）。**同じ回収の2手順のうち片方だけを束縛する形は設計の自己矛盾である。** 材料は増えない — 呼び出し元のコーディネーター bucket は自分の予約行に `callerToken` を持ち（phase 1a / 1b。第6.3節）、`account.callerToken` は phase 2 の `initialize-account` が書いている。
    - **Blocker ではなく本項の規則として扱う理由も書いておく。** `phase = 'done'` が phase 4 の2つの作用（`operations.phase = 'done'` と `credential_locators` の記録）と**同じ `transactionSync` で原子的に書かれる**限り、射程は**進行中および終端した signup**（どちらもログイン手段を持たないアカウント）に留まる。**終端したものは第6.4節 3 のとおり再登録での回復が効かないので、回復可能性を根拠にしない — 判定を支えているのは `callerToken` の束縛と phase 4 の原子性の2つだけである**。**逆に phase 4 を2本の RPC に分けて実装すると**、`credential_locators` が書かれて login が通るのに `phase` が `activating` のままという状態が生じ、`resume-signup` が `poison` に落ちればそれが安定状態になる — その口座は `abandon-account` で破壊できるので、**非原子実装では Blocker 相当になる**。原子性の要求は第6.3節 phase 4 と第6.6節 link 手順4 の両方に明記してある。
  - **`cancel-reservation` にも同じ理由で `callerToken` を課す。この1本は (3-a) の正当化文が届かない。** (3-a) の論拠は「存在しない saga を前進させられない／記録されていない `operationId` で phase を飛ばせない」という**前進**についての主張だが、`cancel-reservation` は**破壊的な補償**であり、しかも第6.4節 3 が残す `active` な孤児 mapping の回収に要るので `status` を問わずに削除する。**signup が完走して phase 4 まで達した mapping 行も `operationId` を保持したままである**（第4.1.1節の「saga コーディネーター状態」に `operationId` があり、昇格時に消す規定は無い。次に上書きされるのは credential 変更 saga の phase 1 のときだけである）。したがって束縛は `operationId` の知識1つに縮退する。**ところが第5.2節 (c) は未認証経路のログに `operationId` を出してよいと明記しており、signup は未認証経路である。** 両者を同時に成立させると、「binding には到達できるが `SESSION_SECRET` を持たない呼び出し元」＋ログ閲覧権限の組み合わせで、**任意の完了済みアカウントのメール / SSO 両方の mapping 行を消せる**。消した後は login も `request-password-reset` も mapping 不在のダミー経路へ落ち、`sweep-orphan-mapping`（`operations` の unlink 行が起点）・`finalize-withdrawal`（`deleting` 前提）・`sweep-reservations`（`reserved` のみ）のいずれの回収対象でもなく、**利用者側に自己回復手段が無い**うえ、空いた canonical で攻撃者がアカウントを取り直せる。**`callerToken` を課すことでこれが閉じる** — 呼び出し元はコーディネーター bucket であり、自分の予約行が `callerToken` を持っている（phase 1a / 1b で載る。第6.3節）ので材料は増えない。トークンは第5.2節 (c) の非露出対象なのでログには出ない。
    - **これにより「`operationId` は capability ではない」が本文で成立する。** 第5.2節 (c) が `operationId` のログ出力を許しているのは、それ単独では何も動かせないという前提の上である。**その前提を破っていたエントリは3本あり、どれも塞いである。** 1本目が `cancel-reservation`（本項の `callerToken`）、2本目が **`abandon-account`**（上記。同じ回収のもう半分であり、`phase != 'done'` は状態であって秘密ではない）、3本目が **`begin-credential-change` の起点 B** で、束縛が「引数の `operationId` が `password_reset_tokens.consumedByOperationId` と一致すること」＋「`credentialId` の一致」だけだった — **前者はログに出る値、後者は第6.1.2節 (C5) が UI へ出すと決めた非秘密なので、束縛の実体がゼロだった**（`dir:g{gen}:b{index}` の `index` を 0..255 で総当たりすれば宛先 bucket も特定できる）。**一回性の `changeAuthToken` へ置き換えて閉じた**（下記 (3-d)・第6.5.1節 起点 B）。**現在、`operationId` を束縛の実体として使っているエントリは1本も無い。**
    - **代替案（`status` を `reserved` に限る / phase 4 で mapping 行の `operationId` を `NULL` にする）は採らない。** 前者は第6.4節 3 が回収したい「phase 3 の途中で `active` へ昇格した行」をそのまま取り逃がす。後者は phase 4 が User Data DO 側の手順なので、全 bucket へ `operationId` を消すためだけの RPC を1周増やすことになり、しかもその RPC 自身の束縛が新しく要る。`callerToken` は**既にその行に載っている値**なので、どちらの代償も払わない。
- **(3-b) `callerToken` が守る群（既存状態の読みと削除）** — `read-own-canonical` / `delete-mapping`（および (2) の `lookup-credential-by-locator`）。**書き側の3本も同じトークンで束縛するが、当てはまらない理由が違うので (3-d) に分けてある。** **この3本は CAS も phase 条件も持たず、初版は「呼び出し元 DO の `userId` と一致する行に限る」だけを守りにしていた。ところが DO 間 RPC には呼び出し元の認証済み識別子が存在しない** — 「呼び出し元 DO の `userId`」は引数として渡るほか無く、bucket 側にそれを検証する材料が無い。**そのままだと `read-own-canonical` は「`userId` を1つ渡せばメール平文が1件返る」復号オラクル、`delete-mapping` は「`userId` を渡すだけで mapping が消える」ロックアウト原始関数になる。`userId` は秘密ではない**（リビジョンの `Actor` や export ヘッダから知りうる。第6.3節）。**したがって `callerToken` を導入する。**
  - **不透明値による束縛の共通規則を1箇所で断定する。射程は `callerToken` と `changeAuthToken` の2列である。** (i) **行側の値が `NULL` の場合は常に不一致とする。** (ii) **引数が欠落・空文字の場合は、比較に入る前に拒否する。** **この規則が無いと、定数時間比較を「両辺をバイト列化して比較する」と素直に実装した時点で `NULL == NULL` が通り、束縛の実体を1つも持たない呼び出し元が起点 B（`begin-credential-change`）や `delete-mapping` を叩ける。** どちらの列も `NULL` を取る局面が設計に実在する — `changeAuthToken` は消費（一回性の CAS）と phase 1 の失効で `NULL` になり（第4.1.1節・第6.5.1節 phase 1）、`account.callerToken` は退会 手順4 で `NULL` になる（第6.7節 手順4）。**本書はセッションの `ep` 欠落（`verify` が `null` を返す）と `typ` 欠落（拒否する）を明示的に fail closed と断定しているので、束縛の実体であるこの2列にも同じ強さの断定を置く。**
  - **`callerToken` は128ビットの不透明値で、signup phase 0 に request Worker の `IdGenerator` が候補 `userId` と同時に採番する。** 保存先は User Data DO 側が `account.callerToken`（`initialize-account` が引数で受け取って書く）、Directory 側が `credential_mappings.callerToken`（phase 1a / 1b の予約行が持ち、link では手順2 の予約が `account` から読んだ値を運ぶ）である。**鍵ローテーションの移送は行ごと引き継ぎ、再採番しない**（第6.8節）。退会で行ごと消える。
...
- **(3-d) saga を新規に開始できる群 — `callerToken` が守る。** `begin-credential-change`（クラス (2)）/ `advance-credential-change` / ローテーション経路の `record-credential-locator` の3本である。**この3本は `operations` 行や `changeState` を新規に作る側なので、(3-a) の「記録されていない `operationId` で phase を飛ばせない」が原理的に当てはまらない。** 束縛を置かないと次の3つが同時に成立する — (i) `begin-credential-change` を無束縛にすると、任意アカウントの `pendingVerifier` を差し替えて `resume-credential-change` の Alarm に phase 2 / 3 を自走させられる（**乗っ取り原始関数**）、(ii) `advance-credential-change` を無束縛にすると `sessionEpoch` と `credentialVersion` だけが前進して Directory 側と不一致になる（**恒久ロックアウト原始関数**）、(iii) ローテーション経路の `record-credential-locator` を無束縛にすると `reserve-credential` + `activate-reservation` と組み合わせて任意アカウントへ攻撃者のクレデンシャルを注入でき、第5.3節 step 5 (ii) の到達性検査を通せる。**同じ locator に対する読み（`lookup-credential-by-locator`）を `callerToken` で束縛しておきながら、より強い書きを無束縛のまま残すのは設計の自己矛盾なので、3本とも `callerToken` で束縛する。** 材料は新しく増えない — 起点 A は phase 0 の A-1 が `account.callerToken` を返し、`advance-credential-change` と `record-credential-locator` は Directory 側の mapping 行が同じ値を持つ。**`begin-credential-change` の起点 B（リセット完了）は未認証経路なので `callerToken` を提示できないが、束縛の強さは落とさない。** `consume-reset-token` が**128ビットの暗号論的乱数 `changeAuthToken` を採番して行に保存し戻り値で返す**ようにし、起点 B はその**定数時間比較を必須ガード**にする。**成功時に同じ `transactionSync` で `changeAuthToken` を `NULL` へ戻す（`値 → NULL` の CAS）ので一回性である。** 形は `callerToken` とまったく同じ（bucket が自分で採番して自分で照合する）なので、**新しい秘密も新しい配布経路も増えない。** この値は第5.2節 (c) の非露出対象に含める。
  - **初版は起点 B の束縛を `consumedByOperationId` + `credentialId` の一致に置いていた。これは束縛として成立しない。** (i) `operationId` は第5.2節 (c) が**未認証経路のログに出してよい唯一の値**であり、リセット完了は未認証経路である。(ii) `credentialId` は非露出リストに入っておらず、(C5) が設定画面へ表示し unlink の引数としてクライアントから受け取ると決めている。(iii) 宛先 bucket も障壁にならない（DO 名は `dir:g{gen}:b{index}` で、`index` を 0..255 で総当たりできる）。(iv) 窓は「直前」ではなく token TTL 全体である — `consume-reset-token` は `usedAt` を立てるだけで行を消さず、phase 1 が消すのは**未使用**トークンだけ、行が消えるのは `sweep-reset-tokens` の期限切れ掃除だからである。しかも「直前に自分が処理した RPC であること」は、本表が `report-login-result` について「DO は RPC を跨いだセッション状態を持たないので実装不能な述語である」と結論づけた形そのものである。**終状態は「攻撃者のパスワードで正規にログインできるアカウント」であり、(3-b) が「正しく塞げている」と宣言した脅威クラス（binding には到達できるが `SESSION_SECRET` を持たない呼び出し元）で成立していた。** 受容済みリスクとして本書が明記しているのは `purge-user-mappings` の恒久ロックアウトだけで、**認可が開く向きの原始関数は1つも受容していない。**
  - **第10.1節との非対称も解消する。** 同節は PITR 経路について「復活した消費済みリセットトークンが `consume-reset-token` → `begin-credential-change` → phase 2/3 で攻撃者の `pendingVerifier` を昇格させる」を**認可が開く唯一の PITR 経路**として特定し必須ステップを置いていた。**PITR を経ない同じ昇格経路が RPC 側に開いたままだったので、片方だけを塞いだ形になっていた。**
  - **代替案（起点 B を廃止し、リセット完了でも一度セッションを発行してから起点 A に合流させる）は採らない。** 認可としては閉じるが、リセット完了**前**にセッションを発行することになり、第6.5.1節の fail closed 順序（phase 1 で旧材料を無効化してから前進する）を崩す。`changeAuthToken` は既存の制約を1つも破らない。**ローテーション経路の `record-credential-locator` に `operations` 行を要求しない、と決め切る** — 要求するとローテーションが動かず、`operations` 行を新規作成して満たす実装にすると無束縛になるので、どちらにも倒さずに `callerToken` を束縛の実体にする。
- **(3-c) 到達制御と監査、＋ bucket が自分で検証できる材料が守る群** — `check-previous-generation` / `purge-user-mappings` / **`list-bucket-user-ids` / `read-schema-version`** / `rotate-encryption` の起動の5本。前4本は追加の束縛を持てず、`rotate-encryption` の起動は**bucket が自分の手元の材料だけで検証できるガードを1つ持つ**（引数の `encryptionGeneration` が state Worker の keyring の active 世代と一致すること。暗号化 keyring は常設バインディングなので手元にある）。
  - **`list-bucket-user-ids` / `read-schema-version` を本表に置く。第9.5節・第11.3節が要求しているのに全数表に無かった。** 第9.5節は「不良 migration が壊した対象集合を作る唯一の経路は Identity Directory bucket の全走査である」と断定し、第11.3節は PITR 手順の第1項にそれを置いている。**要求されているのは2本である** — (i) bucket から `userId` の一覧を返す読み、(ii) 各 User Data DO の `_meta.schema_version` を読む operator RPC。**移送（`rotate-remap`）とは独立に必要である** — 不良 migration からの復旧はローテーションの有無に依存しないので、**#44 の射程外**であり本 Issue が全数表に置く。
  - **`list-bucket-user-ids` が `callerToken` で束縛できない理由は `purge-user-mappings` と同じ形である。** どちらも**対象を特定する前に呼ぶ**エントリなので、対象の `callerToken` をまだ知らない。したがって守りは到達制御と監査だけで、**返るのがその bucket に写像される全利用者の `userId` の一覧**である点は `purge-user-mappings` より露出が広い。**そこで上の (i)(ii) の2つを追加で課す**（ログ非出力と件数上限）。
  - **残余リスクは `purge-user-mappings` と「同じ枠」に丸めない。種類が違うので書き分ける。** どちらも「request Worker 内でのコード実行を得た攻撃者」に対して無力である点は同じだが（maintenance 経路は request Worker 内にあり、そこは `SESSION_SECRET` を持つ。第5.1節 (3-b) の残余リスク）、**`purge-user-mappings` の被害は「攻撃者が `userId` を知っているアカウント」に限られる**のに対し、**`list-bucket-user-ids` は「全アカウントの `userId` の一覧」を返す。** つまりこの1本は、**偽造セッションで叩ける他のエントリ（第6.5.1節 phase 0 の A-1 で `callerToken` が返るなど）の射程を「名前を知るアカウント」から「全アカウント」へ広げる増幅器である。** #38 へ送るのは (a) `purge-user-mappings` と同じ承認手続きと実行監査、(b) **戻り値を作業ログ・チケット・監視系へ貼らないこと**、(c) **この1本の実行頻度そのものを検知対象にすること**（不良 migration からの復旧という契機は稀にしか起きないので、頻度の異常が最も安い検知手段である）の3つで、(c) は `purge-user-mappings` には無い項目である（第11.3節）。
  - **第9.5節の「第6.7節の operator 経路と第6.8節の retirement 走査が既に同じ前提に立っている」という記述は不正確だったので直してある。** `purge-user-mappings` は `userId` を**引数に取る**エントリで一覧を返さず、retirement 走査が数えるのは `previousCount`（bucket ごとの行数）であって `userId` の集合ではない。**「同じ 256 bucket を舐める運用経路がすでにある」という点だけが正しく、一覧を返す口は無かった。**
  - **移送（`rotate-remap`）のチャンク駆動はこの群に置かない。ガードの設計を #44 へ委譲したからである**（第6.8節・上の全数の射程）。**委譲する理由を1つだけ本節に残す** — **bucket は `DIRECTORY_ROUTING_SECRET` を保持しない**（第3.2節の非重複配布）ので、引数で注入された `{ generation, key, bucketCount }` の真正性を判定できる自分の値を1つも持たない。したがって「もう1つガードを足せば閉じる」形ではなく、**移送の実行主体そのものを決め直す判断が要る**。#44 が決着させる論点は「**移送の実行主体と、鍵をどう渡すか**（一時注入という形そのものを含む）」「注入する形を採る場合に bucket がその鍵を検証する手段」「**鍵の漏えいを動機とするローテーションで失効が成立する形**」の3つで、最後のものは「漏えい鍵の保持者も所持証明を満たせるので、所持証明を漏えい時のガードとして数えられない」（#34 R7 security W-001 の判定）ことに由来する。**#37 は移送経路を実装しないので、この論点が未決着のままでも #37 は着手できる**（第6.8節）。
  - **移送が危険なエントリであること自体は本 Issue の結論として残す。** 移送は (a) 移送元 bucket の全利用者を login も `request-password-reset` も解決不能にでき、(b) 移送先 bucket の `lookup-credential`（クラス (2)、無条件応答）から `passwordVerifier` を引き出せる（攻撃者が任意の鍵で駆動できれば移送先 index を自分で計算できる）。**したがって `purge-user-mappings` と同格以上に危険であり、公開ルートを持たない operator 専用 maintenance 経路・実行前承認・実行監査を必須にする**（#38。第11.3節）。**#44 が設計するガードは、この運用ガードの代替ではなく追加である。**
...
- **再 HMAC が必要なローテーション（第6.8節）について、本 Issue が決着させるのは制約2つだけである。** **(i) 再 HMAC の計算は bucket の中で行い、平文 canonical を Worker 境界の外へ bulk で出さない。(ii) routing 鍵を bucket の SQLite にもインスタンスフィールドにも永続化しない。** **この2点が本 Issue の決定であり、#44 は守る**（第6.8節の方針4）。
  - **「誰が起動し、誰が鍵を保持し、鍵をどう渡すか」は本 Issue では決めない。#44 が決める**（第6.8節の論点2）。初版は「operator 専用の maintenance 経路が request Worker 側で起動し、鍵を RPC 引数として bucket へ一時的に注入する」までを決着扱いにしていたが、**一時注入という形は実行主体の選択と不可分である** — 注入する主体が居なくなれば注入という形も消える。決着扱いにすると **#44 が取りうる解（bucket に routing keyring を配る / 専用の maintenance Worker を立てて境界を引き直す）を先に禁じる**ことになる。**正本は #44 側であり、design.md はこの2制約だけを残す。**
  - **(i) を制約として残す理由。** 逆の設計（bucket が平文 canonical を request Worker へ返して向こうで HMAC する）を採らないのは、**全ユーザーのメール平文が bulk で Worker 境界を越えることになり、被害の最大値がはるかに大きい**からである。#44 がどの実行主体を選んでも、この天秤の結論は変わらない。
  - **(ii) を制約として残す理由。** 第3.2節の非重複配布（state Worker は routing secret をバインディングとして持たない）を、保守経路の実装が事後的に壊さないためである。**「保持しない」は永続化の禁止であって、保守経路で鍵が一度だけ引数として渡ることの禁止ではない** — 後者を許すかどうかが #44 の論点である。
  - **注入された鍵の真正性を bucket が検証する手段も #44 が設計する。** **bucket は routing secret を保持しない**ので（上の非重複配布）、引数の `{ generation, key, bucketCount }` と照合できる自分の値を1つも持たない。**この空白は「ガードを1つ足せば閉じる」形ではない** — 検証材料の候補（`encryptedCanonical` と `hmac`）はどちらも **previous 世代の鍵に対する**材料なので、鍵の漏えいを動機とするローテーションでは漏えい鍵の保持者も同じ照合を通せる（#34 R7 security W-001 の判定）。**したがって移送の実行主体そのものを決め直す判断が必要であり、その判断は #44 に属する**（第5.1節 (3-c)・第6.8節）。
  - HMAC-SHA-256 は PBKDF2 と違い CPU 予算を圧迫しないので、DO の中で回してよい（第4.8節の対象外）。

**本節が決めるのは「鍵が世代を持ち、誰がどう配布し、平常時の lookup でどう使うか」までである。ローテーションの手順（移送の実行主体・ガード・巻き戻し・退役）は #44、運用値（保管・頻度・監査の様式）は #38 である。** 3者の境界は第6.8節に一覧として置いた。
...
3. bucket は `{ userId, credentialId, passwordVerifier, status, credentialVersion, usedLocator }` を値として返す。`usedLocator` は実際にヒットした行の locator（世代 + bucket index + 全長 HMAC）であり、`credentialId` はその行が持つ**世代非依存の credential 同一性**である（第6.1.2節）。**見つからない場合もダミーの検証材料を返す**（request Worker 側の定数でもよい）。**mapping 行が credential 変更中（`changeState` が `null` でない。`'pending'` / `'advanced'` のどちらも。第6.5.1節）なら、行が存在しても未登録と同じダミー材料を返す** — 旧検証材料での照合を成立させないためである。**同じく `status != 'active'` の行（signup phase 1a / link 手順2 が書いた予約行）もダミー材料へ倒す** — signup の予約行は phase 1a の時点で `passwordVerifier` を持つので（第6.3節 phase 0・phase 1a）、`status` を見ないと未活性の signup の検証材料が未認証経路へ出る。
...
| **パスワードリセット完了** | `ai_client_connections.createdAtResetVersion` | 下記。**前回のリセット完了以降に作られた接続をすべて自動失効させる**（間に何回パスワード変更が挟まっても射程が消えない） |
...
- **(i) 失効の基準を credential 変更カウンタから切り離す。`account` に `resetVersion`（パスワードリセット完了だけで進む単調増加カウンタ。初期値0）を1列、`ai_client_connections` に `createdAtResetVersion`（作成時点の `account.resetVersion`）を1列足す**（第4.1.1節）。**パスワードリセット完了に限り、第6.5.1節 phase 2 の同じ `transactionSync` で `resetVersion` を1つ進め、「`createdAtResetVersion` が前進前の現在値と等しい接続」を `revoked` にする。** 同じ DO 内の2列で済み、追加の往復が無い。
  - **`credentialVersion` を基準にしてはならない。最頻の乗っ取り系列でちょうど空振りするからである。** 典型手順は「パスワードを握る → AI 接続を作る → **パスワードを自分のものへ変更して正規利用者を締め出す**」であり、この順序では接続の作成時点が `n`、攻撃者の変更で `credentialVersion` が `n+1` へ進むので、被害者のリセット時点の「前進前の現在値」は `n+1` になる。**`n+1` で作られた接続は存在しないので失効対象は0件になり、攻撃者の接続（`n`）が生き残る。** 攻撃者が能動的に変更しなくても、侵害からリセットまでのあいだに正規利用者自身が1回パスワードを変えれば同じことが起きる。**「侵害復旧として不完全では困るので構造的な補償を1つ置く」という上の目的が、最も守りたい系列で達成されない。**
  - **`resetVersion` はリセット完了でしか進まないので、`createdAtResetVersion` が前進前の値と等しいことは「前回のリセット完了以降に作られた接続である」と同値である**（`createdAtResetVersion` が現在値を超えることは無い）。**間にパスワード変更が何回挟まっても射程が消えない。**
  - **通常のパスワード変更（旧パスワードを知っている経路）・SSO link / unlink では `resetVersion` を進めない。** リセットは「アカウントの制御を失った」という利用者の申告であり、変更は日常操作である。両者を区別することで、「パスワードを変えたら最近作った連携だけ切れる」という説明のつかない挙動を避けられる。
  - 失効の対象は「前回のリセット完了以降に作られた接続」に限られるので、**それより前から長く使っている接続は生き残る**。利用者の期待を壊さずに、侵害の窓に持ち込まれた分を切れる。
  - **列を1本増やす代わりに何を得たかを明示する。** `credentialVersion` の再利用は列を増やさない代わりに射程が「直前の credential 変更以降」に縮み、**その縮みが攻撃者の1操作で0になる**。`resetVersion` は縮みの下限を「前回のリセット以降」に固定するので、攻撃者が縮められない。
- **(ii) リセット完了画面に「AI クライアント接続の一覧」と「すべて失効」を必須の導線として出す。** (i) が切れるのは前回のリセット以降に作られた分だけなので、それより前に持ち込まれた接続は利用者の判断で切る。画面要件なので #35 の `spec/pages/` へ送る（第5.1節の SSO link 残余リスクへの導線と同じ画面にまとめる）。
...
5. **読み取りに効く 4 は書き込みには効かないので、書き込み側のガードを別に置く。** 4 が保証するのは「誤った DO から他人のデータが読めない」ことだけで、「誤った DO に書き込まない」ことは保証しない。したがって **signup の RPC エントリ（`initialize-account`）には「対象 DO に `account` 行が既に存在し、かつその `operationId` の `operations` 行が無いなら拒否する」ガードを置く**（第6.3節 phase 2）。これは 1 が破れた場合の二重の防波堤であり、**1 が成立している限り他人の DO に対しては発火しない** — 他人の DO にはその `operationId` の `operations` 行が存在しないので必ず拒否される。
   - **述語を「`account` 行が既に存在するなら無条件で拒否」にしてはならない。** 初版はそう書いていたが、それは第6.4節が saga の前進規則の前提として要求する「phase 2 は冪等」と正面から矛盾する。phase 2 が commit した後に応答が失われると（DO のリセット / `.overloaded` / `ctx.abort()`。第4.7節）コーディネーターは `sagaCommitted` 印を書けず `resume-signup` が phase 2 を再送するので、無条件拒否だと**永久に前進できず、再試行上限で第6.4節 3 の終端が発火し、**正常に作られたアカウントの saga が終端した状態のまま operator 経路へ回る**（#45 が自動回収を入れると `abandon-account` まで進み、アカウントそのものが破棄される）**。利用者から見ると「登録は成功したのに数分後にアカウントが消える」になる。無条件拒否は `operationId` / `payloadDigest` の比較そのものにも到達しないので、同じセルに並ぶ CAS が死ぬ。**述語は上の条件付きの形だけが正しい。**
...
- **(c) メール・SSO 主体の一意性。** 権威は「その canonical を写像する bucket の中の行」である。同じ canonical は同じ鍵・同じ世代なら必ず同じ bucket に落ちるので、**単一世代のあいだは bucket 内の一意制約がグローバル一意性と等価になり、bucket 間の調整は要らない。** これが固定 bucket 分割を採る最大の利点である（第6.2節）。
  ...
  - **previous 世代にトゥームストーンを書く必要は無い。ただし根拠は「previous 世代の行に書き込みが起きないから」ではない。** 正しい根拠は **previous 世代の bucket に新しい canonical が登録されることは無い**（登録は常に active 世代。上の2段規則）ことである。**この根拠は「移送が両世代とも不在になる窓を作らないこと」を前提にしており、その保証は移送の順序に属するので #44 の領分である**（第6.8節の論点1）。したがって 1 をすり抜けた競合は必ず 2 の一意制約が捕まえる。**読み1回で足りる**ので、saga が補償すべき相手（＝書き込んだ DO）は増えない。**単一世代（ローテーション未実行）では 1 が実行されない**（previous 世代が keyring に無いので導出する locator が存在しない）ので、#37 はこの2段規則を「keyring に previous があるときだけ 1 を走らせる」形で実装すれば足りる。
...
- **(d) パスワード認証・パスワードリセットで必要な認証情報の所有境界。** **Directory bucket が持つ。**
  ...
  - **発行済みトークンを無効化する手段を持つ。`password_reset_tokens` 行は対象 credential のキー `credentialId` を持ち、それに対する索引を定義する。** 現行 spec（`spec/database/index.md:77-101`）は `user_id` 列と `prt_user_idx`（用途欄に「ユーザーの既存トークン**無効化**・整理」と明記）を持っており、これを落とすと既存設計からの後退になる。bucket 内では `userId` より credential 単位のほうが自然なので、キーは credential 側に置く。**キーを `(kind, hmac)` にしない理由は第6.1.2節にある** — `hmac` は世代依存の値なので、ローテーション中に previous 世代の bucket で発行されたトークン行を active 世代側の削除要求が取りこぼす。`credentialId` は世代非依存なので、どの bucket にある行も同じキーで引ける。
    - **無効化を発行する箇所は4つで、いずれも同じ `transactionSync` の中で行う。** **本項の発行時**（新しいトークン行を書くのと同じ `transactionSync` で、同じ `credentialId` の**未使用**行を先に全削除する）/ 第6.5.1節 phase 1（credential 変更の起点）/ 第6.6節 unlink 手順3 / 第6.7節 手順3（退会）。
      - **発行時を無効化点に含める、と決め切る。** 含めないと、同じ credential について**未使用トークン行が窓ごとに1本ずつ並存する**（`lastResetRequestedAt` のスロットルが窓を刻むだけで、古い行を落とすのは `sweep-reset-tokens` の期限切れ掃除だけになる）。**第6.2.2節 (b) が「発行される新しいトークンは、第6.1節 (d) の無効化規則によりその credential の未使用トークンを全部置き換えるので、連打が古いトークンを増やすことも無い」と書いている主張は、発行時が無効化点に入って初めて真になる。** #35 へ送るテストケース（第11.1節）も同じ期待値を持つ。
      - **奪還経路そのものは並存があっても閉じていた**（第6.5.1節 phase 1 の2段が**その credential の全トークン行**を対象にするので、被害者の1回の復旧で全部落ちる）。**それでも並存を残さないのは、本項の無効化点の列挙が「全数」を名乗るものであり、列挙が実体と食い違っている状態そのものが本書で繰り返し破れを生んだ形だからである**（一回性を失効と混同していた件と同じ）。**発行時に消すほうが状態が1つ減る。**
      - **スロットル中は新しいトークン行を書かない。したがって既存の未使用行も消さない、と断定する。** 削除は**トークン行の発行と同じ `transactionSync`** に載る操作なので、発行が無ければ削除も無い。第6.2.2節 (b) と第7.6節が「必ず1行書く」と決めているのは**メール送信ジョブの行**であってトークン行ではなく、送らない側の行は「宛先を持たない」中身になる（第7.6節）。**断定を落とすと原始関数になる** — 「スロットル中も発行時の削除だけ走る」実装では、攻撃者が窓の外から連打することで**メールを1通も増やさずに被害者の未使用トークンを消し続けられる**（メール送信ジョブの `operationKey` が刻む窓と `lastResetRequestedAt` の間隔は別のパラメータなので、両者がずれた運用値になれば成立する）。**トークン行の発行と削除を同じ条件（実際に送信可能な行として書くとき）に束ねることで、この面が構造的に消える** — 第6.2.2節 (b) が `lastResetRequestedAt` の更新について既に採っているのと同じ形である。
      - **削除の射程は「未使用行」だけである。消費済み行には触れない。** 消費済み行は `usedAt` と `consumedByOperationId` を持つ監査記録であり、その `changeAuthToken` を落とすのは phase 1 の (2) の役目である（下記）。発行時にそこまで触ると、進行中の credential 変更 saga の起点 B の束縛を新しいリセット依頼が横から失効させられることになる。
    - **無効化の bucket 射程を明示する。「同じ `transactionSync`」は1 bucket 内の原子性であって、複数 bucket にまたがる保証ではない。** 4つの無効化点はいずれも**トークン行が実在する bucket の中**で走るので、対象 `credentialId` の行が複数 bucket に散っている場合は **bucket ごとに発行する**ことになる（第6.1.1節 (R3) の削除とまったく同じ形である）。
      - **単一世代（#37 の前提）では発行は常に1本である。理由を2つ書く。** **(i) トークン行を持ちうる credential は、そのアカウントにつき高々1つである** — トークンを発行するのは `passwordVerifier` を持つ `kind = 'email'` 行だけ（第7.6節）で、**メールクレデンシャルを追加するフローが本設計に存在しない**（第6.6節 unlink (1-a)）ので、`usableForLogin = true` の `kind = 'email'` credential は0個か1個しかない。**(ii) 1つの credential の mapping 行は1世代につき1 bucket にしかない**（DO 名が世代を含むので `credential_mappings.credentialId` は bucket 内 UNIQUE である。第4.1.1節）。したがって**「他 bucket に残った `changeAuthToken`」という状態は単一世代では作れない。**
      - **複数世代（ローテーション中）では対象 bucket が2つになりうる。** 同じ `credentialId` の mapping 行が previous 世代と active 世代の bucket に並存する窓では、トークン行もそれぞれの bucket で発行されうる（本項の「まだ移送されていない credential のトークンは旧世代 bucket で有効なままでよい」）。**このとき無効化点は「`credential_locators` が持つ全世代分の locator へ発行する」形になる**（(R3) と同じ）。**発行の順序と、移送が走っている最中の取りこぼしの扱いは #44 の領分である**（第6.8節の論点1。移送は mapping 行だけを運びトークン行は運ばないので、移送との競合は移送の順序に属する）。**#37 は 1 の形（単一 bucket への1本）を実装すればよい。**
    - **無効化は「行の削除」だけでは閉じない。列にも掛ける、と決め切る。** unlink 手順3 と退会 手順3 はその `credentialId` の行を `usedAt` の有無を問わず**全削除**するので削除だけで足りるが、**credential 変更 phase 1 は行を残す経路を持つ** — `consume-reset-token` は行を消さず `usedAt` を立てるだけで（第5.1節の表）、その同じ行に `changeAuthToken` を載せて返すからである。したがって phase 1 は2段にする。**(1) 対象 `credentialId` の未使用トークン行を全削除する。(2) 残る全行（＝消費済みの行）の `changeAuthToken` を `NULL` にする。**
      - **(2) が無いと「一回性 ≠ 失効」の穴が残る。** `consume-reset-token` に成功した直後に `begin-credential-change` が失敗すると（`.overloaded` — bucket index はリセット URL に載っているので攻撃者は狙う bucket を選べる（第6.2.2節）—・DO のリセット・`ctx.abort()` のいずれでも起きる）、その行は「消費済み・`changeAuthToken` 未使用」になる。`usedAt` が立っているのでリセットのやり直しはできないが、**列は `expiresAt` まで生き残る**。これは事故ではなく安定した中間状態である。起点 B のガードは「引数の `changeAuthToken` が行の値と一致」「行の `credentialId` が対象と一致」の2つだけで、`credentialVersion` も `changeState` も `usedAt` も被害者の復旧の事実も見ないので、**被害者がパスワード変更（起点 A）でもリセット完了（起点 B）でも復旧した後に、温存された値で `begin-credential-change` を叩ける** — 終状態は第5.1節 (3-d) が「乗っ取り原始関数」と呼んだ状態そのものである。**(2) を課すと、被害者のどちらの復旧経路も phase 1 を通るので、温存された値がその時点で失効する。**
      - **起点 B 自身も (2) の対象に含める。** 自分が照合した行も `NULL` になるので、既定の一回性（`値 → NULL` の CAS）がそのまま満たされる。二重に消す形になるが、どちらも同じ `transactionSync` の中なので観測できる中間状態は無い。
      - **代替案（`changeAuthToken` に独立した短い有効期限を持たせる）は採らない。** 列が1つ増えるうえ、**「被害者の復旧で失効する」という性質が得られない** — 攻撃者は復旧の直後に温存した値を使えばよいだけになる。
...
**本節の規則を2つに書き分ける。境界がそのまま #37 と #44 の境界である。**

- **(A) 単一世代でも成立する不変条件 — (R1)〜(R5) と (R8)。** これらは「世代を跨いで同じ credential を指すキーは `credentialId` である」「更新も削除も『その credential の行を全部』を対象にする」という形をしており、**keyring の世代が1つだけのとき（＝ローテーション未実行の初期デプロイ）にも定義され、そのまま成立する**（「全世代」が1世代に退化するだけである）。**#37 はこの6条をそのまま実装する。#44 の結論を待つ必要は無い。**
- **(B) 移送中の状態を前提とする規則 — (R6) / (R7) / (R9)。** いずれも「移送が進行中で、移送元と移送先の両方に行がありうる」ことを前提にしており、**移送そのものが存在しない状態では条件節が真にならない**。**この3条の設計は #44 の領分である**（第6.8節）。本節に残すのは**方針**（何を守りたいか）だけで、**規則の具体（どの状態を印にしてスキップするか / CAS の述語 / 巻き戻しの扱い / 削除進行中の credential との競合）は #44 が決着させる。**
  - **#34 の R5〜R8 で連続して検出された Blocker はすべて (B) の側に属する。** (A) の6条は R5 以降1度も指摘を受けていない。この非対称が、境界をここに引いた実測上の根拠である。
...
- **(R3) 削除は全世代・全 bucket を対象にする。対象の選択キーは `credentialId` である。** unlink（第6.6節）・退会（第6.7節）・signup の敗者補償（第6.4節）と、**#45 が設計する終端後の自動回収**はいずれも、`credential_locators`（または `operations.targetLocators`）から**対象 `credentialId` の全行**を引いて、その全世代分の locator に対して削除を発行し、**「無ければ成功」の冪等操作**にする。「1世代・1 bucket だけを消す」手順は本設計に存在しない。**キーを `(kind, hmac)` にしてはならない** — `hmac` は世代依存なので、世代ごとに違う値になり「同じ credential の別世代の行」を突き合わせられない（第6.1.2節）。
...
- **(R6) 進行中の saga を跨いで行を移送しない。方針としてはこれを守る。規則の具体は #44 が決着させる。** 守りたいものは1つで、**移送は「行の所在」と「その行を前進させるジョブの所在」を引き剥がす** — `resume-credential-change` は phase 1 を書いた bucket の `jobs` にあり、`resume-signup` はコーディネーター bucket の `jobs` にあるのに、移送対象は `credential_mappings` 行だけだからである。引き剥がすと phase 3 が永久に走らず、**旧新どちらのパスワードも通らない状態が恒久化する**（第6.2.2節 (a) の脱出経路 (i) も同時に壊れる）。**#44 が決めるのは「どの状態を移送対象から外すか」の全数と、その判定を読み出し時点で行うことの帰結である** — 少なくとも `changeState = 'pending'` / `status = 'reserved'` / **削除進行中（unlink・退会が対象にした credential）** の3つが候補であり、3つ目は #34 の R8 do-boundary B-001 が「印そのものが `credential_mappings` に存在しない」と特定した穴である。
- **(R7) 移送は移送元・移送先の両側を CAS で守る。方針としてはこれを守る。述語の具体は #44 が決着させる。** 「移送先への書き込み」には前進だけでなく**巻き戻しの破棄**も含む — 片側だけを守ると、移送の窓に入った書き込みが黙って巻き戻る。**#44 が決めるのは3つの述語**（移送先の前進 / 移送元の削除 / 移送先の破棄）**と、破棄が実在する中間状態を壊さないことの証明**である。**(R6) はこの窓を塞がない**という判定だけは本 Issue の結論として残す — (R6) が行を移送対象から外すのは**読み出し時点**の判定であり、移送先に行が書かれた後は active 世代の lookup がそちらへ向くので、**新しい `changeState = 'pending'` が移送先に着地する窓が別に開く**からである。
  - **`credential_locators` 側（移送が追加した新世代行）は「破棄しない」に倒す。これは本 Issue の結論である。** (R8) の単調非減少 upsert により値が正本以上に保たれ、到達性検査が `credentialId` だけを見るので、余分な世代行は認可を緩めない。**CAS を持てない破棄を残すより、残すほうが規則が1つ減る。** ただし**「破棄しない行が孤児として残り続ける」場合の回収**（対象 credential が unlink や退会で消えた場合。R8 B-001）は #44 の論点である。
- **(R8) `credential_locators` の `credentialVersion` / `usableForLogin` / `label` は `credentialId` 単位で管理し、更新は常にその credential の全世代の行へ同時に行う。** (R3) が削除について定めているのと対になる、**更新側の規則**である。具体は2点。
  ...
  - **この規則が無いと何が壊れるか。** (i) 移送が新世代の locator 行を追加した直後に credential 変更が走ると、phase 2 が「当該行」1行だけを進めて新世代行が取り残され、旧世代行が消えた時点で **Directory 側 n+1 / User Data 側 n の恒久不一致**になる（第5.3節 step 5 (iii) が永久に落ちる）。(ii) 逆に移送が古いスナップショットで行を追加すると、新世代行に古い `credentialVersion` が書かれて同じ不一致になる — 単調非減少にすることで塞ぐ。(iii) `record-credential-locator` を「既存行があれば何もしない」no-op にすると、移送の追加が常に空振りして旧世代行の削除後に `credential_locators` が空になり、**到達性検査が移送済みの全利用者を締め出す**（第6.9節）。**したがって upsert であることと単調非減少であることの両方が必須である。**
  - **この3点は単一世代でも意味を持つ。** 「全世代の行へ同時に書く」は1世代なら「その行へ書く」に退化するだけで、**`record-credential-locator` の冪等キーが `(credentialId, generation)` であることと `credentialVersion` が単調非減少であることは、移送の有無に依存しない契約である。** #37 はこの契約をそのまま実装する。
- **(R9) 2世代に行が並存する状態でどちらを正本とするかは、#44 が決着させる。** 本 Issue が残す方針は「**`credentialVersion` が大きい側が正本であり、移送は必ず正本の側へ収束させる**」である。**「移送先に行があれば書かない」だけに倒してはならない**という判定も残す — 移送の中断と再実行のあいだに旧世代 bucket 宛の発行済みトークンでリセットが完走すると、再実行が移送先の古い行をそのまま正本にして**旧 `passwordVerifier` が復活する**（リセットで設定した新パスワードが黙って捨てられる）。**#44 が決めるのは分岐の全数**（行が無い / `userId` が違う / 移送元が新しい / 移送先が新しい / 等しい）**と、それぞれの記録先である。**

**本節の規則が塞いでいる失敗モードを明示する。** **(A) の6条について** — (R2) と (R5) が無いと、ローテーション中のパスワード変更が移送元の行に着地して黙って空振りする（**旧パスワードの復活**）。(R3) と (R4) が無いと、unlink が**自己ロックアウト**と**解除済みクレデンシャルでのログイン**を同時に成立させる。**この2つは単一世代でも成立する** — (R4) は SSO signup がメール canonical にも行を置くため、(R3) は `hmac` を同一性に使うと世代以外の理由でも突き合わせが壊れるためである。(R8) が無いと、`record-credential-locator` の冪等キーと単調性が定まらず、credential 変更と locator 記録がどの順で走ったかによって **Directory 側と User Data 側の `credentialVersion` が恒久的に食い違う**。**(B) の3条について** — (R6) が無いと進行中の credential 変更が phase 3 に到達できず**恒久ロックアウト**、(R7) が無いと巻き戻しが移送先の中間状態を壊して**ログイン不能**、(R9) が無いと移送先の陳腐化を経由して**旧パスワードが正本に戻る**。**(B) 由来の3経路は第6.9節の締め出し経路一覧に「塞ぎ方の設計は #44」として登録してある** — 経路の列挙は本 Issue の成果として残し、塞ぎ方だけを委譲する。**#37 はローテーションを実行しないので、(B) の3経路はいずれも #37 の実装では発火しない**（第6.8節）。
...
- **(C1) 値は `IdGenerator` が採番する128ビットの不透明値である。** canonical からも鍵からも導出しない。`callerToken` とまったく同じ扱いで、**採番点は request Worker である**。
  ...
  - **鍵ローテーションの移送**: 行ごと引き継ぎ、**再採番しない**（第6.8節）。これが「世代非依存」の実体である。
...
- `credential_mappings` 行は `encryptionGeneration` を持ち、**復号は行が宣言した世代の鍵で行う**。routing secret 側の世代（locator の世代）とは**独立した番号体系**である — 2つのローテーションは別々の理由で走るので、世代番号を連動させない。**ただし「番号体系が独立である」ことと「2つのローテーションを同時に走らせてよい」ことは別の主張であり、後者を本 Issue は主張しない。2つのローテーションの関係は #44 が決着させる**（第6.8節の論点3）。
- 再暗号化は bucket の Alarm ジョブ `kind: 'rotate-encryption'` として回す（第7.4節）。走査・チェックポイント・retirement 証明は第6.8節と同じ「bucket ごとの snapshot 置換」の枠組みに乗せる — **記録先も同じ `rotation_checkpoints` テーブルである**（第4.1.1節）。**記録は `rotationKind = 'encryption'` を持ち、置換キーは `(rotationKind, bucketIndex, generation)`、`generation` 列に入るのは `encryptionGeneration` である。この3点は本 Issue の決定である。** **`rotationKind` が無いと、番号体系が独立であるがゆえに `encryptionGeneration = 3` の完了記録と routing `generation = 3` の進捗記録が同一キーへ落ち、片方の完了記録がもう片方の旧鍵破棄条件を誤って成立させる**（第6.8節）。
  - **旧鍵の破棄条件（retirement 証明の述語）と、`previousCount` に何をいつ書くかは #44 が決める**（第6.8節の論点3）。R8 do-boundary B-003 が、初版の2つの規則 — 「移送先で `rotate-encryption` を再投入すれば `previousCount` が0でなくなる」と「`rotate-encryption` は残件が尽きた時点の `previousCount = 0` しか記録しない」— が互いに矛盾しており、**旧暗号鍵の誤退役をどちらの読みでも止められない**ことを特定している。**「証明の述語が矛盾したまま `.thread/34/design.md` に残るくらいなら、述語を #44 の成果物として1箇所で定義するほうが正しい」と判断した。**
  - **`0..N-1` の `N` の定義も同じ理由で #44 に属する。** routing ローテーションが進行中だと、同じ `encryptionGeneration` の行が2つの routing 世代の bucket 集合に散らばるので、**「全 bucket」が2通りに読めて retirement 証明そのものが定義されない。** これは2つのローテーションの関係（論点3）と同じ根である。
  - **#37 への影響は無い。** #37 が実装するのは「`encryptionGeneration` 列」「`rotate-encryption` の起動 RPC とジョブ」「残件条件（active でない `encryptionGeneration` の行が存在すること）」までであり、**退役条件は暗号化鍵の世代を実際に進めるときにしか要らない**（第6.8節の境界表）。
...
- **AAD に `(kind, credentialId, encryptionGeneration)` を束縛する。`hmac` は束縛しない。** 束縛の目的は、DB 書き込み権限を得た攻撃者が bucket 内で暗号文を別の mapping 行へ**付け替える**こと（「B のメールアドレスを A のアカウントの原本にすり替え、リセットメールを別アドレスへ送らせる」経路）を復号失敗として検出することである。`credentialId` は credential ごとに一意なので、**別 credential の行への付け替えは変わらず検出できる**。検出できなくなるのは「同じ credential の previous 世代の行と active 世代の行のあいだで暗号文を入れ替える」場合だけだが、**その2行の平文は同じ canonical なので入れ替えても何も起きない。**
  ...
  - **したがって移送時に再暗号化しない、と決め切る。** 移送は暗号文系の列（`encryptedCanonical` / `encryptionGeneration` / `encryptionNonce`）を**そのまま運ぶ**。代替案（AAD に `hmac` を残したまま移送時に新しい nonce と新しい AAD で再暗号化する）は、平文がその場にあるのでコストは無いが、**再暗号化を `rotate-encryption` ジョブだけの仕事として定義した (b-1) を崩す**（移送側に鍵の使い方をもう1つ持たせることになり、規則が2系統になる）。AAD から `hmac` を外すほうが規則が1つ減る。**なお「2つのローテーションを独立に走らせてよい」という初版の断定は第6.8節で撤回してあるので、この棄却の論拠には数えない** — 棄却は上の1点だけで成立する。
...
2. 鍵ローテーションの再写像（第6.8節）— **bucket の中で復号し、同じ bucket の中で再 HMAC する。平文は DO の外へ出ない**（第5.2.3節が制約として固定した2点のうちの (i)。鍵の渡し方そのものは #44）。
...
- **(a) 認証試行のカウントは `credential_mappings` 行に持つ。射程は「その credential に対するパスワード照合の全数」であって login だけではない。** 行に `{ failedAttempts, nextAttemptAllowedAt }` を足し、照合失敗ごとに `failedAttempts` を進めて指数的に `nextAttemptAllowedAt` を先送りする。**カウンタを進める経路は2本であり、これが全数である** — 第5.3節 step 7（`report-login-result`）と、**第6.5.1節 phase 0 起点 A の A-4（`report-verify-result`）**である。**照合そのものは request Worker で行われる**（第4.8節）ので、bucket はどちらの経路でも結果を報告されて初めて知る。
  - **「進める側」と「0へ戻す側」を書き分ける。全数の宣言は前者についてのものである。** **0へ戻す側は4箇所ある** — 上の2本（成功の報告）に加えて、**第6.5.1節 phase 3**（リセット / パスワード変更の完走。下の脱出経路 (i)）と**第11.3節の PITR restore 後の必須ステップ (iv)**（`failedAttempts` を0・`nextAttemptAllowedAt` を過去へ戻す。締め出し方向へ倒さないため）である。**この2列は `credential_mappings` にあり、同表は第4.1.1節の非集約ストア表（E-2）の射程外なので**（集約でもアダプター内部ストアでもなく、Directory 側の CAS で直列化される行である。第4.1.1節）**列ごとの書き手の全数を機械検査する枠が無い。したがって本項が散文の正本であり、書き手を足したらここに足す。**
  - **A-4 を落とすと、認証済みパスワード変更の旧パスワード照合が「数えない・止めない」面として残る。** 起点 A の A-2 → A-3 は **login step 3〜4 とまったく同じ1回のパスワード認証試行**であり、第5.1節も旧パスワード照合を認可の構成要素として扱っている。更新経路を login 1本に確定させ、`lookup-credential-by-locator` に `nextAttemptAllowedAt` の条件も置かない形では、**セッションを1つ手に入れた攻撃者が旧パスワードを無制限・無記録にオンライン総当たりできる** — `failedAttempts` が進まないので痕跡が1つも残らず、天井も減衰も発火しない。`PlainPassword` の下限は8文字だけで強度要件が無い（`packages/core/src/domain/identity/valueObject.ts` の `PASSWORD_MIN_LENGTH = 8`）ので、当たれば終状態は「攻撃者がパスワードを変更して正規利用者を締め出す」であり、本書が受容している唯一の残余リスク（第5.1節 の SSO link 経路。補償が対で置いてある）より1段強い。**同時に CPU の攻撃面でもある** — 1試行につき A-1（User Data DO）と A-2（Directory bucket）の RPC が2本発生し、標的の bucket は攻撃者が選べる（本節冒頭）ので、下の (c) の WAF を通らずに `overloaded` へ追い込む経路になる。
  - **新しい恒久 DoS は生まれない。** A-4 が進めたカウンタは、下の脱出経路 (i)（リセット完走で `failedAttempts` を0に戻す）でそのまま解消する。**カウンタは credential 単位なので、login 側の総当たりと同じ天井・減衰・非加算の3規則がそのまま効く。****DO 化によって「同じ canonical への試行が必ず同じ bucket の同じ行に集まる」という、カウントに最適な構造が手に入っている** — 共有 DB 時代には無かった性質なので使う。`nextAttemptAllowedAt` 未到達の照合は、第5.3節 step 3 のダミー材料経路へ倒して**成功・失敗の応答を区別させない**（ロックアウトの有無が列挙オラクルにならないようにする）。成功時に `failedAttempts` をリセットする。
  - **この機構はそのまま標的型のアカウントロックアウトに転用できる。天井・減衰・非加算の3点を設計の制約として固定して塞ぐ。** カウンタの単位は canonical credential であって IP でもデバイスでもないので、被害者のメールアドレスを知る攻撃者は誤ったパスワードを投げ続けるだけで `nextAttemptAllowedAt` を先送りできる。素朴に組むと、ロックアウト中の試行も**倒し先（login はダミー経路、パスワード変更は拒否。下記 3）で「失敗」になる**ため `failedAttempts` が減らず、正規利用者が正しいパスワードを入れても「成功時のリセット」に到達できない **恒久的な DoS** になる。
    ...
    3. **ロックアウト中の試行は `failedAttempts` を進めない。** `nextAttemptAllowedAt` 未到達の照合は**カウンタに触らずに終わる。倒し方は経路で違う** — **login（未認証。`lookup-credential`）はダミー材料へ倒す**（成功・失敗の応答差を作らないため）、**パスワード変更（認証済み。`lookup-credential-by-locator`）は明示的に拒否する**（`callerToken` で本人に束縛されているので列挙オラクル回避の根拠が無く、倒すと正規利用者に誤った理由が返り PBKDF2 と報告 RPC が無駄に走る。第5.1節）。**これが無いと攻撃者は先送りを無限に更新できる**ので、天井と減衰があっても意味が無くなる。
  ...
  - **脱出経路を2本残す。** (i) パスワードリセットは `failedAttempts` に影響されない別経路であり、**リセットの完走時に `failedAttempts` を0にし `nextAttemptAllowedAt` を過去へ戻す**（第6.5.1節 phase 3 の同じ `transactionSync`）。(ii) SSO は別 credential なので、その行のカウンタは独立している。**パスワードのロックアウトが SSO ログインを巻き込まない。**
    - **脱出経路 (i) は単一世代（ローテーション未実行）では無条件に成立する。** phase 1 と phase 3 が触る行は同じ bucket の同じ行だからである。**移送中にも成立させるには第6.1.1節 (R6) が要る**（`changeState = 'pending'` の行を移送しないので、phase 3 が触る行は phase 1 が書いた行と常に同じ bucket にある）。**(R6) を採らずに pending 行を移送する設計にすると、phase 3 の対象行がその bucket から消えて脱出経路 (i) が同時に壊れ、パスワード単独アカウント（SSO を持たない利用者）の恒久 DoS が残る。** (R6) の規則の具体は #44 が決着させるので（第6.8節の論点1）、**「ローテーション中も脱出経路 (i) が成立すること」は #44 の受け入れ条件に含まれる。** この依存関係は第6.9節の締め出し経路一覧にも反映してある。
...
- **(c) 標的型の `overloaded` は DO の外で止める。** (a)(b) は DO に到達した後の抑止なので、`overloaded` そのものは防げない。**request Worker の手前、Cloudflare の WAF / Rate Limiting Rules を「パスワード照合を行う全エンドポイント」（login / signup / password reset / **パスワード変更**）に当てる**、を本設計の結論とする。**未認証エンドポイントだけに限定しない** — パスワード変更は認証済みエンドポイントだが、上の (a) のとおり同じ1回のパスワード認証試行であり、限定すると #37 も #38 もこの面に抑止を置く根拠を本書から得られない（第11.3節の #38 への引き継ぎも同じ列挙に揃えてある）。**(c) は (a) の天井設計の前提でもある** — credential 単位のバックオフに天井を置ける（＝無限に伸ばさなくてよい）のは、発信元単位の第一防壁がその手前にあるからである。ルールの具体（キー、閾値、窓、チャレンジの出し方）と (a)(b) の具体値（天井・減衰の係数・依頼間隔）は #38 の運用要件へ送る。**「どのレイヤーで当てるか」だけは本 Issue で決着させる** — DO 側で受けてから数える設計に倒すと、数える前に `overloaded` になるという順序で破れるからである。
...
| phase | 実行場所 | 内容 | 再開可能性 |
|---|---|---|---|
| 0 | request Worker | `IdGenerator` が `operationId` / 候補 `userId` / **`callerToken`**（第5.1節 (3-b)）/ **credential ごとの `credentialId`**（第6.1.2節 (C1)）を**それぞれ新規に採番する**。いずれも**クライアントからは受け取らない**。全 credential の canonical を確定し、locator を安定ソートして**コーディネーター bucket を決める**（下記）。**パスワード signup では `PasswordHasher.hash` で検証材料（`passwordVerifier`）をここで計算する**（第4.8節。DO の外で回す。下記「`passwordVerifier` の書き込み点」） | 冪等キーの起点 |
| 1a | **コーディネーター bucket** | previous 世代が keyring に載っている間は previous 世代の bucket を読んで既存行の不在を確認する（第6.1節 (c)・第6.1.1節 (R1)）。そのうえで予約を取る（`status: 'reserved'`、`{ operationId, credentialId, candidateUserId, callerToken, locators[], reservedUntil }` 付き。**`kind = 'email'` かつパスワード signup なら `passwordVerifier` も同じ予約行に書く**。下記「`passwordVerifier` の書き込み点」）。**`locators[]` は全 credential の locator 一覧**（要素は `credentialId` + `kind` + 全長 HMAC + 世代 + bucket index）、**`reservedUntil` は予約 TTL の絶対時刻**である（第4.1.1節）。**予約行を書くのと同じ `transactionSync` で `sweep-reservations` と `resume-signup` の2ジョブを投入する**（下記）。既に active な mapping があれば敗北して `ConflictError("EMAIL_ALREADY_REGISTERED")` 等。**saga のコーディネーター状態はこの予約行が持つ** | 予約行が冪等キーと全 locator を持つ |
| 1b | 残りの Directory bucket | 同じ確認と予約を、`locators[]` の順に取る（`{ operationId, credentialId, candidateUserId, callerToken, coordinatorLocator, reservedUntil }` 付き。**`passwordVerifier` は載せない** — 1b が扱うのは SSO signup の SSO credential であり、パスワード signup は credential が1つなのでコーディネーターが常にその1件を持つ。下記「`passwordVerifier` の書き込み点」）。**予約行を書くのと同じ `transactionSync` で自 bucket の `sweep-reservations` を投入する**（`resume-signup` は投入しない。前進の駆動はコーディネーターに限るため）。1つでも敗北したら、コーディネーターが `cancel-reservation` で全予約を冪等に削除して saga 全体を敗北させる（phase 2 より前なので巻き戻し） | コーディネーター予約行から再開できる |
...
| 4 | User Data DO | `operations.phase = 'done'`、`credential_locators` に reverse locator（**`credentialId`** + 世代 + bucket index + `kind` + 全長 HMAC + `credentialVersion` + **`usableForLogin`** + **`label`**）を記録する。冪等キーは `(credentialId, generation)` の upsert である（第6.1.1節 (R8)）。**この2つの作用は同じ `transactionSync` で書く**（下記） | 完了 |

**phase 4 の2つの作用（`operations.phase = 'done'` と `credential_locators` への記録）は同じ `transactionSync` で書く、と決め切る。** どちらも User Data DO 側の書き込みなので原子的に書ける。**2本の RPC に分ける実装を禁じる理由は、分けると安定した中間状態が生まれることである** — `credential_locators` が書かれて第5.3節 step 5 (ii) の到達性検査が通る（＝ログインできる）のに `operations.phase` が `activating` のままという状態が作れ、`resume-signup` が `poison` に落ちればそれが恒久化する。その口座は `phase != 'done'` を条件にする `abandon-account` で破壊でき、**`abandon-account` の射程が「進行中の signup（データが空のアカウント）」に留まるという第5.1節 (3-a) の評価が崩れる。** `record-credential-locator` は汎用 upsert（link とローテーションでも共有する。第5.1節）なので、**#37 が phase 4 を「locator の記録 RPC」と「`phase` 前進」の2本として組む余地が実際にある。** **同じ要求は第6.6節 link 手順4 にも掛かる**（あちらは手順4 が `credential_locators` の追加と `operations.phase = 'done'` の2つを持つ）。
...
**`passwordVerifier` の書き込み点を確定する。計算は phase 0（request Worker）、書き込みは phase 1a（`reserve-credential` の引数）である。** 初版はこの点をどの phase にも置いていなかったため、phase 4 の `usableForLogin` 述語（「`kind = 'sso'` であるか、または `kind = 'email'` かつその mapping 行が `passwordVerifier` を持つ」）が**パスワード signup に対して構造的に真になりえなかった**。帰結は広く、(i) login step 4 の照合が成立しない（第5.3節）、(ii) 第7.6節が「送るか送らないかの判定は `passwordVerifier` の有無で行う」と決め切っているのでリセットメールも送られず、第5.2.1節 (a) が「所有の唯一の証明」と断定した復旧経路が存在しない、(iii) 第6.5.1節 起点 A の A-1 は `usableForLogin = true` の行だけを読むのでパスワード変更も成立しない、(iv) 第6.6節 unlink (1-b) のログイン手段の数え上げが常に0になる — **一度もログインできず自己回復手段も持たないアカウントが作られる**。決めは次の4点である。

1. **計算は phase 0 で request Worker が行う**（`PasswordHasher.hash`）。第4.8節の「CPU を食う仕事は DO の外で回す」に合致し、login step 4（`PasswordHasher.verify`）と同じ場所である。
2. **書き込みは phase 1a の予約行である**（`reserve-credential` の引数に `passwordVerifier` を1つ足す。第5.1節の表）。**`initialize-account` / `activate-reservation` / `record-credential-locator` には足さない** — 予約行そのものが後に `active` へ昇格する行なので、昇格時に材料を運び直す必要が無い。
3. **`lookup-credential` は `status != 'active'` の行をダミー材料へ倒す**（第5.1節の表・第5.3節 step 3）。予約行が `passwordVerifier` を持つ以上、この条件が無いと未活性の signup の検証材料が未認証経路（クラス (2)、無条件応答）へ出る。**`changeState = 'pending'` をダミーへ倒す規則と同じ形である。**
4. **SSO signup がメール一意性のために置く行には載せない。** その行の `usableForLogin` が偽になる条件そのものであり（上記）、第7.6節（SSO 専用アカウントにリセットメールを送らない）と第6.1.1節 (R4)（ログイン手段の数え方）がこれに依存している。**したがって `credential_mappings.passwordVerifier` の書き込み点は本節 phase 1a と第6.5.1節 phase 3 の2つだけであり、それが全数である**（第4.1.1節）。
...
|---|---|---|---|
| signup（第6.3節） | phase 2 より前: 一部 bucket の `reserved` 行。phase 2 以降: `active` な `account` と未昇格の予約 / 未記録の reverse locator。**phase 3 の部分成功では `credential_locators` に対応行を持たない `active` な mapping** | コーディネーター bucket の `sweep-reservations`（巻き戻し）と `resume-signup`（前進 / 終端）。**終端は一様である**（`terminalReason` + `poison` + operator エスカレーション。第7.4節）。**残った mapping の自動回収は #45**（下記 3） | **phase 2 の成功を境界に、前が巻き戻し・後が前進。前進不能が確定したら一様な終端へ倒す（下記 3）** |
| SSO link（第6.6節） | 手順2〜3 のあいだは **`reserved` 行**（`reservedUntil` 付き。第6.6節 手順2）。手順3 まで進んで手順4 で落ちると、`credential_locators` に対応行を持たない **`active` な孤児 mapping** | User Data DO の `resume-link`（前進）と、予約段の取り残しを回収する対象 bucket の `sweep-reservations`（第6.6節 手順2 が投入する）。**退会と競合した場合の回収は `finalize-withdrawal` が引き取る**（第6.7節 手順3） | **常に前進**（手順1 で `operations` に記録した時点で利用者の意図が確定しているため）。**ただし対象アカウントが `deleting` / `deleted` に入ったら前進をやめ、退会側の回収へ倒す**（第6.6節・第6.7節 手順3）。**前進不能が確定したら一様な終端へ倒す**（`terminalReason` + `poison` + operator エスカレーション。残った mapping の自動回収は #45。第7.4節） |
...
| credential 変更 / リセット完了（第6.5.1節） | phase 1 完了・phase 3 未完了の間は `changeState` が `null` でない（`'pending'` か `'advanced'`）（旧新どちらのパスワードも通らない） | Directory bucket の `resume-credential-change` | **常に前進**（旧検証材料は phase 1 で既に無効化されているので巻き戻せない） |
| 退会（第6.7節） | `account.status = 'deleting'` のまま、mapping / `credential_locators` が残る | User Data DO の `finalize-withdrawal` | **常に前進** |
| 鍵ローテーションの再写像（第6.8節） | 同じ credential の行が新旧2世代に並存する / `credential_locators` に2世代の行が並存する / 移送をスキップされた行 | **`rotate-remap` チャンク**（Alarm ジョブではない。第6.8節の方針5）。**実行主体は #44 が決める**（第6.8節の論点2）ので本表では名指ししない | **常に前進**（並存する中間状態でも login は通る）。**中間状態の全数と回収規則の設計は #44** であり、本表はこの操作が cross-DO 操作の1つであることの記録として行を残す（第6.8節の論点1・論点2・論点5） |
...
1. **予約 TTL は「バックオフ上限に達するまでの総経過時間 + マージン」より必ず長く取る。本 Issue が固定するのはこの不等式の形と、右辺が「前進のバックオフ上限に達するまでの総経過時間 + マージン」であることまでである** — **#45 が自動回収を設計して後始末の再試行が入ると覆うべき総経過時間はその上限まで伸びるので、右辺の射程は #45 が確定させる**（第7.4節の論点4）。 予約行の `reservedUntil`（絶対時刻。第4.1.1節）は phase 1a / 1b で `now + TTL` として書く。具体値は #38（第11.3節）だが、**この不等式そのものは設計の制約として固定する**。**同じ不等式は SSO link 手順2 の予約にも掛かる**（第6.6節。あちらのバックオフ列は `resume-link` のものである）。
   - **「再開間隔 × 再試行上限」と書いてはならない。再試行は指数バックオフである**（第7.4節「backoff と poison」）。等間隔で見積もると総経過時間を大きく下回るので、**TTL 掃除が下の 3 の終端規則より先に走る**。そのとき掃除されるのはコーディネーター予約行であり、失われるのは `locators[]` / `candidateUserId` / **`callerToken`** の3つで、**`callerToken` は `abandon-account`（第5.1節）の必須ガードなので `jobs.payload` に退避できない**（第7.4節の `payload` 制約が「PII **および再利用可能な秘密**を入れない」と決めており、`callerToken` は第5.2節 (c) の非露出対象である）。
   - **不等式を守れば、掃除と終端の順序は2つの場合に分かれてどちらも閉じる。** **(a) phase 2 が成功済みなら**コーディネーター予約行は `sagaCommitted` 印を持つので（下の 2 が同じ `transactionSync` で書く）**掃除の対象から外れ、回収の材料は残る**。**(b) phase 2 が成功していないなら**予約行は掃除されうるが、**その掃除そのものが正しい巻き戻しである**（`active` へ昇格した行は `status` 条件で掃除されないので、残る孤児 mapping は無い）。したがって (b) では回収すべき残渣が無く、一様な終端（`terminalReason` を残して `poison`）だけで閉じる。**この2場合分けが成立する条件が上の不等式である** — 破れると (a) の側で材料だけが消える。
...
3. **それでも前進できなかった場合の終端を決める。発火条件は「前進不能が確定した場合」であり、`ConflictError` に限らない**（第7.4節「分類 (C) の…終端規則」の方針1 の適用である）。具体的には **(a) 再開が `ConflictError` で確定した場合**（印を書く前に落ち、かつ TTL 経過後に別の利用者へ canonical を取られた場合）と、**(b) バックオフ上限に達した場合**（phase 3 / phase 4 が `.overloaded` や `SQLITE_FULL` で失敗し続けた場合）の2つである。**そのとき `resume-signup` は `terminalReason` を残して `poison` にし、運用へエスカレーションする**（#38。第7.4節の一様な終端）。**「黙って到達不能アカウントを残す」だけは選ばない。**
   - **(b) を条件に含めないと、下記の「恒久的に再登録不能」が終端の記録すら伴わずに残る。** 初版は発火条件を `ConflictError` 限定で書いていたので、`overloaded` 系の連続失敗では終端が1つも走らなかった。**`overloaded` は攻撃者が誘導できる**（第6.2.2節。任意の1 bucket を soft limit へ追い込める）ので例外事象ではなく、**SSO signup は常に2 bucket を跨ぐ**（第6.3節）ので構成としても標準経路である。
   - **終端時に何が残るかを明示する。** phase 3 は逐次なので部分成功しうる（コーディネーターが自分の行を昇格した後、`activate-reservation` の途中で落ちる）。したがって残るのは **`credential_locators` に対応行を持たない `active` な孤児 mapping** と、**未昇格の `reserved` 行**の2種で、**同じ bucket に同居しうる** — SSO signup は2 canonical を持ち、**その2つが同じ bucket に落ちることは 256 分割なら約 1/256 の確率で起きる**（第5.2.5節が bucket の衝突を必然として扱っている）。**この孤児は本設計のどの掃除ジョブの対象でもない** — `sweep-reservations` は `status: 'reserved'` の行しか見ず、しかも `sagaCommitted` 印のある行は作業述語からも駆動源からも構造的に外れる（第7.4節 (1-A)）。`sweep-orphan-mapping` の残件条件は unlink 行に限られ、`finalize-withdrawal` は `deleting` を前提にする。**帰結はそのメールアドレス / SSO 主体が恒久的に再登録不能になることである**（次の signup が phase 1 で `EMAIL_ALREADY_REGISTERED` に敗北する）。**第6.9節の締め出し経路一覧にこの経路を行として登録してある。**
   - **終端は材料を消さない、と決め切る。** コーディネーター予約行は `locators[]` / `candidateUserId` / **`callerToken`** の**唯一の置き場**であり、**`callerToken` は `abandon-account` の必須ガードである**（第5.1節 (3-a)）。しかも `jobs.payload` へも `terminalReason` へも退避できない（第7.4節の制約が「PII **および再利用可能な秘密**を入れない」と決めており、`callerToken` は第5.2節 (c) の非露出対象である）。**したがって終端でこの行を消してはならない**（第7.4節の前方互換点2）。`sagaCommitted` 印のある行は sweep の対象からも外れるので、消さない限り残る。
   - **残った mapping の自動回収は #45 が設計する。** 段の順序と各段の原子性境界・回収の除外単位・`abandon-account` の発行・終端モードの印・後始末そのものが失敗したときの規則・operator 受け口の割り当ては、すべて #45 の論点である（第7.4節の論点1〜5）。**本 Issue が残すのは「発火条件」「一様な終端」「残る中間状態の列挙」「材料の保全」「第6.9節への経路の登録」の5つである。**
   - **回収を `finalize-withdrawal` に任せる形は成立しない。この事実は本 Issue の結論として #45 へ渡す。** 素朴に組むと「`account.status` を `deleting` へ倒して回収は退会経路に任せる」形になるが、**`finalize-withdrawal` は `credential_locators` を唯一の逆引き情報として mapping を消す設計であり（第6.7節 手順3）、`credential_locators` を書くのは phase 4 である。** 終端が発火するのは phase 3 の途中なので、**この時点で `credential_locators` は空**であり、回収対象が1件も見つからない。**したがって #45 は「コーディネーター予約行の `locators[]` を辿る」側で回収を組む必要がある** — その材料が上の前方互換点2 で保全されている理由である。
   - **代替案（phase 4 を分割し、phase 3 の各昇格の直後に `record-credential-locator` を発行して `credential_locators` を常に mapping の上位集合に保つ）は採らない。これも #45 への制約として残す。** 不変条件としては強いが、**昇格前に locator を書くと「mapping が `reserved` なのに `credential_locators` に active 行がある」窓が開き、第5.3節 step 5 (ii) の到達性検査の意味（User Data 側の active 行 = そのアカウントが認めたログイン手段）が緩む**。コーディネーター行が既に `locators[]` を持っている以上、回収は新しい情報を必要としない。
...
- **敗者の冪等補償。** 敗者は自分の `operationId` を持つ行だけを削除する（他人の行には触れない）。**実体は `cancel-reservation` RPC である**（第5.1節。コーディネーター bucket → 各 bucket）。削除は「無ければ成功」の冪等操作にする。敗者が補償の前に落ちても、予約の TTL 掃除（第6.4節）が同じ結果に収束させる。**`cancel-reservation` が `status` を問わないのは第6.4節 3 が残す `active` な孤児 mapping の回収に要るためで（回収の段の設計は #45。第7.4節）、phase 1b の敗北時は対象が `reserved` 行しか無いので挙動は変わらない。**
- **epoch の前進も冪等にする。** パスワード変更・リセット完了・SSO の**解除**・退会は、同じ `operationId` の再送では `sessionEpoch` を**一度だけ**進める。**判定機構は4操作で一様ではないので、両方を明示する。** パスワード変更・リセット完了・SSO の解除の3つは **`operations` 行の存在**で判定する。**退会だけは `account.status` の遷移がこれを兼ねる** — 第6.7節 手順1 は `operations` 行を記録せず（E-2 の `recordOperation` の書き手欄も4箇所で退会を含まない。第4.1.1節）、起点は `account.status = 'active'` のときしか通らないので、2度目の退会要求は epoch ガードと状態照合で拒否される。**悪用可能な穴ではない**（単調増加のカウンタが余分に進むだけである）が、**「4操作に共通の判定機構」と書くと #37 が存在しない `recordOperation` の呼び出し点を探すことになる**ので射程を分ける。**この4つが全数であり、正本は第5.1節である。SSO の link は含まない**（第6.6節 link 手順の末尾）。
...
| 1 | Directory bucket | 対象 `credential_mappings` 行を `{ operationId, pendingVerifier, changeState: 'pending', changeOrigin }` にする。**`changeOrigin` は起点の永続化であり、起点 A なら `'password-change'`、起点 B なら `'reset'` を書く**（値は `begin-credential-change` の引数。第4.1.1節・第5.1節。**phase 2 がこの列を読む**）。**この瞬間から旧検証材料での login 照合を拒否する**（第5.3節 step 3 がダミー材料経路へ倒す）。**同じ `transactionSync` で、その credential のリセットトークン行を2段で無効化する** — **(1) 未使用行を全削除し、(2) 残る全行の `changeAuthToken` を `NULL` にする**（下記・第6.1節 (d)）。同時に自 bucket の job table へ `resume-credential-change` を投入する |
| 2 | User Data DO | `operations` に記録し、**`sessionEpoch` を1つ進め**、**`credentialId` が一致する `credential_locators` の全世代の行**の `credentialVersion` を1つ進め（第6.1.1節 (R8)。「当該行」1行ではない — 1行だけ進めるとローテーション中に世代スキューが残って恒久ログイン不能になる）、**`usableForLogin` を Directory 側が判定した値へ更新する**（第4.1.1節）。**この更新で値が変わることは無い、と断定する** — credential 変更 saga の対象は必ず `usableForLogin = true` の行だからである（起点 A は第6.5.1節 phase 0 の A-1 が `usableForLogin = true` の行だけを読み、起点 B のトークンは `passwordVerifier` を持つ credential にしか発行されない。第7.6節）。**`usableForLogin` を false から true へ遷移させる経路は本設計に存在しない**（第7.6節で決め切ってある）。更新を残すのは (R8) の「全世代の行へ同時に書く」を1本の規則に保つためであって、値の遷移のためではない。**`changeOrigin = 'reset'` の場合に限り、同じ `transactionSync` で `account.resetVersion` を1つ進め、`createdAtResetVersion` が前進前の値と等しい `ai_client_connections` 行を `revoked` にする**（第5.4節 (i)）。**`changeOrigin = 'password-change'`（通常のパスワード変更）では `resetVersion` を進めず、AI 接続にも触らない。判定の権威は mapping 行の `changeOrigin` 列であって、request Worker がその場で知っている文脈ではない** — phase 2 の駆動は `resume-credential-change`（Alarm）でもありうるので、起点が行に永続化されていないと Alarm 再開経路で判定が決まらない（下記）。**成功の確定後、bucket は phase 3 へ進む前に自 bucket のローカル書き込みで `changeState` を `'pending'` から `'advanced'` へ倒す** — これが「phase 2 が適用済みである」ことの bucket 側の唯一の記録であり、`promote-verifier` のガード（第5.1節）と後勝ち規則（下記）がこの値を読む。**#45 が設計する自動回収の判定材料でもある**（第7.4節の前方互換点4） |
| 3 | Directory bucket | `pendingVerifier` を `passwordVerifier` へ昇格し、行の `credentialVersion` を phase 2 と同じ値へ揃え、`changeState` を `null` へ戻す（ガードは `changeState = 'advanced'`。第5.1節）。**`changeOrigin` も同じ `transactionSync` で `null` へ戻す**（saga が終わった行に起点が残ると、次の saga が前の起点を読む）。**同じ `transactionSync` で `failedAttempts` を0にし `nextAttemptAllowedAt` を過去へ戻す**（第6.2.2節 (a) の脱出経路） |

**起点を `credential_mappings.changeOrigin` として永続化する。導出では代替しない、と決め切る。** phase 2 は常に Directory bucket → User Data DO の RPC（`advance-credential-change`）であり、再開は `resume-credential-change` が **mapping 行だけを入力に**駆動する（下記）。したがって起点が行に無ければ、Alarm 再開経路で `resetVersion` の前進と AI クライアント接続の自動失効が決まらない。**帰結は片側へ倒しても閉じない** — 「常に進めない」に倒すと**被害者のリセット完走後も攻撃者が持ち込んだ AI 接続が `exp` まで生き残り**（第5.4節 (i) が `credentialVersion` ではなく `resetVersion` を新設してまで塞いだ経路が再開経路でだけ開く）、「常に進める」に倒すと通常のパスワード変更でも接続が切れて第5.4節の断定に反する。

- **`password_reset_tokens.consumedByOperationId` から導出する形は採らない。** 同列は監査用の記録であって束縛材料ではないと第5.1節 (3-a) が格下げしており、さらに `sweep-reset-tokens` が行を消した後は導出そのものが偽になるので、**判定の権威がトークンの期限に依存してしまう。**
- **`advance-credential-change` の payload に `changeOrigin` を含めてよい。** 値は mapping 行から読むので、再開時にも同じ payload が再構成でき、`payloadDigest` の CAS が `ConflictError` にならない。**行に永続化していない値を payload に含めると再開で digest がずれ、saga が `pending` のまま止まる**（旧新どちらのパスワードも通らない ＝ 第6.9節が塞いだ恒久ロックアウト）。
- **`null` へ戻すのが phase 3 であることの帰結を1つ決め切る — `resume-credential-change` は `changeState IS NULL` を観測したら saga 完了と読み、phase 2 を再発行せずにジョブを `done` にする。** phase 3 は `changeState` の解除と `changeOrigin` の `null` 復帰を**同じ `transactionSync`** で行うので、この2つが別々に観測されることは無い。**判定を「`changeState` を見る」側に置かないと、phase 3 完走後にジョブの完了記録だけが失われた場合（Alarm 実行の末尾で DO がリセットされる。第2.1節 F-4）に、再開が `changeOrigin = null` で payload を組み直して `payloadDigest` の CAS を外し、`ConflictError` を繰り返してバックオフ上限まで進む。** そのとき `changeState` は既に解除済みなので締め出しは起きないが、**`poison` 行と `terminalReason` が正常完了した saga について残り、運用の検知（#38）が偽陽性で埋まる。** 順序を「phase 3 で戻す」に置いたこと自体は正しい — phase 2 より先に戻すと phase 2 の再実行で起点が読めなくなり、戻さないと次の saga が前の起点を読む。**したがって直すのは復帰の位置ではなく、再開側の読み方である。**
- **列は第4.1.1節（E-1）と第1.4節 検査8 の一覧の両方に載せてある**（I-4 の「本文が要求する列が E-1 に無ければ違反であり、直す向きは E-1 の側」に従った）。

**phase 0 の locator 解決規則を定める。起点によって出所が違う。** 第6.1.1節 (R5) の適用であり、**どちらの起点でも canonical からは導出しない。**

- **(起点 A) パスワード変更（認証済み）。** 手順は4段である。**(A-1) セッションから `userId` を得て User Data DO を叩き（クラス (1)。epoch ガードを通る）、`credential_locators` の `kind = 'email'` かつ `usableForLogin = true` の行を読む。** ローテーション中に**同じ `credentialId` の2世代の行**があれば**両方が返り、以降の手順は両方に対して行う**（先に成功したほうを採る。第6.1.1節 (R2)）。同じ RPC で `account.callerToken` も返す。**(A-2) その locator（`(credentialId, generation, bucketIndex)`）で `lookup-credential-by-locator` を叩き（第5.1節のクラス (2)）、`callerToken` を載せて `passwordVerifier` / `credentialVersion` / `changeState` を得る。** **(A-3) request Worker が `PasswordHasher.verify` で旧パスワードを照合する**（第4.8節）。照合が通った locator が phase 1 の対象であり、**phase 1 の `begin-credential-change` にも同じ `callerToken` を載せる**（第5.1節 (3-d)）。**(A-4) 照合の結果を `report-verify-result` で bucket へ報告する**（第5.1節のクラス (2)）。成功なら `failedAttempts` を0に戻し、失敗なら前進させる。
  - **A-4 は省略できない。旧パスワード照合は login と同じ1回のパスワード認証試行だからである。** A-4 が無いと、この経路だけが濫用抑止の射程の外に落ちて**旧パスワードの無制限・無記録なオンライン総当たり**が成立する（第6.2.2節 (a)）。**A-2 の側にも `nextAttemptAllowedAt` 未到達の拒否を課す**（第5.1節。**ダミー化ではない** — ダミーは未認証経路の列挙オラクル回避の手段で、`callerToken` で本人に束縛された本エントリにはその根拠が無い。拒否なら A-3 の PBKDF2 と A-4 の RPC を走らせずに済む）— 数える前に検証材料が出てしまうと、天井・減衰・非加算の3規則が変更経路では効かない。**報告は「応答を返す前に完了させる」**（`report-login-result` と同じ。接続を切ってカウンタを回避させない）。**材料は増えない** — `callerToken` は A-1 が、`credentialId` と locator は A-1 / A-2 が既に返している。
  - **失敗報告がベストエフォートである点は login step 7 と同じ扱いである。** 報告そのものが落ちた場合にカウンタが進まないのは設計の意図どおりで（`report-login-result` の欄と同じ）、攻撃者が報告 RPC だけを落として無制限に試行できるわけではない — 失敗を落とし続けるには request Worker のコード実行が要り、それは第5.1節 (3-b) が明記した残余リスクの枠に入る。
...
- **(起点 B) パスワードリセット完了（未認証）。** locator は**トークン本体が運ぶ `{generation}.{bucketIndex}`** と、`consume-reset-token` が返す `credentialId` から組む（第6.1節 (d)）。範囲検査は locator 導出の前に transport 境界で行う（第5.5節 3）。**旧検証材料の照合は要らない** — トークンの消費そのものが認可である。`consume-reset-token` のガードに「同 bucket にトークン行の `credentialId` と一致する mapping 行が存在すること」を含めるので（第5.1節）、**行の無い bucket でトークンだけが消費されて phase 1 が空振りすることは起きない。**
  - **この起点は `callerToken` を提示できない**（未認証経路なのでセッションが無い）。代わりに **`consume-reset-token` が128ビットの暗号論的乱数 `changeAuthToken` を採番して消費したトークン行に保存し、戻り値で返す。`begin-credential-change` はその定数時間比較を必須ガードにし、成功時に同じ `transactionSync` で `値 → NULL` の CAS で消す**（第5.1節 (3-d)・第4.1.1節）。`consume-reset-token` と `begin-credential-change` は同じ bucket で走るので、照合材料はその場にある。**`consumedByOperationId` も記録するが、それは監査用であって束縛材料ではない** — `operationId` は第5.2節 (c) が未認証経路のログへ出すことを許した値なので capability になりえず、`credentialId` も (C5) が UI へ出す非秘密だからである。**この2つだけを束縛にしていた初版は、binding 到達性を持つ呼び出し元に対する乗っ取り原始関数を残していた**（第5.1節 (3-d) に成立経路と、`changeAuthToken` が既存制約を1つも破らない理由を書いた）。
  - **一回性が「リセットのやり直し」を壊さないことを確認しておく。** `changeAuthToken` を消したあとに同じトークンでやり直すことはできないが、**リセットトークン自体も `consume-reset-token` で使い捨てである**（`usedAt` が立つ）ので、やり直しは元から「新しいリセット依頼から」である。したがって下記の後勝ち規則の対象は**新しい依頼**であり、新しい依頼は新しいトークンと新しい `changeAuthToken` を持つので規則は成立し続ける。第6.1節 (d) が既に受容している「リセットをやり直す」影響と同じ大きさに収まる。
- **(共通) ローテーション中の世代解決。** 起点 A は `credential_locators` の行が世代を持ち、起点 B はトークンが世代を持つので、**どちらも「行が実在する世代」へ着地する**（第6.1.1節 (R2)）。**「active 世代の locator を導出して書く」実装にすると、まだ移送されていない利用者の変更が行の無い bucket を叩いて黙って空振りする。** **この規則は単一世代でも同じ形で成立する**（「行が実在する世代」が1つしかないだけである）ので、#37 はこのまま実装する。**「phase 1 が着地した bucket と phase 3 が触る bucket が常に同じであること」を移送中にも保つ規則は第6.1.1節 (R6) の方針に属し、その具体の設計は #44 である**（第6.8節の論点1）。
...
- **リクエスト跨ぎの冪等性は `changeState`（`null` でない値）+ `operationId` が担う。** 同じ `operationId` の再開は phase 2 / 3 を何度実行しても同じ結果に収束する。**別の `operationId` による新しい変更依頼**が来た場合の扱いは、**行の値で分かれる。決め切る。**
  - **`changeState = 'pending'`（phase 2 未適用）なら後勝ちで `pendingVerifier` と `operationId` を差し替える**（利用者が「リセットをもう一度やり直した」ケースが自然に通る）。差し替えても phase 2 の epoch 前進は `operations` 行で一度だけに保たれる。
  - **`changeState = 'advanced'`（phase 2 適用済み）なら受け付けず `ConflictError` を返す。後勝ちの射程に含めない。** 含めると**認可が開く** — その行は User Data 側の `sessionEpoch` と `credentialVersion` が既に前進した状態なので、`pendingVerifier` だけを差し替えると **phase 3 が「被害者の前進済みバージョン」に「別の依頼の検証材料」を昇格させる**。起点 B の材料は一回性なので同じ依頼の再提示ではできないが、**新しいリセットを1回完走できる攻撃者には成立する**（第6.9節が「攻撃者のパスワードで正規にログインできるアカウント」と呼んだ終状態である）。
  - **`'advanced'` に倒れている窓は phase 3 のローカル書き込み1回ぶんなので短い。** 拒否された利用者は phase 3 の完了後（`changeState` が `null` に戻る）に依頼し直せば通る。**恒久的に `'advanced'` のまま留まるのは phase 3 が恒久的に失敗している場合だけで、それは一様な終端で `terminalReason` が残り operator 経路へ渡る**（第7.4節・下記）。**したがって「拒否」が新しい恒久ロックアウトを作ることはない。****「新しい依頼」であることが要件である** — 起点 A は `callerToken`、起点 B は一回性の `changeAuthToken` を必須ガードにするので、**同じ依頼の材料を後から再提示して差し替えることはできない**（起点 B は成功時に `changeAuthToken` を消す。上記）。
- **`resume-credential-change` がバックオフ上限に達したときに何が残るかを決め切る。残るのは `changeState` が `null` でない mapping 行であり、その状態では脱出経路が3つとも閉じている。** 分類 (C) なので `poison` に落ちると投入点（phase 1 = `begin-credential-change`）からの再投入だけが `pending` へ戻せるが（第7.4節）、**その投入点を叩くには 起点 A（セッション + 旧パスワード照合）か 起点 B（新しいリセットトークン）が要る。ところが `changeState` が `null` でないあいだ脱出経路は3つとも閉じている** — 旧パスワードは `lookup-credential` がダミーへ倒すので login 不能（第5.1節）、新パスワードは phase 3 未完了なので未昇格、リセット依頼も同じダミー化で解決されないためトークンが発行されない。**残るのは既存セッション（TTL 7日）からの 起点 A 再実行だけで、TTL が切れた時点で復旧手段がゼロになる**（`purge-user-mappings` は mapping を消すだけなので復旧ではなく恒久ロックの確定である）。**この経路は第6.9節の締め出し経路一覧に行として登録してある。**
  - **`poison` に至る契機は設計内に実在する。** phase 2 は User Data DO の `account.sessionEpoch` と `credential_locators.credentialVersion` を**書く**。第4.7節は `SQLITE_FULL` を「書き込みだけが恒久的に失敗し、読みと `DELETE` は通る半死状態」「`retryable` false」と定義しているので、**10 GB に達した User Data DO の利用者がパスワードを変更すると phase 2 が毎回失敗して `attempt` 上限に達する**。しかも第4.6節が掲げる逼迫時の導線（ゴミ箱を空にする / エクスポートして削除する）は**ログインを要求する**ので、この順序では到達できない。
  - **本 Issue が置くのは一様な終端だけである。** 前進不能が確定した時点で `terminalReason` を残して `poison` にし、運用へエスカレーションする（#38。第7.4節）。**行を巻き戻して旧 `passwordVerifier` を正本に戻す自動回収の設計は #45 である**（第7.4節の論点1・論点6）。**「黙って中間状態を残す」は選ばないという原則は本 Issue の結論であり、#45 はその上で自動回収を設計する。**
  - **#45 への制約を2つ、本 Issue の結論として残す。** **(1) 巻き戻せても起点 B（`changeOrigin = 'reset'`）の利用者は自力で再試行できない** — 起点 B に来る利用者は**旧パスワードを知らない**（それがリセットを依頼した理由である）ので、旧 `passwordVerifier` が正本に戻っても login できない。しかも発火契機の `SQLITE_FULL` は**書き込みだけが恒久的に失敗する**半死状態なので、**リセットを何度やり直しても phase 2 が毎回失敗する**。第6.2.2節 (a) の脱出経路は (i) リセット完走がこれで塞がり、(ii) SSO はパスワード単独アカウントには存在せず、第4.6節の逼迫時導線はログインを要求する。**したがってパスワード単独アカウントの失念利用者には自力の脱出経路が1本も無く、巻き戻しだけでは閉じない。** **(2) phase 2 が成功した後（`changeState = 'advanced'`）に Directory 側だけを戻してはならない** — そのとき User Data 側は `sessionEpoch` と `credentialVersion` を進めているので、Directory 側だけを戻すと第5.3節 step 5 (iii) が不一致になる（第6.9節が登録している締め出し経路そのものである）。**#45 はこの2つを入力として起点別・phase 別の扱いを決める。**
    - **「恒久的に」不一致とは書かない。射程を正確に書く。** 不一致は後続のリセット / パスワード変更が1回完走すれば解消する（phase 2 が User Data 側を n+2 へ進め、phase 3 が Directory 側を同じ値へ揃えるので、`credentialVersion` は再び一致する。上の phase 3 の「揃える」がこれを保証している）。**恒久化するのは、完走を妨げている原因が恒久的な場合だけである** — 具体的には上で名指しした `SQLITE_FULL`（当該 DO の書き込みが恒久的に失敗する半死状態。第4.7節）である。**したがって operator 側の受け口は正しい** — operator が容量を解放すれば次のリセットが完走して解消する。この射程を書かないと、受け口が「復旧不能」に見えて #38 が手順を書けなくなる。
  - **`changeState` の値域を3値（`null` / `'pending'` / `'advanced'`）にする。これは本 Issue の決定であり、#37 が落としてはいけない前方互換点の1つである**（第7.4節）。phase 2 が成功した直後、bucket は phase 3 へ進む前に**自 bucket のローカル書き込みで `changeState` を `'advanced'` へ倒す**（phase 3 が `null` へ戻す）。**この値が「phase 2 が適用済みか」を表す唯一の記録であり、#45 は上の制約 (2) をこの値でしか判定できない。**
    - **本 Issue の側にも独立した用途が2つある。** `promote-verifier`（phase 3）のガードが `changeState = 'advanced'` かつ `operationId` 一致であること（第5.1節）と、別の `operationId` による新しい依頼の後勝ち規則が `'pending'`（後勝ち）と `'advanced'`（`ConflictError`）で分かれること（上記）である。**したがって3値化は #45 待ちではなく、#37 がそのまま実装する。**
    - **`operations` 行の有無では代替できない。** その行は **User Data DO 側**にあり、それを読む RPC を第5.1節の全数表に持たない。
    - **「その実行の中で `advance-credential-change` が成功したか」でも代替できない。これは失われうる信号である。** phase 2 が成功した直後・`'advanced'` を書く前にエビクションが起きると（第2.1節 F-4）、以後の実行は phase 2 を冪等に再発行するが、その再発行が `.overloaded` で失敗し続けて上限に達しうる。そのとき「その実行では失敗した」を根拠に判定すると、**User Data 側だけが前進した状態を「未適用」と読む**ことになり、上の制約 (2) に正面から反する。**本書は「commit 後に応答が失われる」事象クラスを phase 2 の冪等性の根拠としてすでに認めている**（第6.4節 phase 2 の no-op 規定・第6.5.1節 phase 1 の `changeAuthToken` 温存）ので、同じ事象クラスを判定の側で無視してはならない。
    - **記録は phase 2 の成功「後」に書く。前に書かない。fail-safe の向きをここで決め切る。** 先に書くと「phase 2 が失敗したのに適用済みと記録される」ので、**`promote-verifier` のガードが通って phase 3 が Directory 側だけを昇格させる** — User Data 側の `credentialVersion` は進んでいないので step 5 (iii) が不一致になり、第6.9節に登録した締め出しがそのまま発火する。後に書くと「成功したのに記録前に落ちる」窓が残るが、**その帰結も `credentialVersion` の不一致であって認可は開かない**（`passwordVerifier` には誰も触っていない）。**前者は phase 3 まで進んで固定される破れ、後者は後続の完走1回で解消する不一致なので、後者を選ぶ。**
    - **その窓の射程を正確に書く。** 「phase 2 が成功し、`'advanced'` を書く前にエビクションが起き、**以後 User Data DO が上限まで到達不能であり続ける**」の3つが同時に成立したときだけ「未適用」と読まれる。**次の実行が再発行して成功すれば `'advanced'` へ進む**ので、通常のエビクションでは閉じる。残るのは User Data DO が恒久的に到達不能な場合であり、**そのときは step 5 が同じ DO を叩くので login はどのみち成立しない。** 到達可能に戻った後は不一致が残るが、後続のリセット / パスワード変更が1回完走すれば phase 3 が両側を揃える。**この残余は #38 の運用検知（`poison` 行と容量監視）で拾う。**
    - **値域を1つ広げるだけで、`null` でない値の扱いは変えない。** `lookup-credential` は **`changeState` が `null` でない行**をダミー材料へ倒す（`'pending'` と `'advanced'` を区別しない。第5.1節）。**どちらも fail closed の方向にしか動かない**ので、第6.9節が宣言した「どの中間状態でも認可は開かない」は保たれる。
    - **列は増やさない。** `changeState` は第4.1.1節（E-1）と第1.4節 検査8 の一覧に既にあり、変わるのは値域だけである。
- **phase 1 でリセットトークンを2段で無効化する。「未使用行の全削除」だけでは奪還経路が閉じない。** 段は **(1) 対象 `credentialId` の未使用トークン行を全削除する**、**(2) 残る全行（消費済みの行）の `changeAuthToken` を `NULL` にする**の2つで、**起点 A / 起点 B の両方に課す**（起点 B は自分が照合した行も `NULL` になるので、既定の一回性がそのまま満たされる）。規則の正本は第6.1節 (d) である。
  - **(1) が閉じる経路。** 攻撃者がアカウントを乗っ取ってリセットを1回依頼しトークンを保持 → 正規利用者が気づいてパスワードをリセット（epoch 前進・`credentialVersion` 前進・`passwordVerifier` 差し替え）→ **攻撃者の古いトークンが TTL 内なら依然有効なので、上の後勝ち規則を使ってもう一度リセットを完走し奪い返せる**。未使用行を消すとこれが閉じる。
  - **(2) が閉じる経路 —「一回性 ≠ 失効」である。** `consume-reset-token` は行を消さず `usedAt` を立てるだけで、**その同じ行に `changeAuthToken` を載せて返す**。したがって「消費済み・`changeAuthToken` 未使用」の行は (1) の削除対象に入らず `expiresAt` まで残る。この状態は公開面から到達する安定した中間状態である（`consume-reset-token` の成功後に `begin-credential-change` が `.overloaded` / DO のリセット / `ctx.abort()` で失敗すれば残り、`usedAt` によりリトライもできない）。**温存された値の保持者は、被害者が起点 A でも起点 B でも復旧した後に `begin-credential-change` を叩ける** — 起点 B のガードは `changeAuthToken` の一致と `credentialId` の一致だけで、`credentialVersion` も `changeState` も `usedAt` も被害者の復旧の事実も見ない。**(2) を課すと被害者のどちらの復旧経路も phase 1 を通るので、その時点で失効する。** 上の「同じ依頼の材料を後から再提示して差し替えることはできない」は成立していたが、**攻撃に要るのは再提示ではなく「まだ一度も使っていない材料の温存」**だった。
  - 削除の対象を引く索引と、`verifyAndConsume` の使い捨て性だけでは足りない理由は第6.1節 (d) に書いた。
...
- 窓の中で phase 2 が走れば User Data 側だけが先に進むので不一致となり拒否される。phase 1 だけが走った場合は step 3 が `changeState` の `null` でない値（`'pending'` / `'advanced'`）を見てダミー材料へ倒すので、そもそも照合が成立しない。**どちらの順序でも fail closed になる。**
...
| 2 | Directory bucket | **locator は検証済み IdP アサーション由来の canonical から導出する**（第6.1.1節 (R5)(iii)。link の対象はまだ `credential_locators` に無い credential なので (ii) では手順が決まらない）。previous 世代が keyring に載っている間はその bucket を読んで既存行の不在を確認し（第6.1節 (c)・第6.1.1節 (R1)）、active 世代の bucket に予約を取る（`credentialId` / `callerToken` / **`reservedUntil`**（`now + TTL`。第4.1.1節・第6.4節 1 の不等式が同じく掛かる）付き。既に他アカウントで `active` なら `ConflictError`）。**予約行を書くのと同じ `transactionSync` で、その bucket の job table へ `sweep-reservations` を投入する**（第6.3節 phase 1b とまったく同じ形。「予約行を書いた bucket が自分に投入する」規則を link にも適用する。下記） |
| 3 | Directory bucket | 予約を `status: 'active'` へ昇格する |
| 4 | User Data DO | `credential_locators` に reverse locator（`credentialId` + 世代 + bucket index + `kind` + 全長 HMAC + `credentialVersion` + `usableForLogin`（SSO 行なので常に真）+ `label`（provider 名））を追加し、`operations.phase = 'done'` にする。**この2つの作用は同じ `transactionSync` で書く**（理由は第6.3節 phase 4 と同じ — 分けると「ログインできるのに `phase` が未完了」という安定した中間状態が生まれる） |
...
- **手順2〜4 はすべて冪等である。** 手順2 は同じ `operationId` の予約が既にあれば成功として返し、手順3 は既に `active` なら成功、**手順4 は `(credentialId, generation)` を冪等キーとする upsert である**（既存行があれば `credentialVersion` / `usableForLogin` / `label` を上書きし、`credentialVersion` は単調非減少。第6.1.1節 (R8)・第5.1節の表）。`resume-link` は `operations.phase` を見て中断点から再開する。
  - **初版は手順4 の冪等キーを `(kind, hmac)` と書いていた。これは誤りである。** `credential_locators` の一意性は `(credentialId, generation)` であり（第4.1.1節）、鍵ローテーションの移送は**同じ credential について世代違いの行を追加する**（第6.8節の方針6）。`(kind, hmac)` を冪等キーにすると link だけを見れば動くが、**同じ RPC を共有する移送側の追加が常に no-op になり、移送が旧世代行を消した時点で `credential_locators` からその credential の行が1つも無くなる** — 到達性検査（第5.3節 step 5 (ii)）が移送済みの全利用者を fail closed で締め出す。**冪等キーは全経路で `(credentialId, generation)` に統一する。**
- **手順2 の予約行にも `reservedUntil` を書き、`sweep-reservations` を同じ `transactionSync` で投入する。両方が必要である。** 初版は link 経路にどちらも持っていなかった。**`reservedUntil` を落とすと**、作業述語（`reservedUntil < ?`）から永久に外れる一方で駆動源クエリ（`min(reservedUntil)`）の集合には入る `NULL` 行ができ、第7.4節 (1-A) が `nextRunAt = NULL` を書く（第4.1.1節の `reservedUntil` の項に帰結を書いた）。**投入点を落とすと**、第7.4節 (7) が「投入点の無い周期・反復ジョブは1回完走した時点で恒久的に停止する」と断定している失敗モードが link 経路で発火する — 定常状態では対象 bucket の `sweep-reservations` は残件0で `done` に落ちているので、link が新しい `reserved` 行を作っても誰も `pending` へ戻さない。**その結果、`resume-link` が前進できなくなった時点で（分類 (C) なのでバックオフ上限で `poison` に落ちる）予約行が恒久的に残り、その SSO 主体はロックされる。** 第7.4節 E-3 の `sweep-reservations` の投入点欄に本手順を追記してある。
  - **link の予約行には `sagaCommitted` 印を立てない、と決め切る。** link は単一 bucket の予約なので伝播する相手が無く、TTL 掃除で消えても `resume-link` が手順2 から予約を取り直せる（手順2 は「同じ `operationId` の予約が既にあれば成功」の冪等操作である）。**取り直しのあいだに同じ canonical を別のアカウントが取ったら手順2 が `ConflictError` で敗北し、下の巻き戻しに落ちる** — これは利用者から見て「連携に失敗した」であり、やり直せる。signup の `sagaCommitted` が必要だったのは「phase 2 で `active` なアカウントが完成した後に掃除が先行する」非対称を消すためで（第6.4節 2）、link にはその非対称が無い。
...
- **`resume-link` がバックオフ上限に達したときに何が残るかを決め切る。** `resume-link` は分類 (C) なので、`.overloaded`（第4.7節。リトライ禁止）や `SQLITE_FULL` で失敗し続ければ `poison` に落ちる。**そのとき手順3→4 の落下点で残る `active` な孤児 mapping は、本設計のどの回収経路の対象でもない** — `sweep-reservations` は `status: 'reserved'` しか見ず、`sweep-orphan-mapping` の残件条件は `kind = 'unlink'` 限定（第7.4節 (1-B)）、`finalize-withdrawal` は `deleting` 前提である。**利用者の再試行も効かない** — 再 link は request Worker が新しい `credentialId` を採番する（第6.1.2節 (C1)）ので、手順2 の `reserve-credential` は同じ `(kind, hmac)` に既存の `active` 行がある状態で走って敗北する。`resume-link` の `operationKey` は対象 `operationId` を含むので、`poison` 行の復活規則（第7.4節）も掛からない。**したがって、これは前進ジョブを新設して塞いだはずの状態そのものが単一世代で再生する形である。第6.9節の締め出し経路一覧に行として登録してある。**
  - **本 Issue が置くのは一様な終端だけである。** 前進不能が確定した時点（`ConflictError` またはバックオフ上限到達）で `terminalReason` を残して `poison` にし、運用へエスカレーションする（#38。第7.4節）。**`operations.targetLocators` が持つ全世代の locator へ `delete-mapping` を発行して mapping を回収する自動回収の設計は #45 である**（第7.4節の論点1・論点3）。
  - **#45 への入力として、材料が揃っていることを本 Issue の結論として残す。** 回収に要るのは `operations.targetLocators`（手順1 が記録する）と `account.callerToken`（`delete-mapping` の引数。第6.7節 手順3 と同じ規則）の2つで、**どちらも同じ User Data DO の中にある。** `account.callerToken` は退会が完走しない限り生きている（第6.7節 手順4 が唯一の削除点である）ので、**signup の終端規則が `callerToken` の置き場（コーディネーター予約行）を自分で消しうるのに対し、link は同じ問題を持たない**（第6.4節 3）。**したがって #37 は、終端で `operations` 行を閉じるときに `targetLocators` を空にしてはならない**（第7.4節の前方互換点1）。
  - **終端の向きは巻き戻しである、という判断は本 Issue の結論として残す。** link は「常に前進」が原則だが、**終端では巻き戻しが正しい** — 利用者から見て「連携に失敗した」であり、canonical が解放されるので**同じ SSO 主体で再 link すれば回復する**。これは手順2 が敗北したときに既に採っている扱いと同じである。**前進側に倒す代替（operator 経路を新設して locator を手で書く）は採らない** — 認証状態を operator が書ける経路を1本増やすことになり、第5.1節 (3-d) が `record-credential-locator` について塞いだ「任意アカウントへクレデンシャルを注入する原始関数」と同じ形の口が開く。**#45 はこの向きを入力として手順を設計する。**
  - **退会と競合している場合は本項より退会側の引き取りが先である。** `resume-link` は `account.status != 'active'` を見たら `operations` に触れずにジョブだけを `done` にする（下記）ので、そもそも `poison` には落ちない。**回収は退会 手順3 が行う**（第6.7節）。**一様な終端で `operations` 行を `phase != 'done'` のまま残しても、退会 手順3 の引き取り述語（`kind IN ('link','unlink')` かつ `phase != 'done'`）がその行を拾うので、退会経路は塞がらない**（第6.7節 手順3〜4）。
- **退会が始まったあとの `resume-link` は前進しない、と決め切る。** `account.status` が `active` でなければ手順2〜4 を1つも実行せず、**`operations` 行に触れずに**ジョブだけを `done` にする。**`operations.phase` を `'done'` へ倒してはならない** — 倒すと退会 手順3 の引き取り述語（未完了の link / unlink 行）から外れ、`credential_locators` にも行が無いので**回収経路が1つも無くなる**（第6.7節 手順3）。逆に前進を許すと、`account.callerToken` を材料にする手順4（`record-credential-locator`）が退会 手順4 の後で必ず失敗するうえ、退会中のアカウントに新しいログイン手段を足すことになる。**回収は退会側が行う**（第6.7節 手順3 の引き取り）。`resume-link` が `operations.phase = 'done'` を観測した場合（退会側が先に引き取った場合）も何もせず成功で終わる。
...
3. **Directory bucket** の mapping 行（`encryptedCanonical` を含む）を物理削除する。**削除対象は `credential_locators` に記録された `credentialId` ごとの全世代分**である — 鍵ローテーション中（第6.8節）は active 世代と previous 世代の両方に行が存在しうるので、**両世代の bucket に対して削除を発行し、「無ければ成功」の冪等操作**にする（第6.1.1節 (R3)。unlink 手順3 と同じ規則である）。RPC は `delete-mapping` で、**引数に `credentialId` と `account.callerToken` を載せる**（第5.1節 (3-b)）。この手順は `credential_locators` を読んで locator を得るので、**その行がまだ存在している必要がある**。**同じ `transactionSync` で、その `credentialId` の `password_reset_tokens` 行も全削除する**（第6.1節 (d)）。トークン行を残すと第6.2.1節 (d) の「bucket 側には何も残さない」が守られない。
   - **削除対象集合は `credential_locators` だけでは閉じない。未完了の cross-DO saga 行を引き取る、と決め切る。射程は `kind IN ('link','unlink')` の両方である。** 具体は「**`operations` に `kind IN ('link','unlink')` かつ `phase != 'done'` の行があれば、その行の `targetLocators` が持つ全世代の locator へも `delete-mapping` を発行し、成功したらその行を `phase = 'done'` にする**」（`updateOperation`。第4.1.1節）。
     - **unlink を引き取る理由。** unlink 手順2 が `credential_locators` から対象行を**先に**消すので、その credential の locator は `operations.targetLocators` にしか残っておらず、`credential_locators` だけを見る本手順の削除対象には**入らない**。
     - **link も同じ盲点を持つ。初版は unlink だけを引き取っていた。** link 手順1 は `operations` に `{ kind: 'link', phase: 'reserving', targetLocators }` を記録するが、`credential_locators` へ行を書くのは**手順4** である（第6.6節）。したがって**手順1〜3 のあいだ、link 対象の credential は本手順の削除対象集合に一切現れない。** 単一世代・ローテーション未実行でも次の順序で発火する — (1) 利用者が link を開始（手順1）→ (2) 手順2 で予約、手順3 で `active` へ昇格 → (3) 手順4 の直前で落ちる → (4) 利用者が退会 → (5) 本手順は `credential_locators` を引くので link 対象の mapping を削除せず、引き取り述語が unlink 限定なら link 行も拾わない → (6) 手順4 の前提条件（未完了行0件）が満たされて通過し、`credential_locators` と `account.callerToken` を消して `deleted` にする。**帰結は unlink×退会 とまったく同一である** — (i) `encryptedCanonical` を持つ mapping が bucket に残り第6.2.1節 (d) が破れる、(ii) その SSO 主体が**恒久的に再登録不能**になる、(iii) 回収経路が1つも無い（`sweep-reservations` は `status = 'reserved'` しか見ず、`sweep-orphan-mapping` の残件条件は unlink 行だけ、`resume-link` は `account.callerToken` を失っているので前進できない）。復旧手段が `purge-user-mappings` の 256 bucket 走査しか残らない。**第6.9節が unlink×退会 を Blocker 級として塞いだ以上、link も同じ基準で塞ぐ。**
     - **link 行の正しい終端は前進ではなく回収である。** 退会するアカウントに対する link を完走させても、そのログイン手段は直後に消える。`delete-mapping` は「無ければ成功」の冪等操作なので、手順4 が完走していた場合と二重発行になっても無害である。**逆に「退会の起点で未完了 link を先に完走させる」形は採れない** — 手順1 は `sessionEpoch` を進めて `deleting` へ倒す同期処理であり、link の完走（Directory への RPC 2本）を待てない。
     - **`sweep-orphan-mapping` の残件条件は unlink 限定のままにする、と決め切る**（第7.4節 (1-B)）。同ジョブは削除の再試行であり、link の前進を担うのは分類 (C) の `resume-link` である。link 行を残件条件に混ぜると「前進すべき saga を削除で終端させる」ジョブになり、通常の link（退会と競合しない link）まで巻き込む。**link 行を `phase = 'done'` にする手段は `resume-link` の完走（手順4）と本手順の引き取りの2つであり、それが全数である。**
4. **User Data DO** が `credential_locators` を消し、`account.callerToken` を消し、`account.status = 'deleted'` にする。残すのは**非 PII の tombstone だけ** — 不透明なアカウントキー（= `userId`）、status、epoch、完了時刻。
   - **手順4 の前提条件を1つ置く。「`operations` に `kind IN ('link','unlink')` かつ `phase != 'done'` の行が1件も無いこと」を満たすまで手順4 へ進まない**（満たさないなら `finalize-withdrawal` は手順3 を再試行して次の起動へ回す。第7.4節 (1-B)）。**射程は手順3 の引き取り述語と同じ2 `kind` である** — 片方だけを見ると、見ていない側の mapping が回収前に束縛材料を失う。**`account.callerToken` は `delete-mapping` の唯一の束縛材料なので、これを消すと未完了 saga の mapping を誰も消せなくなる。** その状態は3つの破れを同時に生む — (i) `encryptedCanonical` を持つ mapping が bucket に残って第6.2.1節 (d) の「bucket 側には何も残さない」が破れる、(ii) その canonical が**恒久的に再登録不能**になる（次の signup が `EMAIL_ALREADY_REGISTERED` で敗北する）、(iii) unlink 側については `sweep-orphan-mapping` の残件条件（`operations` の未完了 unlink 行）が**永久に偽にならず**、分類 (B) の残件が単調減少しない（第7.4節 (1-B)）ので tombstone だけになった DO が最小再開間隔で起き続ける。**手順3 の引き取りと手順4 の前提条件は対で入れる** — 前者だけでは順序の保証が無く、後者だけでは引き取る主体が居ない。
   - **`account.callerToken` を消したあとの再入も塞がる。** 手順4 が通った時点で未完了の link / unlink 行は0件なので、`resume-link` は `operations.phase = 'done'` を観測して何もせず終わり（第6.6節）、`sweep-orphan-mapping` は残件0で `done` に落ちる。**tombstone だけになった DO に張り続く Alarm が無いことが、この前提条件で保証される。**
...
Issue #34 の対応項目3 が要求しているのは「DO ID / routing key に PII を使わない**方針**の明記」であり、その方針を成立させるために本 Issue が決めたのは次の8点である。**いずれも「将来ローテーションできる余地を構造として残す」ための決定であって、ローテーションの手順ではない。**

1. **locator は正規化値の HMAC から導出する。生のメールアドレス・SSO subject を DO ID / routing key に使わない**（第5.2節）。
2. **`userId` 由来 locator と credential 由来 locator を分離する。** 前者は鍵に依存させない（`idFromName(userId)`）ので、**鍵ローテーションが User Data DO の同一性＝データ本体に波及しない**（第5.2.2節）。ローテーションで動くのは Directory bucket 内の mapping 行と、User Data DO 側の reverse locator 記録だけである。
3. **鍵は世代（`generation`）を持つ keyring として配布する。** エントリは `{ generation, key, bucketCount }` で、active 1件 + previous 0〜1件である（第5.2.3節）。**`generation` を locator 名（`dir:g{generation}:b{index}`）と keyring エントリの両方に含める。`bucketCount` は keyring エントリだけが持ち、locator 名には入らない**（名前に入るのは世代と index であり、bucket 数は名前からは復元できない。だからこそ keyring が世代のメタデータとして持つ。第5.2.3節・第6.2節）。**bucket 数の変更も世代の変更として表現する。**
   - **初版は主語を誤記して「`bucketCount` を locator 名と keyring エントリの両方に含める」と書いていた。訂正する。** 同一文の後半・第5.2.3節・第6.2節はいずれも「bucket 数そのものは名前に入らない」と述べており、**1文の中で自己矛盾していた**。`bucketCount` を keyring のメタデータとして持つ論拠は「previous 世代を引くときの剰余計算にその世代の bucket 数が必要だが、DO 名からは復元できない」（第5.2.3節）ことなので、名前に入れられるならその論拠ごと消える。**誤記は `.thread/34/adr.md`（ADR-026 / ADR-126）と #44 の Issue 本文の前提欄にも伝播していたので、4箇所すべてを訂正した。**
4. **再 HMAC は bucket の中で行い、平文 canonical を Worker 境界の外へ bulk で出さない。routing 鍵を bucket の SQLite にもインスタンスフィールドにも永続化しない**（第5.2.3節・第6.2.1節 (c) 2）。**この2つの制約が本 Issue の決定であり、#44 は守る。**
   - **「誰が起動し、誰が鍵を保持し、鍵をどう渡すか」— 一時注入という形そのもの — は本 Issue の決定に含めない。#44 が決める**（下の論点2）。初版は「鍵は RPC 引数として一時注入し」までを方針側（再検討しない決着）に置いていたが、**一時注入は実行主体の選択と不可分である**（注入する主体が居なくなれば注入という形も消える）。第5.2.3節が「この空白は『ガードを1つ足せば閉じる』形ではない」として**実行主体の再決定**を要求している以上、注入の形を決着扱いにすると **#44 が取りうる解（bucket に routing keyring を配る / 専用の maintenance Worker を立てて境界を引き直す）を design.md 側が先に禁じる**ことになる。**本表と #44 の Issue 本文の前提欄は、残す2制約だけを載せる形で一致させてある**（第6.4節の cross-DO 操作表も実行主体を名指しせず、第7.4節も「実行主体そのものを何にするかは #44 が決める」と書いてある）。
5. **`rotate-remap` は Alarm ジョブではない。** 根拠は方針4 の制約 (ii) である — **routing 鍵を bucket の SQLite にもインスタンスフィールドにも永続化しない**ので、Alarm が起動した時点で鍵は手元に無い。Alarm ジョブにすると鍵を永続化する実装に倒れ、その制約と第3.2節の非重複配布が同時に壊れる。**この結論は、#44 が第3.2節の非重複配布を保つ限り、どの実行主体を選んでも変わらない**（制約 (ii) が入力だからである）。**ただし #44 が論点2 で「bucket に routing keyring を常設のバインディングとして配る」を選ぶ場合は前提が崩れる** — そのとき Alarm 起動時にも鍵が手元にあるので、この結論と第3.2節の非重複配布を**同時に**見直す判断になる（第3.2節・第5.2.3節。判断を下すのは #44 である）。**#37 は移送を実装しないのでどちらに転んでも影響を受けない。****したがって第7.4節の `kind` 全数表と第4.1.1節の Directory `jobs` 行に `rotate-remap` は無い**（第1.4節 I-3 がこれを検査する）。**`rotate-encryption` はこの制約を受けない** — `IDENTITY_MAIL_ENCRYPTION_KEY` は state Worker の常設バインディングなので Alarm 起動時にも鍵が手元にある。実行主体が2つのローテーションで違うのはこの非対称による。
6. **移送は認証状態の変更ではないので、`credentialVersion` を増やさず、`credentialId` / `callerToken` を再採番しない**（第6.1.2節 (C1)・第5.1節 (3-b)）。**`credentialId` が世代非依存であることが、世代を跨いだ突き合わせを可能にしている唯一の材料である**（第6.1.2節）。
7. **`encryptedCanonical` の AAD は `(kind, credentialId, encryptionGeneration)` を束縛し、`hmac` を含めない**（第6.2.1節 (b-2)）。**したがって移送で AAD が変わらず、暗号文系の3列（`encryptedCanonical` / `encryptionGeneration` / `encryptionNonce`）をそのまま運べる。再暗号化は `rotate-encryption` だけの仕事である。**
8. **リセットトークン行は移送しない**（第6.1節 (d)）。移送対象は `credential_mappings` 行だけであり、旧世代 bucket に残ったトークン行は mapping を失って無効になる。この影響（ローテーション直後のリセット依頼失敗）は受容済みで、告知は #38 へ送る。

**ログに PII と逆引き可能な値を出さない方針も本 Issue の決定である**（第5.2節 (c)）。ローテーションの maintenance 経路については、**注入された鍵と復号結果を含む RPC のログ・トレースを無効化する / チェックポイントを bucket 単位に閉じ、1回の起動で複数 bucket 分の平文を同時にメモリへ載せない / 平文を DO の SQLite にも request Worker のいかなる永続領域にも書かない**の3点を保護規定として置く（第6.2.1節 (c)）。

**危険度の位置づけも本 Issue の結論である。** 移送は (a) 移送元 bucket の全利用者を `lookup-credential` も `request-password-reset` も解決不能にでき、(b) 移送先 bucket の `lookup-credential`（クラス (2)、無条件応答）から `passwordVerifier` を引き出せる。**したがって `purge-user-mappings` と同格以上に危険な操作であり、公開ルートを持たない operator 専用 maintenance 経路・実行前承認・実行監査を必須にする**（第5.1節 (3-c)・#38。第11.3節）。

#### 手順の設計は #44 で行う

**ローテーションの手順そのもの — 移送の phase・ガード・巻き戻し・退役条件の判定・2つのローテーションの関係 — は本 Issue では設計せず、Issue #44 が決着させる。** これは未決の放置ではなく、**設計を別 Issue へ切り出したという決定**である。根拠は3つある。

- **(i) Issue のスコープ。** #34 の対応項目3 が要求しているのは上記「方針」までであり、**鍵の世代管理・ローテーション手順は要求していない**。計画レビュー1周目（coverage S-003）で「AC-14 の鍵の所有者・世代管理は Issue 未要求」と判定し、必須基準から外して任意論点へ降格してある。**ローテーション手順は本設計が自ら広げた ［派生］ 領域である。**
- **(ii) 収束していない。** 手順に由来する Blocker が **R5・R6・R7・R8 の4ラウンド連続**で出続けた（R5: 巻き戻しが移送先行を無条件破棄 / 移送中の login fail closed。R6: 鍵の一時注入が bucket 側で検証不能 / 競合分岐の記録先が無い。R7: `rotation_checkpoints` の共有 / 移送 × `rotate-encryption` / 所持証明が漏えい時に成立しない。R8: 削除対象スナップショット × 移送 / 移送先書き込みが RPC 全数表に無い / 移送先での再投入が `rotate-encryption` の記録規則と矛盾）。**直すたびに、ローテーションが他のすべての saga（unlink / 退会 / credential 変更 / signup 予約 / login）と新しい形で干渉するのが見つかる**という状態だった。レビュー台帳の運用規則（同じ箇所が3回以上再指摘されたら個別修正で追いかけない）に該当する。
- **(iii) 切り出しても本設計は成立する。** 上記「方針」8点はいずれも**手順に依存しない**（列・キー・配布境界・AAD の束縛範囲・実行主体の非対称という構造の決定である）。逆に手順の側は方針を入力として要求するので、**依存の向きは「方針 → 手順」の一方向であり、循環しない。**

**#44 が決着させる論点は次の6つである。** #44 の Issue 本文の「決着させる論点」と同じ列挙であり、**本表はその一覧の正本ではない**（正本は #44 側）。ここに置くのは、本書の読者が「何がここに無いか」を確認できるようにするためである。

| # | #44 が決着させる論点 | 本書での現状 |
|---|---|---|
| 1 | **2世代並存の regime が全 saga と干渉しないこと。** unlink / 退会の削除対象の選択（**削除対象のスナップショットと移送の競合。R8 do-boundary B-001**）/ credential 変更 saga の locator 解決と `credentialVersion` の更新 / signup 予約の TTL 掃除と saga 再開 / login の到達性検査（移送中に fail closed に落ちる窓）/ ロックアウト脱出経路 | 第6.1.1節 (R6)(R7)(R9) が現状の案。方針だけを残し、規則の具体を委譲した |
| 2 | **移送（`rotate-remap`）の実行主体・鍵の渡し方（一時注入という形そのものを含む）・ガード・RPC エントリとしての位置づけ。** 移送先 bucket への書き込みと巻き戻しの破棄を含む（**RPC 全数表に無くガード未定義。R8 do-boundary B-002**） | 第5.1節の RPC エントリ表から外し、外した理由と全数の射程の限定を同表に書いた。**本書が残すのは方針4 の2制約だけである**（平文 canonical を bulk で越境させない / routing 鍵を bucket へ永続化しない）— 実行主体と鍵の渡し方は名指ししない（第5.2.3節・第6.4節の cross-DO 操作表・第7.4節も同じ扱いに揃えてある） |
| 3 | **routing 鍵ローテーションと暗号化鍵ローテーションの関係。** `rotation_checkpoints` の共有と退役条件の相互干渉、`rotate-encryption` の記録規則（**R7 do-boundary B-001 / R8 do-boundary B-003**） | `rotationKind` 列と置換キーは第4.1.1節に残す。退役条件の判定規則と記録の契機を委譲した（第6.2.1節 (b-1)） |
| 4 | **鍵漏えい起因のローテーションで、漏えい鍵の保持者が失効手段を無効化できない形。** previous 世代の鍵の所持証明は漏えい時にガードとして数えられない（**R7 security W-001**）ので、代替のガードが要る | 第5.1節 (3-c)・第5.2.3節に「bucket は routing secret を保持しないので照合材料を持たない」という空白の性質だけを残した |
| 5 | **巻き戻し時の中間状態の扱い** | 第6.1.1節 (R7) に方針だけを残した |
| 6 | **上記を機械検証できる不変条件**（第1.4節 I-1〜I-8 と同じ形式） | 第1.4節の形式が入力になる |

#### #37 は「ローテーション未実行」を前提に着手できる

**初期デプロイでは keyring の世代は1つだけであり、previous 世代のエントリは存在しない。したがって2世代並存の状態も、移送中の状態も存在しない。** #37 はこの前提で着手する。**#44 の結論を待つ必要は無い。**

**境界を表で明示する。左列が #44 の結論に依存しないもの、右列が依存するものである。**

| #37 が実装する範囲 | #44 の結論を待つ範囲 |
|---|---|
| keyring の型・構築時検査（`generation` の一意性 / active ちょうど1件 / previous 0〜1件 / `bucketCount ≥ 1`）と request Worker への配布（第3.2節・第5.2.3節） | 世代を1つ増やす操作そのもの（active の追加と previous への降格） |
| DO 名 `dir:g{generation}:b{index}` の組み立てと、リセットトークンが運ぶ `generation` / `bucketIndex` の範囲検査（第6.1節 (d)） | 複数世代が keyring に載った状態での lookup 順序の実運用 |
| `credentialId` を突き合わせキーにする全述語 — 到達性検査 / ログイン手段の数え上げ / 削除対象の選択 / `credentialVersion` の更新範囲 / `record-credential-locator` の冪等キー `(credentialId, generation)` と単調非減少 upsert（第6.1.1節 (A)・第6.1.2節） | 同じ credential が2世代の行を持つ状態でのそれらの振る舞いの検証 |
| `credential_mappings` / `credential_locators` / `password_reset_tokens` / `rotation_checkpoints` の列（`generation` / `encryptionGeneration` / `rotationKind` を含む。第4.1.1節） | `rotation_checkpoints` へ**何をいつ書くか**（記録の契機と `previousCount` の意味） |
| `rotate-encryption` の**起動 RPC とジョブの存在**（第5.1節・第7.4節）と残件条件（`encryptionGeneration` が active でない行が存在すること） | `rotate-encryption` の**退役条件と記録規則**、および移送側からの再投入経路 |
| AES-256-GCM の AAD を `(kind, credentialId, encryptionGeneration)` に束縛すること（第6.2.1節 (b-2)） | 暗号化鍵の世代を実際に進める手順 |
| `delete-mapping` を「対象 `credentialId` の全世代分へ発行する冪等操作」として実装すること（世代が1つなら発行は1本。第6.1.1節 (R3)） | 削除の対象集合が移送で増える場合の扱い（R8 do-boundary B-001） |
| **移送経路（`rotate-remap`）は実装しない** | 移送のすべて |

**「実装しない」の射程を1点だけ限定する。** #37 は**移送を駆動するコード**を書かないが、**移送が後から入る余地を壊す実装もしない。** 具体的には (a) `credential_locators` の一意性を `(credentialId, generation)` にする、(b) `record-credential-locator` を no-op ではなく upsert にする、(c) `credential_mappings.credentialId` に bucket 内 UNIQUE を張る、(d) 削除・数え上げ・到達性検査のキーを `(kind, hmac)` ではなく `credentialId` にする — **この4点は移送が無くても実装でき、落とすと #44 がどう設計しても後から入れられなくなる**（第6.9節の締め出し経路一覧の該当4行が根拠である）。

#### 「旧世代 locator が0件である」ことの証明の枠組み

**証明の枠組みは本 Issue で決める。** 全 bucket の checkpoint scan を**加算集計**するのではなく、**bucket ごとの snapshot 置換**にする。記録先は `rotation_checkpoints` で、置換キーは `(rotationKind, bucketIndex, generation)` である（第4.1.1節）。#19 のレビュー指摘 B-IDDS6-001 が記録した3つの穴への扱いは次のとおりで、**3つとも枠組みの側で閉じる。**
...
| checkpoint が加算更新で snapshot 置換になっていない | **bucket ごとの snapshot 置換にする。** 置換キーは `(rotationKind, bucketIndex, generation)` で、加算カウンタは持たない |

**枠組みの上で「いつ・何を書くか」と「どの述語で0件を判定するか」は #44 が決める**（上の論点3）。**`rotationKind` で絞らずに数えると、片方のローテーションの完了記録がもう片方の旧鍵破棄条件を成立させる**という判定だけは本 Issue の結論として残す（第4.1.1節・第6.9節の該当行）。
...
- **第2の規則を置く。fail closed が「利用者を締め出す」方向へ働く経路は列挙して塞ぐ。** 「開かない」だけを規則にすると、締め出す側の破れが設計から見えなくなる。初版は該当経路を「1つだけ」と断定し、次に4つ・8つ・12・14・16・18・20・21・22へ広げたが、実際には次の23経路がある。**行番号では指さない**（行が増えるたびに取り残されるため。第6.1節 (d) が本書内の自己参照について置いた規則と同じ理由である）。**根が共通の群が2つある** — `hmac`（世代依存の値）を credential の世代非依存な同一性として使っていたことを根とする4行（第6.1.2節）と、「routing 鍵のローテーションと暗号化鍵のローテーションを独立に走らせてよい」という未検証の1文を根とする2行（第6.8節の論点3）である。**新しい cross-DO 操作を足したら、この一覧にも行を足す。加えて — 3ラウンド目に追加した4行はいずれも「新しい操作」ではなく既存操作どうしの相互作用だったので — cross-DO 操作表（第6.4節）に載っている操作の組み合わせについても、片方が中間状態を持つ間に他方が走る場合を検査する。**

**塞ぎ方の設計を #44 へ委譲した行が7つある**（塞ぎ方の欄に **#44** と書いてある）。**経路の列挙は本 Issue の成果として残し、塞ぎ方だけを委譲する** — 列挙を落とすと #44 が入力を失うからである。**7行はいずれも「移送が進行中である」ことを条件節に持つので、ローテーションを実行しない #37 の実装では発火しない**（第6.8節）。

| 締め出す経路 | 塞ぎ方 |
|---|---|
| 予約 TTL 掃除と saga 再開の競合（ログイン手段を持たない `active` アカウント） | 第6.4節で3段（TTL の下限不等式・`sagaCommitted` 印・終端）で塞ぐ。**本 Issue が残すのは前2段と、終端が一様であること（`terminalReason` + `poison` + operator エスカレーション）である。TTL の不等式の右辺に後始末の上限まで含めるかと、終端時の自動回収は #45**（第6.4節 1・3・第7.4節の論点3・論点4） |
| 鍵ローテーション中、移送済みユーザーの login が到達性検査に落ちる（第6.8節） | **本 Issue が残す結論は「到達性検査は世代を条件に含めない」**（照合は `credentialId` だけを見る。第5.3節 step 5 (ii)）**と「`credential_locators` に同じ credential の両世代の行が並存してよい」の2点である。** 移送のどの段でどちらへ着地させるか（順序）の設計は **#44**（第6.8節の論点1） |
| SSO link の部分失敗で mapping が残り、その主体が永久ロックになる（第6.6節）。**落下点によって残るものが違うので両方を塞ぐ** — 手順3→4 では `active` な孤児 mapping、手順2〜3 では `reserved` 行である | `resume-link` で前進させる。`operations` 行に `targetLocators` を持たせて前進を可能にする。**ただし前進不能が確定したとき（`ConflictError` またはバックオフ上限到達）は一様な終端へ倒す** — `terminalReason` を残して `poison` にし、operator 経路へエスカレーションする（第6.6節・第7.4節「分類 (C) の…終端規則」）。**終端の向きが巻き戻しであること**（利用者から見て「連携に失敗した」であり、canonical が解放されるので再 link で回復できる）**は本 Issue の結論だが、`targetLocators` の全世代へ `delete-mapping` を発行して回収する手順の設計は #45**（第7.4節の論点1）。**#37 は `targetLocators` と `account.callerToken` を終端後も残す**（同節の前方互換点1・3）。**`reserved` 行の側は、手順2 が `reservedUntil` を書き、同じ `transactionSync` で対象 bucket へ `sweep-reservations` を投入することで回収される**（第6.6節 手順2・第7.4節 E-3 の投入点欄）。**どちらか一方を落とすと閉じない** — `reservedUntil` が `NULL` だと作業述語から永久に外れ、投入点が無いと定常状態で `done` に落ちた `sweep-reservations` が二度と `pending` へ戻らない（第7.4節 (7)）。**`resume-link` が分類 (C) なので `poison` に落ちうるという認識は、`reserved` 行の側だけでなく `active` な孤児 mapping の側にも掛かる** — したがって前進ジョブだけでは閉じず、終端の扱いが要る。**その終端が一様であることまでが本 Issue の成果であり、自動回収が入るまでこの行は operator 経路で受ける。** |
...
| **ローテーション中の credential 変更が phase 3 に到達できず恒久的に `pending` になる**（第6.8節 × 第6.5.1節）。旧新どちらのパスワードも通らず、第6.2.2節 (a) の脱出経路 (i) も同時に壊れる | 方針は第6.1.1節 (R6)（進行中の saga を跨いで行を移送しない）。**スキップ対象の全数と、判定を読み出し時点で行うことの帰結の設計は #44**（第6.8節の論点1） |
| **移送の巻き戻しが、移送先に着地済みの credential 変更の中間状態を無条件に破棄する**（第6.8節 × 第6.5.1節 phase 1）。移送先に行が書かれた後は lookup がそちらへ向くので `changeState = 'pending'` / `pendingVerifier` が移送先に載る一方、移送元側の CAS は移送元の変化（遅れて届く `report-login-result` で足りる）で失敗する。破棄すると Directory 側 n / User Data 側 n+1 の恒久不一致になり**ログイン不能**になる | 方針は第6.1.1節 (R7)（移送元・移送先の両側を CAS で守り、破棄も CAS の対象にする）。**3つの述語（前進 / 移送元の削除 / 移送先の破棄）の設計は #44**（第6.8節の論点5）。**`credential_locators` 側の追加行は破棄しない**は本 Issue の結論である（(R8) の単調性で正本以上に保たれる） |
...
| **複数クレデンシャル signup の phase 3 部分成功で `active` な孤児 mapping が残り、その credential が恒久的に再登録不能になる**（第6.4節 3） | **一様な終端へ倒す** — `terminalReason` を残して `poison` にし、operator 経路へエスカレーションする（第6.4節 3・第7.4節「分類 (C) の…終端規則」）。**発火条件は `ConflictError` に限らず「前進不能が確定した場合」である** — バックオフ上限到達（`.overloaded` / `SQLITE_FULL` の連続失敗）も含める。限定すると `overloaded` 系の経路で終端が1度も発火せず、記録すら残らない。**この経路を「黙って残す」ことはしないが、残った mapping を自動で回収する設計は #45 である**（第7.4節の論点1）。**#37 はコーディネーター予約行（`locators[]` / `candidateUserId` / `callerToken` の唯一の置き場）を終端で消さない**（同節の前方互換点2）— 消すと #45 がどう設計しても回収の材料が無い |
| **`resume-credential-change` が `poison` に落ちて `changeState` が `null` でない値（`'pending'` / `'advanced'`）のまま恒久化し、脱出経路が3つとも閉じる**（第6.5.1節 × 第7.4節）。旧パスワードは `lookup-credential` のダミー化で login 不能、新パスワードは phase 3 未完了で未昇格、リセット依頼も同じダミー化で解決されずトークンが発行されない。分類 (C) なので投入点（起点 A / 起点 B）からの再投入だけが `pending` へ戻せるが、その投入点を叩く材料が上の3つと同じものである。**残る手段は既存セッション（TTL 7日）からの 起点 A 再実行だけで、TTL が切れた時点で復旧手段がゼロになる。** 契機は設計内に実在する — 10 GB に達した User Data DO では `SQLITE_FULL`（書き込みだけが恒久的に失敗し `retryable` false。第4.7節）で phase 2 が毎回失敗し、逼迫時の導線（第4.6節）はログインを要求するので到達できない | **一様な終端へ倒す** — `terminalReason` を残して `poison` にし、operator 経路へエスカレーションする（第6.5.1節・第7.4節）。**行を巻き戻して旧 `passwordVerifier` を正本に戻す自動回収の設計は #45 である**（第7.4節の論点1・論点6）。**本 Issue が #45 への制約として残すのは2点** — **(1) 起点 B（`changeOrigin = 'reset'`）は巻き戻しだけでは閉じない**（旧パスワードを知らない利用者は巻き戻っても login できず、発火契機の `SQLITE_FULL` は書き込みだけが恒久的に失敗するのでリセットをやり直しても同じ結果になり、SSO も逼迫時導線も使えないため自力の脱出経路が1本も無い）、**(2) phase 2 の成功後に Directory 側だけを戻してはならない**（step 5 (iii) が不一致になり、同じ表の別の行になる）。**判定材料は `changeState` の3値である**（`'pending'` = phase 2 未適用 / `'advanced'` = 適用済み）。`operations` 行は User Data DO 側にあり読む RPC が無く、「その実行で phase 2 が成功したか」は commit 後に応答が失われる事象クラスで壊れるので、**判定材料を行の値として持つのは本 Issue の決定であり #37 が実装する**（第6.5.1節 phase 2・第4.1.1節・第7.4節の前方互換点4） |
...
| **ローテーション中の credential 変更が `credential_locators` の片側の世代の行だけを進め、step 5 (iii) が恒久的に不一致になる**（第6.8節 × 第6.5.1節 phase 2）。パスワードを変えた直後から一切ログインできず、`sessionEpoch` も進んでいるので既存セッションも死ぬ | 第6.1.1節 (R8) — `credentialVersion` を `credentialId` 単位で管理し、更新は全世代の行へ同時に行う。`record-credential-locator` を `(credentialId, generation)` の upsert（`credentialVersion` は単調非減少）にする。**この塞ぎ方は移送の手順に依存しないので本 Issue で決着済みであり、#37 がそのまま実装する** |
| **`record-credential-locator` の冪等キーを `(kind, hmac)` にすると、移送による追加が常に no-op になり旧世代行の削除後に `credential_locators` が空になる**（第6.6節 link 手順4 × 第6.8節）。到達性検査が**移送を終えた全利用者**を締め出す | 冪等キーを `(credentialId, generation)` に統一する（第6.1.1節 (R8)・第6.1.2節）。`hmac` が世代依存であることが根因なので、世代非依存の `credentialId` を突き合わせキーにする |
| **`encryptedCanonical` の AAD が `hmac` を束縛していると、移送された全行が復号不能になる**（第6.2.1節 (b-2) × 第6.8節）。リセットメールの宛先組み立てが壊れ、**アカウント回復手段そのものが失われる** | AAD を `(kind, credentialId, encryptionGeneration)` にして移送で変わらないようにする（第6.2.1節 (b-2)）。付け替え防止は `credentialId` の一意性で維持される |
| **移送先の陳腐化を区別しない移送規則が、リセットで設定した新パスワードを捨てて旧 `passwordVerifier` を正本に戻す**（第6.8節）。締め出しではなく**認可が開く**方向の破れだが、(R8) と対で扱わないと失敗モードが入れ替わるだけなので同じ表に並べる | 方針は第6.1.1節 (R9)（`credentialVersion` が大きい側を正本とし、移送を正本の側へ収束させる。「行があれば書かない」に倒してはならない）。**分岐の全数と記録先の設計は #44**（第6.8節の論点1） |
| **移送への鍵注入が bucket 側で検証されず、任意の鍵と世代で移送を駆動できる**（第5.1節 (3-c) × 第6.8節）。移送元 bucket の**全利用者**が login も `request-password-reset` も解決不能になり（復旧は 256 bucket の PITR しか無い）、同時に攻撃者が計算できる移送先 bucket の `lookup-credential` から **`passwordVerifier` が引き出せる**。締め出しと認可漏れが同時に成立する唯一の経路である | **bucket は `DIRECTORY_ROUTING_SECRET` を保持しないので、注入された鍵と照合できる自分の値を1つも持たない**（第3.2節の非重複配布）。候補（`encryptedCanonical` と `hmac`）はいずれも previous 世代の鍵に対する材料なので、**鍵の漏えいを動機とするローテーションでは漏えい鍵の保持者も同じ照合を通せる**。**ガードの設計は #44**（第6.8節の論点2・論点4）。本 Issue が残すのは (i) 移送を RPC エントリ全数表から外したこと（第5.1節）、(ii) 公開ルートを持たない operator 専用 maintenance 経路・実行前承認・実行監査を必須にすること（#38。第11.3節）、(iii) **#37 は移送経路を実装しない**ことの3点である |
| **`rotation_checkpoints` を2つのローテーションが区別列なしで共有し、片方の完了記録がもう片方の旧鍵破棄条件を成立させる**（第6.2.1節 (b-1) × 第6.8節）。移送が終わっていない状態で operator が previous 世代の routing 鍵を破棄すると、**未移送 bucket の全利用者**が `lookup-credential` も `request-password-reset` も恒久的に解決不能になる（復旧は 256 bucket の PITR しか無い）。routing ローテーションの走査対象 bucket は、その直前まで `rotate-encryption` が走っていた bucket そのものなので、同居は偶然ではなく構造的である | **`rotationKind`（`'remap'` / `'encryption'`）を1列足し、置換キーを `(rotationKind, bucketIndex, generation)` にする**のは本 Issue の結論である（第4.1.1節）。**両ローテーションの退役条件をどの述語で判定するか（`rotationKind` の絞りを含む）の設計は #44**（第6.8節の論点3） |
| **移送が古い `encryptionGeneration` の行を `rotate-encryption` 完走済みの bucket へ運び込み、再武装経路が無いまま暗号化 previous 鍵が退役する**（第6.8節 × 第6.2.1節 (b-1)）。運ばれた行の `encryptedCanonical` が恒久的に復号不能になり、リセットメールの宛先組み立てが壊れて**アカウント回復手段そのものが失われる**。`hmac` での login は通り続けるので**最初のリセット依頼まで顕在化しない** | **塞ぎ方の設計は #44**（第6.8節の論点3）。R8 do-boundary B-003 が、初版の塞ぎ方（移送先での `rotate-encryption` 再投入）が `rotate-encryption` の記録規則（完走時に `previousCount = 0` しか書かない）と矛盾して成立しないことを特定している。**2つのローテーションを同時進行させない運用制約と、それが破られたときに構造で止める仕組みの両方が #44 の論点である** |
| **`cancel-reservation` の `status` 非依存削除が、`operationId` の知識だけで完了済みアカウントの mapping を消せる**（第5.1節 × 第5.2節 (c) × 第6.4節 3）。第5.2節 (c) が未認証経路のログへ `operationId` を出すことを許しているので、ログ閲覧権限と binding 到達性の組み合わせで発火する。消えた行はどの回収ジョブの対象でもなく利用者に自己回復手段が無いうえ、空いた canonical で攻撃者がアカウントを取り直せる | ガードに **`callerToken` の定数時間比較**を足す（第5.1節 (3-a) の但し書き）。呼び出し元のコーディネーター bucket は自分の予約行にこの値を持っているので材料は増えず、`callerToken` は第5.2節 (c) の非露出対象なのでログには出ない。これで「`operationId` は capability ではない」が本文で成立する |
| **`begin-credential-change` の起点 B（リセット完了）の束縛が `operationId` + `credentialId` だけで、乗っ取り原始関数が残る**（第5.1節 × 第5.2節 (c) × 第6.1.2節 (C5)）。締め出しではなく**認可が開く**方向の破れだが、根が `cancel-reservation` の行と同じ（第5.2節 (c) がログ出力を許した値を束縛に使った）なので同じ表に並べる。`operationId` は未認証経路のログに出る値、`credentialId` は設定画面へ出す非秘密、宛先 bucket は `index` の総当たりで特定できるので、束縛の実体がゼロだった。終状態は**攻撃者のパスワードで正規にログインできるアカウント**である | `consume-reset-token` が **128ビットの一回性 `changeAuthToken`** を採番・保存・返却し、起点 B がその**定数時間比較を必須ガード**にする。成功時に同じ `transactionSync` で `値 → NULL` の CAS で消す（第5.1節 (3-d)・第6.5.1節 起点 B・第4.1.1節）。材料は増えない（bucket が自分で採番して自分で照合する。`callerToken` と同じ形）。第10.1節が PITR 経路について同じ昇格経路を塞いでいたのに RPC 側が開いていた非対称も、これで解消する |
| **退会が未完了の cross-DO saga（link / unlink）を引き取らず、退会後に孤児 mapping が残る**（第6.6節 link 手順1〜4 / unlink 手順2 × 第6.7節 手順3〜4）。**2つの `kind` がまったく同じ盲点を持つ** — unlink 手順2 は対象 `credentialId` を `credential_locators` から**消して**locator を `operations.targetLocators` へ退避し、link は手順1〜3 のあいだ `credential_locators` へ**まだ書いていない**（記録は手順4）。どちらも**退会 手順3 が `credential_locators` から引く削除対象に入らない**。手順4 が `account.callerToken` を消すと `delete-mapping` の束縛材料も失われる。帰結は (i) `encryptedCanonical` を持つ mapping が bucket に残って第6.2.1節 (d) が破れる、(ii) そのメールアドレス / SSO 主体が**再登録不能**になる、(iii) unlink 側では `sweep-orphan-mapping` の残件（`operations` の未完了 unlink 行）が永久に減らず、分類 (B) の残件が単調減少しないので tombstone の DO が最小再開間隔で永久に起き続ける。**link 側は R8 で unlink を塞いだときに漏れており、単一世代・ローテーション未実行でも発火する** | **退会 手順3 の削除対象集合に「`operations` の `kind IN (link, unlink)` かつ `phase != done` の行が持つ `targetLocators` の全世代分」を加え、削除が成功したその行を `phase = done` にする。手順4（`credential_locators` と `account.callerToken` の削除）は「未完了の link / unlink 行が0件になったあと」に限る**（第6.7節 手順3〜4・第7.4節 (1-B)）。**あわせて `resume-link` は `account.status != 'active'` を見たら前進せず、`operations` 行に触れずにジョブだけを `done` にする**（第6.6節。`phase` を `done` へ倒すと引き取り述語から外れて回収経路が消える）。これで退会経路でも残件が単調減少し、`sweep-orphan-mapping` / `resume-link` / `finalize-withdrawal` のどれが先に走っても同じ結果に収束する（`delete-mapping` は「無ければ成功」の冪等操作なので二重発行は無害である） |
| **`changeAuthToken` の一回性が失効を兼ねず、消費済み・未使用の値が被害者の復旧をまたいで奪還権として残る**（第5.1節 (3-d) × 第6.5.1節 phase 1 × 第6.1節 (d)）。締め出しではなく**認可が開く**方向の破れだが、根が起点 B の束縛（同じ表の1つ上の行）と同じなので並べる。`consume-reset-token` は行を消さず `usedAt` を立てて同じ行に `changeAuthToken` を載せて返すので、phase 1 が「**未使用**トークン**行**」だけを消す規則では**消費済み行の列が残る**。この状態は公開面から到達する安定した中間状態である（`consume-reset-token` の成功後に `begin-credential-change` が `.overloaded` / DO のリセット / `ctx.abort()` で失敗すれば残り、`usedAt` によりリトライ不能）。保持者は被害者が起点 A / 起点 B のどちらで復旧した後でも `begin-credential-change` を叩ける — ガードは `changeAuthToken` と `credentialId` の一致だけで、`credentialVersion` も `changeState` も `usedAt` も復旧の事実も見ない。**終状態は「攻撃者のパスワードで正規にログインできるアカウント」である** | **phase 1 の無効化を行の削除だけでなく列にも掛ける** — 同じ `transactionSync` で **(1) 対象 `credentialId` の未使用トークン行を全削除し、(2) 残る全行の `changeAuthToken` を `NULL` にする**（起点 A / 起点 B の両方に課す。第6.1節 (d)・第6.5.1節 phase 1）。**`NULL` の行は照合で常に不一致とする**（第5.1節 (3-b) の共通規則）。**「一回性 ≠ 失効」である** — 一回性が保証するのは「使われたら消える」ことだけで、攻撃に要るのは再提示ではなく**まだ一度も使っていない材料の温存**だからである。代替案（`changeAuthToken` に独立した短い有効期限を持たせる）は「被害者の復旧で失効する」性質が得られないので採らない |
| **`finalize-withdrawal` が `poison` に落ちて `account.status = 'deleting'` が恒久化し、mapping と `credential_locators` が残る**（第6.7節 × 第7.4節）。契機は他の分類 (C) と同じで、退会 手順2〜3 が `SQLITE_FULL`（第4.7節）や対象 bucket の `.overloaded`（第6.2.2節。攻撃者が誘導できる）で失敗し続ける場合である。**利用者側の再投入も効かない** — 投入点（第6.7節 手順1）は `account.status = 'active'` のときしか通らず、退会済みの主体はログインもできない。帰結は第6.7節が自ら列挙している2つで、(i) `encryptedCanonical` を持つ mapping が bucket に残って第6.2.1節 (d) の「bucket 側には何も残さない」が破れる、(ii) **そのメールアドレス / SSO 主体が恒久的に再登録不能になる**（次の signup が `EMAIL_ALREADY_REGISTERED` で敗北する）。**上の除外条項が退会の `deleting` を「この一覧の対象外」として名指ししているのは、前進が完走するか終端が発火するかのどちらかになるという意味であって、恒久化した状態が一覧から漏れてよいという意味ではない** | **一様な終端へ倒す** — `terminalReason` を残して `poison` にし、operator 経路へエスカレーションする（第7.4節）。**受け口は `purge-user-mappings` である**（`userId` をキーに取るので、退会中のアカウントの mapping には届く。第6.7節「最後の砦」）。**自動回収は #45**（第7.4節の論点1）。**認可は開かない** — epoch ガードと login step 5 (i) の状態照合が二重に拒否する（第5.3節・第5.4節） |

- **意図的に可用性を落とす中間状態は上の一覧に含めない。ただし除外の根拠は「短時間だけ」ではない。「前進が完走して抜けるか、前進不能が確定して終端が発火し、その状態が記録されて受け口へ届くかのどちらかである」ことである。** credential 変更の `changeState` が `null` でない期間（`'pending'` / `'advanced'`）（旧新どちらのパスワードも通らない。第6.5.1節）と退会の `deleting` は、**利用者の操作の結果として生じ、前進ジョブが完走するか、前進不能が確定して一様な終端（`terminalReason` + `poison` + operator エスカレーション。第7.4節）が発火するかのどちらかになる**ので、この一覧の対象外である。
  - **「短時間だけ閉じる」を根拠にしてはならない。** 前進ジョブは分類 (C) なのでバックオフ上限で `poison` に落ちうるので、実行時間の上界は設計として保証されていない。**根拠を保証されていない前提に置いていた初版は、`resume-credential-change` の `poison` が恒久ロックアウトであるという反例を抱えていた**（上の表に行として足した）。
  - **除外条項の成立条件を正確に書く。「必ず自動で抜ける」ではない。** 一様な終端が保証するのは**「黙って残らないこと」** — 終端の理由と `operationId` が `terminalReason` に残り、operator 経路へ届くこと — **までである**（第7.4節）。**自動で抜ける（巻き戻る）ようにする設計は #45 であり、それが入るまでのあいだ、恒久化した状態は上の表に行として登録され、塞ぎ方の欄が #45 を指す。** この区別を書かずに「必ず抜ける」とだけ書くと、除外条項が反例を持つ。**認可はどちらの場合も開かない**ので、第6.9節の宣言そのものは一様な終端だけで満たされる（残るのは可用性である）。
  - **`poison` 行そのものの検知と手動再投入は #38 の領分である**（第11.3節）。**現在の第11.3節は `done` / `poison` の保持期間しか送っていなかったので、検知・再投入の項目を1つ足してある。**
...
| `finalize-withdrawal` | User Data DO | 第6.7節 手順1（`account.status = 'deleting'` と同じ `transactionSync`）**および第5.1節（`abandon-account` エントリが `account.status` を `deleting` へ倒すのと同時に投入する。そのエントリを発行する主体の設計は #45 だが、投入するのはエントリの定義そのものである）**。**投入点は2つあり、これが全数である**（I-1） | **(C) 一回性** | 退会の手順3〜4 を前進させる（第6.7節）。到達不能な signup を破棄したときの**アカウント側の後始末**も同じ経路に乗る（第5.1節）。**mapping の回収は別である** — 終端の時点では `credential_locators` が空なので本ジョブでは回収できない（第6.4節 3） |
...
| `resume-link` | User Data DO | 第6.6節 link 手順1（`operations` 行の記録と同じ `transactionSync`） | **(C) 一回性** | SSO link saga の前進（第6.6節）。手順3 と手順4 の間で落ちたときに `active` な孤児 mapping を**解消する唯一の経路である**（手順4 まで前進させて解消する。前進不能が確定したときの自動回収は #45。第7.4節） |
...
| `sweep-reservations` | Directory bucket | 第6.3節 phase 1a / 1b **および第6.6節 link 手順2**（予約行と同じ `transactionSync`。**予約を書いた bucket が自分に投入する** — 3箇所とも同じ形である） | **(A) 時刻駆動** | 予約の期限切れ掃除（第6.4節） |
| `sweep-reset-tokens` | Directory bucket | 第6.1節 (d)（トークン行の発行と同じ `transactionSync`） | **(A) 時刻駆動** | リセットトークンの期限切れ行掃除（第6.1節 (d)） |
| `rotate-encryption` | Directory bucket | 第6.2.1節 (b-1)（operator 専用 maintenance 経路の「`rotate-encryption` の起動」RPC が自 job table へ投入する。第5.1節のクラス (3)）。**これが本 Issue で確定している唯一の投入点である** — 移送側からの再投入を投入点に足すかは #44 の論点である（第6.8節の論点3） | **(B) 残件駆動** | メール暗号鍵ローテーションの再暗号化（第6.2.1節 (b-1)） |

**`rotate-remap`（routing secret ローテーションの再写像）は本表に無い。Alarm ジョブではないからである。** 再 HMAC には routing key が要るが、**その鍵を bucket の SQLite にもインスタンスフィールドにも永続化しない**と定めてあるので（第5.2.3節・第6.8節の方針4 の制約 (ii)）、**Alarm 起動時には手元に無い**。Alarm ジョブにすると #37 が鍵を永続化する実装へ倒れ、その制約と第3.2節の秘密の配布境界が同時に壊れる。**鍵の渡し方そのものは #44 が決めるが、この結論は #44 が第3.2節の非重複配布を保つ限りどの形を選んでも変わらない**（制約 (ii) が入力だからである）。**唯一の例外は #44 が論点2 で「bucket に routing keyring を常設のバインディングとして配る」を選ぶ場合で、そのときは非重複配布と本結論を同時に見直す判断になる**（第3.2節・第6.8節の方針5）。**したがって `rotate-remap` は Alarm ジョブにしない**（第6.8節の方針5）。**実行主体そのものを何にするかは #44 が決める**（第6.8節の論点2）が、**Alarm ジョブでないことは本 Issue の結論なので本表には現れない** — 第1.4節 I-3 がこれを検査する。`rotate-encryption` が本表に残るのは、`IDENTITY_MAIL_ENCRYPTION_KEY` が state Worker の常設バインディングで Alarm 起動時にも存在するためである。
...
**`cancel-reservation` と `abandon-account` は本表に現れない。ジョブではなく RPC だからである。** この2本は「前進」ではなく**補償と回収**であり、`resume-signup` ジョブの実行中に同期の RPC として発行される（第6.5節の敗者補償と、#45 が設計する終端後の自動回収）。**ジョブ種別を足さない、と決め切る。** 独立した `kind` にすると回収が別の Alarm 起動へ持ち越されて「`deleting` へ倒したが mapping が回収されていない」窓が新しく開くうえ、**その `kind` の投入点・再武装分類・E-5 の類型が #45 の結論待ちになり、本表と第4.1.1節と第7.7節が同時に未確定になる。** **#45 は回収を `resume-signup` / `resume-link` の実行の中に置く前提で設計する。**
...
- **再投入は `nextRunAt` を「早める方向にのみ」更新し、遅らせない。射程は実行可能集合（`status IN ('pending','running')`）の行である。** 既存行の `nextRunAt` より早い値で再投入されたら更新し、同じか遅ければ何も書かずに成功を返す。`purge-trash` の retention 短縮はより早い `nextRunAt` を渡すので前倒しされ、`send-mail` の連打は同じ窓に対して同じ `nextRunAt` を渡すので**行がまだ実行可能集合にいるあいだは**何も起きない（＝重複が捨てられる）。**行が `done` へ落ちた後の連打は本規則では決まらない** — そちらは下の `status` 別の規則が決める。
- **`status` 別の規則が「早める方向にのみ」より優先する、と決め切る。** 優先順位を書かないと、`done` / `poison` の行への再投入が「方向で決まる」のか「`status` で決まる」のか読み手に委ねられ、`send-mail` のような (C) 一回性ジョブでは**通常事象として**両者が衝突する（下記）。**優先させる理由は構造的である** — 完了時に `nextRunAt` を `NULL` にする規定を置いてあるので（下記「backoff と poison」）、`done` / `poison` の行には比較相手の `nextRunAt` が存在せず、**「早める方向」という判定そのものが定義されない。**
...
- **`status = 'poison'` の行に対する再投入は、再武装分類にかかわらず同じ行を `pending` へ戻して `attempt` を0にし、`nextRunAt` / `payload` / `payloadDigest` を引数の値で置き換える。別行は作らない。** `operationKey` は「そのジョブの同一性」なので、行を増やすと同一性の意味が壊れる。`poison` から戻すのは、`terminalReason` が残る一方で**利用者操作による明示的な再投入まで拒み続ける理由が無い**ためである（`terminalReason` は上書きせず残す）。
- **`status = 'done'` の行に対する再投入で `pending` へ戻すのは、再武装分類が (A) / (B) の5種に限る。分類 (C) の7種は `done` の行を復活させず、何も書かずに成功を返す。** 更新の形は `poison` からの復帰と同じである（`attempt` を0、`nextRunAt` / `payload` / `payloadDigest` を引数の値で置き換え、別行は作らない）。
  - **(A)(B) を復帰させる理由。** この経路が無いと、**定数 `operationKey` を持つ周期・反復ジョブが1回完走した時点で prune 保持期間ぶん再投入を受け付けなくなる**（下記 (7)）。平常時にはどのジョブも必ず `done` へ落ちるので、投入点からの復帰が唯一の再起動手段である。
  - **(C) を復帰させない理由。** (C) の `done` は「その `operationKey` が表す一回分の仕事が完了した」という意味である。7種はいずれも `operationKey` に対象の同一性を含む（`reindex` / `migrate-bulk` は `targetVersion`、`resume-*` / `finalize-withdrawal` は対象の `operationId` ないしアカウント、`send-mail` は対象 canonical の全長 HMAC + 依頼の窓。第6.2.2節 (b)）ので、**同じキーの再投入は新しい仕事ではなく重複依頼である。**
  - **分類で分けるのが要点で、`payloadDigest` の一致では分けられない。** (A)(B) の5種は定数キーであり投入点が毎回同じ payload を渡すので digest も一致する。digest の差で判定する形にすると、周期ジョブの正当な復帰と (C) の重複依頼が同じ形になって区別できない。
  - **これで `send-mail` の同窓連打が閉じる。** `send-mail` は Alarm が数秒で拾って `done` へ落ちる (C) なので、同じ窓での2回目以降の依頼は**通常事象として「`done` に対する再投入」になる**。「`done` は必ず復活させる」と書くと、依頼のたびに行が `pending` へ戻って Alarm が起き直し、**行数は1本に収束するのに起床回数と rows written が依頼回数に比例する**（`setAlarm` 1回は課金対象の1行書き込みである。第2.1節 F-24）。二重送信そのものは第6.2.2節 (b) が塞いでいる（窓内の再依頼は「宛先を持たない側」の行として書かれる）ので利用者影響は無く、本規則が閉じるのは課金と DoS の面である。**第7.6節の「連打はジョブ行1本に収束する」も、この規則があって初めて起床回数まで含めて成立する。**
...
- **(1-A) 分類 (A)（時刻駆動の周期ジョブ）の駆動源クエリは「そのジョブの作業述語から時刻条件だけを外したもの」と定義する。** **作業述語と駆動源が同じ行集合を指すことが規則の要である**（ずれると下の (5) の失敗モードが開く）。**本表は時刻駆動の周期ジョブ専用である** — 時刻列を持たないジョブに `min(...)` は定義できないので、分類 (B) は本表に行を持たない。
  ...
  | `sweep-reservations` | `WHERE status = 'reserved' AND reservedUntil < ? AND sagaCommitted IS NULL` | `WHERE status = 'reserved' AND sagaCommitted IS NULL` の `min(reservedUntil)`（**`reservedUntil` は NOT NULL なので `min(...)` が `NULL` になる状態は作れない**。第4.1.1節） |
...
- **(1-B) 分類 (B)（残件駆動の反復ジョブ）は残件条件だけを持ち、次の起動時刻は「最小再開間隔」で決める。** これらのジョブの作業述語には**時刻条件が無い**（回収すべき対象がいつからあるかは問題にならない）ので、(1-A) の「時刻条件を外す」という操作そのものが定義されない。**したがって駆動源と作業述語は同一である。**
  ...
  **残件があれば `nextRunAt` を `now + 最小再開間隔` に置いて `pending` へ戻し、無ければ `done` にする。** 1回の起動で処理する件数には (iii-a) と同じ上限を掛けるので（下記）、残件が多いときは最小再開間隔ごとに刻んで進む。**最小再開間隔は (1-A) の安全弁クランプと同じ値を使う** — 用途が「時刻を持たないまま即時再発火するのを避ける」で同じだからである。**分類 (B) に対して「早める方向にのみ」は適用しない**（(1-A) と同じ理由）。
  ...
  - **`sweep-orphan-mapping` の残件は退会経路でも単調減少する。両ジョブの生存期間が噛み合っていることを明示する。** `delete-mapping` の唯一の束縛材料は `account.callerToken` であり（第5.1節 (3-b)）、それを消すのは退会 手順4 である。**未完了の unlink 行が残っている状態で手順4 が走ると、`sweep-orphan-mapping` は残件を永久に減らせなくなる**（束縛材料が失われるので `delete-mapping` を発行できず、`phase = 'done'` にも到達できない）。塞ぎ方は第6.7節の2点で、**どちらも本 Issue の結論である** — (i) **退会 手順3 が未完了 unlink の `targetLocators` も削除対象に取り込み、成功したらその unlink 行を `phase = 'done'` にする**、(ii) **手順4 は「未完了 unlink 行が0件」を前提条件にし、満たさないなら手順3 を再試行して次の起動へ回す**。これで**どちらのジョブが先に走っても同じ結果に収束し**（`delete-mapping` は「無ければ成功」の冪等操作なので二重発行は無害である）、退会経路でも残件が単調減少する。**(i) だけでは順序の保証が無く、(ii) だけでは引き取る主体が居ないので、対で必要である。**
    - **退会の引き取りと手順4 の前提条件の射程は `kind IN ('link','unlink')` だが、本表の残件条件は unlink 限定のままである、と決め切る。** 引き取り側を広げたのは、link も手順1〜3 のあいだ `credential_locators` に行を持たないという同じ盲点を持つからである（第6.7節 手順3）。**一方こちらを広げてはならない** — 本ジョブは削除の再試行であり、link の前進を担うのは分類 (C) の `resume-link` である。link 行を残件条件に入れると、退会と競合しない通常の link まで「前進すべき saga を削除で終端させる」対象になり、しかも `resume-link` と本ジョブが同じ `operations` 行を逆向きに動かす。**link 行を `phase = 'done'` にする手段は `resume-link` の完走と退会の引き取りの2つであり、それが全数である**（本表の残件条件はどちらも参照しない）。
  ...
  - **`sagaCommitted` 印のある `reserved` 行は作業述語からも駆動源からも外れる。** 回収するのは終端後の operator 経路（`cancel-reservation` の operator 実行。受け口の割り当ては #45）と、そこで `poison` になった行の運用エスカレーション（#38）であって `sweep-reservations` ではない。**駆動源からも外すのが要点で、外さないと `min(reservedUntil)` が恒久的に過去へ固定される。**
...
- **(7) 分類 (A)(B) の5種は、投入点と再武装の両方を持って初めて閉じる。** 再武装だけでは足りない — (1-A) / (1-B) はどちらも「集合が空なら `done`」なので、**平常時（ゴミ箱が空 / 予約が0件 / 未使用トークンが0件 / 未回収の unlink が0件 / 再暗号化残が0件）にはどのジョブも必ず `done` へ落ちる**。そこから `pending` へ戻す唯一の手段が、上の再投入規則（**分類 (A)(B) について** `done` / `poison` の行を `pending` へ戻す規則）であり、それを発行するのが `kind` 全数表の「投入点」欄である。**投入点の無い周期・反復ジョブは、1回完走した時点で恒久的に停止する。** これが第1.4節 I-1 を不変条件に置いた理由である。

**実行可能集合を1つ定義し、以下の述語をすべてそこへ帰着させる。** **実行可能集合 = `status IN ('pending','running')` の行である。** `done` / `poison` は実行可能集合の外にあり、**そこへ戻す唯一の手段は上の再投入規則（`enqueueJob`）である。ただし分類 (C) の `done` 行は戻す手段を持たない終端である**（上の `status` 別の規則）。 この定義を置かないと、claim の述語と `deleteAlarm()` の述語が別々の読みを持ち、下の2つがどちらも壊れる。

**claim と完了の CAS。** 1件ずつ `UPDATE jobs SET status='running', leaseUntil=?, ownerToken=? WHERE operationKey=? AND (status='pending' OR (status='running' AND leaseUntil < ?))` で claim し、0行更新なら他が持っているとみなして次へ進む。完了も `WHERE operationKey=? AND ownerToken=?` の CAS で行う。**第2の選言に `status='running'` を必ず含める** — 落とすと、`done` / `poison` の行が過去の `leaseUntil` を保持したままこの述語に一致し、**prune の保持期間のあいだずっと再 claim・再実行の対象になる**（`finalize-withdrawal` のような一回性ジョブが完了後に再実行される）。ジョブは冪等なので（第7.7節 項3）実害は限定的だが、**下の「lease の用途を DO リセット時の回収手段だけに限定できる」という断定がそのままでは成立しない。** DO は single-threaded なので同一 DO 内で claim が競合することは無いが、**lease は「実行中に DO がリセットされた」場合の回収手段として必要である**（第2.1節 F-4 のエビクション）。期限切れ lease は専用の索引から reclaim する。

**backoff と poison。** 失敗時は `attempt` を進め、指数バックオフで `nextRunAt` を先送りする。上限回数を超えたら `status='poison'` にして `terminalReason` を残し、ホットパスの索引から外す。**`done` / `poison` のどちらへ落とすときも、同じ `transactionSync` で `completedAt` に現在時刻を書き、`leaseUntil` / `ownerToken` / `nextRunAt` を `NULL` にする**（第4.1.1節。`completedAt` が `pending` / `running` の行で `NULL` であるのと対になる列ごとの状態規定である）。**`nextRunAt` を残さないのが要点である** — 残すと下の `deleteAlarm()` の述語が prune されるまで真にならず、恒久起床ループが開く。`done` と `poison` は別々の保持期間で prune し、走査を bounded に保つ。

**分類 (C) の cross-DO saga 前進ジョブが `poison` に達したときの終端規則の正本は本項である。** 分類 (C) の `done` は復活させない（上の `status` 別の規則）ので、**`poison` は「その saga がもう誰にも前進させられない」ことを意味する**。ところが saga の中間状態は残る。**本 Issue が決着させるのは「残った状態をどう扱うか」の骨格 — 一様な終端 — までであり、自動回収（巻き戻し）の設計は Issue #45 が決着させる。** これは未決の放置ではなく、**設計を別 Issue へ切り出したという決定**である（第6.8節が鍵ローテーションの手順について採ったのとまったく同じ形である）。

#### 本 Issue が決着させた方針

1. **発火条件は「前進不能が確定した場合（`ConflictError` またはバックオフ上限到達）」である。`ConflictError` に限定してはならない。** **`.overloaded`（第4.7節。リトライ禁止）・`SQLITE_FULL`・連続エビクション（第2.1節 F-4）で失敗し続けてバックオフ上限に達する経路では `ConflictError` が出ない**ので、限定すると終端が1つも走らないまま `poison` になる。**しかも `overloaded` は攻撃者が誘導できる** — 第6.2.2節が「任意の1 bucket を選んで 1,000 req/s の soft limit へ追い込める」と断定しているので、`resume-*` が Directory RPC を通る瞬間を狙い続ければバックオフ上限まで押し切れる。**したがって発火条件を例外事象として扱ってはならない。**
2. **終端は `kind` によらず一様である。** **`poison` へ落とすのと同じ `transactionSync` で `terminalReason` を書き**（「backoff と poison」の段落の規定どおり `completedAt` / `leaseUntil` / `ownerToken` / `nextRunAt` も同時に確定させる）、**operator 経路へエスカレーションする**（#38）。**`kind` ごとに終端の入り方を変えない。**
3. **「黙って中間状態を残す」は選ばない。** 第6.4節が signup について置いたこの基準は、分類 (C) の全 `kind` に等しく掛かる。**一様な終端が満たすのは「残った状態が記録され、受け口へ届くこと」であって、自動で解消することではない。** 自動で解消させる設計が #45 である。
4. **`terminalReason` に PII と再利用可能な秘密を載せない**（`jobs.payload` と同じ制約。第7.4節）。**本 Issue が載せると決めたのは (a) 終端の理由**（`ConflictError` / バックオフ上限）**と (b) `operationId`** である。**`callerToken` は載せない**（第5.2節 (c) の非露出対象である）。**残渣の所在をどこまで載せるかは #45**（下記の論点5）。**ただし射程を1つ固定する — #45 が残渣の所在を載せる場合も、第5.2節 (c) の非露出対象（canonical / `hmac` / locator / `callerToken` / `changeAuthToken` / `passwordVerifier` / リセットトークン）は載せられない。** 「PII と再利用可能な秘密でないこと」だけを制約にすると **locator は形式的に通過する**（PII でも秘密でもない）が、第11.3節 (i) は `terminalReason` をメトリクスへ添えると決めているので、**載せた瞬間に bucket 分割の写像が運用面へ出る** — 第5.2節末尾が「運用画面への露出」を HMAC を使う理由に数えている、まさにその面である。**載せる必要があるなら、`terminalReason` を運用系へ素通しする第11.3節 (i) の側を同時に変える。**
5. **一様な終端で第6.9節の宣言は保たれる。** 残った中間状態はどれも**認可が開く方向へは倒れない** — `credential_locators` に対応行を持たない `active` な mapping は login step 5 (ii) の到達性検査で拒否され（第5.3節）、`changeState` が `null` でない mapping は `lookup-credential` がダミー材料へ倒す（第5.1節）。**残るのは可用性の問題であって、認可は開かない。** 締め出しの側は第6.9節の一覧に行として登録済みであり、**塞ぎ方の欄が #45 を指す。**

**終端時に残りうる中間状態と、回収の材料の置き場を分類 (C) の全 `kind` について並べる。この列挙そのものが本 Issue の成果であり、#45 の入力である。**

| `kind` | 終端時に残りうる中間状態 | 回収の材料と置き場 |
|---|---|---|
| `resume-signup` | phase 3 の部分成功では `credential_locators` に対応行を持たない **`active` な孤児 mapping**、phase 1b の途中なら **`reserved` 行**。どちらもコーディネーター bucket に同居しうる（第6.4節 3） | コーディネーター予約行が持つ `locators[]` / `candidateUserId` / **`callerToken`**（第6.3節 phase 1a・第4.1.1節）。**この行が3つの唯一の置き場であり、`jobs.payload` へ退避できない** |
| `resume-link` | 手順3→4 の落下点では **`active` な孤児 mapping**、手順2〜3 では **`reserved` 行**（第6.6節） | `operations.targetLocators`（手順1 が記録する）と `account.callerToken`（同じ User Data DO の `account` 行にある。第6.6節） |
| `resume-credential-change` | `changeState` が `null` でない mapping 行（`'pending'` = phase 2 未適用 / `'advanced'` = 適用済み）と、それに伴う `pendingVerifier` / `changeOrigin` / `operationId`（第6.5.1節） | mapping 行そのもの。**`changeState` の3値が「phase 2 が適用済みか」の唯一の判定材料である**（第6.5.1節 phase 2・第4.1.1節） |
| `finalize-withdrawal` | `account.status = 'deleting'` のまま、mapping / `credential_locators` が残る（第6.7節） | `credential_locators` と `operations.targetLocators`、および `account.callerToken`（手順4 まで消さない。第6.7節） |
| `send-mail` | **残らない。** 巻き戻す状態を持たず、窓が変われば新しい `operationKey` になるので投入点から回復する（第7.6節） | 不要 |
| `reindex` | **利用者データの状態としては残らない。** 索引が古いまま残るだけで、投入点が migration ゲートなので `schema_version` が進んだ後は冪等にスキップされる（第9.2節） | `migration_progress.cursor`（第9.3節） |
| `migrate-bulk` | 同上。migration が途中まで進んだ状態が残る（第9.3節） | `migration_progress` の `targetVersion` / `step` / `cursor`（第9.3節） |

**operator 経路は2本ある。その存在は本 Issue の結論である。** **`purge-user-mappings`**（第6.7節「最後の砦」。`userId` をキーに取り、自 bucket 内の全 mapping 行を削除する冪等 RPC）と **`cancel-reservation`**（第5.1節。`operationId` 一致 + `callerToken` で `status` を問わず削除する冪等操作）である。**どの残渣をどちらが受けるかの割り当ては #45 である**（下記の論点5）— **`purge-user-mappings` は `userId` をキーに取るので、`candidateUserId` しか持たない予約行の残渣には届かない**（第4.1.1節）。**「operator へ送る」と書いただけでは届かない残渣がある**というこの事実は本 Issue の成果として残し、割り当てを委譲する。

- **`cancel-reservation` は operator が単独で実行できる形ではない。この限界も本 Issue の結論として残す。** 同エントリのガードは `callerToken` の定数時間比較を含むが（第5.1節 (3-a)）、**`callerToken` を返す口は第6.5.1節 phase 0 の A-1（クラス (1)。当該利用者の有効なセッションが要る）だけである。** operator 専用の maintenance エントリはどれも返さず、上の方針4 は `terminalReason` に載せないと決め、`jobs.payload` も同じ制約で不可、第5.2節 (c) はログ・トレース・監視系への露出を禁じている。**したがって operator は `operationId` しか持てない。****材料の受け渡しをどう設計するかは #45 の論点5 に含める。**
  - **埋め方の向きを1つ禁じておく。`callerToken` を `terminalReason` / 運用画面 / メトリクスへ出す形で受け渡してはならない。** 出すと第6.9節の該当行が閉じた経路（`operationId` は第5.2節 (c) がログ出力を許した値なので、`callerToken` も出れば binding 到達性 + ログ閲覧権限で**任意の完了済みアカウント**の mapping を `status` 非依存に削除できる）がそのまま再び開く。**`cancel-reservation` は `status` を問わないので、被害は終端した saga に限定されない。****この禁止は #45 と #38 の両方に掛かる。**

#### 自動回収の設計は #45 で行う

**終端時の自動回収 — 段の順序と各段の原子性境界・終端モードの印・後始末そのものが失敗したときの規則と再試行上限・受け口の割り当て・起点別の扱い — は本 Issue では設計せず、Issue #45 が決着させる。** 根拠は3つある。

- **(i) Issue のスコープ。** #34 の対応項目4 が要求しているのは「非同期処理・Alarm ジョブ・UoW 契約の設計」までであり、**cross-DO saga の異常系の自動回収は要求していない**。第6.9節が要求しているのは「どの中間状態でも認可は fail closed に倒れること」と「締め出し経路を列挙して塞ぐこと」であり、**前者は上の方針5 で満たされ、後者は経路の列挙が本 Issue の成果として残る。****自動回収は本設計が自ら広げた ［派生］ 領域である。**
- **(ii) 収束していない。** 自動回収に由来する Blocker が **R11・R12・R13 の3ラウンド連続**で出続けた（R11: 起点 B は巻き戻しても旧パスワードを知らないので恒久締め出しになる。R12: 回収の第1段が `abandon-account` の必須ガード `callerToken` の唯一の置き場を消しうる。R13: 終端モードの印に指名した行の削除を**同じラウンドの別の修正が別の段へ移していた** / 回収の除外単位を bucket にすると同 bucket に同居した2行目を取り残す）。**R11 以降の設計 Blocker 5件のうち4件がこの領域であり、R13 の2件は R12 の修正どうしが同じラウンドの中で互いを壊した形だった。** レビュー台帳の運用規則（同じ箇所が3回以上再指摘されたら個別修正で追いかけない）に該当する。**直すたびに「材料の寿命」「判定信号の永続性」「段どうしの順序」で新しい破れが見つかる**という状態である。
- **(iii) 切り出しても本設計は成立する。** 上の方針5点はいずれも**自動回収の手順に依存しない**（発火条件・終端の一様性・記録の制約・中間状態と材料の所在という構造の決定である）。逆に自動回収の側は方針を入力として要求するので、**依存の向きは「一様な終端 → 自動回収」の一方向であり、循環しない。**

**#45 が決着させる論点は次の7つである。** #45 の Issue 本文の「決着させる論点」と同じ列挙であり、**本表はその一覧の正本ではない**（正本は #45 側）。ここに置くのは、本書の読者が「何がここに無いか」を確認できるようにするためである。

| # | #45 が決着させる論点 | 本書での現状 |
|---|---|---|
| 1 | **自動回収を `kind` ごとにどう組むか。段の順序と各段の原子性境界**（`transactionSync` に入れられるのは自 DO のローカル書き込みだけで、DO 間 RPC はその外である。第2.1節 F-7・第6.9節 制約1） | 中間状態と材料の置き場を上の表に残した。段の構成そのものを委譲した |
| 2 | **「終端モードに入った」ことをどう永続化するか。** 前進を再試行しないことを次の起動が読める形にする | `jobs.terminalReason` 列は第4.1.1節にある。**一様な終端は `poison` と同時にしか書かないので、書く契機を前倒しする形は #45 が置く** |
| 3 | **後始末そのものが失敗したときの規則と再試行上限。** `resume-link` / `resume-signup` では**発火の契機と後始末の失敗の契機が同一である**（どちらも対象 bucket の `.overloaded` であり、攻撃者が誘導できる。第6.2.2節） | 上の方針2 は後始末を持たないので `poison` は無条件に書かれる。**後始末を入れたときの「backoff と poison」への例外規則は #45 が置く** |
| 4 | **材料の寿命。** 回収の各段で `callerToken` / `locators[]` / `targetLocators` が生きていること。**予約 TTL の不等式**（第6.4節 1）が覆うべき総経過時間の射程もここに属する | 材料の置き場は上の表に、**終端が材料を消さないこと**は下の前方互換点に残した。**不等式の右辺に後始末の上限まで含めるかは #45**（第6.4節 1） |
| 5 | **operator 経路への受け渡し。** 残渣の種類ごとにどのエントリが受け口になるかと、`terminalReason` に何を載せるか | 経路が2本あることと、`purge-user-mappings` では届かない残渣があることを上に残した。`terminalReason` に載せる2つと載せない2つも上の方針4 で確定してある |
| 6 | **起点別の扱い。** 巻き戻しても自力回復できない起点（credential 変更の `changeOrigin = 'reset'` は旧パスワードを知らない）をどう扱うか | `changeOrigin` 列と `changeState` の3値は第4.1.1節にある。**使い分けを委譲した**（第6.5.1節） |
| 7 | **上記を機械検証できる不変条件**（第1.4節 I-1〜I-8 と同じ形式）。とくに「正本と適用先の散文の整合」と「同一ラウンド内で入れた修正どうしの整合」を検査する形 | 第1.4節の形式が入力になる。**この2つの検査は本 Issue では設計できていない**（第11.4節） |

#### #37 は「一様な終端」だけを実装すれば着手できる

**#37 が実装するのは上の方針5点だけであり、#45 の結論を待つ必要は無い。** 自動回収は後から足せる — **足すために #37 が落としてはいけない前方互換点が4つある。落とすと #45 がどう設計しても後から入れられなくなる。**

1. **`operations.targetLocators` を、#45 の自動回収が入るまで消さない。** 一様な終端には消す契機が無いので、**「終端で `operations` 行を閉じるときに `targetLocators` を空にする」実装にしない**という形で守る（第6.6節・第6.7節・第4.1.1節）。
2. **コーディネーター予約行（`locators[]` / `candidateUserId` / `callerToken` を持つ行）を、#45 の自動回収が入るまで消さない。** `sagaCommitted` 印のある `reserved` 行は上の (1-A) の作業述語からも駆動源からも外れるので、**終端が消さない限り残る**（第6.3節 phase 1a・第6.4節 3）。
3. **`account.callerToken` を退会完走時以外に消さない。** 削除点は第6.7節 手順4 の1つだけである。**`delete-mapping` と `abandon-account` の必須ガードであり、`jobs.payload` にも `terminalReason` にも退避できない**（第5.2節 (c) の非露出対象である）。
4. **`credential_mappings.changeState` を3値（`null` / `'pending'` / `'advanced'`）で実装する。** `'advanced'` は **phase 2 の適用を bucket 側に記録する値**であり（第6.5.1節 phase 2）、**これが無いと #45 は「phase 2 が適用済みか」を判定する材料を持てない**（`operations` 行は User Data DO 側にあって読む RPC が無く、「その実行で phase 2 が成功したか」は commit 後に応答が失われる事象クラスで壊れる）。**#34 の側にも独立した用途がある** — `promote-verifier`（phase 3）のガードが `'advanced'` であること（第5.1節）と、別の `operationId` による新しい依頼の後勝ち規則が `'pending'` と `'advanced'` で分かれること（第6.5.1節）である。

**`poison` 行そのものの検知と手動再投入は運用側の項目として #38 へ送る**（第11.3節）。**本項が決めるのは「`poison` に落ちる瞬間に設計側が何を記録し、どこへ渡すか」であって、落ちた行の運用ではない。**
...
**代わりに「回数」だけで有界にする。3階層の件数上限を置き、CPU を件数の積で押さえる。** **「時計を読まない」の射程はチェックポイント予算の打ち切り条件だけである、と明記する。** 打ち切り条件は「同一コード実行中の経過時間の差分」に依存し、そこは進まないので（第2.1節 F-32）件数だけで書く。**一方、本節が同時に要求している時刻列 — claim の `leaseUntil < ?`、backoff の `nextRunAt` 先送り、`completedAt` の記録、prune の保持期間判定、最小再開間隔のクランプ — はいずれも起動をまたいだ絶対時刻の比較であり、`Date.now()` は起動ごとに I/O を経て進むのでそちらは成立する。** 射程を書かずに「時計を読む実装を1箇所も置かない」と書くと、**DO がリセットされた場合の唯一の回収手段である lease が実装できなくなる**（下記 claim の第2の選言）。**成立しないのは同一コード実行中の経過時間の差分だけである。**
...
3. **(iii) 1件が大きいジョブ（全件再インデックス、bulk migration、退会時の一括削除）はジョブ自身が内部カーソルを持ち、カーソルを進めてコミットしてから次のチャンクへ進む。** ここに**2つの**上限を置く — **(iii-a) 1チャンクで触ってよい行数の上限**と、**(iii-b) 1回の Alarm 起動で同一ジョブについて回してよいチャンク反復回数の上限**である。**(iii-b) で中断するときは、進捗カーソルのコミットと同じ `transactionSync` で `status` を `pending` へ戻し、`ownerToken` と `leaseUntil` を解放してから次の Alarm へ回す。**
   - **解放したジョブは同じ Alarm 起動の中で再 claim しない、と決め切る。** ジョブランナーはその起動で (iii-b) により解放した `operationKey` を除外集合に持ち、以降の反復の claim 対象から外す。**規則が必要なのは、解放直後の行が claim 述語（下記 `status='pending' OR (status='running' AND leaseUntil < ?)`）に一致するからである** — `nextRunAt` も過去のままなので、除外しないと外側の25件を使い切るまで同じジョブを回し続けられる。**それを許すと (iii-b) を置いた目的が失われる**（1件のジョブが1起動で 中間上限 × 25 回のチャンクを回せることになる）と同時に、第9.2節が「外側の25件は `migrate-bulk` 1件に対して一度も発火しない」と断定している読みが成立しなくなる。**除外集合は1回の起動の中だけで持つ揮発値であり、`jobs` に列を足さない**（次の起動では通常どおり claim される — 解放の目的は次の起動での即時再開である）。
...
**カーソルの永続先を確定させる。** (iii) は「カーソルを進めてコミットする」ことを要求するので、**カーソルの置き場所が全数表のどこかに無ければ実装できない。** 決めは2点である。

- **永続カーソルを持つのは `reindex` / `migrate-bulk` の2種で、置き場所はどちらも `migration_progress`（`targetVersion` / `step` / `cursor` / `updatedAt`。第4.1.1節）、書き込み口は `setMigrationCursor` である**（第8.2節）。**主キーは `(targetVersion, step)`** なので、同じ `targetVersion` から `reindex` と `migrate-bulk` の両方が投入されても行が衝突しない。**両者の投入点はどちらも migration ゲートであり、そこには必ず `targetVersion` がある**（第9.2節）ので、`reindex` だけ別の置き場所を用意する理由が無い。第9.2節が初版で書いていた「`reindex` は `migration_progress` ではなく自分の内部カーソルで進む」という排除は**撤回する**（同節）。
- **`finalize-withdrawal` / `purge-trash` は永続カーソルを持たない。根拠は「作業述語そのものが進捗を表す」ことであり、それが成り立つ作業を2つに広げる — 行の削除、または自己消尽する述語で書ける更新である。**
  - **削除側。** 作業述語（`WHERE trashed = 1 AND purge_after < ?` / 退会対象の残存行）は消えた行を自動的に外すので、次のチャンクは「まだ残っている行の先頭」から始めれば正しい。**カーソルを持たせると、削除で行が消えた後のカーソル値の意味が定義できない。**
  - **更新側は `purge-trash` の retention 一括再計算だけである。** そちらの述語は「まだ新しい値になっていない行」として書き、更新した行がその場で述語から外れる形にする（第7.5節が述語そのものを持つ）。**述語が単調に縮むので、削除側と同じ根拠が届く。**
  - **自己消尽しない UPDATE をこの2種に足してはならない。** 述語が縮まないので (iii-b) の中断やエビクションのたびに先頭からやり直しになり、大きく育った DO では完了しない。**これは第9.2節が `reindex` の「内部カーソル」について特定した失敗モードと同型である** — 足す必要が生じたら、そのジョブは永続カーソルを持つ側（`migration_progress`）へ移す。
- **`jobs.payload` へカーソルを持たせる形は採らない。** `payloadDigest` の照合対象は「`nextRunAt` を除いた payload」と定義されているので（上記）、カーソルを更新するたびに digest がずれて再投入が `ConflictError` になる。**`jobs` に `cursor` 列を足す形も採らない** — `jobs` は両クラスで共有する12列の表であり、User Data DO の2種のためだけに13列目を足すと、Directory bucket 側の6種が常に `NULL` を持つ列が1つ増える。

**(iii) の対象は内部カーソルを持つ4種 — `reindex` / `migrate-bulk` / `finalize-withdrawal`（退会の一括削除）/ `purge-trash` である**（うち永続カーソルを持つのは上記のとおり前2種である）。 `purge-trash` を含めるのは、`HardDeletePolicy.expandTargets` の展開件数が利用者のゴミ箱の大きさに比例するためである（第7.5節）。**残り8種（`sweep-orphan-mapping` / `resume-link` / `send-mail` / `resume-signup` / `resume-credential-change` / `sweep-reservations` / `sweep-reset-tokens` / `rotate-encryption`）は1件の仕事量が構造的に有界なので (iii) の対象外である** — ただし **`sweep-*` 3種と `rotate-encryption`** は1回の起動で処理する行数に (iii-a) と同じ上限を掛け、残りを次の起動へ回す。**「次の起動」を張るのは上の「周期・反復ジョブの再武装規則」であり、どの表を読むかは再武装分類で決まる** — 分類 (A) の `sweep-reservations` / `sweep-reset-tokens` は (1-A) の駆動源クエリ表（作業述語から時刻条件だけを外したもの）を、分類 (B) の `sweep-orphan-mapping` / `rotate-encryption` は (1-B) の残件条件表を、それぞれ完了時に読み直して `nextRunAt` を設定する。**`sweep-*` という接頭辞で一括りにしない** — `sweep-orphan-mapping` の作業述語には時刻条件が無く、(1-A) の規則が構造的に当てはまらないからである。
...
- **通常の DO 入力（RPC）での再設定は、DO facade のラッパーが発行する。実行主体をここで名指しする。射程は `run()` を呼ぶ RPC に限らず、全 RPC エントリである（例外は第9.2節「ゲートを通さない2本」の `read-schema-version` / `list-bucket-user-ids` だけで、それが全数である）。** `enqueueJob` は `UnitOfWorkContext` の同期メソッドでトランザクション内に閉じる（第8.2節）一方、`setAlarm` はトランザクションの外なので、誰がいつ呼ぶかを決めないと #37 に落ちる。**規約は次の4点である。**
  ...
  - **(4) ラッパーは `run()` を呼ばない RPC エントリでも再設定を行う。** 発火点は「migration ゲートと本体処理の後」であり、**`run()` の有無で分岐させない。** **これが無いと、ゲートが投入したジョブに arming の主体が存在しない** — 第9.2節のゲートは `run()` の**前**に自分の `transactionSync` で走り、しかも「`schema_version` の読み取りから全 DDL ステップの適用まで `await` を1つも挟まない」と定められているので、`sync()` を要する `setAlarm` を**ゲートの中では発行できない**。したがって arming は「ゲートの外・本体処理の後」でしか行えないが、`run()` を呼ばない RPC（`UnitOfWorkContext` を経由しない projection 読み。第4.3節 行10 の `SearchIndexPort.query` が該当しうる）では発火点が存在しないことになる。**dormant DO では実害になる** — `deleteAlarm()` 規則により実行可能集合が空の DO には alarm が張られていないので、「新リリース → 読み取りだけの RPC が来る → ゲートが DDL を適用し `schema_version` を進めて `migrate-bulk` / `reindex` を投入 → alarm が張られない」の順序で**投入されたのに起動されないジョブ行**が残る。`migrate-bulk` は第9.3節の両対応読みがあるので正しさは保たれるが、**`reindex` は第9.2節 条件2 の4段分解の2段目が止まり、参照先を切り替えたあとの検索が空を返す方向に倒れうる。** 射程を全 RPC へ広げるのが安全側で、`await` はゲートの外なので第9.2節の「`await` を挟まない」排他条件は破れない。**投入が無かった起動では `setAlarm` の発行値が既存値と同じになるので、(2) のインスタンスフィールド比較が余分な書き込みを落とす。**
...
**この積が「1件のジョブの上界」ではないことを明示する。** 25 は**別々のジョブが25件**並ぶ場合の外側の枠であり、**1件のジョブが1回の起動で触る行数の上界は「中間上限 × 内側上限」である** — (iii-b) で解放したジョブを同じ起動で再 claim しないと決めてあるからである（上記 (iii)）。**区別しないと1件あたりの上界が25倍ずれる**（出発点の初期値では 20 × 1,000 = 20,000 行 と 500,000 行 の差になる）。**上界の見積りは実装判断に直接効く** — CPU 予算超過の帰結はエラーではなくエビクションであり、「途中まで進んで黙って落ちる」形でしか現れないからである。第9.2節が `migrate-bulk` について「外側の25件は一度も発火しない／効く保護は中間と内側の2つだけ」と断定しているのは、この読みに立っている。

**チューニング定数の置き場を決める。値の決定主体（#37 → #38）とは別の話である。** **3階層の予算（ジョブ25件 / チャンク反復回数 / 1チャンクの行数）・最小再開間隔・`done` / `poison` の prune 保持期間・lease 期限の既定値は、leaf モジュール（`packages/core/src/lib/` もしくはジョブ契約の型定義側）に置き、DI や実行部から value-import させない、と断定する。** **理由は #40 のクローズコメントが #37 へ引き継いだ論点2 である** — 現行の `packages/core/src/application/di/env.ts` は `application/workers/eventRelayWorker.ts` / `outboxPrune.ts` から `DEFAULT_*` を **value-import** しており、「合成ルート（`di/`）→ worker 実装」という**逆向きの依存**を作っていた。しかもその value-import が、トップレベル Worker がモジュールスコープの `crypto.randomUUID()` を評価してしまう唯一の流入経路だった（#40 の実測。第11.2節末尾）。**Outbox 廃止で `env.ts` ごと対象消滅するが（第11.2節）、新しい設計は同種の定数を再導入するので、素直に「ジョブ実行部モジュールに `DEFAULT_*` を置いて `di/` から value-import する」と実装すると同じ逆向き依存を新構成で再生産する。**
...
- **retention 設定の変更時。** `TrashRetentionDays` は `User` の属性なので変更も同じ DO 内で起きる。変更したトランザクションの中で **ゴミ箱内の全項目の `purge_after` を一括再計算し**、新しい `min(purge_after)` で `enqueueJob('purge-trash', ...)` を発行する。ゴミ箱の件数は利用者1人分なので一括更新で足りる（件数が大きい場合は第7.4節のチェックポイント分割に乗せる）。
  ...
  - **チェックポイント分割に落ちたときの進捗は、永続カーソルではなく自己消尽する作業述語で表す。** `purge_after` は `RetentionPolicy` により `(trashedAt, trashRetentionDays)` の純関数なので（算出規則そのものは変えない。上記）、再計算の作業述語は **`WHERE trashed = 1 AND purge_after <> <新しい `trashRetentionDays` で算出した値>`** ＝「まだ再計算していない行」と書ける。更新した行はその場で述語から外れるので**述語が単調に縮み**、次のチャンクは残っている行の先頭から始めれば正しい。**これで `purge-trash` は永続カーソルを持たずに済み、第7.4節「カーソルの永続先」の除外根拠（作業述語が進捗を表す）がこの UPDATE にも届く。** 再計算の途中で `trashRetentionDays` がもう一度変更されても先頭からやり直しにはならない — 述語は新しい値に対して定義され直され、既に新しい値になっている行は外れるだけである。
  - **再計算の残件があるあいだは削除フェーズを回さない、と決め切る。** `purge-trash` は1回の起動でまず再計算述語が空になるまでチャンクを回し、空になってから削除述語（`WHERE trashed = 1 AND purge_after < ?`）へ進む。**順序を決めないと延長方向で誤削除が起きる** — 延長の再計算が済んでいない行は古い（短い）`purge_after` を保持しており、その状態で削除述語を回すと利用者が延ばしたはずの項目が消える。逆に**削除が次の起動へ遅れるのは安全側である**（retention は「少なくともこの期間は保持する」規定なので、遅れは規定違反にならない）。
...
- **`purge-trash` は内部カーソルを持つジョブである。** `expandTargets` の展開件数は利用者のゴミ箱の大きさに比例するので、第7.4節 (iii) の対象に含める（`reindex` / `migrate-bulk` / `finalize-withdrawal` と同じく、1チャンクの行数上限とチャンク反復回数上限を種別ごとに持つ）。**retention 設定の変更に伴う `purge_after` の一括再計算も同じチャンク分割に乗せる。ただし進捗を表すのは永続カーソルではなく上記の自己消尽する作業述語である**（`purge-trash` は永続カーソルを持たない。第7.4節「カーソルの永続先」）。**再計算フェーズが先、削除フェーズが後**という順序も上記のとおり固定する。
...
- **同じ canonical への連打はジョブ行1本に収束する。登録済みでも未登録でも同じである。** `operationKey` を「対象 canonical の全長 HMAC + 依頼の窓」から決定的に導くので、第7.4節の「同じキーの再投入は既存行に収束する」がそのまま効く（第6.2.2節 (b)）。
  - **収束の内容を1つに固定する — 収束するのは行数だけでなく、書き込みと Alarm 起床もである。** 同じ窓のあいだ、2回目以降の依頼は (i) 行が実行可能集合にいれば「早める方向にのみ」により（同じ `nextRunAt` を渡すので）何も書かず、(ii) 行が `done` へ落ちた後は `status` 別の規則により（`send-mail` は分類 (C) なので）復活させず何も書かない。**どちらの経路でも `setAlarm` は増えない。** したがって書き込みと起床は**窓の数**に比例し、依頼回数には比例しない。**`done` からの復帰を分類で限定していない設計では、この段落は成立しない**（起床と rows written が依頼回数に比例する）ので、第7.4節の分類別規則がここの前提である。
...
- **`collectEvents` は消えるが、その位置を `enqueueJob` が引き継ぐ**（第7.3節）。`collectEvents` が占めていた「**トランザクション内の唯一の副作用登録点**」というスロットは空にしない。設計の複数箇所が「ジョブ投入は本体更新と同一 `transactionSync` に入っていなければ意味がない」ことを要求している — 第7.5節（`purge_after` の最小値から `purge-trash` を投入）、第7.6節（外部 I/O は必ずトランザクションでジョブ行を書く）、第6.2.2節 (b)（レート制限時もジョブ行を必ず書く）、第9.2節（migration ゲートの中でのジョブ投入）。
  ...
  - **`migration_progress` のカーソル更新も同じ位置に置く**（第9.3節。`setMigrationCursor`）。**書き手は `migrate-bulk` と `reindex` の2種で、これが「内部カーソルを永続化するジョブ」の全数である**（第7.4節「カーソルの永続先」）。`ctx.storage.sql` を usecase から直接触る形は採らない — レイヤー違反であり、UoW を通らない書き込み経路を作るからである。
...
**起動タイミングは、DO の全 RPC エントリ「および `alarm()`」の先頭に置いた冪等なゲート関数である。ただし例外が2本あり、それが全数である**（下記「ゲートを通さない2本」）。 `blockConcurrencyWhile` は使わない。**`alarm()` については「先頭」に置くものが本節のゲートと第7.4節の再武装の2つあるので、順序を第7.4節で確定させてある — (1) 再武装 + `await ctx.storage.sync()` → (2) 本節のゲート → (3) 仕事 である。** (1) の `await` はゲートに入る前に完了するので、下記「ゲート関数は同期関数とし `await` を1つも挟まない」という排他条件は破れない。
...
2. FTS5 の全件再インデックス（トークナイザや正規化規則を変えたとき。第5.2.1節 (d) の canonical 規則変更も同種である）。**external-content の `CREATE VIRTUAL TABLE ... USING fts5(...)` 自体は shadow table を作るだけなので単発でよい**が、既存行を索引に載せる作業は本条件に当たる（external-content は作成時に content テーブルから自動で populate されない。第7.1節）。**手段として `INSERT INTO search_fts(search_fts) VALUES('rebuild')` は採らない、と決め切る。** 公式 `fts5.html` が「This command first deletes the entire full-text index, then rebuilds it based on the contents of the table or content table」と定めているとおり、これは**1文で全索引を消して全行から作り直す単一 SQL 文**であり、中断・再開の単位が存在しない（`SAVEPOINT` も使えない。第2.1節 F-8。公式ページには `'rebuild'` を分割・再開する手段の記載が無く、増分実行を許すのは既存索引のセグメント統合（`'merge'` / `'usermerge'` / `'automerge'`）だけである）。**これは条件4 の `CREATE INDEX` が分割不能であるのと同じ性質なので、同じ扱いにする** — 「必ず分割する」条件の中に分割不能な手段を選択肢として並べない。`'rebuild'` を選ぶと `reindex` に与えた3つの保護（(iii-a) の行数上限 / (iii-b) のチャンク反復上限 / `migration_progress` の永続カーソル。第7.4節）が**すべて空振りし**、10 GB まで育った DO では下記の失敗モードのまま**途中まで進んで黙って落ちる**。**したがって `reindex` は projection の全行再実行として実装する**（1行ずつ「旧値で delete → 新値で insert」。第7.1節）。**進捗は `migration_progress` の `(targetVersion, step)` カーソルに乗る**（第7.4節「カーソルの永続先」）。
   - **`tokenize` オプションそのものを変える場合は、条件4 (b) と同型の多段 forward-only へ分解する。** tokenizer は `CREATE VIRTUAL TABLE` の `tokenize` オプションで設定する値であり（公式 `fts5.html`: 「The CREATE VIRTUAL TABLE 'tokenize' option is used to configure the specific tokenizer used by the FTS5 table.」）、**既存の FTS5 表の tokenizer を変更する手段は公式ページに記載が無い**（記載の不在からの推論 — `fts5.html` は fts5 表に対する `ALTER TABLE` に一切触れていない）。したがって分解は「新しい `tokenize` の仮想表を作る（単発）→ `reindex` が projection の全行再実行でチャンク投入する → 検索の参照先を新しい表へ切り替える → 旧仮想表を落とす」の4段である。**正規化規則だけを変える場合はこの分解は要らない** — fog の正規化は projection（`search_entries` への書き込み）側で行うので（第7.1節）仮想表の定義が変わらず、全行の delete → insert をチャンクで回すだけで足りる。
...
**ゲートを通さない2本を明示する。第5.1節の `read-schema-version` と `list-bucket-user-ids`（どちらも operator 専用の診断エントリ。第9.5節）である。これが例外の全数である。** どちらも **(a) 本節のゲートを通さず、(b) 第9.4節の fail-closed も通さず、(c) 第7.4節 (4) の arming（ラッパー末尾の `setAlarm` + `sync()`）も行わない。**

- **理由は、この2本が「ゲートが止めるべき状況を診断するための口」だからである。** 通すと3つが同時に壊れる。**(i) 未 migrate の DO を1つずつ舐めて回る診断が、その走査の副作用として全 DO に DDL を適用し `migrate-bulk` / `reindex` を投入して alarm を張ることになる** — 第9.5節が「不良 migration が壊した対象集合を作る」ために用意した口が、まさにその不良 migration を全 DO へ配って回る。**(ii) コードをロールバックした後は fail-closed が発火するので、`_meta.schema_version` の値ではなく `SystemError` が返る** — 第9.4節が止める対象そのものを読みたいのに読めない。**(iii) `list-bucket-user-ids` は Directory bucket 側でも同じ2つが起きる。**
- **安全である根拠は、この2本が書かない・返さないことに尽きる。** `read-schema-version` が読むのは `_meta.schema_version` の整数1つだけで、**スキーマの形に依存しない**（`_meta` は第4.1.1節のとおり両クラスが持つ固定の表であり、migration で形が変わらない）。`list-bucket-user-ids` が読むのは `credential_mappings.userId` だけで、こちらも列の追加・削除に影響されない。**どちらも1行も書かないので、未 migrate の DO を新スキーマ前提の SQL で壊す経路が無い**（第9.4節が fail-closed にした理由は「知らない列に `INSERT` してデータを壊す」ことであり、読みだけならその損害が発生しない）。
- **`read-schema-version` の `userId` は呼び出し元供給ではない。** 引数は `list-bucket-user-ids` が返した値、すなわち **Directory bucket の RPC 戻り値**であり、第5.5節 1 の3経路のうち (iii) に当たるサーバー永続値である。**したがって第6.2節の判断軸 (iv)（未認証入力で新しい DO オブジェクトを無制限に生成できる面を作らない）に触れない** — この2本は公開ルートを持たない operator 専用 maintenance 経路にしかなく、未認証の総当たり面ではない（第5.1節 (3-c)）。**#38 の運用手順には「`list-bucket-user-ids` の戻り値以外の `userId` を `read-schema-version` へ渡さない」を書く**（第11.3節）。

**ゲートが投入したジョブの arming の主体を第7.4節が名指ししている。** ゲートは `await` を1つも挟めないので `sync()` を要する `setAlarm` を自分の中では発行できず、arming は「ゲートの外・本体処理の後」に DO facade のラッパーが行う。**その射程は `run()` を呼ぶ RPC に限らず全 RPC エントリである**（第7.4節「Alarm の再設定規則」(4)）。**限らないと決め切ったのは、`run()` を呼ばない読み取り専用 RPC がゲートを通過してジョブを投入したときに arming の主体が存在しなくなるからである** — dormant DO には次の DO 入力が来ないので、投入されたジョブ行が起動されずに残る。
...
- **条件2（FTS5 の全件再インデックス）は `reindex`。** 同じくゲートの中で `enqueueJob('reindex', ...)` を発行する。**`migrate-bulk` に畳まない** — `reindex` は本体テーブルを一切書き換えず projection だけを作り直すジョブなので、両対応の読み取り（第9.3節）が要らず、失敗しても本体データが壊れない。**ただしカーソルの置き場所は `migrate-bulk` と共通で `migration_progress` である**（`setMigrationCursor`。第7.4節「カーソルの永続先」）。**初版は「`migration_progress` ではなく自分の内部カーソルで進む」と書いていた。これは撤回する** — その「内部カーソル」の永続先が第4.1.1節（`jobs` は12列）にも第8.2節の書き込み口にもどこにも無く、**`reindex` だけが (iii-b) の中断のたびに先頭からやり直しになって、大きく育った DO では永久に完了しない**（(iii-b) を置いた目的そのものが失われる）。`migration_progress` の主キーは `(targetVersion, step)` なので、同じ `targetVersion` から両方が投入されても行は衝突しない。`spec/database/index.md` のトークナイザ方針や第5.2.1節 (d) の canonical 規則を変えたときは、`schema_version` を進めるのと同じゲートからこのジョブが投入される。
...
- **1回で完了しない migration の部分適用を記録する。** `migration_progress` テーブルに `{ targetVersion, step, cursor, updatedAt }` を主キー `(targetVersion, step)` で持ち、**カーソルを進めた分をコミットしてから次の Alarm を張る。** これは任意の最適化ではなく必須である（第9.2節の「黙ってリセットされる」失敗モードのため）。**`migrate-bulk` と `reindex` の両方がこの表を使う**（第7.4節「カーソルの永続先」）— `reindex` の再開位置を別の場所に持たせると、第4.1.1節にも第8.2節の書き込み口にも現れない永続状態が1つ増える。
...
**fail-closed にする。** `_meta.schema_version` がコード側の期待する最大バージョンより大きい場合、その DO は**リクエストを受け付けず** `SystemError` を返す。**射程から外れるのは第9.2節「ゲートを通さない2本」（`read-schema-version` / `list-bucket-user-ids`）だけであり、それが例外の全数である** — この2本は fail-closed が発火している DO の `schema_version` を読むための口なので、fail-closed に掛けると第9.5節の対象特定手段が「止まっている DO については使えない」ことになる。**1行も書かないので、本節が守ろうとしている「知らない列への `INSERT` でデータが壊れる」損害は発生しない。**

**この fail-closed も `alarm()` に掛ける。** 掛けないと、ロールバック直後に古いコードの `alarm()` が新スキーマを触る経路が開いたままになり、第9.2節でゲートを `alarm()` へ広げた意味が半分になる。**fail-closed で止まった DO の `alarm()` は、ジョブを実行せずに一定間隔で `setAlarm` を張り直し、`await ctx.storage.sync()` で永続化を確認してから戻る。** poison にはしない — 原因はデータではなくデプロイ状態であり、正しいコードが戻れば次の起動で自然に回復するべきものだからである。張り直しは第7.4節の「ハンドラ先頭での再武装」と**同じ規則**に乗る（`now + 再開間隔` の固定間隔 + `sync()`）。規則を2箇所に分岐させない — fail-closed の DO は仕事をしないので実害は小さいが、ここだけ `sync()` を省くと規則が2つになる。運用は「fail-closed で止まっている DO の存在」をメトリクスで検知する（#38。第11.3節）。

**間隔にバックオフは掛けない、と決め切る。** 第7.4節の先頭再武装は `now + 再開間隔` の**固定間隔**であり、下記のとおり fail-closed が発火するのはその再武装より後の (2) なので、**「(3) を飛ばして戻る」だけで規則が成立する。** バックオフを掛けるには (2) のゲートの後で `setAlarm` を上書きする**2本目の規則**が要り、それは上段の「規則を2箇所に分岐させない」と衝突する。**間隔を伸ばす動機も無い** — 原因はデータではなくデプロイ状態なので、試行を重ねても状況は変わらず、回復（正しいコードの再デプロイ）の検知を遅らせるだけである。**fail-closed が長引く場合の間隔調整が要るなら、それは第7.4節の `再開間隔` そのものの運用値として #38 が決める**（第11.3節の「fail-closed で止まっている DO の検知」と同じ枠）。
...
- **対象を特定する唯一の経路は Identity Directory bucket の全走査である。** `0..N-1` の全 bucket を舐めて `credential_mappings.userId` を集めれば全 `userId` を列挙できる。**「同じ 256 bucket を舐める operator 経路がすでにある」という点では第6.7節（逆引き情報ごと失われた場合の mapping 削除）と第6.8節の retirement 走査と同型だが、どちらも `userId` の一覧を返す口は持っていない** — `purge-user-mappings` は `userId` を**引数に取る**エントリであり、retirement 走査が数えるのは `previousCount`（bucket ごとの行数）である。**したがって本手順には専用の RPC エントリが2本要る。**
- **その2本を第5.1節の RPC エントリ表に置いてある** — **`list-bucket-user-ids`**（Directory bucket から `userId` の一覧をカーソルで刻んで返す読み）と **`read-schema-version`**（対象 User Data DO の `_meta.schema_version` を読む）である。どちらも operator 専用 maintenance 経路のクラス (3)、群は (3-c)（到達制御と監査が守る）で、`list-bucket-user-ids` には**返す `userId` をログ・トレースに出さない**ことと**1回の呼び出しの件数上限**が追加で課されている。**移送とは独立に必要なので #44 の射程外である。**
...
- **Directory bucket の PITR は、消費済み・削除済みのリセットトークン行を復活させる。これは3つ目の独立した穴であり、上の2つの必須ステップ（どちらも User Data DO 側の操作）では塞げない。** 上の3点（Directory mapping がゲート / `account.status` が権威 / 片方だけでは復活しない）は**認可を閉じる向き**についての論証だが、この経路は**認可を開く向き**に倒れる。
  - **経路。** 乗っ取り被害者がリセットで攻撃者を締め出した後に bucket を T0 へ restore すると、**(a) `credential_mappings` の `passwordVerifier` / `credentialVersion` が n へ巻き戻り、(b) 攻撃者が保持していたリセットトークン行が `usedAt = null` で復活する**（第6.5.1節 phase 1 の未使用トークン一括削除も、`consume-reset-token` の使い捨て記録も、どちらも巻き戻る）。**(a) 単独なら fail closed で止まる** — 第5.3節 step 5 (iii) の `credentialVersion` 照合が User Data 側の n+1 と不一致になるからで、上の `credentialVersion` の項が述べているのはこの向きである。**ところが (b) は照合を回避せずに前進させて解消する** — `consume-reset-token` → `begin-credential-change`（**起点 B の束縛は `consume-reset-token` が採番する `changeAuthToken` だが、攻撃者はトークンを保持しているので自分で `consume-reset-token` を叩いて新しい `changeAuthToken` を受け取れる。束縛はこの経路を止めない** — 第5.1節 (3-d) の `changeAuthToken` が塞ぐのは「トークンを持たない呼び出し元が起点 B を叩く」経路であって、「復活したトークンを持つ攻撃者」ではない）→ phase 2 が `sessionEpoch` と `credentialVersion` を n+2 へ進める → phase 3 で攻撃者の `pendingVerifier` が昇格する。**終状態は攻撃者のパスワードで正規にログインできるアカウントである。** トークン TTL は時間オーダー（第6.1節 (d)）なので成立するのは「直近数時間へ戻す restore」に限られるが、**それは PITR の典型的な使い方そのものである。**
...
2. **リセット完了画面に必須の導線を2つ置く** — **クレデンシャル一覧**（覚えの無い SSO 連携を解除できる。第5.1節の残余リスクの唯一の対策）と、**AI クライアント接続一覧 + 「すべて失効」**（第5.4節 (ii)。`createdAtResetVersion` による自動失効が切るのは前回のリセット完了以降に作られた分だけなので、それより前に持ち込まれた接続は利用者の判断で切る）。**2つは同じ画面にまとめる。** この画面は第10.1節の PITR 復旧手順（restore 後に全接続を失効させ利用者に再接続させる）からも流用される。
...
| `spec/testcases/identity/changePassword.md` | `:7` / `:19` | **(B)**。あわせて **credential 変更 saga（第6.5.1節）の中間状態**（`changeState` が `null` でない間 — `'pending'` / `'advanced'` — は旧新どちらのパスワードも通らない。値域が3値であることと `'advanced'` が phase 2 の成功を記録する値であることも期待値に含める。第4.1.1節・第6.5.1節）と `sessionEpoch` の前進をケースに足す。**さらに濫用抑止のケースを3つ足す**（第6.2.2節 (a)・第6.5.1節 phase 0 起点 A の A-4）— (i) **旧パスワードの照合失敗が `failedAttempts` を進める**（`report-verify-result`。login の失敗と同じカウンタである）、(ii) **`nextAttemptAllowedAt` 未到達の変更試行は `lookup-credential-by-locator` が**明示的に拒否される**（ダミー材料へ倒すのではない。認証済み経路なので列挙オラクル回避の根拠が無い。第5.1節・第6.2.2節 (a) 3）**、(iii) **照合成功で `failedAttempts` が0に戻る**。**`resume-credential-change` が前進不能を確定したときに `terminalReason` を残して `poison` になり、`changeState` が `null` でないまま残る**（旧新どちらのパスワードでもログインできない）ケースも足す（第6.5.1節・第7.4節の一様な終端）。**巻き戻して旧パスワードで login できるようにするのは #45 なので、その結論が出るまでこのケースの期待値を変えない** |
...
| `spec/testcases/identity/loginWithPassword.md` | 11ケース全部が「`findByEmail` を引いて `PasswordHasher.verify` する」現行フロー前提で、第5.3節が足した手順が1つも無い | 次の6ケースを足す。(i) **到達性検査**（step 5 (ii)）— Directory に mapping が残っているが `credential_locators` に active 行が無いと拒否される。(ii) **`credentialVersion` 不一致**（step 5 (iii)）で拒否される。(iii) **`changeState` が `null` でない間**（`'pending'` / `'advanced'`）は行が存在してもダミー材料が返り照合が成立しない。(iv) **`nextAttemptAllowedAt` 未到達**の試行はダミー経路へ倒れ、成功・失敗を区別できない（第6.2.2節 (a)）。(v) **step 7 の報告** — 成功で `failedAttempts` がリセットされ、失敗で前進する。(vi) **鍵ローテーション中**に `credential_locators` が両世代の行を持っていてもログインできる（到達性検査が `credentialId` だけを見るため。第6.1.1節・第6.8節）。「同一メールの `SsoUser`」ケースは第6.6節のクレデンシャル集合へ読み替える |
...
| `spec/testcases/identity/listAiClientConnections.md` | 「active と revoked の混在」ケースはあるが、**リセット完了による自動失効**のケースが無い | 「**パスワードリセットを完走すると、`createdAtResetVersion` が前進前の `account.resetVersion` と等しい接続（＝前回のリセット完了以降に作られた接続）だけが `revoked` になり、それより古い接続は `active` のまま残る**」ケースを足す（第5.4節 (i)）。**「接続を作る → パスワードを変更する → リセットを完走する」の順で、変更を挟んでも接続が失効すること**もケースとして足す（`credentialVersion` を基準にしていた初版はここで空振りしていた）。通常のパスワード変更では失効しないことも足す |
...
| **削除** | `packages/core/src/application/di/env.ts`（57行） | **`TuningEnv`（`OUTBOX_BATCH_SIZE` / `OUTBOX_LEASE_MS` / `OUTBOX_MAX_ATTEMPTS` / `OUTBOX_RETENTION_MS`）と `readRelayTuning` / `readPruneTuning`。Outbox 廃止に伴い全体が対象消滅する。** **このファイルは `../workers/eventRelayWorker`（`DEFAULT_BATCH_SIZE` / `DEFAULT_LEASE_MS` / `DEFAULT_MAX_ATTEMPTS`）と `../workers/outboxPrune`（`DEFAULT_OUTBOX_RETENTION_MS`）を value-import している唯一のプロダクションモジュールであり、`application/workers/` を削除した時点でコンパイル不能になる。** #37 の Issue 対応項目5 が名指しする「調整用 env」の実体でもある。**道連れになるテストが3本ある** — `packages/core/src/application/di/__tests__/serverCloudflare.test.ts`（`readRelayTuning` / `readPruneTuning` の検証。**移植先が無いので削除する**）、`packages/core/src/application/identity/__tests__/eventDecoders.test.ts`（`domain/common/event.ts` / `domain/identity/events.ts` / `application/identity/eventDecoders.ts` ごと消える）、`packages/core/src/application/__tests__/helpers.ts`（`D1OutboxRepository` / `D1IdempotencyStore` / `D1UnitOfWorkProvider` / `WorkerContainer` を組み立てるハーネスなので**作り直す**）。`apps/web/app/worker/cloudflare/__tests__/` に付けたのと同じ但し書きである。**あわせて `readRelayTuning` / `readPruneTuning` を再輸出している `di/serverCloudflare.ts` と、それを呼ぶ `apps/web/app/worker/cloudflare/handlers.ts` は既に別行で「作り直す」「削除」に入っている** | 第7.3節・第7.4節 |
...
| **改修** | `packages/core/src/application/errors.ts` | `SystemErrorCode` に **`ServiceOverloaded` と `StorageCapacityExceeded` の2値を追加する**（現行は `DatabaseError` / `DataIntegrityError` / `CryptoError` / `SessionError` / `NetworkError` / `ExternalApiError` の6値）。**`RETRYABLE_SYSTEM_CODES` には入れない** — どちらも retryable false である。**`RETRYABLE_SYSTEM_CODES` の中身そのものも変えない**（`NetworkError` / `ExternalApiError` の2値のまま。`:206-210`）— とくに **`DatabaseError` を足さない。** 足すと既存の全 DB 経路の `retryable` が一斉に反転するので、第4.7節 行3（`ctx.abort()` / DO のリセット）は表の側を **false** に合わせてある | 第4.7節 |
...
- **trigram / `bm25` の再確認 spike。** 第2.1.1節の手順を着手時に1回走らせる。#37 の Issue が要求する tokenizer 実環境検証と同じ作業である。**あわせて第11.4節の表のうち「#37 / 着手時の spike」を含む行すべてを同じ着手時にまとめて走らせる。第11.4節は11行あり、うち9行が #37 の着手時 spike を含む**（うち2行は #37 が根拠値を出して #38 が運用値を確定する2段の分担である。**残り2行は鍵ローテーションの手順（#44）と cross-DO saga の異常系の自動回収（#45）で、決める主体が別 Issue なので spike の対象ではない**）。trigram / `bm25` の再確認以外の**残り8件**は、`snippet()` / `highlight()` の可用性 / `transactionSync` のネスト可否 / Alarm・RPC の CPU リセット契機 / 単一 SQL クエリの結果セット合計サイズ上限 / チェックポイント予算の中間・内側の初期値 / `SELECT changes()` の意味論 / `transaction()` コールバックの `async` 可否 / `sql.exec()` が `Date.now()` を進めるか、である。**件数を書き換えるときは第11.4節の表を数え直す。**

**本設計が新しく導入した「テーブル定義の外の作業」のうち、既存ファイルの変更ではなく DO 側の新規実装になるものを、#37 が拾い落とさないように名指ししておく。** 上の表は**既存ファイル**の削除・作り直し・改修の一覧なので、DO 側に新しく足す機構はどの行にも現れない。**次の6つは第5.1節・第6.x節・第7.4節の本文が正本であり、本節は所在の索引としてだけ持つ。**

- **`report-verify-result`**（第5.1節のクラス (2)）— パスワード変更経路の照合結果を bucket へ報告する。`report-login-result` と同型。あわせて `lookup-credential-by-locator` に `nextAttemptAllowedAt` 未到達の**拒否**（ダミー化ではない）と、`callerToken` 照合を先に評価する順序を実装する（第6.2.2節 (a)）。
- **`list-bucket-user-ids` / `read-schema-version`**（第5.1節のクラス (3)、群は (3-c)）— PITR / 不良 migration の対象特定。**この2本だけは migration ゲート（第9.2節）・fail-closed（第9.4節）・第7.4節 (4) の arming を通さない。** ラッパーの分岐が1つ増える。
- **分類 (C) の一様な終端**（第7.4節の正本）— 発火条件（`ConflictError` またはバックオフ上限到達）、`poison` と同じ `transactionSync` で `terminalReason`（終端の理由と `operationId`。PII と再利用可能な秘密は載せない）を書くこと、operator 経路へのエスカレーション（#38）。**自動回収（巻き戻し）は実装しない — 設計が #45 だからである。** ただし**前方互換点4つ**（`operations.targetLocators` / コーディネーター予約行 / `account.callerToken` を終端で消さない、`changeState` を3値で実装する）は落とさない（第7.4節「#37 は『一様な終端』だけを実装すれば着手できる」）。
- **`credential_mappings.changeState` の3値化**（`null` / `'pending'` / `'advanced'`）— phase 2 の成功を bucket 側に記録する値。`lookup-credential` は `null` でない値をすべてダミーへ、`promote-verifier` は `'advanced'` だけを通す（第5.1節・第6.5.1節）。
- **リセットトークンの発行時の未使用行削除**（第6.1節 (d) の無効化点の1つ目）— **スロットル中はトークン行を書かないので削除も走らない**。
- **phase 4 の原子性** — signup phase 4 と link 手順4 の2作用（`operations.phase = 'done'` と `credential_locators` の記録）を同じ `transactionSync` で書く（第6.3節・第6.6節）。

**本設計が新しく導入した列が変更対象一覧に与える影響。** 下の13行はいずれも新設テーブルの列なので DDL 側の追加作業は増えないが、**テーブル定義の外に作業を1つずつ生む**。
...
| **`password_reset_tokens.changeAuthToken`** / `consumedByOperationId` | Directory `password_reset_tokens` | **`changeAuthToken` は `consume-reset-token` が採番する128ビットの暗号論的乱数で、`begin-credential-change` の起点 B（未認証）の唯一の束縛材料である**（定数時間比較を必須にし、成功時に同じ `transactionSync` で `値 → NULL` の CAS で消す。第5.1節 (3-d)・第6.5.1節 起点 B）。**一回性とは別に失効経路が要る** — credential 変更 phase 1 は「未使用トークン行の全削除」に加えて「**残る全行の `changeAuthToken` を `NULL` にする**」を同じ `transactionSync` で行う（第6.1節 (d)・第6.5.1節 phase 1）。**行の値が `NULL` なら常に不一致、引数が欠落・空なら比較前に拒否**という共通規則も実装する（`callerToken` と共通。第5.1節 (3-b)）。**この2つを落とすと、消費済み・未使用の値が被害者の復旧をまたいで奪還権として残り、`NULL == NULL` が通る実装が起点 B を無条件開放にする**（第6.9節の該当2行）。第5.2節 (c) の非露出対象に含めるので、RPC の引数・戻り値ロギングを有効化しない。**`consumedByOperationId` は監査用の記録に格下げした** — `operationId` は未認証経路のログへ出してよい値なので capability にできず、**それを束縛にしていた初版は乗っ取り原始関数を残していた**（第5.1節 (3-a)(3-d)）。**#37 は2列とも実装するが、認可の判定に使うのは `changeAuthToken` だけである** |
| **`credential_mappings.changeOrigin`** | Directory `credential_mappings` | **credential 変更 saga の起点（`'password-change'` / `'reset'`）を永続化する列である。** phase 1 が `begin-credential-change` の引数から書き、phase 3 が `null` へ戻す（第6.5.1節）。**`advance-credential-change` の payload にこの値を載せるので、`payloadDigest` の CAS が Alarm 再開でも成立する。** **この列が無いと phase 2 が `resetVersion` を進めるかどうかを Alarm 再開経路で決められない** — `resume-credential-change` は mapping 行だけを入力に駆動するからである。「常に進めない」に倒すと被害者のリセット完走後も攻撃者の AI 接続が `exp` まで生き残り、「常に進める」に倒すと通常のパスワード変更でも接続が切れる。**`password_reset_tokens.consumedByOperationId` からの導出では代替しない**（監査用に格下げした列であり、`sweep-reset-tokens` が行を消した後は導出が偽になる） |
...
| **`account.resetVersion` / `ai_client_connections.createdAtResetVersion`** | User Data DO `account` / `ai_client_connections` | **パスワードリセット完了でしか進まない単調カウンタと、その値の写しである**（第5.4節 (i)）。第6.5.1節 phase 2 が**起点 B（リセット完了）に限り**同じ `transactionSync` で `resetVersion` を1つ進め、`createdAtResetVersion` が前進前の値と等しい接続を `revoked` にする。**起点 A（通常のパスワード変更）では進めない。** **`credentialVersion` で代用してはならない** — 最頻の乗っ取り系列（接続作成 → 攻撃者によるパスワード変更 → 被害者のリセット）で失効対象が0件になり、補償が空振りする。AI 接続の作成経路（#13 の `/authorize` 承認時）が `account.resetVersion` を読んで写す必要があるので、**#13 の接続行生成にも1項目増える** |
| **`rotation_checkpoints.rotationKind`** | Directory `rotation_checkpoints` | **2つのローテーション（`rotate-remap` / `rotate-encryption`）が同じ表を共有するための区別列である**（第6.8節）。置換キーが `(rotationKind, bucketIndex, generation)` になる。**列と置換キーは #37 が実装する**（DDL と `RotationCheckpointStore` の口。第4.1.1節）。**一方「どの述語で旧鍵破棄条件を判定するか」「いつ・どの値を記録するか」は #44 待ちである**（第6.2.1節 (b-1)・第6.8節の論点3）。**絞りを落とすと片方の完了記録がもう片方の旧鍵破棄条件を成立させる**という判定だけが本設計の結論なので、#37 は**この列を落とさない**こと以外にローテーション側の実装義務を負わない |
| `rotation_checkpoints.conflictCount` / `lastConflictAt` / `lastConflictCredentialId` | Directory `rotation_checkpoints` | 移送先に `userId` の違う行があって移送を見送った場合の**記録先である**。**移送は `jobs` 行も `operations` 行も持たないので `poison` / `terminalReason` を使えない**（第7.4節）ため、この3列がその代わりになる。**列は #37 が実装するが、書き込みは移送経路だけなので #37 の実装では1度も書かれない。記録の規則と運用エスカレーションの受け口は #44 → #38 待ちである**（第6.8節の論点1） |
| `credential_mappings.encryptionNonce` | Directory `credential_mappings` | AES-256-GCM の nonce を**独立列**に持つ。AAD に `(kind, credentialId, encryptionGeneration)` を束縛するので、暗号文への連結にしない（第4.1.1節・第6.2.1節 (b-2)）。**移送では再暗号化しない** — AAD が世代非依存なので暗号文は3列そのまま運べる（第6.8節の方針7）。**この列と AAD の束縛範囲は #37 が実装する**（移送の有無に依存しない） |
...
| H-8 | `pnpm start` / `pnpm preview` が起動不能 | **真因（`eventRelayWorker.ts` の module-scope `crypto.randomUUID()`）は第7.3節の削除で消える。#40 は 2026-07-29 に `NOT_PLANNED` でクローズ済みだが、クローズコメントが #37 へ引き継いだ論点2つ（worker identity の注入 / チューニング定数を leaf モジュールへ）は新構成でも有効である**（第11.2節末尾）。**ただし `.tpl` の `main` が TS ソースを指す問題は別で、#36 が「`main` の修正は redirect 設定が無い経路（`wrangler deploy --dry-run` など）のために依然必要」と結論しているとおり #37 の対応項目8 で直す**（上の `render-wrangler.ts` の行。#36 の作業ログは未 commit なので、この結論は本行を正本として引き継ぐ） |
...
**#40 の扱い。#40 は 2026-07-29 に `NOT_PLANNED` でクローズ済みである。真因は第7.3節の削除で消えるが、クローズコメントが引き継いだ論点は新構成でも有効なので、対象消滅で閉じない。**

- **真因と消滅。** `pnpm start` / `pnpm preview` の起動不能は `packages/core/src/application/workers/eventRelayWorker.ts` のモジュールスコープ `crypto.randomUUID()` が原因であり、同ファイルは第7.3節で削除される。**流入経路は `server.cloudflare.ts → di/serverCloudflare.ts → di/env.ts → workers/eventRelayWorker.ts` の value-import 1本だけで、#40 のクローズ前調査が「トップレベル Worker が `eventRelayWorker.ts` から必要としているのは `DEFAULT_*` 定数のみ」と実測している。** その `env.ts` も上の変更対象一覧で削除対象に入れてある。
- **#37 へ引き継がれた論点1（worker identity は入口が採番して注入する）。** application 層から randomness を構造的に消す方針であり、**DO 構成でも同じ制約が効くので方針はそのまま必要である**（`alarm()` / RPC ハンドラの中で `IdGenerator` から採るのは可、モジュールスコープでの生成は不可）。第7.4節の `ownerToken`（claim した実行主体の識別子）がこの方針の適用先である。
- **#37 へ引き継がれた論点2（チューニング定数の置き場）。** 「実装モジュールから leaf モジュールへ。合成ルート（`di/`）→ worker 実装という逆向きの依存を作らない」であり、**`env.ts` の value-import がまさにその逆向き依存の実例だった。** 新しい設計は同種の定数（第7.4節の3階層予算・最小再開間隔・prune 保持期間・lease 期限）を再導入するので、**置き場所の規則を第7.4節に断定として置いてある**（leaf モジュールに置き、DI や実行部から value-import させない）。値そのものは #37 → #38 の2段のままである。
- **#24 へ引き継がれた論点3（ビルド成果物を workerd で起動するスモークテスト）は #37 の範囲ではない。** 本 Issue も判定しない。

### 11.3 #38 への引き継ぎ — 運用ドキュメント化が必要な事項 ［参考］

- **PITR の手順** — ローカル workerd では使えないので staging での実施手順。復旧単位が DO 1個であることの設計上の帰結は第10.1節で決着済みなので、ここへ送るのは手順（対象 bookmark、実施日時、User Data DO と Directory bucket の照合、後片付けの記録）だけである。**restore 直後の必須ステップを、restore した DO のクラス別に書く**（第10.1節）。**User Data DO を戻した場合は2つ** — **(i) `sessionEpoch` を現在時刻由来の十分大きな単調値へ強制的に進める、(ii) その User Data DO の `ai_client_connections` を全件 `revoked` にして利用者に再接続させる**（AI クライアントトークンは Directory を1度も参照しないので、第10.1節の3点では塞げない独立した穴である）。**Identity Directory bucket を戻した場合も2つ** — **(iii) その bucket の `password_reset_tokens` を全行削除する**（消費済み・未消費を問わない。復活した消費済みトークンは `consume-reset-token` → `begin-credential-change` → phase 2/3 の経路を**前進させて**攻撃者のパスワードを昇格させうる。認可が開く向きに倒れる唯一の PITR 経路である）、**(iv) `failedAttempts` を0・`nextAttemptAllowedAt` を過去へ戻す**（締め出し方向へ倒さないため）。**(iii)(iv) は「復旧できないなら全部切る」型の既定手順であり、実行の可否を運用者に判断させない。****対象 DO の特定手段（Identity Directory bucket の全走査で `credential_mappings.userId` を集める）**を手順の第1項に置く — Worker から namespace を列挙する API が無いので、これが唯一の経路である（第9.5節・第2.1節 F-5）。**使う RPC は `list-bucket-user-ids` と `read-schema-version` の2本で、どちらも第5.1節の全数表に置いてある**（群は (3-c)）。**#38 が運用手順として書くのは、この2本に `purge-user-mappings` と同じ守り（実行前の承認手続き / 誰が・いつ・どの bucket 範囲に対して実行したかの記録）を課すことと、`list-bucket-user-ids` が返す `userId` を作業ログ・チケットへ貼らない運用である。**あわせて「**PITR は個別救済の最後の手段であり、全ユーザー規模の巻き戻しには使わない**」を運用の原則として書く（第9.5節）。
...
- **`jobs.poison` 行の検知と手動再投入** — 本 Issue が決めたのは「`poison` に落ちる瞬間に設計側が何を記録し、どこへ渡すか」（第7.4節「分類 (C) の cross-DO saga 前進ジョブが `poison` に達したときの終端規則」の一様な終端）であって、落ちた行の運用ではない。**#38 が書くのは4つである** — (i) `poison` 行の検知手段（`status = 'poison'` の存在をメトリクスに出す。`terminalReason` を添える）、(ii) **分類 (C) の全 `kind` についての手動再投入手順**（第7.4節の `status = 'poison'` への再投入規則が `pending` へ戻すので、operator 経路は「同じ `operationKey` で `enqueueJob` を1回発行する」で足りる）、(iii) `SQLITE_FULL` に起因する `poison` の切り分け（第4.7節。容量監視の項目と対で読む）、**(iv) `poison` 行としては現れない終端の検知**。**(iv) が要る理由を明示する** — 終端は当該 DO の `jobs` 行への書き込みなので、**その書き込み自体が恒久的に失敗する状態（当該 DO の `SQLITE_FULL`）では `poison` も `terminalReason` も残らず、`poison` 行の存在を見る検知が発火しない。** 代わりに見るのは **(a) 容量監視（下の項目）と (b) 同一 `operationKey` の claim が繰り返され続けること**（`jobs` の `attempt` が伸び続けるが `status` が実行可能集合から出ない）の2つである。**#38 はこの2つを `poison` 行の検知と並べて書く。**
  - **残った中間状態の回収手順そのものは #45 待ちである。** 本 Issue の一様な終端は「残った状態が記録され、受け口へ届く」ところまでしか保証しない（第7.4節）。**残渣の種類ごとにどの operator エントリ（`purge-user-mappings` / `cancel-reservation`）が受け口になるかは #45 の論点5 なので、#38 はその結論を待って回収手順を書く。** それまでのあいだ #38 が書けるのは上の (i)〜(iv) の検知と再投入だけであり、**「`poison` を見つけたら何をするか」の後半は空欄のままでよい、と明示する** — 埋まっていない事実を運用手順が隠すほうが危険である。
- **`list-bucket-user-ids` の実行監査**（第5.1節 (3-c)）— #38 が書くのは3つで、これが全数である。**(a) `purge-user-mappings` と同じ承認手続きと実行監査**（誰が承認し、誰が・いつ・どの bucket 範囲に対して実行したか）、**(b) 戻り値（`userId` の一覧）を作業ログ・チケット・監視系へ貼らないこと**、**(c) この1本の実行頻度そのものを検知対象にすること**。**(c) が独立した項目である理由** — 不良 migration からの復旧という契機は稀にしか起きないので、**頻度の異常が最も安い検知手段である**。しかも本エントリは全アカウントの `userId` を返すので、偽造セッションで叩ける他のエントリの射程を「名前を知るアカウント」から「全アカウント」へ広げる増幅器であり（第5.1節 (3-c)）、`purge-user-mappings` には無い性質である。**`read-schema-version` については「`list-bucket-user-ids` の戻り値以外の `userId` を渡さない」を手順に書く**（第9.2節）。**保持期間（下の prune の具体値）とは別の項目である** — 保持期間は「いつ消すか」であり、こちらは「消す前に気づくか」である。
- **容量監視** — 「本体 + FTS インデックスの合計で 10 GB」の監視閾値、逼迫時の利用者向け導線（第4.6節）。**あわせて「逼迫時の導線がログインを要求する」ことの帰結を書く** — 10 GB に達した利用者がパスワード変更を試みると phase 2 が `SQLITE_FULL` で失敗し続け、`resume-credential-change` が一様な終端で `poison` になる（第6.5.1節・第7.4節）。**利用者から見て「パスワードが変わらないうえ、旧新どちらのパスワードでもログインできない」という症状になるので、問い合わせの切り分けに含める。****症状の重さは #45 が自動回収（巻き戻し）を入れるまで変わらない** — 巻き戻しが入れば「旧パスワードでログインできる」まで軽くなるので、#38 は #45 の結論が出た時点で本項の症状記述を更新する。
- **コスト** — rows written の内訳（trigram の増幅、`setAlarm`）（第10.2節）。
- **鍵ローテーションの運用手順 → #44 待ち。** 本 Issue が #38 へ確定して送れるのは、**手順に依存しない2点**だけである — **(i) ローテーション開始を告知し、直後のリセット依頼失敗を想定する**（旧世代 bucket のリセットトークン行が mapping を失って無効になる。第6.1節 (d)）、**(ii) maintenance 経路のログ・トレースを平文が残らない構成にする**（第6.2.1節 (c)・第6.8節の保護規定）。**残り（keyring の世代を進める操作、移送の実行、旧鍵破棄の判定、中断・再開の手順）は #44 が手順を決着させたあとにしか書けない**（第6.8節）。**#38 はこの項目を「#44 の成果物を入力とする」ものとして扱い、#44 が完了する前に手順書を書き始めない。**
- **2つのローテーションの排他と順序 → #44 待ち**（第6.8節の論点3）。**routing 鍵（`DIRECTORY_ROUTING_SECRET`）のローテーションと暗号化鍵（`IDENTITY_MAIL_ENCRYPTION_KEY`）のローテーションが互いに干渉することは本 Issue で判明している** — (i) 同時進行中は暗号化側の retirement 証明の「全 `0..N-1` bucket」が2通りに読めて定義されない、(ii) 移送が古い `encryptionGeneration` の行を再暗号化済み bucket へ運び込む。**排他をどう課すか（運用制約か構造か、その両方か）と、破られたときの検知手段は #44 が決める** — 本 Issue の初版が置いた構造側の保険（移送先での `rotate-encryption` 再投入）は `rotate-encryption` の記録規則と矛盾して成立しないことが R8 で判明しているので、**その前提に立つ検知手順（`previousCount` が0に戻らないことを見る）を #38 が先に書いてはならない。** **`rotationKind` 列が存在すること**（第4.1.1節）**と、旧鍵破棄の判定クエリで `rotationKind` を絞る必要があること**だけは本 Issue の結論である。
- **routing 鍵の漏えいを契機とするローテーションの手順 → #44 待ち**（第6.8節の論点4）。**本 Issue が確定させたのは問題の性質だけである** — bucket は `DIRECTORY_ROUTING_SECRET` を保持しないので注入された鍵と照合できる自分の値を持たず、照合材料の候補（`encryptedCanonical` / `hmac`）はいずれも previous 世代の鍵に対する材料なので、**漏えい鍵の保持者も同じ照合を通せる**（第5.1節 (3-c)・第5.2.3節）。順序を逆にすると失効させたい鍵の保持者が失効操作そのものを乗っ取れる。**「鍵の世代を進めるより先に maintenance 経路の到達性（binding 構成 / デプロイ資格情報）を再発行する」は #44 が採る可能性の高い順序だが、それだけで失効が成立するかは #44 の受け入れ条件である。** #38 は #44 の結論を待って手順書を書く。
- **移送（`rotate-remap`）の監査要件 — 一部は #44 待ちだが、運用ガードの必要性そのものは本 Issue の結論である。** 移送は **`purge-user-mappings` と同格以上に危険**である（第5.1節 (3-c)）— 破壊（移送元 bucket の全利用者が login も `request-password-reset` も解決不能になり、復旧は 256 bucket の PITR しか無い）に加えて、**移送先 bucket の `lookup-credential` から `passwordVerifier` を引き出せる**という検証材料の窃取まで成立する。**したがって #38 は #44 の結論を待たずに次の2つを書ける** — (i) **実行前の承認手続き**、(ii) **誰が・いつ・どの世代／どの bucket 範囲に対して実行したかの記録を必須にする**（`purge-user-mappings` と同じく「無ければ実行しない」と書く）。**#44 待ちは残り2つである** — (iii) 走査を中断・再開する手順と `rotation_checkpoints.previousCount` の読み方（記録の規則が #44 の論点3 だから）、(iv) `conflictCount` が増えたときのエスカレーション手順（記録の契機が #44 の論点1 だから）。
...
- **パスワード照合を行う全エンドポイントのレート制限** — Cloudflare WAF / Rate Limiting Rules を **login / signup / password reset / パスワード変更**に当てる際のキー・閾値・窓・チャレンジの出し方と、`failedAttempts` のバックオフ係数・リセット依頼間隔の具体値（第6.2.2節）。**パスワード変更は認証済みエンドポイントだが射程に入る** — 起点 A の A-2 → A-3 が login step 3〜4 と同じ1回のパスワード認証試行であり、`report-verify-result`（第5.1節）でカウンタも共有するからである。認証済み側のキーはセッション由来の主体も併用できるので、**閾値は未認証側と同じ値にしないこと**を検討材料として送る（第6.2.2節 (a)(c)）。
...
- **リセット完了後の利用者向け導線と説明** — クレデンシャル一覧と AI クライアント接続一覧（「すべて失効」つき）を提示する画面は #35 が仕様化するが、**「パスワードを変えても AI クライアント接続は有効のままである」「リセット完了で自動失効するのは前回のリセット完了以降に作られた接続である」**の説明は #38 の運用ドキュメントと利用者向け文言に書く（第5.4節）。**「直近の credential 変更以降」ではない** — 基準は `account.resetVersion` であり、途中のパスワード変更では進まないので射程が消えない。
- **予約 TTL の具体値** — 「**前進のバックオフ上限に達するまでの総経過時間** + マージン」より長く取るという不等式の形は設計側で固定済みなので、値だけを決める（第6.4節 1）。**ただし右辺の射程は #45 待ちである** — 自動回収が入って後始末の再試行が加わると覆うべき総経過時間が伸びるので（第7.4節の論点4）、**#38 は #45 の結論が出るまで値を確定させない。** 到達不能アカウントの終端（`poison` + エスカレーション）の運用受け口も含む。
...
本 Issue で結論を出さなかったのは下表の11件だけである。いずれも**誰がいつ決めるかを割り当ててある**。**種類は4つに分かれる** — (i) 未確認の事実で、設計がそれに依存しないよう組んであるもの、(ii) 結論は出ているが依拠する事実が実測1件なので再確認が要るもの（`.adr/003-sqlite-fts5-only-search.md` が「再確認が覆れば本決定そのものが成立しない」と明示している）、(iii) 決定は済んでいて値だけが2段階で決まるもの、**(iv) 設計そのものを別 Issue へ切り出したもの（2件。鍵ローテーションの手順 → #44 / cross-DO saga の異常系の自動回収 → #45）**。**「本設計への影響」欄でどれかが分かる。**

| 論点 | 決める主体 | いつ | 本設計への影響 |
|---|---|---|---|
| **鍵ローテーションの手順**（移送の phase・ガード・巻き戻し / 移送中の2世代並存の規則 = 第6.1.1節 (R6)(R7)(R9) / 2つのローテーションの関係と退役条件 / 漏えい起因のローテーションでの失効の成立性）。**「設計そのものを切り出した」種類の行の1つである**（もう1つは次の行） | **#44** | #44 の設計時 | **#37 の着手には影響しない。** 初期デプロイでは keyring の世代が1つだけで2世代並存の状態が存在しないので、**#37 は「ローテーション未実行」を前提に着手できる**（第6.8節に #37 / #44 の境界表を置いた）。**方針側（世代付き keyring・`userId` 由来 locator と credential 由来 locator の分離・`credentialId` の世代非依存性・AAD の束縛範囲・`rotationKind` 列）は本 Issue で決着済みであり、#44 はそれを入力として手順だけを設計する** |
| **cross-DO saga の異常系の自動回収**（分類 (C) の前進ジョブが `poison` に達したときの巻き戻し。段の順序と原子性境界 / 終端モードの印 / 後始末の失敗時の規則と再試行上限 / 材料の寿命 / operator 受け口の割り当て / 起点別の扱い / 上記の機械検査）。**「設計そのものを切り出した」種類の行のもう1つである** | **#45** | #45 の設計時 | **#37 の着手には影響しない。** 本 Issue は一様な終端（`terminalReason` + `poison` + operator エスカレーション）を決着させており、**#37 はそれだけを実装すれば着手できる**（第7.4節に #37 / #45 の境界と前方互換点4つを置いた）。**第6.9節の「どの中間状態でも認可は fail closed に倒れる」という宣言は一様な終端だけで満たされる** — 残るのは可用性であり、恒久化した経路は同節の一覧に行として登録され塞ぎ方の欄が #45 を指す |
