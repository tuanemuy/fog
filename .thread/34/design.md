...
| (2) | `lookup-credential`（login step 3 / リセット依頼の解決） | Directory bucket | request Worker（未認証） | 無条件に応答し、**中身を均一化する** — 未登録 / **`status != 'active'`（＝ signup / link の予約行）** / `changeState = 'pending'` / `nextAttemptAllowedAt` 未到達はすべてダミー検証材料へ倒す（第5.3節・第6.2.2節）。**`status` の条件が要るのは、signup phase 1a の予約行が `passwordVerifier` を持つからである**（第6.3節）— 落とすと、まだ活性化していない signup の検証材料が未認証経路へ出る |
...
- **`resume-credential-change` がバックオフ上限に達したときの終端を決める。巻き戻す。** 分類 (C) なので `poison` に落ちると投入点（phase 1 = `begin-credential-change`）からの再投入だけが `pending` へ戻せるが（第7.4節）、**その投入点を叩くには 起点 A（セッション + 旧パスワード照合）か 起点 B（新しいリセットトークン）が要る。ところが `changeState = 'pending'` のあいだ脱出経路は3つとも閉じている** — 旧パスワードは `lookup-credential` がダミーへ倒すので login 不能（第5.1節）、新パスワードは phase 3 未完了なので未昇格、リセット依頼も同じダミー化で解決されないためトークンが発行されない。**残るのは既存セッション（TTL 7日）からの 起点 A 再実行だけで、TTL が切れた時点で復旧手段がゼロになる**（`purge-user-mappings` は mapping を消すだけなので復旧ではなく恒久ロックの確定である）。
  ...
  - **phase 2 が成功した後に上限へ達した場合は巻き戻さない。** そのとき User Data 側は `sessionEpoch` と `credentialVersion` を進めており、Directory 側だけを戻すと第5.3節 step 5 (iii) が恒久的に不一致になる（第6.9節が登録している締め出し経路そのものである）。**したがって (ii) 側**（`terminalReason` を残して運用エスカレーション。#38）**に倒す。**
  - **判定は `operations` 行の有無では行わない。「上限に達したその実行の中で `advance-credential-change` が成功したか」で行う。** `operations` 行は **User Data DO 側**にあり、それを読む RPC を第5.1節の全数表に持たないので Directory bucket の判定材料にできない（しかも本規則の発火契機が `SQLITE_FULL` である場合、書き込みベースの冪等再実行では成功と失敗を区別できない）。**判定材料は手元にある** — `resume-credential-change` は再開のたびに phase 2 を（`operationId` + `payloadDigest` の CAS により冪等に）再発行する設計なので、**上限に達した実行は必ずその回の phase 2 の結果を持っている。** 失敗していれば (i)（起点 A なら (i) だけ、起点 B なら (i) + (ii)）、成功していて phase 3 が失敗し続けているなら (ii) である。**後者で (i) が実行できないことは整合している** — phase 3 は自 bucket のローカル書き込みなので、それが恒久的に失敗する状態（Directory bucket 側の `SQLITE_FULL`）では巻き戻しの書き込みも通らない。
...
- **意図的に可用性を落とす中間状態は上の一覧に含めない。ただし除外の根拠は「短時間だけ」ではなく「前進または巻き戻しが必ず終端する」である。** credential 変更の `changeState = 'pending'`（旧新どちらのパスワードも通らない。第6.5.1節）と退会の `deleting` は、**利用者の操作の結果として生じ、前進ジョブが完走するか、前進不能が確定して終端規則が発火するかのどちらかで必ず抜ける**ので締め出しではない。
...
| `spec/testcases/identity/changePassword.md` | `:7` / `:19` | **(B)**。あわせて **credential 変更 saga（第6.5.1節）の中間状態**（`changeState: 'pending'` の間は旧新どちらのパスワードも通らない）と `sessionEpoch` の前進をケースに足す。**さらに濫用抑止のケースを3つ足す**（第6.2.2節 (a)・第6.5.1節 phase 0 起点 A の A-4）— (i) **旧パスワードの照合失敗が `failedAttempts` を進める**（`report-verify-result`。login の失敗と同じカウンタである）、(ii) **`nextAttemptAllowedAt` 未到達の変更試行は `lookup-credential-by-locator` がダミー材料へ倒すので照合が成立しない**、(iii) **照合成功で `failedAttempts` が0に戻る**。**`resume-credential-change` が前進不能を確定したときに `changeState` / `changeOrigin` / `pendingVerifier` / `operationId` が `null` へ戻り、旧パスワードでログインできる**ケースも足す（第6.5.1節の終端規則） |
