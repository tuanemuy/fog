...
### 再指摘の集約 — 鍵ローテーションを別 Issue へ切り出す

`design.md:6.8/鍵ローテーションと他 saga の相互作用` を根とする Blocker が **R5・R6・R7・R8 の4ラウンド連続**で出ている。
...
| R8 | do-boundary B-001（削除対象スナップショット × 移送）/ B-002（移送先書き込みが RPC 全数表に無い）/ B-003（(X3) が記録規則と矛盾） |

本台帳の運用規則（3回以上の再指摘は個別修正で追いかけず、別 Issue を起票するかユーザーに判断を委ねる）に従いユーザーに判断を仰ぎ、**別 Issue に切り出す**方針で合意した。

**スコープ上の根拠**: Issue #34 の対応項目3 が要求しているのは「DO ID / routing key に PII を使わない方針の明記」までで、鍵の世代管理・ローテーション手順は要求していない。計画レビュー1周目（coverage S-003）で「AC-14 の鍵の所有者・世代管理は Issue 未要求」と判定し必須基準から外している。ローテーション手順は設計側が自ら広げた ［派生］ 領域である。

**起票した Issue: #44**（[設計] Identity Directory の鍵ローテーション手順を、全 saga との相互作用を含めて設計する）

| Key | 初出 | 判定 | 理由（一行） | 再指摘 |
|---|---|---|---|---|
| `design.md:6.8/鍵ローテーションと他 saga の相互作用（構造）` | R5 | defer → **#44** | 4ラウンド連続の再指摘。Issue #34 が要求していない ［派生］ 領域なので別 Issue へ切り出す。R8 の B-001 / B-002 / B-003 を含む | 4 |
| `design.md:5.1/begin-credential-change 起点 B の束縛` | R8 | fix | `operationId`（ログ出力可）＋ `credentialId`（UI 露出）だけでは乗っ取り原始関数が残る。レビューが具体的な修正案（一回性 `changeAuthToken`）を提示している | 0 |
...
## Round 10（最終ラウンド。レビュー上限に到達）

**レビューループは最大ラウンド数（10）に達したが、`fix` 判定の指摘がゼロになるラウンドには到達しなかった。** `_shared/references/review-loop.md` の規定に従い強制終了し、残っている指摘を `.thread/34/review/review-010.md` にまとめてユーザーの判断に委ねる。**PR は Draft のまま維持し、Phase 4 のブラウザ検証には進まない。**

鍵ローテーション由来の指摘は R9 / R10 ともゼロ。残る指摘はすべて**直近2ラウンドで新設した機構の射程の穴**で、各レビュアーが「1〜2行の追記で閉じる」と評価している。

| Key | 初出 | 判定 | 理由（一行） | 再指摘 |
|---|---|---|---|---|
| `design.md:6.4/分類 (C) の cross-DO saga が poison に達したときの終端` | R10 | **未対応** | 設計自身が Blocker 級と判定した孤児 mapping が単一世代のまま再生する | 0 |
| `design.md:6.5.1/認証済みパスワード変更経路の試行制限` | R10 | **未対応** | `failedAttempts` の更新経路が login の1本に確定し、WAF の射程も未認証のみ | 0 |
| `design.md:6.5.1/changeAuthToken の NULL 化が同 bucket に限られる` | R10 | **未対応** | 複数クレデンシャル利用者で他 bucket の行が失効しない | 0 |
| `design.md:6.5.1/changeOrigin の null 復帰が phase 3` | R10 | **未対応** | phase 4 の再実行で起点が読めない | 0 |
| `design.md:5.1/abandon-account の束縛が operationId のみ` | R10 | **未対応** | 「`operationId` を束縛の実体に使うエントリは1本も無い」が偽になる | 0 |
| `design.md:9.5/bucket 全走査の RPC エントリが全数表に無い` | R10 | **未対応** | 不良 migration 復旧手順が要求している。#44 の射程外 | 0 |
| `design.md:6.1/リセットトークン無効化点の全数の食い違い` | R10 | **未対応** | 「新しいトークンの発行が未使用トークンを全部置き換える」と3箇所の全数が矛盾 | 0 |
| `design.md:6.9/resume-credential-change の poison が締め出し経路一覧に無い` | R10 | **未対応** | 除外条項が「短時間だけ閉じる」を前提にしている。上記1件目の一般形 | 0 |
| `design.md:7.4/E-3 の finalize-withdrawal の投入点欄の取りこぼし` | R10 | **未対応** | `abandon-account` が漏れており I-1 と検査2 では構造的に検出不能 | 0 |
| `design.md:6.8/routing keyring を bucket に配るかが両方書かれている` | R10 | **未対応** | 自己矛盾。#44 本文とも食い違い | 0 |
| `design.md:1.4/検査8・検査9 の字下げが値に混入する` | R10 | **未対応** | 逐語実行すると誤検出になる（検査自体は正しく動く） | 0 |
| `PR #43 本文/件数・規模が現物と食い違う` | R10 | fix | 機械検証17件 → 20件 / 11章73節 → 11章55節。PR 本文を更新して解消 | 0 |

**Round 10 集計**: fix 1（PR 本文）/ 未対応 11 / wont-fix 0 / defer 0（生指摘 B2 + W8 + 検査手順1 + Notes 4 のうち、Notes 4件は各レビュアーが Blocker / Warning 相当ではないと明示）
...
| 通算 fix | 153 |
| 通算 wont-fix | 2 |
| 通算 defer | 1（**#44**。4ラウンド分の Blocker 8件 + R8 async B-001 を集約） |
| 未対応で残した指摘 | 11（`.thread/34/review/review-010.md` に記載） |
| 構造的修正（3ラウンド以上の再指摘を集約） | 3件（`credentialId` の導入 / 「全数」表の不変条件と機械検査 / 鍵ローテーションの #44 切り出し） |

### wont-fix の記録（後片付け時に確認する）

| Key | 判定理由 | 転記先 |
|---|---|---|
| `plan.md:AC表の23→19統合`（計画レビュー R2 cov S-001） | 検証粒度を落とす損失が大きく、番号の付け替えが台帳の参照を壊す | `.thread/34/adr.md` ADR-013 |
| `design.md:7.7/類型名と第7.4節 (iii) の対象集合が別の軸`（R7 async N-011） | レビュアー自身が「必須ではない」と明記。両者が別の軸であることは本文から読み取れる | 転記不要（誤りではなく補足の提案） |

いずれも「指摘は正しいが意図的に逸脱している」type ではないため、コードの why not コメントへの転記は不要。
