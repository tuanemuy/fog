...
- **対応する受け入れ基準:** AC-8
- **目的:** 1662行の作業ログの本文を1文字も壊さずに supersede ポインタが入っていることを確認する
- **手順:**
  1. 追加行数・削除行数を数える
...
- **手順:**
  1. ラベルが付いていない見出しを洗い出す

     ```bash
     grep -nE '^#{2,4} ' .thread/34/design.md | grep -vE '［(Issue 要求|派生|参考)］'
     ```
  2. ラベル別の節数を数える

     ```bash
     for l in 'Issue 要求' '派生' '参考'; do
       printf '%s: %s\n' "$l" "$(grep -cE "^#{2,4} .*［${l}］" .thread/34/design.md)"
     done
     ```
  3. 未決を示す語を全文走査する

     ```bash
     grep -nE '検討する|今後検討|TBD|要検討|未定|決めきれない|〜次第|次第である|見込み|暫定|保留とする' .thread/34/design.md
     ```
  4. 前方依存の解消を確認する（第4〜6章に暫定表現が残っていないか）

     ```bash
     awk '/^## 4\./,/^## 7\./' .thread/34/design.md | grep -nE '暫定|見込み|次第|従属'
     ```
- **期待結果:** 手順1 は第1章の H1 とラベル運用の説明節を除いて空。手順2 は3ラベルとも1以上。手順3 のヒットが、`［参考］` 節・第7.2.1節（検索 API の仕様 → #35 へ委譲）・第11.4節（未決事項）・第5.2.3節（鍵の所有者と世代管理）に**限られる**。手順4 が空
- **確認ポイント:** 手順3 のヒット行はファイル名だけでは判定できないので、必ず**その行がどの節に属するか**を見る。とくに次の7箇所は AC-5 が名指しで対象に含めているので、結論位置に未決語があってはいけない — 「FTS5 の同期更新」「Outbox / relay / consumer / DLQ の廃止範囲」「Alarm ジョブ」「trash retention の期限処理」「外部 I/O を永続ジョブに残す境界」「UoW 契約」章・「スキーマバージョン管理と lazy migration」章、加えて「FTS5 のみで日本語全文検索が成立する根拠」と「分割方式」（`.adr/003` と #37 の前提がここに懸かる）。意味レベルの断定判定は人間判断項目 H-5 で行う
...
- **手順:**
  1. 章の存在を確認する
     ...
     ```
  2. 7項目のキーワードが「保持データ範囲」節の表に現れるか走査する

     ```bash
     awk '/^### .*保持データ範囲/,/^### /{print}' .thread/34/design.md \
       | grep -nE 'ユーザー単位設定|AI client|AiClientConnection|memo|revision|topic|document|source link|trash|retention|FTS5|冪等'
     ```
- **期待結果:** 手順1 で「User Data DO」章と 4.1〜4.8 相当の節が出る。手順2 で7項目（ユーザー単位設定 / AI client connections / memos・memo revisions / topics・documents・document revisions・source links / trash・retention 状態 / FTS5 インデックス / 冪等化・非同期処理状態）すべてに対応する行が確認できる
...
- **手順:**
  ...
  3. PII 3点（生値を使わない / HMAC・hash を使う / ログ・URL・エラーに出さない）を走査する
     ...
     ```bash
     awk '/^### .*DO ID \/ routing key と PII/,/^### 5\.3/' .thread/34/design.md \
  ...
  4. canonical 化の4項目に対応する節があるか
     ...
     ```
- **期待結果:** 手順1 で「ルーティング」章と 5.1〜5.5 相当の節が出る。手順2 で session / token → `userId` → DO locator の経路が追える。手順3 で (a)(b)(c) の3点が結論として書かれている。手順4 で4つの節タイトルがすべてヒットする
...
- **手順:**
  ...
  2. 解決責務の4サブ項目を確認する
     ...
     ```bash
     awk '/^### .*解決責務/,/^### .*分割方式/' .thread/34/design.md \
       | grep -nE '正規化メール|findByEmail|SSO|providerSubject|findBySsoIdentity|一意性|パスワードリセット|所有境界'
     ```
  3. 分割方式が決め切られているかを見る

     ```bash
     awk '/^### .*分割方式/,/^#### .*canonical credential/' .thread/34/design.md \
       | grep -nE '採用|採らない|決定|単一グローバル|bucket|credential 単位|#37'
     ```
- **期待結果:** 手順1 で6論点すべての節がヒットする。手順2 で (a) 正規化メール → `userId` / (b) SSO provider + subject → `userId` / (c) メール・SSO 主体の一意性 / (d) パスワード認証・リセットの認証情報の所有境界 が個別の結論として読める。手順3 で3案（単一 / 固定 bucket / credential 単位 DO）が4軸で比較され、**採用案が断定されている**
...
- **手順:**
  ...
  3. 述語の定義が表より前に書かれているか
     ...
     ```bash
     awk '/^### .*ユーザー境界に閉じないものの帰属/,/^### 4\.4/' .thread/34/design.md | head -40
     ```
  4. 表の各行に行き先が入っているか（補助）

     ```bash
     awk '/^### .*ユーザー境界に閉じないものの帰属/,/^### 4\.4/' .thread/34/design.md \
       | grep -E '^\|' | grep -vcE 'User Data DO に閉じる|Directory|Account Home|不要になる'
     ```
- **期待結果:** 手順1 が **85**（実測値。台帳が更新されていれば数は動く）。手順2 の `comm` 出力に残る `ADP-*` が、**述語に該当しない（＝ユーザー境界に閉じる）ものだけ**である。手順3 で述語 (a)(b)(c) の定義が表の前に置かれている。手順4 のカウントがヘッダー行と区切り行の2行のみ（＝全データ行に行き先がある）
- **確認ポイント:** **主判定は手順2 の走査そのもの**であって行数ではない。plan.md の表は8カテゴリ29行だが、これは走査の結果であって目標値ではない（1周目7件 → 2周目16行 → 3周目25行 → 29行と3周連続で拡大している）。手順2 で残った `ADP-*` は1件ずつ述語を当て直す — (a) `userId` を第一引数に取らないポート（引数オブジェクトの中に `userId` がある場合も該当）、(b) 引き方の経路に `user_id` が入っていないテーブル、(c) 台帳の粒度で捕まらない次元（DI 構成・ジョブ・spec 上の未設計領域）
...
- **手順:**
  1. 結論の所在を見る
     ...
     ```bash
     awk '/^### .*クラス構成と責務分界/,/^### 3\.2/' .thread/34/design.md | grep -nE 'Account Home|採用|採らない|畳む|2クラス|3クラス'
     ```
  2. 未決事項の節に現れないことを確認する

     ```bash
     awk '/^### .*未決事項/,0' .thread/34/design.md | grep -n 'Account Home'
     ```
- **期待結果:** 手順1 に「採用する / しない」の断定と理由がある。手順2 の**出力が空**
...
- **手順:**
  1. 必須パスの機械チェック
     ...
     ```
  2. `package.json` のスクリプト群への言及を確認する

     ```bash
     grep -nE 'deploy:(staging|production)|db:apply|db:execute|db:migrate' .thread/34/design.md | head
     ```
  3. 対象ファイルが実在することを確かめる（引き継ぎ先が存在しないパスを指していないか）
     ...
     ```
- **期待結果:** 手順1・手順3 の出力が空。手順2 で deploy 系（非 dry 12本 / dry を含め24本）と D1 前提の db 系7本への言及がある
...
- **手順:**
  1. ADR の件数を数える
     ...
     grep -nE '^## ADR-0(2|3)[0-9]' .thread/34/adr.md
     ```
  2. 追記が無い場合の明記を探す
  ...
  3. `.adr/` の3件と内容重複していないかを見る
     ...
     grep -E '^## ADR-0(2|3)[0-9]' .thread/34/adr.md
     ```
- **期待結果:** 手順1 の件数が **着手時点の19件から増えている**（ADR-020 以降が存在する）。増えていない場合は手順2 で「昇格を見送った判断が無かった」旨が明記されている。手順3 で追記分の主題が `.adr/002〜004` の主題（ランタイム・データ配置 / 検索方式 / 非同期処理）と重複していない
- **確認ポイント:** 存在チェックだけでは着手前の19件で空振りするので、**必ず件数の増加**を見る（着手時点の実測は19件）
...
- **手順:**
  ...
  3. 双方向参照の成立を見る
     ...
     ```
- **期待結果:** 手順1 の `MISSING:` が、**先行ブランチにしか存在しないパス（`.thread/19/*`）と本 Issue で新設したファイルの相互参照以外**に出ない。手順2 の出力が空（無修飾の `ADR-004` は `.thread/1/adr.md` / `spec/adr/` / `.adr/` の3文書を指しうるため、`.thread/1/adr.md` ADR-046 の規約でパス付きが必須）。手順3 で `.adr/` → design.md と design.md → `.adr/` の参照が両方向とも出る
- **確認ポイント:** 手順1 は候補一覧なので、ADR 番号の短縮表記（`` `.adr/002` `` のような拡張子なしの言及）と先行ブランチのパスは人間が除外して判定する。手順2 でヒットした行は、見出し自身（`## ADR-004: ...` の形）なら許容してよい
...
`git diff --name-status main...HEAD` は commit 前だと空を返す（着手時点の実測でも空）。この状態で確認項目1・4・5・6 を実行すると「違反が無い」ではなく「差分が無い」で通ってしまう。**必ず `git status --porcelain` を先に見て、`?? .thread/34/` が残っていないこと（＝成果物が commit 済みであること）を確認してから機械検証に入る。**
