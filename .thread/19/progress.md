...
- User DataをAccount Homeのcanonical `userDataObjectName`からだけ解決し、
  schedule→restart→restored session verify→undoを行うoperator HTTP/CLI
...
- local-only lifecycle Worker/CLIの13操作と各直後検索assert、CI実行、
