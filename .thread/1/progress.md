...
### 1. 旧コスト・旧アルゴリズムの保存ハッシュが残る間の等時間化

- **内容:** 未登録メール・SSO アカウントの経路もダミーハッシュへの verify で鍵導出1回分を払う（ADR-026 でタイミングオラクル自体は解消済み）。ダミーの反復回数は `DEFAULT_PBKDF2_ITERATIONS` と、アルゴリズム識別子は `ALGORITHM_ID` と、それぞれ型で結ばれている（ADR-034 / #20）ので既定値の変更には追随するが、**変更前に書かれた行は旧コストまたは旧アルゴリズムのまま**残るため、そのアカウントへの誤パスワードは未登録アドレスより安く済む。
- **理由:** 解消には rehash-on-login（読み取り専用ユースケースに書き込みが入る設計判断）が必要で、Issue #18 に切り出し済み。
- **影響範囲:** 本番データが1行も存在しない現状では差は生じない（#20 の方式変更 SHA-256 → SHA-512 も初回デプロイ前に行ったので、旧アルゴリズムの行は本番に生まれていない）。本番に行が生まれたあとで反復回数やアルゴリズムを動かした時点から #18 が入るまでの間だけ、旧パラメータのアカウントについて存在推測の材料が残る。
...
台帳は記憶ではなく `grep -n "spec-sync" .thread/1/adr.md` の全ヒットから作る。ADR を追記したら同じ grep を回して差分を転記する。
...
- **`ADP-identity-012` のハッシュ方式** — spec は「Argon2id 等」と例示するが、実装は WebCrypto PBKDF2-HMAC-SHA512（ADR-003 / 方式は #20 で SHA-256 から変更）。spec が「アダプター責務」と明記しているため乖離ではないが、記録として残す。
