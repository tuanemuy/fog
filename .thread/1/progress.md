...
- **残差の大きさ:** 旧 `pbkdf2-sha256` 形式の行への誤パスワードは、**未登録アドレスより約 97ms 速い**（`SHA-256 @ 210,000` = 30.2ms に対し現行ダミーの `SHA-512 @ 210,000` = 127.2ms。CI x86_64 実測 — `.thread/1/adr.md` ADR-003「実測結果（#20 / 2026-08-07）」節）。#20 以前は反復回数を一度も上げていなかったので残差は文字どおりゼロだった。**向きは既存のものと同じだが、大きさは桁で変わっている** — 97ms はネットワークノイズに埋もれる量ではなく、「そのアドレスは登録済みで、かつ #20 以前に作られた」を判別できる。
