...
**訂正（2026-08-07 / #20）— 本ファイル全体に掛かる宣言。** 本ファイル内で `pbkdf2-sha256` / `PBKDF2-HMAC-SHA256` / `PBKDF2-SHA256` に言及する記述（ADR-003 / 014 / 021 / 026 / 027 / 033 / 034）は、いずれも **#20 以前の方式名**である。#20 が動かしたのは**アルゴリズムだけ**なので、この宣言の射程もアルゴリズム識別子だけに掛かる。
...
- 方式名の書き分けは節の性格で決めてある。**当時の観測・当時の決定を述べる行（Context / Decision）は原文を残し、括弧書きで現行値を添えた。** 今も有効な仕組み・今も参照される定義を述べる行（Consequences など）は本文を現行値へ直した
...
- **SHA-512 を選んだ理由は我々の判断である。** SHA-512 は 64bit 演算主体で、32bit 演算に最適化された GPU / ASIC での並列効率が SHA-256 より低いと一般に言われており、同じサーバー負荷あたりの攻撃者コストが高い。**これは OWASP が当該行の回数を低く設定した理由ではない**（OWASP の表は防御側コストのキャリブレーションであって意図の記述ではない。上記 Decision の訂正ブロックを参照）
