...
**訂正（2026-08-07 / #20）— 本ファイル全体に掛かる宣言。** 本ファイル内で `210,000` / `pbkdf2-sha256` / `PBKDF2-HMAC-SHA256` に言及する記述（ADR-003 / 014 / 021 / 026 / 027 / 033 / 034）は、後述の訂正で本文を直した行を除き、いずれも **#20 以前の事実・当時の決定**であって現行値ではない。**現行の方式と反復回数は、本 ADR の「訂正（2026-08-07 / #20）」節と「実測結果（#20 / 2026-08-07）」節を見よ。**
...
> **訂正（2026-08-07 / #20）— OWASP 引用の取り違え。** 210,000 は OWASP Password Storage Cheat Sheet（2023 版）の **PBKDF2-HMAC-SHA512 の行**の数字であり、PBKDF2-HMAC-SHA256 の行は **600,000** である。表の2つの数字は**各アルゴリズムで防御側の所要時間がおおむね揃うようにキャリブレートされた**もので、SHA-512 は1反復あたりの CPU コストが SHA-256 より高いから回数が少ない。したがって「210,000 は SHA-256 の OWASP 推奨」という上記の帰属は誤りで、SHA-256 のままなら推奨の約 1/3 のコストだった。
>
...
- **測定環境:** GitHub Actions `ubuntu-latest`（x86_64）の `integration` ジョブ。ワークフロー実行 URL: <https://github.com/tuanemuy/fog/actions/runs/31121514993>（**attempt 4**。attempt 1〜3 は GitHub 側のランナー確保失敗 `The job was not acquired by Runner of type hosted` による全ジョブ cancelled で、計測とは無関係）
...
- **確定した判定ゲートの行: `G-1`（`t_A ≤ 2.0 × t_B` → 127.2 ≤ 172.4）→ 案 A。** 比 1.476 はノイズ再測帯（1.8〜2.2）の外なので CI の再実行は行っていない
...
**未登録・SSO の分岐でも `PasswordHasher.verify` を1回走らせてから同じエラーを投げる。** 照合先はモジュール定数のダミーハッシュで、アダプターの本番パラメータ（PBKDF2-HMAC-SHA512 / 210,000 回）で生成した固定値。保存ハッシュは自己記述形式なので、テスト用に低い反復回数のハッシャーを差し替えても、このダミーは自分の宣言どおりのコストで検証される（テストの既定は `FakePasswordHasher` なので実コストは発生しない）。
...
ADR-026 の等時間化は `DUMMY_PASSWORD_HASH`（`pbkdf2-sha512$210000$…` の文字列定数）に verify を1回走らせることで成立している。ここに2つの穴が残っていた。
