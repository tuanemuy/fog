...
## ADR-003: パスワードハッシュ方式 — WebCrypto PBKDF2-HMAC-SHA512

**訂正（2026-08-07 / #20）— 本ファイル全体に掛かる宣言。** 本ファイル内で `pbkdf2-sha256` / `PBKDF2-HMAC-SHA256` / `PBKDF2-SHA256` に言及する記述（ADR-003 / 014 / 026 / 027 / 034）は、いずれも **#20 以前の方式名**である。#20 が動かしたのは**アルゴリズムだけ**なので、この宣言の射程もアルゴリズム識別子だけに掛かる。

- **反復回数 210,000 は #20 の後も現行値である。** `210,000` / `210_000` / `210000` を含む記述はそのまま読んでよい。誤っていたのは「これが SHA-256 に対する OWASP の推奨である」という**帰属だけ**であって、数字そのものは動いていない（下記 Decision の訂正ブロックを参照）
- 方式名の書き分けは**記述の性格**で決めてある。**当時の観測・当時の決定を述べる行は原文を残し、現行値と取り違えられうる行には括弧書きで現行値を添えた**（取り違えの経路が無い行には添えていない。たとえば下記 Context の選択肢の列挙は「当時どの候補を並べたか」を述べているだけで現行値の宣言ではないので、上記第1段落の宣言がそのまま覆う）。今も有効な仕組み・今も参照される定義を述べる行は本文を現行値へ直した。節の見出し（Context / Decision か Consequences か）はこの性格とおおむね一致するが、規則の本体は節名のほうではない。**例外は ADR-014 の Context にある `verify` 失敗の2分類**で、これは当時の観測ではなく**今も有効な失敗分類の定義**（どちらの失敗がどの `SystemErrorCode` に翻訳されるか）なので本文を現行値へ直し、旧形式も読める側に含まれる旨を同じ行に添えてある
- **現行の方式と反復回数は、本 ADR の「訂正（2026-08-07 / #20）: PBKDF2-HMAC-SHA512 へ切り替える」節と「実測結果（#20 / 2026-08-07）」節を見よ。**
...
**(d) WebCrypto PBKDF2-HMAC-SHA256 を採る。**（当時の決定。→ 下記「訂正（2026-08-07 / #20）: PBKDF2-HMAC-SHA512 へ切り替える」節で SHA-512 へ変更した）パラメータは 16 byte のランダム salt・32 byte 出力・**既定**反復回数 210,000（OWASP の PBKDF2-SHA256 推奨）。保存形式は

```
pbkdf2-sha256$<iterations>$<saltBase64>$<hashBase64>
```

> **訂正（2026-08-07 / #20）— OWASP 引用の取り違え。** 210,000 は OWASP Password Storage Cheat Sheet の **PBKDF2-HMAC-SHA512 の行**の数字であり、PBKDF2-HMAC-SHA256 の行は **600,000** である。**出典:** <https://cheatsheetseries.owasp.org/cheatsheets/Password_Storage_Cheat_Sheet.html>（PBKDF2 節。2026-08-07 参照）。同節が挙げる3行は次のとおりで、どれがどれかはこの表の全体を見れば一意に決まる。
>
> | アルゴリズム | 推奨反復回数 |
> |---|---|
...
>
> 回数がこの順に少なくなるのは、1反復あたりの CPU コストがこの順に高くなるからである。**チートシート自身はこれらの設定を（防御側にとって）等価な選択肢として並べているだけで、「各アルゴリズムで防御側の所要時間が揃うようにキャリブレートした」という説明は我々の読みであり、出典の文言ではない。** したがって「210,000 は SHA-256 の OWASP 推奨」という上記の帰属は誤りで、SHA-256 のままなら推奨の約 1/3 のコストだった。**反復回数 210,000 そのものは #20 の後も現行値である**（誤っていたのは帰属だけ）。
>
> **現行の方式と保存形式は下記「訂正（2026-08-07 / #20）: PBKDF2-HMAC-SHA512 へ切り替える」節のとおり**（`pbkdf2-sha512$<iterations>$<saltBase64>$<hashBase64>`、既定反復回数 210,000）。上のコードブロックは #20 以前の保存形式である。
...
上記の OWASP 引用の訂正を受け、**案 A（PBKDF2-HMAC-SHA512 + 210,000）と案 B（PBKDF2-HMAC-SHA256 + 600,000）のどちらを採るか**を、事前定義の判定ゲート（`.thread/20/steps.md` ステップ2 / `.thread/20/adr.md` ADR-001）へ実測値を当てはめて確定した。

捨てプローブの置き場所は今回 `packages/core/src/application/identity/__tests__/_probe.integration.test.ts` で、2026-07-25 のプローブ（`adapters/webcrypto/__tests__/`）とは異なる。`.adr/001` 以降 `vitest.config.integration.ts` の `include` が許可リストへ絞られ、`adapters/webcrypto/**` はそこに載っていないためで、設定ファイルへ一時的な穴を開けずに済む配置を選んだ（確認後に削除済み）。

**G-0（`PBKDF2 + SHA-512 @ 210,000` の疎通）: 通過。** 先行実測（ローカル workerd / Apple Silicon）で `supported: true`、CI ランでも同じく `supported: true`。workerd の WebCrypto は PBKDF2 + SHA-512 を実装している。

計測（1 導出あたり / ミリ秒。**各条件 n = 3** で、min / 中央値 / max は3標本そのものである）:

| 条件 | 環境 | min | 中央値 | max |
|---|---|---|---|---|
| `SHA-512 @ 210,000`（`t_A`） | CI（x86_64） | 126.6 | **127.2** | 139 |
| `SHA-256 @ 600,000`（`t_B`） | CI（x86_64） | 86.2 | **86.2** | 86.4 |
| `SHA-256 @ 210,000`（参考） | CI（x86_64） | 30 | 30.2 | 30.2 |
| `SHA-512 @ 210,000` | ローカル（Apple Silicon） | 45.4 | 45.6 | 45.8 |
| `SHA-256 @ 600,000` | ローカル（Apple Silicon） | 46.4 | 46.8 | 47.0 |
| `SHA-256 @ 210,000` | ローカル（Apple Silicon） | 16.2 | 16.4 | 16.6 |

- **測定環境:** GitHub Actions `ubuntu-latest`（x86_64）の `integration` ジョブ。ワークフロー実行 URL: <https://github.com/tuanemuy/fog/actions/runs/31121514993>。**上表の CI 行は attempt 4 の値である。** このランは attempt 4 まで再実行されており、内訳は次のとおり。
  - **attempt 1〜2:** GitHub 側のランナー確保失敗（`The job was not acquired by Runner of type hosted even after multiple attempts`）で全ジョブ cancelled。計測は出ていない
  - **attempt 3 / attempt 4:** どちらも `integration` ジョブが完走し、**完全な計測を1組ずつ出した**（プローブ入りのコミットなので REPORT テスト自体は故意に失敗する。R-2b のとおりそれが計測成功の印である）
- **attempt 3 の計測（結論の頑健性の裏付け）:** `SHA-512 @ 210,000` 中央値 **116.4ms** / `SHA-256 @ 600,000` 中央値 **77.2ms** / 比 **1.508**。上表の attempt 4 は 127.2 / 86.2 / 比 **1.476**。**2組の独立した計測が、どちらも同じゲートの行（`G-1`）に落ちた。** 中央値の絶対値は共有ランナーの負荷で 9% ほど動いたが、判定を決める比は 1.48〜1.51 に収まっており、どちらの組でも案 A で一致する
- **ローカル行の出どころ:** 表のローカル（Apple Silicon）3行は**計画確定時（2026-08-07）の先行実測**であり、`.thread/20/adr.md` ADR-001 / `.thread/20/steps.md` が「`SHA-512 @ 210k` 中央値 45〜47ms / `SHA-256 @ 600k` 中央値 47〜49ms」とレンジで書いているのは**同じランの丸め**である（上表が実値）。判定には使っていない
- **判定に使ったのは CI 実測のみ。** ローカル（Apple Silicon / ARM）の数字は比較に使っていない。M シリーズは ARMv8.2 の FEAT_SHA512 を持ちうるため、ローカル実測は SHA-512 に系統的に有利な方向へ偏るためである
- **比が環境で動いた。** ローカルの `t_A / t_B` は **0.974** だったのに対し、CI（x86）では **1.476**。SHA-NI が SHA-256 側だけを加速するという想定どおりの動き方であり、「ローカルではなく x86 で測る」と決めた理由がそのまま観測された。CI は案 A にとって保守側（不利側）の条件である
- **確定した判定ゲートの行: `G-1`（`t_A ≤ 2.0 × t_B` → 127.2 ≤ 172.4）→ 案 A。** 比 1.476 はノイズ再測帯（1.8〜2.2）の外なので**ゲート由来の再測は不要だった**（CI の再実行はランナー確保失敗という別の理由で発生しており、その結果得られた attempt 3 の比 1.508 も同じ `G-1` の行に落ちる）
- **参考値の連続性:** `SHA-256 @ 210,000` の CI 中央値 30.2ms は、上記 2026-07-25 の実測（workerd 16ms）および ADR-033 の 33ms と同じ桁で、共有ランナーとローカルの速度差の範囲に収まる
- **観測項目（判定には使っていない）:** `t_A` の中央値 **127.2ms が 100ms を超えている**（`t_B` の 86.2ms も参考として記録する）。絶対値の上限は判定ゲートに置いていないので**案の選択は変えない**（理由は `.thread/20/adr.md` ADR-001: このスコープでは上限に触れても取れる行動が「案 B を選ぶ」しかなく、そこでは相対比較が既に案 B を選んでいるため上限は判定に一度も寄与しない。加えてこれは共有ランナーの絶対値であって本番の値ではない）。重さそのものが問題なら別 Issue として起票する
- **未認証経路の CPU 増幅（判定には使っていない / 追跡先は #18）:** `t_A` = 127.2ms は**未認証リクエスト1本あたりの CPU コスト**でもあり、#20 以前の出荷構成（`SHA-256 @ 210,000` = 30.2ms）比で **4.2 倍**である。ADR-026 の等時間化により `burnVerificationTime` は**未登録アドレスへのログイン試行にも同じ1導出を払わせる**ので、この増幅を引き出すのにアカウントの存在すら要らない。CF Paid の 30 秒予算に対して1リクエストは問題にならないが、それは1 invocation の上限の話であって、同時実行と CPU 時間課金の増幅は別問題である。**レート制限の実装は本 Issue のスコープ外で、受け皿は [#18](https://github.com/tuanemuy/fog/issues/18)** — `spec/domains/identity.md` は `failedAttempts` / `nextAttemptAllowedAt` を定義しているが、実装はリポジトリに存在しない

### 訂正（2026-08-07 / #20）: PBKDF2-HMAC-SHA512 へ切り替える

上記の実測とゲート `G-1` により、**方式を PBKDF2-HMAC-SHA512 + 反復回数 210,000 に変更する。** これは事実の誤りの訂正ではなく**決定の変更**なので、Decision 本文を書き換えずここに追記する。

- 保存形式は `pbkdf2-sha512$<iterations>$<saltBase64>$<hashBase64>`。salt 16 byte・出力 32 byte・既定反復回数 210,000 は据え置き（PBKDF2 のブロック数は `ceil(dkLen / hLen)` なので、32 byte 出力は SHA-256 でも SHA-512 でも 1 ブロック）
- **旧 `pbkdf2-sha256$` 形式は読み取り専用の枝として残る。** `verify` は引き続き読めるが、書き出す識別子は `pbkdf2-sha512` の1種類だけになる。Decision が謳っていた「識別子付きエンコードによる無停止移行」を初めて実際に行使した変更であり、退役条件は `.thread/20/adr.md` ADR-002 にある
- **SHA-512 を選んだ理由は我々の判断である。** SHA-512 は 64bit 演算主体で、32bit 演算に最適化された GPU / ASIC での並列効率が SHA-256 より低いと一般に言われており、同じサーバー負荷あたりの攻撃者コストが高い。**これは OWASP が当該行の回数を低く設定した理由ではない**（チートシートは各設定を並べているだけで、行ごとの回数の理由を述べていない。上記 Decision の訂正ブロックを参照）
- 変更は本番データが1行も存在しない時点で行ったので、ハッシュの移行処理は発生しない
...
- トレードオフ:
  ...
  - 反復回数 210,000 は Cloudflare Workers の CPU 予算（無料プランで数十 ms）を超える可能性がある。**これは「CF を本番採用したら考えること」ではなく、統合テストが workerd プールで走る以上、本 Issue の実行可能性に直結する**（上記 Decision の実測手順を参照）。CF ランタイムを本番採用する場合は反復回数の見直し、または Argon2id(WASM) への移行を検討する
    ...
    > **訂正（2026-08-07 / #20）— CPU 予算の数字。** Workers の CPU 予算は Free で **10ms**、Paid の既定で **30 秒**であり、「無料プランで数十 ms」は誤り。加えて #34 で **CF Paid** の採用が確定しているため、**この懸念そのものが現行の前提では成立しない** — 30 秒の予算に対して 1 導出あたり数十〜百数十 ms は問題にならない（実測値は下記「実測結果（#20 / 2026-08-07）」節）。したがって「CF を本番採用する場合は反復回数の見直しを検討する」という結びも、CPU 予算を理由とする限りは不要になっている。
  ...
  - 反復回数を**ファクトリ引数**にしたことで、呼び出し側が誤って低い値を渡す余地が生まれる。既定値を持たせたうえで、本番の配線（4本の DI ファクトリ）は既定値をそのまま使い引数を渡さない運用にする。環境変数からは設定できないままなので、「環境ごとに強度が揺れない」という当初の意図は保たれている
    ...
    > **補足（2026-08-07 / #20）— ピンの残穴。** #20 で入れたアルゴリズム／反復回数の型ピン（ADR-034 / AC-9）が覆うのは `DEFAULT_PBKDF2_ITERATIONS`（既定値）どうしのドリフトであって、**ファクトリ引数で構築されたハッシャーではない**。本番の配線が `iterations` を渡せば ADR-026 の等時間化は型検査にもテストにも掛からずに壊れる（ダミー側が高くなり、オラクルが反転する）。防護は上記の運用規約だけである。テストで塞ぐ案は本番強度の実導出を踏むテストを2件から3件へ増やして `.thread/20/plan.md` R-6 と衝突するため採らず、**トレードオフとしてここに記録して閉じる**。アルゴリズム側は引数が無いので同じ穴は無い。
...
1. 保存済みハッシュが `pbkdf2-sha512$<iterations>$<salt>$<hash>` 形式として読めない（形式不正・base64 破損・反復回数が非整数）。#20 以前に書かれた `pbkdf2-sha256$…` も読める側に含まれる
...
- トレードオフ:
  - Cloudflare だけは依然としてリクエスト毎の検証で、起動時には検出されない。`.thread/1/testing.md` と PR 説明の「起動時エラー」はランタイム別に書き分ける必要がある
...
**未登録・SSO の分岐でも `PasswordHasher.verify` を1回走らせてから同じエラーを投げる。** 照合先はモジュール定数のダミーハッシュで、アダプターの本番パラメータ（PBKDF2-HMAC-SHA256 / 210,000 回。#20 以降は PBKDF2-HMAC-SHA512 / 210,000 回）で生成した固定値。保存ハッシュは自己記述形式なので、テスト用に低い反復回数のハッシャーを差し替えても、このダミーは自分の宣言どおりのコストで検証される（テストの既定は `FakePasswordHasher` なので実コストは発生しない）。
...
- トレードオフ:
  - アプリケーション層のモジュール定数がアダプターの保存形式（`pbkdf2-sha512$...`）を1つ抱える。ポートに「ダミーハッシュ」を生やす案もあったが、spec が定義するポートの面（`hash` / `verify`）を実装都合で広げるほうが害が大きいと判断した。定数の役割と失効時の挙動は JSDoc に書いてある
  - 未登録アドレスへのログイン試行も本物と同じ CPU を消費するので、認証エンドポイントのレート制限の必要性は（N-005 の指摘どおり）むしろ上がる。本 Issue の範囲外で、**受け皿は [#18](https://github.com/tuanemuy/fog/issues/18)**。#20 でこの1導出は 30.2ms → 127.2ms の **4.2 倍**になったので、#18 の優先度判断の入力も変わっている（ADR-003「実測結果（#20 / 2026-08-07）」節）
...
- **実ハッシャーでの担保を1件足す**。TC-loginWithPassword-009 は実 PBKDF2（1,000回）を注入する唯一のテストなので、そこで `users.password_hash` が `pbkdf2-sha256$1000$` で始まり平文を含まないことを表明する（#20 以降この表明は `pbkdf2-sha512$1000$` を見る）。フェイクの側で縛れるのは「フェイクの出力が保存されている」ことまでで、「実アダプターの出力形式が保存されている」ことは実物でしか見られない
...
- 良い点: 定数の陳腐化が1ケースで落ちる（実測: 反復回数を 21,000,000 に書き換えると `DATA_INTEGRITY_ERROR` で失敗する）。実ハッシャーでの検証コストは本番パラメータ 210,000 回でも workerd 上で 33ms なので、統合スイートの実行時間に影響しない（当時の実測。#20 で方式が SHA-512 へ変わった後の数字は ADR-003 の「実測結果（#20 / 2026-08-07）」節を見よ）
...
ADR-026 の等時間化は `DUMMY_PASSWORD_HASH`（`pbkdf2-sha256$210000$…` の文字列定数。#20 以降は `pbkdf2-sha512$210000$…`）に verify を1回走らせることで成立している。ここに2つの穴が残っていた。
...
- **訂正（review-003-frontend W-002）**: 当初この行は「全箇所で反映されることを確認した」と書いていたが、**画面端に接する余白は4トークンで尽きていなかった**。シート本文の下端（`AppShell` の `<main>` 内側の `pb-2xl`）が固定値のまま残っており、実測は 40px（= `--space-2xl`）で変化しなかった。5つ目のトークン `--sheet-pad-b` は .thread/1/adr.md ADR-049 で足している。「全箇所」と書けるのは棚卸しの根拠を示せるときだけ、という記録として残す
...
## ADR-046: 出荷コードからの ADR 参照は `.thread/1/adr.md` をパスで明示する
...
選択肢は3つあった。(a) `.thread/1/adr.md` の判断を `spec/adr/007-*.md` 以降として切り出す。(b) コード側の参照を全件パス付きに直す。(c) `.thread/1/adr.md` の採番を 100 番台へ振り直す。

### Decision

**(b) を採る。** 44箇所すべてを `.thread/1/adr.md ADR-NNN` の形にした（`(ADR-008)` → `(.thread/1/adr.md ADR-008)`）。1行に複数の兄弟参照がある場合は先頭だけを修飾する（`(.thread/1/adr.md ADR-005 / ADR-010)`）。
...
- 良い点: `grep -rnoE 'ADR-[0-9]+' packages/core/src apps/web/app infra` の全ヒットが、直前のパスによって一意に解決する。番号空間の衝突は解消した（本 PR 完了時点で52箇所すべてが `.thread/1/adr.md ` 直後に現れることを実測）
- トレードオフ: 出荷コードが issue 単位の作業ディレクトリを参照し続ける歪みは残る。`.thread/1/adr.md` を `spec/adr/007-*.md` 以降へ切り出す作業は未了で、その時点で52箇所の再置換が必要になる
