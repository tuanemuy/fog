...
- 方式名の書き分けは**記述の性格**で決めてある。**当時の観測・当時の決定を述べる行は原文を残し、括弧書きで現行値を添えた。** 今も有効な仕組み・今も参照される定義を述べる行は本文を現行値へ直した。節の見出し（Context / Decision か Consequences か）はこの性格とおおむね一致するが、規則の本体は節名のほうではない。**例外は ADR-014 の Context にある `verify` 失敗の2分類**で、これは当時の観測ではなく**今も有効な失敗分類の定義**（どちらの失敗がどの `SystemErrorCode` に翻訳されるか）なので本文を現行値へ直し、旧形式も読める側に含まれる旨を同じ行に添えてある
