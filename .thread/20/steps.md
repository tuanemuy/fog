...
- **変更内容:**
  ...
  2. **9-13 行（「意図的にスコープ外とした項目 1. 旧コストの保存ハッシュが残る間の等時間化」）** — ここも過去のログではなく現在の残課題を語る生きた記述で、**アダプター JSDoc（ステップ5【案 A】6-7）と対になっている**（`.thread/1/adr.md` の ADR-034（Consequences の「残る限界」）が「この限界は `DEFAULT_PBKDF2_ITERATIONS` の JSDoc と `progress.md` に書いた」と明記）。片方だけ直すと対が壊れる。
