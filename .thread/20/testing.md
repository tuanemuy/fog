...
**確認項目1〜4 は案 A / 案 B の両方で実施する。確認項目5〜6 は案 A を採った場合のみ。**
...
### 2. 保存されたハッシュが確定した方式のアルゴリズム識別子・反復回数になっている
...
- **期待結果:**
  ...
  |---|---|---|
  | 案 A | `pbkdf2-sha512` | `210000` |
  | 案 B | `pbkdf2-sha256` | `600000` |
...
- **確認ポイント:**
  ...
  - 反復回数が `210000` のまま案 B を名乗っている／`600000` なのに識別子が `pbkdf2-sha512` になっている、のような**識別子と回数のちぐはぐ**が無いこと。ゲート（steps.md ステップ2）が選んだ案の行と厳密に一致すること
...
- **確認ポイント:**
  ...
  - 警告が出た場合、案 A なら `DUMMY_PASSWORD_HASH_ALGORITHM_ID` の取り残し、案 B なら `DUMMY_PASSWORD_HASH_ITERATIONS` の取り残しを疑う（ただし型ピンがあるので本来コンパイルが通らないはずで、警告が出たなら型ピンのほうが壊れている）

### 4. 変更前に作成したアカウントで引き続きログインできる

- **対応する受け入れ基準:** AC-7（案 A）/ 旧コスト行の後方互換（案 B）
...
- **手順:**
  ...
  4. 保存された値が変更前の形式であることを確認する（案 A / 案 B とも `pbkdf2-sha256$210000$` で始まる）
...
- **確認ポイント:**
  ...
  - **案 A ではこれが旧形式（`pbkdf2-sha256$`）読み取り枝の唯一の実機確認**であり、`hashFor()` の旧枝が生きていることを意味する（AC-7）
  - **案 B では旧コスト（210,000）の行が新コスト（600,000）のハッシャーで読めることの確認**になる。`verify` が保存値のコストで導出していなければここで落ちる
  - 手順6 が失敗する場合、案 A なら `hashFor()` から `pbkdf2-sha256` 枝を落としている、案 B なら `parse()` に反復回数の一致判定を持ち込んでいる

### 5. 【案 A のみ】旧形式の低コストフィクスチャを注入した行でログインできる
...
### 6. 【案 A のみ】旧形式の行はログイン成功後も書き換わらない
...
- **目的:** 反復回数（案 B）または1反復あたりのコスト（案 A）が上がった結果として、ログイン・登録が**利用者から見て破綻していない**ことを確認する。判定ゲートの入力ではなく、確定した案が実用に耐えるかの観測
- **手順:**
...
- **期待結果:** ログイン送信の POST が体感で待たされる感じにならず、pending 表示（「ログイン中…」）が数秒間残るようなことがない。ローカル workerd での1導出は先行実測で 45〜49ms なので、**サーバー関数の往復が数百 ms のオーダーに収まっていれば正常**
- **確認ポイント:**
  - **これは絶対値の合否判定ではない。** steps.md ステップ2 は「中央値が 100ms を超えても案の選択は変えない」と決めており、重さそのものが問題なら別 Issue に切り出す。ここで見るのは「桁が変わっていないか」だけ
  ...
  - 案 A で桁違いに遅い（1秒級）場合は、`derive()` に渡す hash 名が固定されずに毎回 SHA-512 の巨大な反復を回している、あるいは反復回数を取り違えている可能性がある。確認項目2 で保存値を読み直す
...
- **正直な限界（重要）:** **応答時間の差が体感で区別できないことは、この手順では検証できない。** 両者の差は1導出ぶん（数十 ms）を等時間化した残差であり、`pnpm dev` の HMR・ネットワーク・ブラウザ描画のノイズのほうが桁で大きい。DevTools の Time を並べても有意差の有無は判定できない。**タイミングオラクルそのものの検証は自動テスト（`identity.integration.test.ts` の「ダミーハッシュを本番ハッシャーが読めること」）と確認項目3 の警告ログに委ねる。** ここで確認するのは表示の同一性だけである
...
- **手順:**
  1. 次の SQL を `apps/web/broken.sql` に書いて実行する（値はどちらの案でも「読めない識別子」になるものを選ぶ。案 A なら `argon2id`、案 B なら `pbkdf2-sha512` でもよい）
...
- **目的:** 1反復あたりのブロック長が変わる案 A で、長い入力に対する `importKey` / `deriveBits` が壊れていないことを確認する（SHA-512 のブロックは 128 byte で SHA-256 の倍）
- **手順:**
