...
| TC-outboxDelivery-028 | 投入と同じトランザクションのあとの setAlarm | spec/testcases/async/outboxDelivery.md#L36 | 終端行しか残っておらず deleteAlarm() 済みの DO へ enqueueEvent すると、そのトランザクションのあと4本の min の合成で setAlarm が張られ、外から起こさなくても次の起床で relay されれば PASS（TC-outboxDelivery-016 の裏返し。enqueueJob にも同じ規約が掛かる） |
