...
|----|------|---------|------------------------------|
| DOM-identity-001 | User エンティティ | spec/domains/identity.md#User | 認証方式の判別可能ユニオンを持たない。保有クレデンシャルの非 PII 要約集合（`{ credentialId, kind, label, usableForLogin }`。1件以上）と trashRetentionDays・OCC version を持ち、email / passwordHash / provider / providerSubject は持たない。registerWithPassword / registerWithSso / addCredential（**呼び出し元は `linkSsoCredential` だけで、追加できるのは `kind: "sso"` の要素に限る**。メールクレデンシャルを後から追加する経路は無い）/ removeCredential（`kind: "sso"` のみ受け、**解除後に `usableForLogin` が真の要素が残らなければ** BusinessRuleError(LastCredentialRemoval)）/ changeTrashRetentionDays を純関数ファクトリで提供する。**「ログイン手段の数」は `usableForLogin` が真である要素の `credentialId` の異なり数**であり、要素数でも `kind` でも決まらない。パスワードの変更・リセットは本エンティティの遷移ではない |
...
| DOM-identity-031 | PasswordResetTokenPort.issue | spec/domains/identity.md#PasswordResetTokenPort | 対象クレデンシャル単位の発行（`issue(credentialId, now): { token, tokenId }`）。**生トークンと `tokenId` の両方を返す** — 生トークンは**アダプターの外へ出るが、ユースケースはこれを読まない・保持しない・どこへも渡さない**（現状の読み手は0件で、送信時は送信材料 RPC が `tokenId` から導出し直す）。返し続けるのは payload の必須項目 `tokenId` を同じ戻り値で得るためであり（返さない形だと payload の必須項目を得る経路が無い）、ユースケースが payload へ載せるのは `tokenId` だけである。**`token` をログ・DTO・payload・Queue メッセージへ載せない**ことは signature が守らないので実装レビューの確認項目とする。同じクレデンシャル宛の未使用トークンをすべて置き換える。`tokenId` は `mintDecoyTokenId` と**同一の生成器**（同じ CSPRNG・同じ長さ・同じ符号化）から採り、**`IdGenerator` を使わない**（UUIDv7 等は時刻由来で `token_id` の生成規則と食い違う）。同期契約。置き場は認証情報側（Identity Directory） |
...
| DOM-identity-033 | MailSender.sendPasswordResetMail | spec/domains/identity.md#MailSender | リセットリンクメールを送る（`sendPasswordResetMail(to, resetToken, providerIdempotencyKey)`）。**`providerIdempotencyKey` は at-least-once 下で重複メールを止める唯一の機構**であり、引数で受け取れないと機構が成立しない（抑止は配送状態ではなくこのキーが担う。spec/async/index.md）。**値は consumer が組み立てず送信材料 RPC の応答として発行元 DO から受け取る**（`event.id` から DO 側で決定的に導く。`outbox_events` の列ではない）。宛先実在性起因の失敗をユーザー応答に反映しない。**実装できる API が非同期しか無いため Promise 契約のまま残る。引数が増えても Promise 例外は2件のまま動かない**（例外は列挙であって導出規則ではなく、判定基準は引数の数ではない。domains/index.md） |
...
| DOM-identity-048 | ファクトリ / 遷移の戻り値の形 | spec/domains/identity.md#共通の契約 | **イベントを発行する**ファクトリ / 遷移の戻り値が `{ entity, eventDrafts }` であり、状態遷移とイベントの発生を1つの戻り値で表す。**この形は将来イベントを発行する遷移が現れたときの契約であり、現在インスタンスは0件である** — 既存の遷移（`registerWithPassword` / `addCredential` 等）はすべて `): User;` のままで、memo / knowledge の「状態遷移は次状態のエンティティだけを返す」も動かない。**この形はエンティティ遷移から出るイベント専用であり、遷移を伴わない発行点は draft ファクトリから出す**（DOM-identity-052。domains/index.md「ドメインイベントの契約」） |
| DOM-identity-049 | identity.passwordResetRequested | spec/domains/identity.md#identity.passwordResetRequested | **本 spec で定義される唯一のイベント型である**（全数は spec/async/index.md）。発行点は `requestPasswordReset` のトランザクションで、**その窓での最初の依頼のときだけ4ケース（登録済み / 未登録 / SSO 専用 / スロットル中）とも必ずちょうど1行**、既に発行済みの窓なら4ケースとも1行も書かない。`aggregateId` は**スロットル窓のキー**（`claimWindow` に渡した `windowKey`。`credentialId` は未登録の宛先に存在せず有無が観測可能な差になるので使えない）。payload は `tokenId` / メール種別の**2つだけ**で、**メールアドレス・生トークン・`userId` を載せず、`tokenId` を nullable にしない**（送らない側では宛先の有無から独立に生成した不透明値を置く）。**宛先 DO の routing key は payload に入れない** — relay が publish 時に Queue メッセージへ押す項目であって、ドメインの契約ではない（`EventId` と同じ扱い）。**登録・パスワード変更・保持期限変更のイベント型は定義しない**（consumer も明示的な監査要件も無い） |
...
| DOM-identity-051 | PasswordResetTokenPort.mintDecoyTokenId | spec/domains/identity.md#PasswordResetTokenPort | どのトークンも指さない `tokenId` を1つ採る。**「送らない側」（未登録 / SSO 専用）の payload にデコイを置く唯一の口である** — ユースケースが `container.idGenerator.next()` で代用すると、時刻由来の UUID になって送らない側の payload だけ形が割れ、payload そのものが列挙オラクルになる。**`issue` が返す `tokenId` と同一の生成器**（同じ CSPRNG・同じ長さ・同じ符号化）から採ることが契約であり、**4ケースで生成器が同一であること**がここと DOM-identity-031 の対で担保される。行を1つも書かず、`verifyAndConsume` で解決できる値も返さない。**同期契約**（`Promise` 例外は2件のまま動かない）。置き場は認証情報側（Identity Directory） |
| DOM-identity-052 | passwordResetRequestedDraft（draft ファクトリ） | spec/domains/identity.md#draft-ファクトリ | `passwordResetRequestedDraft({ windowKey, tokenId, mailKind, now })` が `identity.passwordResetRequested` の draft を返す。**`identity.passwordResetRequested` はエンティティ遷移から出ない**（`requestPasswordReset` は `User` も `CredentialMapping` も遷移させない）ので `{ entity, eventDrafts }` の形では出せず、**遷移を伴わないイベントは draft ファクトリから出す**という共通契約（domains/index.md）の唯一のインスタンスである。**`type` の文字列・`aggregateId` の選び方・payload の形を決める唯一の場所**であり、ユースケースは戻り値を `enqueueEvent` へ渡すだけ（リテラルで組み立てない。組み立てると「4ケースで一字も違わない行」を保証する主体が居なくなる）。`aggregateId` には `windowKey` をそのまま入れ、`occurredAt` には引数の `now` を使う（時計にも `IdGenerator` にも触らない）。**`tokenId` の出所はファクトリの外**（送る側は `issue` の戻り値、送らない側は `mintDecoyTokenId`）で、形の同一性はポート側の契約が担保する。**routing key は引数に取らない**（ドメインは自分がどの DO に載っているかを知らない） |
