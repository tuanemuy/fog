...
| DOM-identity-031 | PasswordResetTokenPort.issue | spec/domains/identity.md#PasswordResetTokenPort | 対象クレデンシャル単位の発行（`issue(credentialId, now): { token, tokenId }`）。**生トークンと `tokenId` の両方を返す** — 生トークンはアダプターに留まり、ユースケースが payload へ載せるのは `tokenId` だけである（返さない形だと payload の必須項目を得る経路が無い）。同じクレデンシャル宛の未使用トークンをすべて置き換える。`tokenId` は `mintDecoyTokenId` と**同一の生成器**（同じ CSPRNG・同じ長さ・同じ符号化）から採り、**`IdGenerator` を使わない**（UUIDv7 等は時刻由来で `token_id` の生成規則と食い違う）。同期契約。置き場は認証情報側（Identity Directory） |
...
| DOM-identity-048 | ファクトリ / 遷移の戻り値の形 | spec/domains/identity.md#共通の契約 | `{ entity, eventDrafts }`。状態遷移とイベントの発生を1つの戻り値で表す。イベントを発行しない遷移では `eventDrafts` が空になる。**この形はエンティティ遷移から出るイベント専用であり、遷移を伴わない発行点は draft ファクトリから出す**（DOM-identity-052。domains/index.md「ドメインイベントの契約」） |
