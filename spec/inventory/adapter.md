...
| ADP-account-001 | schema: account | spec/database/index.md#account | 単一行テーブル。`status`（active/deleting/deleted）・`session_epoch`（セッション失効の唯一の権威）・`deleted_at`・`caller_token`（ログ・エラー・`terminal_reason` に出さない。**消すのは退会の完走時だけで、それ以外の経路では消さない**）・`reset_version`（**リセット完了だけで進む単調増加カウンタ**。`credential_version` で代用しない）・OCC `version` を持つ。退会後も非 PII の tombstone として残る |
...
| ADP-credential-locators-001 | schema: credential_locators | spec/database/index.md#credential_locators | 保有クレデンシャルの逆引き。原本も検証材料も持たない。`credential_id` / `kind` / `hmac` / `generation` / `bucket_index` / `credential_version` / `status`（値域は `active` の1値のみ。除去は物理削除）/ `usable_for_login` / `label`（非 PII）を持ち、**PK は複合キー (`credential_id`, `generation`)**（`cl_credential_uq`。**サロゲートの `id` 列は置かない**）、ほかに `cl_hmac_uq`（kind, hmac, generation）を張る。**到達性の照合は `credential_id` だけを見て `generation` を含めない**（ローテーション中は2世代が並存する）。**書き込み口は UoW コンテキストの `credentialLocatorStore` だけ**で、書き込み箇所は連携完了時の upsert / 連携解除・退会時の削除 / クレデンシャル変更適用時の全世代更新の3つである |
...
| ADP-jobs-001 | schema: jobs（User Data DO） | spec/database/index.md#jobs | Alarm ジョブの多重化テーブル（12列）。`operation_key` PK・`kind`・`payload`（**PII と再利用可能な秘密を入れない**）・`payload_digest`（`next_run_at` を除いた payload で照合）・`attempt` / `next_run_at` / `status`（pending/running/done/poison）/ `lease_until` / `owner_token` / `provider_idempotency_key` / `terminal_reason` / `completed_at`。`jobs_runnable_idx` / `jobs_lease_idx` / `jobs_completed_idx` を張る。claim と完了は CAS（第2の選言に `status='running'` を必ず含める）。終端は一様で、前進不能は `terminal_reason` を残して `poison` にし operator 経路へエスカレーションする。prune 専用の `kind` は置かない。**usecase からの書き込み口は `enqueueJob` だけ**で、再投入の収束規則は3つ — `next_run_at` は早める方向にのみ更新する / `poison` の行は `pending` へ戻して `attempt` を 0 にする / `done` の行を戻すのは再武装する5種（`purge-trash` / `sweep-reservations` / `sweep-reset-tokens` / `sweep-orphan-mapping` / `rotate-encryption`）に限り、残る7種は復活させない |
...
| ADP-identity-022 | CredentialLocatorStore.findByCredentialId | spec/domains/identity.md#credentiallocatorstore | `cl_credential_uq` の前方（`credential_id`）で引く。**PK は (`credential_id`, `generation`) の複合キーだが、到達性の照合は `credential_id` だけを見て `generation` を含めない**（ローテーション中は2世代が並存する）。不在は null |
...
| ADP-trash-006 | TrashQueryPort.findEarliestPurgeAfter | spec/domains/trash.md#trashqueryport | **`purge-trash` ジョブの駆動源の読み側**。同じ3索引の先頭から `min(purge_after)`（`WHERE status = 'trashed'`）を返し、ゴミ箱が空なら null。ソフトデリート・保持日数変更・ジョブ完了時の再武装が次の起床時刻の材料として読む（**投入は早める方向にのみ効く**。spec/database/index.md#jobs） |
