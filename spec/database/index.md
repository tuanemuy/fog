...
fog の永続化スキーマ。**Cloudflare Workers + ユーザー単位 SQLite-backed Durable Objects**（CLAUDE.md「Reference runtime」）を前提とし、型は SQLite の型アフィニティ（TEXT / INTEGER）で記す。実装先は Durable Object のアダプター（`packages/core/src/adapters/cloudflare/`）である。

- 入力: [spec/domains/](../domains/index.md) の全エンティティ・VO・ライフサイクル状態・リポジトリのクエリパターン
- 関連 ADR: [ADR-001](../adr/001-restore-document-without-topic.md) / [ADR-003](../adr/003-source-link-after-hard-delete.md) / [ADR-004](../adr/004-domain-boundaries.md)
- **本ファイルが `spec/` 側のスキーマの正本である。** 変更は本ファイルを直す
...
- **書き込みの単位**: 書き込みは DO 内蔵の SQLite に対する単一の同期トランザクションで確定する。**本体行と検索 projection（`search_entries` / `search_fts`）も同じトランザクションに入る**（domains/search.md「インデックスの維持」）。**イベント行（`outbox_events`）も同じトランザクションに入る** — (1) 業務データの書き込み、(2) FTS5 projection の更新、(3) `enqueueEvent` によるイベント行の追加、の3つが1つの `transactionSync` の中で一度に確定し、**rollback すると3つとも巻き戻る**
...
アカウントの状態・失効の権威・退会 tombstone。**単一行のテーブルである**（その DO の中に1人分しか存在しない）。単一行制約の掛け方は実装裁量とする。
...
- **`version` 列は保持するが、本 spec の範囲では `version` を条件に含む更新を発行する操作が無い。** `AccountStore` の5メソッド（`find` / `advanceSessionEpoch` / `advanceResetVersion` / `initializeCallerBinding` / `beginDeletion`。domains/identity.md）はいずれも `version` を読まず進めない。**新設の2つも `version` と `updated_at` の扱いは既存の前進メソッドと同じである** — `version` は進めず、`updated_at` には書き込みのたびに現在時刻を書く。列を落とさないのは、「集約ルートは `version` を持つ」という全数（後述の OCC の表）を崩さないためである。**`version` を条件に含まないだけで、条件付き更新そのものは存在する** — 下の `status` の遷移がそれである
- **`caller_token` と `status` の書き手を確定させる。** **`caller_token` を書くのはアカウントの初期化1箇所だけである**（新規登録 saga の phase 2 = `initialize-account` が業務書き込みと同じ `transactionSync` で書く。`spec/usecases/identity.md` の `registerWithPassword` 手順5 / `registerOrLoginWithSso` 手順4）。**消すのは退会の完走時だけである**（上の列定義）。**書かないと、認証情報側からこの DO へ発行する操作（放棄・写像の削除・逆引きの記録）の束縛が照合規則により全経路で拒否側へ倒れる**
- **`status` の `deleting` への遷移を書く経路は2つで、これが全数である** — (1) 退会の起点（**退会 saga そのものは本 spec の範囲外であり、投入点は実装が DO の RPC 側で決める**）と、(2) **新規登録 saga が終端したときのアカウント候補の放棄**（`abandon-account`。`status='active'` を条件に含む条件付き更新であり、効果とガードの評価順序の正本は [recovery/index.md](../recovery/index.md)）。`deleted` への遷移と `deleted_at` は退会の完走が書く

### user_settings

identity の `User` のユーザー単位設定。**単一行のテーブルである**（`account` と同じく業務上の主キーを持たない。単一行制約の掛け方は実装裁量とする）。`UserSettingsRepository` は `find()` で引き、`findById` を持たない（他の `userId` を渡せるという読み方を残さないため。domains/identity.md）。
...
- **書き込み口は UoW コンテキストの `credentialLocatorStore` だけである。** 書き込み箇所は4つで、これが全数である — (1) 新規登録・SSO 連携の完了時の upsert、(2) SSO 連携解除・退会時の削除、(3) クレデンシャル変更の適用時の全世代更新、(4) **鍵ローテーションの移送による新世代行の追加**（同じ `record` の upsert を `record-remapped-locator` 経由で使う。見送り判定を含む呼び出し規約の正本は [rotation/index.md](../rotation/index.md)）。`usable_for_login` と `label` の値は Identity Directory 側が判定して引数で運ぶ（この表は自分では判定しない）。**移送は旧世代の locator 行を削除しない**（余剰世代行を回収しない理由も同ファイル）
...
- `listRevisionSummaries` / `findRevision` は PK 前方一致（`memo_id`）で引く。追加インデックス不要
...
| `body` | TEXT | NOT NULL。空文字可・**400,000 文字以下**は VO で検証（下の「本文上限のバイト予算」） |
...
**`topic_id` に FK を張らない理由（設計判断）**: 個別削除済みドキュメントが残ったままトピックがハードデリートされるケースが正当に存在する（ADR-001。ゴミ箱内ドキュメントの `topic_id` は消滅済みトピックを指したままになり、復元時に `moveToTopic` で差し替える）。`ON DELETE CASCADE` はユーザーが明示していない不可逆削除（ADR-001 で不採用）を、`RESTRICT` は正当なトピックハードデリートの阻害を招く。NOT NULL のため `SET NULL` も不可。したがって「active なドキュメントの `topic_id` は実在する Live トピックを指す」の保証はユースケース（作成時の検証と insert が単一の `transactionSync` に収まることによる直列化、restore 時の呼び出し側保証）に置く。

インデックス:

| 名前 | 定義 | 用途 |
|---|---|---|
| `docs_topic_active_idx` | (`topic_id`) WHERE `status = 'active'` | このインデックスを引くのは 3 メソッド: `listActiveSummariesByTopic`（トピック詳細）/ `listActiveSummariesByTopics`（トピック一覧）/ `listActiveByTopic`（セット削除対象の確定。OCC トークン付きの entity を返す） |
...
`DocumentBody` の 400,000 コードポイントという上限は、1 行の予算（「物理境界」の単位の読み方により 2,000,000 バイト）から逆算した値である。**逆算を拘束するのは本文以外が最も重い `document_revisions` の行**であり、`documents` の行ではない。

**この逆算が拘束条件に採るのは 2 MB/行 だけであり、「100 KB/文」は使わない。** 本文は bind パラメータとして渡され、SQL 文のテキストには現れないためである。上限ちょうどの本文を最悪ケースの符号化（4 バイト/コードポイント）で書き込む経路は [testcases/knowledge/createDocument.md](../testcases/knowledge/createDocument.md) の境界値ケースが実 DO SQLite に対する統合テストとして押さえており、この大きさの bind 値が 1 文で通ることはそこで観測できる（**行サイズの予算に対する網ではない。上の「単位の読み方」のとおり、開発ランタイムは 2 MB/行 を強制しない**）。**この観測を、bind 値が「100 KB/文」に算入されないことの一般規範としては使わない** — 同じ上限を `outbox_events.payload` / `jobs.payload` に課している記述はこの注記の射程外であり、bind 値と文のテキスト長のどちらに「100 KB/文」が掛かるかは別途決定する。

| 行 | 本文以外の最大バイト数 | 内訳 |
|---|---|---|
| `documents` | 約 979 | `title` 200 文字 × 4 = 800 + ID 群 3 本（`id` / `topic_id` / `trashed_with`）× 36 = 108 + INTEGER 列 6 本（`latest_revision_number` / `trashed_at` / `purge_after` / `version` / `created_at` / `updated_at`）× 8 = 48 + `status` 最大 7（`'trashed'`）+ 行エンコードのヘッダ 16 |
| `document_revisions` | 約 2,149 | `title` 800 + `change_reason` 200 文字 × 4 = 800 + `actor_client_name` 100 文字 × 4 = 400 + ID 群 3 本（`id` / `document_id` / `actor_connection_id`）× 36 = 108 + INTEGER 列 2 本（`revision_number` / `created_at`）× 8 = 16 + `actor_type` 最大 9（`'ai_client'`）+ ヘッダ 16 |

「ID 群」の 1 本 36 バイトは、値オブジェクトが持つ長さ上限ではなく「共通方針」の ID 規則（`IdGenerator` が採番する UUIDv7）の帰結であり、**この表からは導けない**。`actor_connection_id` は `ai_client_connections.id` の参照値なので同じ根拠に乗る。

```
2,000,000 − 2,149 = 1,997,851
1,997,851 ÷ 4（最悪ケース: 1 コードポイント 4 バイト）= 499,462 コードポイント（算術上の天井）
採用値 400,000 → 最悪 400,000 × 4 + 2,149 = 1,602,149 バイト = 予算の 80.1%
```

天井をそのまま採らないのは、逆算に使った数字（2 MB の解釈・列構成から数えたヘッダ推定）のどれかが数 % 外れただけで保証が破れるためである。残す約 20%（約 398,000 バイト）の予備の用途は3つあり、**この3つは同じ 398 KB を食い合う。同時に3つぶんの余裕があるのではなく、他が消費していない場合にだけそれぞれの上界が成り立つ**。

1. 「2 MB」の単位解釈（「物理境界」）と、列構成からは数え切れない SQLite の行エンコードの内部オーバーヘッド
2. `search_entries` の NFKC 膨張（下の限界の1文）— **1 と 3 がゼロのときに限り** 約 1.24 倍まで吸収する
3. `documents` / `document_revisions` への将来の列追加

したがって「予備があるから列を足してよい」と「予備があるから NFKC 膨張は大丈夫」を同時に主張することはできない。

**限界**: この上限が行サイズについて保証するのは `documents` と `document_revisions` の行である。`search_entries.body` は NFKC 正規化後の値を持ち、1 コードポイントが最大 18 コードポイントへ展開する入力ではこの逆算が成り立たない。**この行を押さえる手当ては検索 projection 側の切り詰めであり、別途決定する。**

**前提**: この値を**下げる**変更は保存済みの行に対して破壊的である（読み取りも削除も `DocumentBody.create` を通る）。400,000 への引き下げは「それを超える行を持つ Durable Object が存在しない」という前提の上に立つ。**その根拠は、上限を超える行を書けた配備の形跡がリポジトリの中に無いことである** — Pulumi のスタック設定はコミットされる運用でありながら 4 本とも `accountId` が `REPLACE_WITH_CF_ACCOUNT_ID` のまま（`zoneName` を持つものは `example.com` のまま）で、どのスタックも実アカウントに構成されていない。ローカル開発の DO データ（`.wrangler/state`）は破棄可能なので残らない。**この根拠は「リポジトリの中に形跡が無い」までしか言えないという限界を持つ** — 描画済みの wrangler config も Pulumi の state もリポジトリの外にあるので（前者はルートの `.gitignore` が無視し、配備済みでも同じく「無い」）、配備の不存在は証明できない。**前提が破れたことの signal と直し方は [domains/index.md](../domains/index.md)「再水和の再検証」が持つ**。
...
| `body` | TEXT | NOT NULL。当時の本文の全文スナップショット。**`documents.body` と同じ 400,000 文字上限が効く**（`### documents`「本文上限のバイト予算」。逆算を拘束するのはこの行である） |
| `actor_type` | TEXT | NOT NULL, CHECK (`actor_type IN ('user','ai_client')`) |
...
| `body` | TEXT | NOT NULL。本文。**NFKC 正規化 + trim 済みの値**を入れる。**この列は正規化後の値を持つため、`### documents`「本文上限のバイト予算」が与える行サイズの保証の外である** |
...
| 名前 | 定義 | 用途 |
|---|---|---|
| `jobs_runnable_idx` | (`status`, `next_run_at`) WHERE `status IN ('pending','running')` | claim（実行可能集合の絞り込み。`next_run_at` の昇順はこの索引では解けずソートが要る）と Alarm の張り直し |
| `jobs_lease_idx` | (`status`, `lease_until`) WHERE `status = 'running'` | Alarm の張り直し（leased 行の `min(lease_until)`）。**先頭列は部分述語と冗長だが必須である** — 部分索引の述語は `WHERE` の項を消化しないので、先頭列が無いと等値でシークできず `jobs_completed_idx` に落ちる |
| `jobs_completed_idx` | (`status`, `completed_at`) | `done` の prune（保持期間を過ぎた行を有界に削除する）と、`poison` の一覧（`list-poisoned-jobs`。`completed_at` 昇順がこの索引で解ける） |

- **この表に外部 I/O を伴う `kind` は存在しない。** local job はすべて DO ローカルで完結する。**この表の行はネットワークに出ない**と読んでよい
...
- **再投入（`enqueueJob`）の収束規則は3つで、これが全数である。** `operation_key` の欄が言う「同じキーの再投入は既存行に収束する」だけでは、`kind` によって逆向きの更新が要求されるためである。**`status` 別の (2)(3) は (1) と `payload_digest` 列の規則の両方に優先する** — `done` / `poison` の行は `next_run_at` が `NULL` なので「早める方向」という判定そのものが定義されず、payload の差も終端済みの行では競合ではなく次の一回分の入力だからである
  ...
  - **(2) `status = 'poison'` の行への再投入は、`kind` によらず同じ行を `pending` へ戻し、`attempt` を 0 に、`completed_at` を `NULL` にして、`next_run_at` / `payload` / `payload_digest` を引数の値で置き換える。別行は作らない**（`operation_key` はそのジョブの同一性なので、行を増やすと同一性の意味が壊れる）。`terminal_reason` は上書きせずに残す
    - **`completed_at` の `NULL` 復帰を (2)(3) にも課すのは、終端でない行に `completed_at` が残ると列定義（`pending` / `running` では `NULL`）に違反するからである。** 同じ逆向き遷移を行う `requeue-poisoned-job` / `requeue-quarantined-event` だけがこれを守る形にすると、**同じ不変条件が同じ表の2つの遷移で守られたり破られたりする。** **`poison` を恒久保持にしたことでこの経路は常態化した** — 定数 `operation_key` を持ち終端モードを持たない2種（`finalize-withdrawal` / `sweep-orphan-mapping`）は、次の退会 / 解除が投入点から再投入した瞬間に必ず (2) を通る
  - **(3) `status = 'done'` の行を `pending` へ戻すのは、再武装する5種（`purge-trash` / `sweep-reservations` / `sweep-reset-tokens` / `sweep-orphan-mapping` / `rotate-encryption`）に限る。戻すときは (2) と同じく `completed_at` を `NULL` にする。残る6種は `done` の行を復活させず、何も書かずに成功を返す。** 5種を復帰させるのは、定数 `operation_key` を持つこれらが1回完走した時点で prune の保持期間ぶん再投入を受け付けなくなるからである（平常時はどれも必ず `done` へ落ちるので、投入点からの復帰が唯一の再起動手段になる）。残る6種の `done` は「その `operation_key` が表す一回分の仕事が完了した」という意味なので、**同じキーの再投入は新しい仕事ではなく重複依頼である** — 復活させると、たとえば `resume-credential-change` の重複依頼や `finalize-withdrawal` の二重投入が、完了済みの saga を起こし直して起床回数と書き込み行数を依頼回数に比例させる。**`payload_digest` の一致では5種と6種を分けられない**（5種は投入点が毎回同じ payload を渡すので digest も一致する）
...
- **claim と完了は CAS で行う。** `UPDATE jobs SET status='running', lease_until=?, owner_token=? WHERE operation_key=? AND (status='pending' OR (status='running' AND lease_until < ?))` の 0 行更新を「他が持っている」とみなす。**第2の選言に `status='running'` を必ず含める**（落とすと `done` / `poison` の行が過去の `lease_until` を保持したまま再 claim の対象になる）。完了も `WHERE operation_key=? AND owner_token=?` の CAS。**`outbox_events` の claim / finalize も同じ形の CAS であり、述語キーは `id` である**（`jobs` の `operation_key` に対応する位置。`UPDATE outbox_events SET status='publishing', lease_until=?, owner_token=? WHERE id=? AND (status='pending' OR (status='publishing' AND lease_until < ?))` と `WHERE id=? AND owner_token=?`。値域が `publishing` に変わるだけ）
  - **「上限件数まで claim」の実装形を確定させる。** `UPDATE ... LIMIT` / `DELETE ... LIMIT` は `SQLITE_ENABLE_UPDATE_DELETE_LIMIT` を有効にしたビルドでしか使えず、**DO の SQLite で使える保証が無い**ので、これを前提にした書き方をしない。**実行可能集合を `next_run_at` の昇順で上限件数だけ `SELECT` し、行ごとの CAS を1文ずつ発行する**（0 行更新の行は他が持っているものとして飛ばす）。**「索引順」と書かない** — 実行可能集合の索引（`jobs_runnable_idx` / `outbox_runnable_idx`）は `(status, next_run_at)` なので**索引順は `status` 主・`next_run_at` 従**であり、`pending` の行が lease 満了済みの `running` / `publishing` の行より必ず先に並ぶ。`pending` が供給され続けるかぎり lease 満了行が索引順の後ろで飢え続け、**DO reset で生じた行がいつまでも回収されない**（`jobs_lease_idx` / `outbox_lease_idx` は Alarm の張り直しの材料であって、claim の入口ではない）。**`status` が2値なので、この順序は宣言済みの索引だけでは1本のシークにならず、実行可能集合ぶんのソートが要る。** これは Alarm の張り直しを4本の min へ分解したのと同じ種類のコスト判断であり、**あちらは分解でソートを避けたが、こちらは順序が正しさ（飢餓の回避）に効くのでソートを受け入れる**。件数上限は取得件数に掛かるのであってソート対象には掛からないので、**実行可能集合が大きい DO では毎起床このソートが載る**ことを運用値（件数上限）の材料に含める。**この `SELECT` の述語は「実行可能集合 ∧ `next_run_at <= now`」である** — 実行可能集合そのものは `status` だけで定義され、時刻の条件は claim の選択にだけ掛かる（後述の「Alarm の多重化」が正本）。**掛け忘れると backoff が無効化される**（上の CAS は `status` と `lease_until` しか見ないので、`next_run_at` を先送りした行がその場で再実行される）。`WHERE id IN (?, …)` のような一括形へまとめる場合は、**件数上限を 100 bind parameter の内側に収める**（CLAUDE.md「Storage limits」。bulk insert を chunk するのと同じ制約であり、素朴に展開すると bind の上限が件数上限の実質的な天井になる）
...
- **backoff と終端。** 失敗時は `attempt` を進めて指数バックオフで `next_run_at` を先送りする。上限を超えたら `status='poison'` にして `terminal_reason` を残し、ホットパスの索引から外す（**その行に後始末の段が実在する場合だけは、`poison` の前に終端モードを1段挟む**。次のバレット）。**`done` / `poison` のどちらへ落とすときも、同じトランザクションで `completed_at` に現在時刻を書き、`lease_until` / `owner_token` / `next_run_at` を `NULL` にする**
  - **`terminal_reason` は終端状態でしか載らない列ではない。** cross-DO saga の前進ジョブのうち後始末（自動回収）を持つ3種は、前進不能が確定した時点で**`status` を `pending` に保ったまま `terminal_reason` を書いて終端モードへ入る**（`attempt` を 0 に戻し、`next_run_at` は**通常の backoff 規則が `attempt = 0` について定める値**とし — **突入のための待ちを1つも挟まず、新しい運用値も置かない** — `lease_until` / `owner_token` を解放し、**`completed_at` は書かない**）。以後その行は、**後始末の段が実在するあいだは**前進を1段も実行せず後始末の段だけを実行し、後始末が完走したら `done`、**焼き切れるか材料を見失ったら** `poison` へ落ちる（**行の状態が差し替わって段が消えた場合と再投入された場合は前進から再開する** — 段の選択規則の正本は recovery/index.md）。**`status IN ('pending','running')` かつ `terminal_reason IS NOT NULL` が終端モードの定義であり、その全数と段の順序の正本は [recovery/index.md](../recovery/index.md) である**（本ファイルは書く列だけを定める）。**突入の書き込みが起きるのは後始末の段が実在するときだけである** — 判定は `kind` 単位ではなく行の状態も見る（`resume-credential-change` は `change_state='pending'` のときだけ突入する）。**どの段を実行するかは `terminal_reason` の有無ではなく「後始末の段が実在するか」で決まる**ので、`requeue-poisoned-job` と収束規則 (2) が `terminal_reason` を残したまま `pending` へ戻した行は、段が実在しなければ前進から再開する
...
- **終端は一様である。** 前進不能が確定したジョブは、後始末を持つなら終端モードを経て、持たないならその場で `terminal_reason` を残して `poison` にし、operator 経路へエスカレーションする（後述）。**「黙って中間状態を残す」は選ばない。** 材料の寿命のうち **実装が落としてはならない前方互換点3本は本ファイルが各テーブルの節で持つ** — `account.caller_token`（消すのは退会の完走時だけ）/ `operations.target_locators`（後始末が完了しても空にしない）/ `credential_mappings` のコーディネーター予約行（後始末の最終段が `jobs` 行を終端へ落とすのと同じ `transactionSync` で消す）。**巻き戻し（自動回収）の具体 — 段の順序・原子性境界・終端モードの印の意味・後始末の再試行上限・材料の寿命の導出 — の正本は [recovery/index.md](../recovery/index.md) である**ので、本ファイルには書かない
...
- **1回の起動で触る量は件数だけで有界にする。** 経過時間では測らない（`Date.now()` はコード実行中に進まない）。ジョブ件数・チャンク反復回数・1チャンクの行数の3階層の上限を置く。**spike は `jobs.kind` の handler が1つでも実装された時点で実施する。それまでの値はプラットフォーム上限と1件の過去実測からの導出であって実走行の測定ではない。** **relay パスは独立した件数上限を持つ**（上限を共有すると片方の滞留がもう片方を飢えさせる。後述の「Alarm の多重化」）
...
- **prune 専用の `kind` は置かない。** ジョブランナーが1回の起動の末尾で、保持期間を過ぎた `done` を上限件数だけ削除する（`jobs_completed_idx` から引く）。**`outbox_events` の prune も同じ末尾処理に載せる**（`outbox_completed_idx` から引き、`published` を削除して `quarantined` は残す）
  - **終端行しか残っていない DO は起きないので、保持期間を過ぎた `done` 行も次の投入まで残る**（`outbox_events` の `published` と同じ性質。prune はジョブランナーの起動末尾でしか走らず、実行可能集合が空の DO は定義上 `deleteAlarm()` 済みである）。**`poison` を恒久保持にしたことでこの状態は珍しくなくなる** — `poison` 行を持つ DO は実行可能集合が空のまま起きないので、同じ DO の `done` 行も残る。10 GB の見積りにはこの分を含める
  - **`poison` は prune しない。恒久保持する**（正本は [recovery/index.md](../recovery/index.md)）。`poison` 行は残渣についての唯一の記録であり、かつ `requeue-poisoned-job` の対象そのものなので、保持期間で消すと**残渣だけが残って記録が消える** — 「黙って中間状態を残す」を選ばないという上の原則が prune によって破れる。**束縛の相手は設定値ではなく operator の反応時間なので、保持期間の不等式としては書けない**（`outbox_events` の `quarantined` を恒久保持にしたのと同じ論拠である）。**したがって `poison` 行は自動では減らず、減らす手段は `requeue-poisoned-job`（`done` へ進めば prune の対象に戻る）と operator の明示削除の2つだけである**（10 GB に算入する。**増加が有界であることは無条件では言えない** — 1件が増えるには前進と後始末の2本ぶんのバックオフ上限を焼き切る必要があるが、**新規登録の起点は未認証**なので、共有 bucket を継続的に `.overloaded` へ保てる主体は `poison` を積める。塞ぐのは発信元単位のレート制限であり、それは transport 境界の責務で本ファイルの範囲外である。正本の限界の記載は [recovery/index.md](../recovery/index.md)）
...
- **`rotate-remap`（写像鍵ローテーションの再写像）は Alarm ジョブではない**ので `kind` に現れない。写像鍵を Alarm 起動時に手元へ持てないためである。**実行主体は request Worker の operator 専用 maintenance 経路によるチャンク駆動であり、鍵はコミットメント照合つきの一時注入で渡す**（正本は [rotation/index.md](../rotation/index.md)）。**Outbox relay も `jobs.kind` ではない** — 起床時刻の権威が二重化し、backoff の単位（outbox は行ごと、ジョブ行は `attempt` 1本）も食い違うためである
...
```text
setAlarm( min(
    min(jobs.next_run_at)          WHERE status IN ('pending','running')   AND status = 'pending',  -- jobs_runnable_idx
    min(jobs.lease_until)          WHERE status = 'running',                                        -- jobs_lease_idx
    min(outbox_events.next_run_at) WHERE status IN ('pending','publishing') AND status = 'pending', -- outbox_runnable_idx
...
- **式は4本の min の合成として書く。索引で解ける形にするためである。** 4本はいずれも宣言済みの索引の先頭列等値 + 次の列の最小で引ける（上のコードブロックの注記が対応を示す）。**ただし runnable 側の2本は、部分索引の述語（`status IN (…)`）を構文的に満たす形で書かないとその索引が選ばれない** — プランナは `status = 'pending'` が述語を含意することを推論しないので、素朴に `WHERE status = 'pending'` とだけ書くと `*_completed_idx` に落ちる。**lease 側の2本にはこの条件が要らない**（索引定義の先頭列が等値を受けるため）。`min(max(next_run_at, lease_until))` を1本の SQL として発行すると**式を key にした索引が無いので実行可能集合の全走査になり、そのコストが backlog の件数に比例して毎起床かかる。**
...
DO ローカル Outbox のイベント行。**業務データの書き込み・FTS5 projection の更新と同じ `transactionSync` の中で追加され、Alarm relay が Queue へ publish する**。**Identity Directory DO も同じ13列・同じ索引を持つ**（後述）。**専用の Outbox DO は作らない** — 業務更新とイベント行の追加を原子的に書けるのは発行元 DO のローカル表だけだからである。
...
| 名前 | 定義 | 用途 |
|---|---|---|
| `outbox_runnable_idx` | (`status`, `next_run_at`) WHERE `status IN ('pending','publishing')` | relay の claim（実行可能集合の絞り込み。`next_run_at` の昇順はこの索引では解けずソートが要る）と Alarm の張り直し |
| `outbox_lease_idx` | (`status`, `lease_until`) WHERE `status = 'publishing'` | Alarm の張り直し（leased 行の `min(lease_until)`）。**先頭列は部分述語と冗長だが必須である** — 部分索引の述語は `WHERE` の項を消化しないので、先頭列が無いと等値でシークできず `outbox_completed_idx` に落ちる |
...
- **共通化する規約**（**列名と状態遷移そのものは規約の項目として数えず、上の列表と `status` の欄が持つ** — 同名・同意味の列は各列定義が、値域の差は下の「値域の差は分離する規約として数えない」が正本である）: Alarm scheduler（張り直しの式と lease の算入）/ backoff（`attempt` を進めて指数バックオフで先送り、上限超過で終端）/ lease（claim の CAS と `lease_until` 満了による回収）/ prune（ジョブランナーの起動末尾で、保持期間を過ぎた行を上限件数だけ削除）。**ランナーの実装は2表で共有し、削除の対象集合も対応する** — **どちらの表も終端2値のうち「正常に完了した側」だけを消し、「異常終端した側」は恒久保持する**（`jobs` は `done` を消して `poison` を残し、`outbox_events` は `published` を消して `quarantined` を残す）。**恒久保持の論拠も共通である** — 束縛の相手が設定値ではなく operator の反応時間なので、保持期間の不等式としては書けない（`jobs` の prune の項 / [recovery/index.md](../recovery/index.md)）
- **分離する規約**は2つで、これが全数である。
  1. **同一性と収束の有無** — `jobs` は `operation_key` で収束し、収束規則3つが載る。**`outbox_events` は例外なく「1イベント1行・不変」であり、収束しない。** 2回起きた事実を1行に畳むと片方が配送されない
  2. **終端時に `NULL` にする列** — `outbox_events` は `lease_until` / `next_run_at` の**2つだけ**であり、**`owner_token` は残す**（`jobs` は3つとも `NULL` にする）。落とすと呼び出しガードが `published` の行に対して必ず失敗し、正常系の配送が全滅する。**射程は終端（`published` / `quarantined`）だけである** — 上限未到達の失敗で `pending` へ戻すときは両表とも `owner_token` を解放する（`jobs` の「backoff と終端」）。行が再び claim を待つ状態へ戻る以上、呼び出しガードの照合材料として残す理由が無いためである
- **値域の差は分離する規約として数えない。** `pending` / `publishing` / `published` / `quarantined` と `pending` / `running` / `done` / `poison` は**同じ形の4状態に別の名前を付けたものであり**（`published` は「Queue へ渡した」であって「処理された」ではない、という読み替えだけが `outbox_events` 固有である）、**そこから prune の対象集合の差は導かれない** — 上の共通規約が両表を「正常完了だけを消す」に揃えたからである。**以前はこの差から prune の対象集合の差が導かれると書いていたが、`poison` を恒久保持にした時点でその導出は成立しない**

その他:

- **`dedupe_key` を置かない。** 同一窓への連打の抑止は行の一意制約ではなく、`reset_request_windows` を読むスロットル判定（同じ transaction の中）が担う
...
| `kind` | TEXT | NOT NULL。操作の種別。**値域トークンは `signup` / `link` / `unlink` / `credential-change` / `withdrawal` の5つで、これが全数である**（読み手が値で行を選ぶので、日本語の呼び名だけでは実装が確定しない — `abandon-account` のガードは `signup` を、後始末の L1 と退会側への要求は `kind IN ('link','unlink')` を述語に持つ。[recovery/index.md](../recovery/index.md)）。**書き手は `recordOperation` だけであり、どの段がどの値を書くかは [usecases/identity.md](../usecases/identity.md) が持つ**。**本 spec が書き手を持つのは `signup` / `link` / `unlink` の3つで、残る2つは段が本 spec の範囲外の saga のための予約である** — `credential-change` は Identity Directory 側が所有する saga なので `operations` を持つ DO をまたぐ設計が要り、`withdrawal` は退会 saga が自分の手続きを記録する場合の置き場である。**どちらも記録するかどうかと段の形は実装が決め、どちらであっても `finalize-withdrawal` の冪等性キーには入らない**（キーは `jobs.operation_key` の DO ごとの定数だけである。[async/index.md](../async/index.md)）。**新規登録 saga の終端規則による放棄経路はこの表に行を作らない**（[recovery/index.md](../recovery/index.md)） |
| `payload_digest` | TEXT | NOT NULL。同じ `operation_id` に違う payload が来たら `ConflictError`。**値はキー順を正規化した canonical JSON をハッシュせずそのまま格納したものである。したがって材料に再利用可能な秘密（トークン・鍵）を入れない** — 入れた値はこの列へ平文で永続化される（**格納形がハッシュでないことはアダプターのテストが押さえるが、材料の選び方を縛るのはこの記述だけであり、検査する型もテストも無い**） |
...
- **`target_locators` は消さない。後始末が完了して `phase='done'` を書くときも空にしない**（消すと回収の材料が失われ、空にしても得るものが無い。二重発行の冪等性の材料として残す。正本は [recovery/index.md](../recovery/index.md)）
...
| カラム | 型 | 制約 |
|---|---|---|
| `encrypted_canonical` | TEXT | **NOT NULL**。canonical 原本の暗号文。**`kind` を問わず全行が持つ** — `kind = 'email'` はメール canonical、`kind = 'sso'` は `provider + U+0000 + subject` の canonical である。予約行を書く時点で暗号化して書く。原本が無い行は再 HMAC の材料を持たず写像鍵ローテーションで移送できないため、nullable にしない（[rotation/index.md](../rotation/index.md)） |
| `encryption_generation` | INTEGER | **NOT NULL**。暗号鍵の世代。**写像鍵の世代とは独立した番号体系である** |
| `encryption_nonce` | TEXT | **NOT NULL**。AES-256-GCM の96ビット nonce。**独立列に持ち、暗号文に連結しない。** 行ごと・書き込みごとに再生成し、使い回さない |
...
**リセット依頼のスロットル判定の列はこの表に置かない。** 権威は `reset_request_windows`（後述）ただ1つである — この表の行が置かれるのは登録・予約の経路だけで**未登録の canonical には行が無く**、ここに載せると4ケース一様性が構造的に成立しないからである。**列削除の DDL 分類は「データ量に依存する（分割か回避が要る）」の (i) 側である**（後述）。

**saga コーディネーター状態:**

| カラム | 型 | 制約 |
|---|---|---|
| `operation_id` | TEXT | nullable。予約を取った操作。**予約の確定（`activateReservation`）では消さない** — 昇格後も残ることが、`resume-signup` の冪等性キーの片側（[async/index.md](../async/index.md) の全数表）が成立する条件である。次に上書きされるのはクレデンシャル変更 saga の開始時であり、**したがって可変列である** — **終端した saga の後始末はこの列を「行を引く鍵」にも削除の条件にも使わず、保存済みの locator の PK で対象行を名指しする**（使うと、同じ行にリセットが着地して値が変わった瞬間に回収が空振りする。[recovery/index.md](../recovery/index.md)） |
| `candidate_user_id` | TEXT | nullable。確定前の `userId` 候補。**予約の確定（`activateReservation`）では消さない** — 終端した saga の後始末（S2）が放棄先の特定に使う材料であり、消すのは後始末の最終段だけである（[recovery/index.md](../recovery/index.md)） |
...
| `locators` | TEXT | nullable。コーディネーター行が持つ全クレデンシャルの locator 一覧（JSON 配列。**要素は `credential_id` + `kind` + 全長 HMAC + 世代 + bucket index であり、`operations.target_locators` と同じ形である** — 後始末が要素ごとに写像の削除を発行し、かつ**コーディネーター行を `credential_id` の一致で除外する**ので、`credential_id` を含むことが成立条件である。[recovery/index.md](../recovery/index.md)）。**予約の確定（`activateReservation`）では消さない** — `candidate_user_id` と同じく、消すのは後始末の最終段だけである |
...
| カラム | 型 | 制約 |
|---|---|---|
| `caller_token` | TEXT | **NOT NULL**。この写像の所有アカウントが提示すべき不透明値。User Data DO 側の `account.caller_token` と同じ値。ログ・エラーに出さない。**予約行を書く3箇所（新規登録 saga の予約2つと SSO 連携の予約）はいずれも値を書くので、型で NOT NULL を強制できる**（照合側の規則 — `NULL`・空・規定長未満は常に不一致 — は `change_auth_token` と同じ形で残し、型が破られた場合の defense-in-depth とする。[rotation/index.md](../rotation/index.md) の `record-remapped-locator` も同じ規則で照合する） |
...
- **終端の後始末が終わるまで予約行を消さない。** `locators` / `candidate_user_id` / `caller_token` を持つコーディネーター予約行は、**中断した saga を回収するための唯一の材料**であり、期限切れ掃除（`sweep-reservations`）がこれを先に消すと材料が失われる。**消すのは後始末の最終段だけであり、その段は `jobs` 行を終端へ落とすのと同じ `transactionSync` で消す**（段の順序と材料の寿命の正本は [recovery/index.md](../recovery/index.md)）。**掃除に先を越されないことは予約 TTL の不等式が担保する** — 右辺は「前進のバックオフ上限に達するまでの総経過時間 + 後始末のバックオフ上限に達するまでの総経過時間」の2項 + マージンであり、実値は運用設計が定める（不等式の正本は [recovery/index.md](../recovery/index.md)）
- **lookup の世代順序（stub 選択アダプターの契約）。** canonical からの解決（`findByEmail` / `findBySsoIdentity` を含む未認証経路の全部）は、**stub 選択アダプター（request Worker）が keyring の active → previous の順に各世代の locator を導出して bucket を引き、最初にヒットした行を使う**（単一世代では1回に退化する。DO facade・リポジトリの signature は変わらず、各 bucket の中では従来どおり PK 引きである）。**両世代とも外れた場合は、active をもう1回だけ再プローブしてから「未登録」と扱う** — 逐次2プローブは原子でないので、active の空振りと previous の空振りの間に移送（import → 移送元削除）が完走すると、登録済みの行を両世代とも取り逃す窓がある。移送の宛先は（巻き戻し中を含め）常に active なので、再プローブ1回でこの窓は閉じる。**リセット依頼はこの順序でヒットした世代の bucket で依頼一式（窓の判定・行の解決・発行・イベント行）を完結させ、どの世代にもヒットしなければ active 世代で完結させる** — 未移送の利用者の依頼が previous 側に、未登録宛の依頼が active 側に落ち、4ケースの一様性（同じ bucket の中で1トランザクション）はどの世代でも保たれる。**ローテーション中に限り、この世代プローブの回数差（active でヒット = 1回 / それ以外 = 2回）が窓の外で観測できる時間側の残差になる** — 受容し、[rotation/index.md](../rotation/index.md)「受容した残余」に列挙する。機構の検証は TC-keyRotation-035 / 036 / 040 が担う
- **予約の書き込み（3箇所）には2つの規約が掛かる**（正本は [rotation/index.md](../rotation/index.md)）— (i) **予約の世代ガード**: 自 bucket の写像世代が鍵コミットメントの active 世代と一致しなければ拒否する（保存済みの世代つき座標を再駆動する saga が previous 世代へ行を作る経路を塞ぐ。`previousCount = 0` の恒久性の成立条件）。(ii) **退役証明の無効化**: 予約行を書く同じトランザクションで、増えた行が属する世代の `rotation_checkpoints` 行を削除する
- **この表を読み書きする口は UoW コンテキストの3メンバーで全数である** — 読みの `credentialMappingReader`（導出済み locator 引きと `credential_id` 引き）、手続きの6段を持つ `credentialMappingWriter`、照合結果を書き戻す `credentialAttemptRecorder` である。**したがって書き込み経路は 6 + 1 = 7 つで、これが全数である**（各操作の名前・効果・冪等性と3規則の正本は [domains/identity.md](../domains/identity.md)）。**期限切れ掃除（`sweep-reservations`）と鍵ローテーションの移送は同じ行に対してアダプターが行うので口を通らない**（`jobs` / `outbox_events` と同じ形）
- **OCC の `version` を持たない。** 書き込みはすべて単一の CAS 文で直列化されており、同じ行に対する「読んで判断して書く」がリクエストを跨がない。**述語に入る列の全数は [domains/identity.md](../domains/identity.md) の3規則が持つ**。汎用の OCC を重ねると CAS 条件と `version` のどちらが権威かが二重になる。**したがって `CredentialMappingRepository` は `ExpectedVersion` を取らない**（domains/identity.md）
...
- **書き込み口は UoW コンテキストの `resetThrottleStore` の `claimWindow` ただ1つであり、これが全数である。** その窓の最初の依頼なら行を作って `true`、既存の窓なら `last_requested_at` だけを更新して `false` を返す。**「読み」と「計上」を2つの書き込み箇所として数えない** — 判定と計上は1回の呼び出しで原子的に行われる。戻り値の `boolean` が「イベント行を書くか」と「リセットトークンを発行するか」の**両方を決める唯一の分岐**である。**掃除の `DELETE` は `sweep-reset-tokens`（アダプター）が同じ行に対して行うので口を通らない**（`jobs` / `outbox_events` と同じ形）
...
- **窓の長さと `expires_at` の猶予は運用値であり、実値の確定は運用設計が持つ**（`jobs` の件数上限と同じ形）。窓の長さは (a) スロットルの強度、(b) この表の行数（= 10 GB 算入分）、(c) `sweep-reset-tokens` の起床頻度を同時に決める。**値の選び方には制約が1本掛かる — `スロットル窓の長さ < リセットトークンの TTL` を満たすこと。** 満たさないと、リンクが期限切れになった利用者が窓の明けるまで再送を得られず、`spec/scenario/account.md` の異常系（「リセットリンクが期限切れの場合、その旨が表示され再送をやり直せる」）と衝突する。**この制約は配送の運用値の制約2本（`jobs` の prune の項 / async/index.md「運用値の制約」）の外側にある別の1本であり、あちらの「2本で全数」を開かない**
  - **窓の長さは単一の設定値であり、`windowKey` の導出（ユースケース側）と `expires_at` の算出（アダプター側）が同じ値を読む。2箇所に別々の定数を置かない。** 同じ運用値が2層で必要になるのは、`claimWindow(windowKey, now)` が窓長を引数に取らないからである。**「別々の定数を置かない」の実体は、2層とも同じ DI コンテナから同じ値を受け取ることである** — ユースケース側の受け取り口は `container` の運用値（`spec/usecases/identity.md` の共通事項）であり、どちらの層にも定数を直書きしない。ズレると静かに壊れる — アダプター側が短いと、まだ有効な窓の行が `sweep-reset-tokens` に消され、`claimWindow` が同じ窓で2度目の `true` を返す（下の世代跨ぎで例外的に許容している破れが、設定ミスで恒常化する）
...
- **`key_generation` の読み手は診断である** — 索引にも `claimWindow` の述語にも現れない（世代は既に `window_key` へ畳み込まれているので PK にも入らない）。運用者が「どの写像鍵世代で導かれた行か」を判別するために持つ。旧世代の行は誰にも引かれないまま `expires_at` で掃除されるので、**世代を述語にした一括削除は置かない**
  ...
  - **世代を進める主体は写像鍵のローテーション（[rotation/index.md](../rotation/index.md)）である。** `rotate-encryption` はメール暗号鍵の世代を進めるジョブであり、この列は動かさない
- **行は依頼回数ではなく窓の数に比例して増える。** ただし**未登録アドレス宛の依頼でも行が増える**ので、**掃除が追いつかない、または窓の中で distinct なアドレスが大量に撒かれると** 10 GB 上限へ向かって伸び続ける（`requestPasswordReset` は未認証で叩け、窓キーは canonical ごとに独立なので、**canonical 単位のスロットルは異なるアドレスを撒く経路を抑止しない**）
  - **逼迫の影響はこの bucket に写像を持つ全ユーザーへ及ぶ。** Identity Directory DO は bucket 共有であり、逼迫した DO は書き込みだけが失敗するので、**同じ bucket の他ユーザーの新規登録・クレデンシャル変更・リセット依頼が巻き添えで止まる**（User Data DO の 10 GB と違い、単独ユーザーに閉じた話ではない）。発信元単位のレート制限は transport 境界の責務であり、本ファイルの範囲外である
- OCC の `version` は持たない（非集約ストア）
...
User Data DO 側と**同じ13列・同じインデックス・同じ規則**である。Outbox relay の実装も2クラスで共有する。違うのは `type` の値域だけで、こちらは `identity.passwordResetRequested` の1種である（全数表は [async/index.md](../async/index.md) を参照）。**User Data DO 側のイベント型は初期0件だが、表と機構は両クラスに置く**。
...
- **置換キー（PK）は (`rotation_kind`, `bucket_index`, `generation`) である。** 記録は置換で行う。**置換に加えて、行の削除がある** — 写像行を増やす書き込み（予約 / import / `rotate-encryption` の書き換え）が「退役証明の無効化」として、増えた行の属する世代の checkpoint 行を同じトランザクションで消す（契機の正本は [rotation/index.md](../rotation/index.md)）。`rotationCheckpointStore` は置換と削除の両方の口を持つ
- 衝突の3列（`conflict_count` / `last_conflict_at` / `last_conflict_credential_id`）は `rotation_kind = 'remap'` の行だけが使う。移送は `jobs` 行も `operations` 行も持たないので、`terminal_reason` を記録先にできないためである。**値はそのチャンクでの再検出数の snapshot であり、累積しない**（衝突行は解消されるまで毎チャンク再検出される。正本は [rotation/index.md](../rotation/index.md)）
- OCC の `version` は持たない
- **書き込み口は UoW コンテキストの `rotationCheckpointStore` だけである。** 書き手は2群 — **snapshot 置換を書く2つ**（写像鍵の移送 `remap` とメール暗号鍵の再暗号化 `rotate-encryption`）と、**退役証明の無効化として行を削除する側**（写像行を増やす書き込み = 予約 / import / `rotate-encryption` の書き換え。契機の正本は [rotation/index.md](../rotation/index.md)）— である
- **記録の契機（各チャンク完了トランザクションでの snapshot 置換）・読み方（`rotation_kind` で絞った全 bucket × `previous_count = 0` の退役条件）・ローテーションの手順そのものの正本は [rotation/index.md](../rotation/index.md) である。** 本ファイルは列と用途までを定める
...
- **順位付けは `bm25` で行い、タイトルを本文より重く見る重み付けを行う。裏付けは実測**（公式ドキュメントに記載は無い）。**重みとページサイズの実値は本ファイルに固定しない** — 実装側が持ち、実環境で再検証した結果を spec へ反映する
...
- **ゲートは初期化分岐と移行分岐に分かれる。射程を DO クラスで分ける — 初期化分岐に制限が掛かるのは User Data DO だけである。** **User Data DO の初期化分岐（`_meta` の行が無い DO に対して全テーブルを作る側）を実行してよいのは `initialize-account`（新規登録 saga の phase 2）だけであり、これが全数である。** User Data DO の他の全エントリは未初期化の DO に対して**1行も書かずに fail closed で戻る**（**応答の種別は `SystemError` である**。「epoch 不一致と同じ扱い」は倒す向きの説明であって応答の種別ではない — 未初期化はセッションの失効ではなく、その DO にアカウントが存在しないという基盤側の状態である）。**Identity Directory bucket の初期化分岐に制限は掛けない** — 制限が要るのは**名前の空間が利用者入力から導かれ、上限を持たない側だけ**だからである（bucket の名前は `dir:g{世代}:b{番号}` で番号は keyring の `bucketCount` で有界であり、最初の予約や最初の `claimWindow` がその bucket を初期化するのは正常系そのものである。**制限を掛けるとどの bucket も一度も初期化できず、登録もリセット依頼も成立しない**）。**理由は「User Data DO を実体化してよい経路を1本に絞る」ことである** — 絞らないと、正しく署名された・存在しない `userId` のセッションや、自動回収が未初期化の DO へ発行する `abandon-account` が、**空の DO（全テーブルと `_meta` だけを持ち `account` が0行の DO）を作る。** 空の DO は `credential_mappings` に行を持たないので `list-bucket-user-ids` から到達できず、DO の内部 ID から `userId` へは戻せない（後述の PITR の項）ので、**回収も発見もできない**（正本は [recovery/index.md](../recovery/index.md)）
  - **初期化分岐だけは、`initialize-account` の業務書き込み（`account` / `user_settings`）と同じ `transactionSync` で確定する。** 移行分岐は従来どおりゲート自身のトランザクションで確定し、本体の成否に依存させない（下の「データ書き換えを伴う部分はジョブへ逃がす」の断定は移行分岐についてのものであり、そのまま維持される）。**分けるのは、本体が落ちたときに DO が0バイトへ戻らなければ同じ空 DO が残るからである**
  - **診断エントリ `read-schema-version` は従来どおりゲートを通らないので、未初期化の DO を「未初期化」として返す**（1行も書かない）
- **未初期化の DO に対する既定の応答（`SystemError`）には例外が2群あり、これが全数である。性格が違うので分けて書く。** **(a) ゲートを通らない2本** — operator 専用の診断エントリ `read-schema-version` / `list-bucket-user-ids`。どちらも1行も書かず、スキーマの形に依存しない値だけを読むので、未 migrate の DO を壊す経路が無い。**(b) ゲートは通るが、未初期化を分岐名で返す1本** — `abandon-account` は未初期化の DO に対して**1行も書かずに `nothing-to-abandon` を返す**（`SystemError` にしない）。**これが無いと、phase 2 を一度も成功させずに終端した新規登録 saga の後始末が構造的に全滅する** — 後始末は「進めない」を失敗と読んではならないので、`SystemError` を受けると恒久的に再試行して回収可能な残渣まで巻き添えにする（正本は [recovery/index.md](../recovery/index.md) の評価順序 (1)）
...
- **本ファイルの形は初回の `CREATE TABLE` で丸ごと作る。** 実装先の DO のアダプター（冒頭）は本ファイルの現在の形をそのまま張るので、`ALTER TABLE DROP COLUMN` を発行する必要はない。**未配備のあいだは、後から足す表も新しいバージョンを起こさず初回のバージョンの DDL を拡張する — `schema_version` を上げるのは最初の本番配備以降である。** 一方がバージョンを起こし他方が拡張すると、コードより新しい version を見た DO が fail-closed で沈黙する側へ倒れ、しかも**一度も実行されない migration step** が残る。既に初期化済みの開発環境の DO は拡張後の表を持たないので作り直す（配備前なので影響はそこに限られる）。**配備後に同じ変更を行う場合の分類は次のとおりである。**
  - **`outbox_events`（両クラス）と `reset_request_windows` の追加は単発適用である。** `CREATE TABLE` はデータ量に依存せず、索引も**空テーブルへの `CREATE INDEX`** なので安い。**`migrate-bulk` は行のコピーのための機構であり、コピーすべき行が0件なら出番が無い。** DO 全体が 10 GB まで育っていても、新設・空の表には関係しない（上の多段分解は「**既に大きく育った既存テーブル**へ索引を足す」場合の逃がし方であって、この場合には当たらない）
  - **`jobs` からの列の削除、`credential_mappings` からの列の削除は、いずれも (i) の列削除に当たる。** 表の追加側だけ分類を書いて列削除側を書かないのは非対称なので、両方を書く
...
- 射程から外れるのは診断エントリ2本だけである。fail-closed で止まっている DO の存在は運用側がメトリクスで検知する
...
- **PITR は失効を巻き戻す。** User Data DO を戻すと `session_epoch` と `ai_client_connections.status` が過去へ戻り、Identity Directory bucket を戻すと消費済み・削除済みのリセットトークン行が復活する。したがって復旧には必須ステップが伴う — User Data DO 側は `session_epoch` を現在時刻由来の十分大きな単調値へ進め、`ai_client_connections` を全件 `revoked` にする。Identity Directory bucket 側は `password_reset_tokens` を全行削除し、`failed_attempts` を 0・`next_attempt_allowed_at` を過去へ戻す。**復旧できないなら全部切るのが既定である。手順・承認・監査の実体は運用設計が定める**
...
ジョブが一様な終端（`terminal_reason` + `poison`）に達したときのエスカレーション先として、**operator 専用の maintenance 経路が存在する**。**残渣を直接消せるエントリは `purge-user-mappings`（退会の最後の砦）の1つだけであり、`jobs.kind` の11種には入らない**（ジョブではなく RPC である）。**`cancel-reservation`（予約と孤児写像の取り消し）はこの経路に数えない** — `callerToken` を必須ガードに持ち、その値を返す口は当該利用者の有効なセッションを要する経路だけなので**operator は単独で実行できず、呼び出し元は自動回収の段（コーディネーター bucket / User Data DO）である**（saga の内部エントリとしての正本は [recovery/index.md](../recovery/index.md)。**operator は `callerToken` を持てない**以上、このエントリを operator 経路として数えることはできない）。**したがって終端した saga に対する operator の入口は後述の `requeue-poisoned-job` である。**診断用の `read-schema-version` / `list-bucket-user-ids` と、`rotate-encryption` の起動（全数表が名指しする唯一の投入点）も同じ経路に属する。**写像鍵ローテーションの4エントリ（`remap-chunk` / `import-remapped-mappings` / `record-remapped-locator` / `read-rotation-checkpoint`）も同じ経路に属し、ガードと手順の正本は [rotation/index.md](../rotation/index.md) である** — `record-remapped-locator` だけは宛先が User Data DO で、残る3つは Identity Directory bucket 宛である。**4エントリはいずれも `jobs` / `outbox_events` の実行可能な行を増やさないので、Alarm の張り直しの射程に入らない**（`rotate-encryption` の起動との違いはここにある）。**行を書く経路には「Alarm の多重化」の張り直しの規約がそのまま掛かる** — `rotate-encryption` の起動も `requeue-quarantined-event` の再駆動（後述）も、そのトランザクションのあとに4本の min の合成で `setAlarm` を張る。**読みしかしないエントリ**（`read-schema-version` / `list-bucket-user-ids` / `list-quarantined-events` / `read-rotation-checkpoint`）**は行を増やさないので張らない。** **到達制御・監査ログ・運用手順の実体は運用設計が定める。**
...
**返す件数にも上限を置く。** `quarantined` は恒久保持で、隔離は Queue producer binding の障害などで**一斉に起きうる**（`outbox_events` の「その他」）ので、一斉隔離のあとにこのエントリを叩くのは運用上の既定動作である。上限を置かないと、この経路だけが行数に比例して伸びる唯一の無制限経路になる（claim・prune・チャンク反復・bind パラメータには上限が置かれている）。**順序は `completed_at`（= 隔離された時刻）の昇順とし、超過分はその続きを引く** — `outbox_completed_idx` が `(status, completed_at)` なので、この順序だけは宣言済みの索引で解け、ソートが要らない（`created_at` 順にすると索引が無く、claim と同じソートをもう1箇所増やすことになる）。**上限の実値と、続きの引き方（カーソルかオフセットか）は運用設計が定める** — 到達制御・監査ログ・運用手順と同じく maintenance 経路の実体だからである。**`requeue-quarantined-event` は単体 RPC なので据え置きでよいが、その帰結として一斉隔離からの復旧は隔離件数ぶんの RPC になる** — 一括再駆動を足すかは運用設計の材料である。
...
- **`setAlarm` の張り直しは省けない。** `quarantined` は実行可能集合に入らないので（「Alarm の多重化」）、**隔離行しか残っていない DO は定義上 `deleteAlarm()` 済みである** — これはまさに operator が再駆動する典型的な状況であり、張り直さないと「行は `pending` に戻ったが誰も起こさない」という、下の「起きるが1行も進まない」より静かな停止になる。**規約の正本は「Alarm の多重化」の張り直しの項**であり、ユースケース側の投入点（`enqueueJob` / `enqueueEvent`）と同じ規約である。この経路だけを例外にしない。**`list-quarantined-events` は読みのみなので張り直さない**
- **`next_run_at` を書かないと再駆動が黙って空振りする。** 終端時に `NULL` にされる列なので（`outbox_events` の「分離する規約」2.）、`status` だけ戻すと (1) Alarm の張り直しの `min(outbox_events.next_run_at)` が SQL の `min()` の NULL 無視でこの行を拾わず、(2) claim の対象を選ぶ述語（`next_run_at <= now`）にも掛からず、(3) それでも実行可能集合は空でないので `deleteAlarm()` もされない、という「起きるが1行も進まない」状態が残る。`attempt` の 0 復帰も `completed_at` の `NULL` 復帰も `jobs` の収束規則 (2)（`poison` → `pending`）と同じ形である — **同名・同意味の列なので不変条件（終端でない状態では `NULL`）は両表に掛かり、終端から実行可能集合へ戻す遷移を持つ経路はエントリでも収束規則でも一様にこれを書く。** `outbox_events` の再駆動については、上の4列がこの表の全数である
...
- **`quarantined` の行を減らす手段は、この再駆動（`published` へ進めば prune の対象に戻る）と operator による明示削除の2つだけであり、自動では減らない**（`outbox_events` の「その他」/ 「物理境界」の 10 GB の項）。**明示削除の口は本ファイルでは名前まで確定させない** — 到達制御・監査ログ・運用手順と同じく、maintenance 経路の実体は運用設計が定める

**`jobs` の `poison` に対する導線も同じ経路に属する** — `list-poisoned-jobs`（一覧）と `requeue-poisoned-job`（`pending` へ戻す再投入）である。**どちらも `jobs.kind` の11種には入らない**（ジョブではなく RPC である）。**`poison` は自動では減らないので、`quarantined` と同じ形の導線が要る**（`jobs` の prune の項）。**終端した cross-DO saga に対する operator の入口はこの2本であり、残渣を直接消すエントリを operator に開かない**（理由と受け口の割り当ての正本は [recovery/index.md](../recovery/index.md)）。

**`list-poisoned-jobs` が返す列を確定させる。** 返すのは `operation_key` / `kind` / `attempt` / `completed_at` / `terminal_reason` の5つであり、**`payload` は返さない** — `payload` は終端の原因を読むための列ではない（それは `terminal_reason` である）ので返す理由が無く、`list-quarantined-events` が同じ理由で `payload` を外したのと揃える。**順序は `completed_at`（= 終端した時刻）の昇順とし、件数上限を置いて超過分はその続きを引く** — `jobs_completed_idx` が `(status, completed_at)` なのでこの順序は宣言済みの索引で解け、ソートが要らない。**上限の実値と続きの引き方（カーソルかオフセットか）は運用設計が定める。** 読みのみなので `setAlarm` を張り直さない。

**`requeue-poisoned-job` が書く列を確定させる。** 同じトランザクションで `status='pending'` / `next_run_at = 現在時刻` / `attempt = 0` / `completed_at = NULL` の4つを書き、`terminal_reason` は残す（`lease_until` / `owner_token` は終端時に `NULL` になっているのでそのままである）。**そのトランザクションのあと、4本の min の合成で `setAlarm` を張り直す。**

- **書く列は上の4つで全数である。** 内訳は **収束規則 (2)（`poison` → `pending`）が書く6列から `payload` / `payload_digest` を除いた4列**（`status` / `attempt` / `next_run_at` / `completed_at`）である。`payload` を置き換えないのは、再投入ではなく再駆動であり実行する仕事が同じだからである。**`completed_at` を書くのは、終端でない行に `completed_at` が残ると列定義（`pending` / `running` では `NULL`）に違反するからである** — 同じ理由が収束規則 (2)(3) にも掛かっており、**逆向きの遷移を持つ経路のあいだにこの列の扱いの非対称は無い**
- **`setAlarm` の張り直しは省けない。** `poison` は実行可能集合に入らないので（「Alarm の多重化」）、**終端行しか残っていない DO は定義上 `deleteAlarm()` 済みである** — `requeue-quarantined-event` とまったく同じ理由であり、この経路だけを例外にしない
- **`terminal_reason` を残す理由は `quarantined` の再駆動と同じである** — 終端の原因は再駆動の後も運用者が読む唯一の記録だからである。**段の選択はこの列の値を読まない**（実行するのは、その行に後始末の段が実在すればその後始末、実在しなければ前進である。[recovery/index.md](../recovery/index.md)）。**前進をやり直させたい場合は、利用者が新しい `operationId` で操作をやり直す**（`operation_key` が変わるので別の行になる）
- **`poison` の行を減らす手段は、この再駆動（`done` へ進めば prune の対象に戻る）と operator による明示削除の2つだけである**（`quarantined` と同じ形。明示削除の口の名前も同じく運用設計が定める）

## 本ファイルで定義しないテーブル

- **OAuth 2.1 の `jti` 一回性テーブル。** 認可コードは署名済みの自己完結値なので永続化せず、User Data DO に置くのは交換済みコードの `jti` を短期間だけ記録する表だけである。**その定義は AI クライアント接続（OAuth 認可・一覧・失効）の設計の範囲であり、本ファイルでは名前を確定させない。** OCC の `version` は持たない（一回性の記録なので集約ではない）
- **検索の不透明カーソルが指す期限付きスナップショットの物理形。** ドメイン側で決まっているのは契約（同じカーソルからは同じ集合が読める / 期限切れのカーソルは拒否される / カーソルは不透明である。domains/search.md）だけで、**物理形は実装が決める** — 期限付きの表・DO ストレージの一時キー・安定順位による再実行のいずれでも契約を満たせるうえ、寿命と粒度はストレージ上限に依存する判断だからである。**採用した形はカーソル自体への埋め込みである** — 順位順の ID 集合（種別 1 byte + UUID 16 byte）を署名付き base64url に詰め、集合の上限は 500 件、期限は 30 分。表を増やさず、読み取りで何も書かないので 10 GB 逼迫時にも検索は動く。上限を超える一致は読めず、`count` は切った後の件数である（値の根拠と改訂の条件は実装の `stores/searchCursor.ts` の JSDoc と統合テストが持つ）
...
| ジョブの claim（実行可能集合を `next_run_at` の昇順で上限件数まで） | `jobs_runnable_idx`（集合の絞り込みまで。`status` が2値なので順序は索引では解けず、実行可能集合ぶんのソートが要る。`jobs` の claim の項） |
| lease 満了の `running` 行の回収 | 上の「ジョブの claim」と同じ経路（実行可能集合の `SELECT` と主キー CAS）で拾う。**`lease_until` を絞り込みに使わないので専用の索引を持たない** — `jobs_lease_idx` は Alarm の張り直しの材料であって claim の入口ではない（`jobs` の claim の項） |
| 完了ジョブ（`done`）の prune と、終端ジョブ（`poison`）の一覧（`list-poisoned-jobs`。`completed_at` 昇順・件数上限） | `jobs_completed_idx`（**`poison` の一覧もこの索引で解けてソートが要らない** — prune 専用の部分索引へ縮めるとこの経路が解けなくなる。`outbox_completed_idx` と同じ形である） |
| relay の claim（実行可能集合を `next_run_at` の昇順で上限件数まで） | `outbox_runnable_idx`（同上。ソートが要る点も `jobs` と同じ） |
| lease 満了の `publishing` 行の回収 | 上の「relay の claim」と同じ経路（同上）。**`outbox_lease_idx` も Alarm の張り直しの材料であって claim の入口ではない** |
| 配送済み（`published`）イベントの prune | `outbox_completed_idx` |
| Alarm の張り直し（4本の min の合成。「Alarm の多重化」） | `jobs_runnable_idx`（`status IN ('pending','running') AND status='pending'` の `min(next_run_at)`。**素朴に `status='pending'` とだけ書くと選ばれない** — 発行形の正本は「Alarm の多重化」）/ `jobs_lease_idx`（`status='running'` の `min(lease_until)`）/ `outbox_runnable_idx`（`status IN ('pending','publishing') AND status='pending'` の `min(next_run_at)`。同上）/ `outbox_lease_idx`（`status='publishing'` の `min(lease_until)`） |
...
| 写像鍵の移送の走査（`remap-chunk` の `credentialId` 昇順 + 開始位置） | `cm_credential_id_uq` |
| 移送の残行数集計（`previousCount`）と `rotate-encryption` の残件走査（`encryption_generation ≠ active`） | **全走査（受容）** — `≠` 述語と COUNT に効く索引を張らず、bucket の行数に比例するコストをチャンクごとに1回受け入れる。どちらの述語も自己消尽する（行は移送・書き換えで減る）ので収束は保たれ、コストはチャンクサイズの運用値の材料に含める |
