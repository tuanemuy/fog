...
| `kind` | TEXT | NOT NULL。操作の種別。**値域トークンは `signup` / `link` / `unlink` / `credential-change` / `withdrawal` の5つで、これが全数である**（読み手が値で行を選ぶので、日本語の呼び名だけでは実装が確定しない — `abandon-account` のガードは `signup` を、後始末の L1 と退会側への要求は `kind IN ('link','unlink')` を述語に持つ。[recovery/index.md](../recovery/index.md)）。**書き手は `recordOperation` だけであり、どの段がどの値を書くかは [usecases/identity.md](../usecases/identity.md) が持つ**。**本 spec が書き手を持つのは `signup` / `link` / `unlink` の3つで、残る2つは段が本 spec の範囲外の saga のための予約である** — `credential-change` は Identity Directory 側が所有する saga なので `operations` を持つ DO をまたぐ設計が要り、`withdrawal` は退会 saga が自分の手続きを記録する場合の置き場である — **実装は `finalize-withdrawal` の初回起床で `operation_id = 'withdrawal'` の行を 1 つ作り（`phase='deleting'`、`target_locators` は `credential_locators` の snapshot）、2 世代並存中に写像の削除が no-op で返った時刻を要素の `noopSince` として持つ（rotation/index.md の no-op 確定の記録先）**。**段の形はそれ以上に定めず、どちらであっても `finalize-withdrawal` の冪等性キーには入らない**（キーは `jobs.operation_key` の DO ごとの定数だけである。[async/index.md](../async/index.md)）。**新規登録 saga の終端規則による放棄経路はこの表に行を作らない**（[recovery/index.md](../recovery/index.md)） |
