...
| 認証済みユーザー | 投稿する | 同一 `transactionSync` の中で投稿したメモのエントリが `search_entries` / `search_fts` に作られ、直後の検索からヒットする | |
...
| 認証済みユーザー | `MemoRepository.insert` / `insertRevision` で DB 例外が発生する | `SystemError(DatabaseError)`。トランザクションはロールバックされ、メモ・リビジョン・インデックスエントリのいずれも記録されない | |
