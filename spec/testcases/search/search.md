...
| 一致するドキュメントが複数トピックにまたがってヒットする | 検索する | 各ドキュメント項目に所属トピックの `topicName` が付与される。トピック名の解決は `topicId` を重複除去したうえで `TopicRepository.listSummariesByIds` の一括呼び出し1回で行われる（N+1 にしない） | |
