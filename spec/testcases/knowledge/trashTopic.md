...
|---|---|---|---|
| `active` トピックの配下に active ドキュメント 2 件が存在する | トピックを削除する | トピックは `status: "trashed"`（`trashedAt: now`, `wasArchived: false`）、配下 2 件は `TrashedDocument`（各 `trashedWith = topic.id`）になる。`topic.trashed` 1 件 + `document.trashed` 2 件のイベントが同一 UoW で記録され、`trashedDocumentIds` に 2 件の ID が返る | |
...
| トピックが存在する | `DocumentRepository.save` で DB 例外が発生する | `SystemError(DatabaseError)`。ロールバックされイベントも記録されない | |
