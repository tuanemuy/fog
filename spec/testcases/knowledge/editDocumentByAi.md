...
| 本文が上限近くの active ドキュメントが存在する | 適用結果が 400,001 文字になるパッチで編集する | 適用結果の `DocumentBody.create` で `BusinessRuleError(DocumentBodyTooLong)`（境界値） | |
| active ドキュメントが存在する | `mode: "replaceAll"`, `body` を 400,001 文字で編集する | `BusinessRuleError(DocumentBodyTooLong)` | |
