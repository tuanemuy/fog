...
|---|---|---|---|
| 認証済みユーザー | `name: "読書メモ"`, `description: "本の要約置き場"` で作成する | `status: "active"`, `version: 0` のトピックが作成され、`id` / `name` / `description` / `createdAt` / `updatedAt` を含むビューが返る。トピックはインデックスエントリを持たないので projection の更新は発生しない | |
...
| 認証済みユーザー | `TopicRepository.insert` で DB 例外が発生する | `SystemError(DatabaseError)`。トランザクションはロールバックされ、トピックは作成されない | |
