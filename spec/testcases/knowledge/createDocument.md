...
| `active` トピックが存在する | `body` をちょうど 400,000 文字で作成する | 正常に作成される（境界値: 最大長ちょうどは許容） | |
| `active` トピックが存在する | `body` を 400,001 文字で作成する | `BusinessRuleError(DocumentBodyTooLong)`。ドキュメントは作成されない | |
...
| 作成が完了しており、トランザクション境界をまたいでトピックの OCC トークンを保持している書き手がいる（エッジケース: トピック touch の OCC 競合） | そのトークンでトピックを `save` する | 手順 6 の touch がトピックの `version` を消費済みのため `save` が 0 行更新となり `ConflictError("OPTIMISTIC_LOCK_FAILURE")`。利用者は再試行。**この形を取れるのは `TopicRepository` を直接呼ぶ経路だけであり、現行のユースケースには存在しない** — ユースケースはいずれもトピックの読みと `save` を 1 つの `transactionSync` に収めるため、境界をまたいでトークンを持ち越せない | |
| 同一トピックに対する `createDocument` と `updateTopic`（アーカイブ切替等）が並行する | 両者を実行する | UoW コールバックが単一の `transactionSync` であるため 2 つは完全に直列化され、後から入った側は先行が進めた `version` の上に積まれる。トピックの `version` は 2 進み、どちらも `ConflictError` にならない。「ゴミ箱内トピック配下に active ドキュメントが生まれる」レースを排除しているのはこの直列化であり、touch の OCC ではない | |
