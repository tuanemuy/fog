...
| セッションは有効だが `userId` が未初期化の User Data DO を指す | 現在のユーザー情報を取得する | `SystemError(NOT_INITIALIZED)`。migration ゲートが1行も読まずに拒否するので、`user_settings` の読みに到達しない（認証ミドルウェアはこの1コードだけを未認証へ畳む） | |
...
| メールのクレデンシャルの逆引きが失われている、または原本を復号できない | 現在のユーザー情報を取得する | `SystemError(DataIntegrityError)`。アカウントが生きているあいだ消す経路が無いので、空の `email` や欠けた要素を描画せずドリフトとして扱う | |
