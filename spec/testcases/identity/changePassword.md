...
| 中間状態（`changeState` が `'pending'`）のまま前進不能が確定した | 一様な終端に落ちる | 終端に至った事実が記録として残り（`terminal_reason` に載るのは理由トークンと `operationId` だけ）、**そのうえで後始末が手続きを巻き戻す** — `pendingVerifier` / `changeState` / `changeOrigin` が `null` に戻り旧検証材料が正本のまま残るので、**原則として旧パスワードでログインして変更をやり直せる**（**ユーザー単位設定側の更新がコミットした後に巻き戻った窓に当たっていた場合だけは版照合で拒否されるので、そのときはリセットの依頼1回で回復する**。recovery/index.md「起点別の扱い」）。**後始末が完走すればジョブは `done` に落ち、operator の受け口（`list-poisoned-jobs` / `requeue-poisoned-job`）には現れない** — 運用へエスカレーションされるのは後始末が焼き切れるか材料を見失った場合だけである（[recovery/index.md](../../recovery/index.md) 段 C1） | |
...
| `changeState` が `'advanced'`（ユーザー単位設定側への反映が済んでいる）まま前進不能が確定した | 一様な終端に落ちる | **巻き戻さない。** `poison` と `terminal_reason` が残って operator 経路へエスカレーションされ、原因の解消とジョブの再投入まで中間状態が続く。巻き戻すと認証情報側だけが旧材料へ戻ってログインの到達性検査の版照合が外れる（新しい締め出しを作る向きには倒さない。[recovery/index.md](../../recovery/index.md)） | |
| `changeState` が `'pending'` のまま前進不能が確定したあと、別の `operationId` の新しい変更依頼が後勝ちでその行を差し替えた | 終端モードの後始末が走る | **何も書かずにジョブが `done` になる**（巻き戻しの CAS 条件が `operationId` の一致を含むので0行になる）。新しい依頼の `pendingVerifier` は巻き戻されない。**条件を落とした実装は他人の進行中の依頼を巻き戻すのでこのケースで落ちる** | |
