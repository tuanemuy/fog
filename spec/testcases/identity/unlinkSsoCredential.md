...
[usecases/identity.md](../../usecases/identity.md) の unlinkSsoCredential に対するテストケース。書き込みはユーザー単位設定側（User Data DO）と認証情報側（Identity Directory）の2つの物理境界にまたがるので、順序と中間状態で利用者に何が観測されるかも対象に含める。**解除対象を作る唯一の経路は `linkSsoCredential` である**ため、正常系の前提はいずれも連携済みの状態から始める。

| 前提条件 | 操作 | 期待結果 | 実装ステータス |
|---|---|---|---|
| パスワード登録のアカウントが `linkSsoCredential` で SSO を連携済み（`kind: "sso"` と `kind: "email"`（`usableForLogin: true`）の2件を持つ） | SSO の `credentialId` を指定して解除する | `User.credentials` から当該要素が消えて `version` が +1 され、`CredentialLocatorStore.deleteByCredentialId` で逆引きが消え、`AccountStore.advanceSessionEpoch` が呼ばれる。認証情報側では写像行とそのクレデンシャル宛のリセットトークン行が消える。正常終了（`void`） | |
...
| 中間状態のまま前進不能が確定した | 一様な終端に落ちる | 終端に至った事実が記録として残り、運用へエスカレーションされる。**この操作には巻き戻しが無い** — 解除はユーザー単位設定側で既に確定しており、前進（削除の再試行）そのものが回収だからである。**したがって終端後は認証情報側の写像行が残り、その SSO 主体を再連携できないまま operator の受け口（ジョブの再投入）を待つ。** ログインは到達性検査が拒否し続けるので認可はどの向きにも開かない（[recovery/index.md](../../recovery/index.md)） | |
| `linkSsoCredential` で連携済みのアカウント | 解除を実行する | 逆引きを消す**前に**、その `credentialId` の写像材料が**全世代分**手続きの記録へ退避され、同じトランザクションで `sweep-orphan-mapping` が投入される。**これが本ジョブの唯一の投入点**であり、投入が無いと認証情報側に残った写像が回収されず同じ SSO 主体を二度と連携できなくなる（1世代分だけ控えると回収されない世代が残る） | |
