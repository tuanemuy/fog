...
| 中間状態のまま前進不能が確定した | 一様な終端に落ちる | 終端に至った事実が記録として残り、**そのうえで後始末が手続きを巻き戻す** — 手続きの記録が持つ写像材料の全世代へ取り消しが発行され、`reserved` 行も `active` な孤児写像も同じ1つのエントリで解放されるので、**同じ SSO 主体で連携をやり直せる**。手続きの記録は `phase` が完了になるだけで、退避した写像材料は空にされない。**後始末が完走すればジョブは `done` に落ち、operator の受け口には現れない** — 運用へエスカレーションされるのは後始末が焼き切れるか材料を見失った場合だけである（[recovery/index.md](../../recovery/index.md) 段 L1〜L3） | |
| 連携の手続きが中断したまま退会が始まっている（`account.status` が `active` でない） | 前進のジョブが起きる | **終端モードへ入らない。** 手続きの記録に触れずジョブだけが完了になり、回収は退会側の引き取りが行う。**手続きの記録の `phase` を完了へ倒す実装は、退会側の引き取り述語（未完了の連携 / 解除の行）からその行を外して回収経路を消すのでこのケースで落ちる**（[recovery/index.md](../../recovery/index.md)） | |
