...
| ゴミ箱内のメモが復元前にドキュメントの出典（「削除済み」表示中）である | 復元を実行する | 出典リンクは保持されたままであり、trash 側の追加操作なしで復元される。`memo.restored` により search consumer が再インデックス・出典先ドキュメントのエントリ再 upsert を行う契機となる | |
