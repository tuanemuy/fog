...
**本体を書くトランザクションの中で projection を更新する。** 別ストアではないので配送する必要が無く、**検索ドメインには外部 transport（キュー・ワーカー）が登場しない。** これは検索についての限定であり、リポジトリ全体の断言ではない — 外部への配送は DO ローカル Outbox が担う（契約と全数は [async/index.md](../async/index.md)）。**検索用イベントも indexer consumer も置かない**（`.adr/003` / `spec/adr/005`）。
...
**この11の契機はイベントではない。** いずれも本体の書き込みと同じ `transactionSync` の中で直接行われる同期実行であり（[async/index.md](../async/index.md) の判定規則1）、`outbox_events` にも `jobs` にも行を作らない。**検索用イベントと indexer consumer を置かない**（`.adr/003` / `spec/adr/005`）ので、search ドメインはイベント型を1つも定義しない。
