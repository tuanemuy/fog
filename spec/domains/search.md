...
メモ・ドキュメントを横断する全文検索（SQLite FTS5）を担うドメイン。エンティティは持たず、値オブジェクト・ポートが中心となる（[ADR-004](../adr/004-domain-boundaries.md)）。検索は SQLite FTS5 の全文検索のみとし、インデックスは本体更新と同一トランザクションで維持する。
...
**本体を書くトランザクションの中で projection を更新する。** 別ストアではないので配送する必要が無く、**検索ドメインには外部 transport（キュー・ワーカー）が登場しない。** これは検索についての限定であり、リポジトリ全体の断言ではない — 外部への配送は DO ローカル Outbox が担う（契約と全数は [async/index.md](../async/index.md)）。**検索用イベントも indexer consumer も置かない**。
...
**この11の契機はイベントではない。** いずれも本体の書き込みと同じ `transactionSync` の中で直接行われる同期実行であり（[async/index.md](../async/index.md) の判定規則1）、`outbox_events` にも `jobs` にも行を作らない。**検索用イベントと indexer consumer を置かない**ので、search ドメインはイベント型を1つも定義しない。
