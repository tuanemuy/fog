...
|---|---|---|
| id | `UserId` | required |
| credentials | `readonly CredentialRef[]` | required。保有クレデンシャルの要約（下記）。**集合を作る操作が1件以上を保ち、組み立て直しは検査しない**（後述の不変条件） |
...
- **`credentials` に `usableForLogin: true` の要素が1件以上あることを、集合を作る操作が保つ。** 生成のファクトリ（`registerWithPassword` / `registerWithSso`）は満たさない集合を渡されると `BusinessRuleError(LoginMethodRequired)` を投げ、最後のログイン手段を解除する操作は `BusinessRuleError(LastCredentialRemoval)` で拒否する。数えるのは要素数ではなく**`usableForLogin` が真である要素の `credentialId` の異なり数**である（同じクレデンシャルが複数の表現を持ちうるため、要素数で数えるとログイン手段が0のアカウントを作れてしまう。`usableForLogin` を見ないと SSO 専用アカウントのメール要素まで数に入る）
- **保存済みの行から `User` を組み立て直す経路はこの不変条件を検査しない。** `credentials` は `CredentialLocatorStore` の投影であり、登録の手続きはユーザー単位設定側の行を phase 2（手順5）で、最初の逆引き行を phase 4（手順7）で書く（[usecases/identity.md](../usecases/identity.md)、段の呼び名の正本は [recovery/index.md](../recovery/index.md)）ので、**クレデンシャル集合が空の `User` は進行中の手続きの正当な中間状態である**。読み直しで検査すると、その窓が `SystemError(DataIntegrityError)` に変わる
...
- バリデーション: 次の順序で行う。違反は `IdentityErrorCode.InvalidEmail`
  ...
  6. domain 部は NFKC 正規化して lowercase 化し、非 ASCII を含む場合は punycode（IDNA、ASCII 形式）へ変換する。**その変換のあとで末尾のドット1個を落とし**（FQDN 表記を同一メールボックスとして畳む）、**落とした結果が空、または依然 `.` で終わるものは拒否する**。畳みを変換の後に置くのは、UTS46 がラベル区切りとして扱う4文字（`.` `．` `。` `｡`）をすべて `.` へ写すのが変換であり、そこが全数を1箇所で拾える唯一の位置だからである。NFKC が畳むのは `．` だけなので、変換より前に置くと `。` `｡` を取りこぼし、canonical が自分自身の不動点にならない
...
**認証情報の所有者とユーザー単位設定の所有者は、別々のポートである。** 2つは物理境界が分かれているので、1つのリポジトリポートには畳まない。
...
- **濫用抑止には3つの規則を課す。** カウンタの単位はクレデンシャルであって発信元ではないので、被害者のメールアドレスを知る攻撃者は誤ったパスワードを投げ続けるだけで `nextAttemptAllowedAt` を先送りできる。素朴に組むと正規利用者が正しいパスワードを入れても「成功時のリセット」に到達できない恒久的な締め出しになるため、次の3つを規則として置く（**具体値**（天井の秒数・減衰の係数・初期のバックオフ幅）**は運用側で決める**）。
  ...
  2. **時間減衰を置く。** **`nextAttemptAllowedAt`（最後に次の試行が許された時刻）からの経過時間**に応じて `failedAttempts` を減らす。閾値未満の失敗ではこの時刻は失敗そのものの時刻であり、閾値を超えてロックアウトが載ったあとは「ロックアウト明け」が起点になる。成功だけがリセットの契機である設計にしない。**減衰は読み取り時に効き、下げた値が保存値へ届くのは失敗側の書き戻しが観測値の CAS に一致したときだけである**（「照合結果の報告」）
...
- **3規則の置き場は分かれる。** 天井と減衰の算式はログインのユースケースが持ち（具体値は `container` の運用値から読む。[usecases/identity.md](../usecases/identity.md) の共通事項）、書き戻す文は上の `CredentialAttemptRecorder` が持つ。**規則 iii はユースケースが報告を呼ばないことで守られ、ストア側に「制限中なら無視する」分岐は無い。**
...
- **登録・変更・解除の手順そのものは単一のメソッドに畳めない。** 認証情報側とユーザー単位設定側の2つを跨ぐため、順序と再開の規則を持つ手続きとして usecases/identity.md に書く。**ただし各段が呼ぶ書き込み操作の名前と契約はここで固定する** — 畳まないことと契約を書かないことは別である。
  ...
  | `activateReservation` | 予約の確定 | 予約行を有効な写像へ昇格し、写像先の `userId` を確定する | 同じ手続き ID の行にだけ効く |
  | `cancelReservation` | 予約の取り消し・終端した手続きの回収 | **呼び出し元が保存済みの座標で名指しした写像行**を取り除く。**`status` も手続き ID も条件に含めない**（敗北した予約行だけでなく、終端した手続きが残した有効な写像行も対象になり、**手続き ID は別の手続きが上書きしうる可変の値なので条件に使えない**）。**束縛は2つで、これが全数である** — 呼び出し元束縛の不透明値の一致と、**行のクレデンシャル識別子が名指しした座標のものと一致すること**である（後者を落とすと、同じ canonical が空いた後に別の手続きが取り直した行を消す）。**同じトランザクションでその `credentialId` 宛のリセットトークン行も全削除する**（写像を失ったトークン行を残さない。`deleteMapping` と同じ規則） | 「無ければ成功」の冪等操作で、**応答は「成功」1値。束縛の2条件は削除の CAS 述語そのものであり、不一致は「行に触れない」ことを既に意味するので、行が無い場合と区別しない**（区別すると呼び出し元へ他アカウントの行の存在が漏れ、失敗と読んだ後始末が恒久再試行に落ちる。`abandon-account` が不一致をエラーへ倒すのとは逆であり、その理由も含めて正本は [recovery/index.md](../recovery/index.md)） |
  ...
  | `promoteVerifier` | 検証材料の昇格 | 保留の検証材料を正本にし、`credentialVersion` を**呼び出し元が渡す値**へ揃え（正常系では `CredentialLocatorStore.advanceCredentialVersion` の戻り値。**`'advanced'` からの再開では phase 2 を再発行せず、`CredentialLocatorStore.findByCredentialId` で読み直した現在値** — `advanceCredentialVersion` は冪等でないので再発行は版を1つ余分に進める。正本は [recovery/index.md](../recovery/index.md)）、`changeState` / `changeOrigin` を `null` へ戻す。**同じトランザクションで `failedAttempts` を 0 に、`nextAttemptAllowedAt` を過去へ戻す**（濫用抑止からの脱出経路） | **条件は2つで、これが全数である** — `changeState` が `"advanced"` であることと、**行の手続き ID が呼び出し元の手続きの値と一致すること**。**後者は読み取り時点の確認では代用できない**（読みと書きのあいだにユーザー単位設定側への RPC が挟まるので、その窓で後勝ちに差し替えられた他の手続きの保留材料を昇格させてしまう）。一致しなければ0行で、呼び出し元は「自分の残渣はもう無い」と読む（正本は [recovery/index.md](../recovery/index.md)） |
  | `deleteMapping` | 解除・退会 | 対象 `credentialId` の写像行と、そのクレデンシャル宛のリセットトークン行を消す。**対象は `userId` で束縛し、呼び出し元束縛の不透明値の一致も必須ガードに持つ** | 「無ければ成功」の冪等操作。**`userId` が未確定の予約行には発行できない**（束縛の片方が成立しないため）。**これが、終端した手続きの回収に本メソッドではなく `cancelReservation` を使う理由である**（正本は [recovery/index.md](../recovery/index.md)） |

  **これらは `CredentialMappingRepository` のメソッドではない。** 手続きの各段が認証情報側へ発行する操作であり、`find*` の3本のようにドメイン型を返す読み取りとは性質が違う。**置き場は認証情報側の `UnitOfWorkContext` に生やす書き込み口（`CredentialMappingWriter`）であり、読みのポートとは分ける** — 「写像行への書き込み口は UoW コンテキストが全数である」という規約をこの表にもそのまま掛けるためである（列と CAS の書式は `spec/database/index.md` が持つ）。**6操作には次の3規則が掛かり、これが全数である。**

  1. **各操作は単一の CAS 文である。** 述語に入るのは `operation_id` / `change_state` / `status` / `saga_committed` / `caller_token` / `credential_id` のいずれかで、**汎用 OCC の `version` は使わない**（この表は `version` 列を持たない）。**matched-row count を読み戻すのは、答えが呼び出し元へ届く4操作だけである** — `activateReservation` / `beginCredentialChange` / `promoteVerifier` は戻り値の `boolean` として、`reserveCredential` は 0 行を `ConflictError` へ翻訳するために読み戻す。**`cancelReservation` / `deleteMapping` は読み戻さない** — 束縛の条件は削除の CAS 述語そのものなので、読み戻して得られる「当たらなかった」は「行が無い」と「束縛が合わない」のどちらかであり、この2つを区別しないと決めたのが規則2 の `void` 側である。区別しないと決めた答えは、読み戻しても捨てるほかない
  2. **戻り値の意味は2つに揃える** — `boolean` は「CAS が行に当たったか」であり（`activateReservation` / `beginCredentialChange` / `promoteVerifier`）、`void` は「当たったかを返さないと決めた」である（`reserveCredential` / `cancelReservation` / `deleteMapping`。`reserveCredential` は敗北を `ConflictError` で返すので戻り値を持たない）
  3. **同じトランザクションへ巻き込む副作用は操作の一部であって呼び出し元の責務ではない**（`cancelReservation` / `deleteMapping` のリセットトークン行の全削除、`beginCredentialChange` の未使用トークンの全無効化、`promoteVerifier` の濫用抑止カウンタのリセット）

  **`usableForLogin` は列を持たず導出する。** 権威は `coordinate.kind` が `'sso'` であることと `passwordVerifier` の有無であり（`credential_locators.usable_for_login` の定義と同じ規則）、**この2つを持つ操作は値を引数に取らない。**

  ```ts
  ReserveCredentialParams = { coordinate, operationId, candidateUserId, callerToken,
                              sealedCanonical, passwordVerifier, reservedUntil, coordinatorRole };
  ActivateReservationParams = { coordinate, operationId, userId };
  ```

  逆引き（`CredentialLocatorStore.record`）へ写す値は、呼び出し元が同じ2つの材料から導く。第2の権威になる列も引数も足さない。

  **中間状態のあいだは旧新どちらのパスワードでもログインできない**（fail closed）。前進不能が確定した場合の終端は一様である（記録を残して運用へエスカレーションする）。**終端の具体的な手順の正本は [recovery/index.md](../recovery/index.md) である** — 要点は3つで、**(i) `changeState` が `'pending'`（ユーザー単位設定側への反映が未了）なら手続きを巻き戻す**（`pendingVerifier` / `changeState` / `changeOrigin` を `null` へ戻し、旧検証材料を正本のまま残す）、**(ii) `'advanced'`（反映済み）なら巻き戻さない**（ユーザー単位設定側が版を進めているので、認証情報側だけを戻すと到達性検査の版照合が外れて新しい締め出しを作る）、**(iii) 巻き戻しは `changeOrigin` で分岐しない**（戻す先は「依頼が無かった状態」であり起点によらず同じである。起点で分かれるのは巻き戻した後に利用者が自力で完走できるかだけで、`'reset'` は旧パスワードを知らないぶん**リセットの再依頼**で回復する）。

#### 照合結果の報告（7つめの書き込み口）

**ログイン照合と現在パスワード照合の結果を写像行へ書き戻す操作は、上の6操作のどれでもない。** 手続きの段ではないので `CredentialMappingWriter` には入れず、**`CredentialAttemptRecorder`（1メソッド）として UoW コンテキストの独立したメンバーに置く。** 読み1 + 書き6 + 報告1 で、写像行を読み書きする口はこれが全数である。
...
- **成功側は引数を持たない。** `failedAttempts` を 0 に戻すこととロックアウトの解除を文の側に固定し、「リセットしない成功」を呼び出し元が書けないようにする
- **失敗側の値は呼び出し元が算出して渡す。** 規則 i（天井）と規則 ii（時間減衰）は運用値に基づく方針であってストアの保存形ではないので、ストアには置かない。**算出の材料になった減衰前の `failedAttempts` を `observedFailedAttempts` として同行させる** — 下の CAS の立会人であって、保存する値ではない
- **失敗側の書き戻しは、呼び出し元が読んだ値に対する CAS である。** 呼び出し元は行を読み、鍵導出を1回払ってから報告するので、同じクレデンシャルに重なった照合はどれも同じ読みから算出している。ストアは保存された `failedAttempts` が `observedFailedAttempts` と**一致するとき**（読んでから誰も書いていない）だけ `failedAttempts` / `nextAttemptAllowedAt` を報告値のまま書き、**一致しないとき**（並列の試行が窓の内側で報告した）は `failedAttempts` を行自身の値から1つ進め、`nextAttemptAllowedAt` は保存値と報告値の遅いほうを取る（時刻を持たない報告は保存値を動かさない）。重なった報告を1つの前進に畳むと、並列度を上げるだけでロックアウトへ到達しない当て推量が続けられ、早いほうの時刻を取ると直前の報告が書いたロックアウトを解除してしまう。**一致側が、規則 ii の減衰が保存値へ届く唯一の経路である** — 保存値が下がらない形にすると 0 への道が成功だけになり、静かな期間のあとの2回の打ち間違いが正規利用者を締め出す
- **規則 iii（制限中の照合はカウンタを進めない）は「呼ばない」ことで守る。** ストア側に「制限中なら無視する」分岐を置くと、規則を知らない呼び出し元がそれを通せてしまい、責任の所在が二重になる
- **行を選ぶ述語は座標（`kind` / 全長 HMAC / `credentialId`）だけで、手続き ID も `status` も含めない。** 行が無ければ no-op である（ログインは既に `INVALID_CREDENTIALS` を返しており、数える相手がいない）。**上の観測値の CAS は列の更新式の側にあり、行の選択には掛からない**

#### 契約を満たす実装の形（読みの対）

**`findByEmail` / `findBySsoIdentity` はどちらの物理位置にも単独では実装できない。** canonical から写像行の座標を導くには写像鍵が要り、その鍵は request Worker にしか配布されない（[rotation/index.md](../rotation/index.md)）ので bucket の中では引けない。一方で行は bucket の中にあり到達手段は非同期の RPC なので、同期契約のポートとして request Worker 側にも実装できない。**したがってこの契約は2つの実装の対で満たす。**

- **request Worker 側** — canonical から locator を導き、keyring の順に bucket を選ぶ（lookup の世代順序の正本は `spec/database/index.md`）
- **bucket の中** — `CredentialMappingReader` が locator を不透明な写像材料として受け取って行を引く。メソッドは `findByLocator(kind, mapping)` と `findByCredentialId(credentialId)` の2本で、どちらも同期契約である

読みが返すのは `CredentialMappingRecord`（= `CredentialMapping` + `passwordVerifier` + `sealedCanonical`）である。**ドメイン型に載せない2つを、行の随伴情報として同じ戻り値へ載せる** — 検証材料の照合は Durable Object の外で、canonical の復号はトランザクションの外で行うので、どちらも行がトランザクションを出るときに同行しなければならない。`CredentialMapping` 自身は上の9フィールドのまま動かない。**検証材料が bucket の外へ出るのはログインの行解決の応答1本だけである** — ポートの戻り値の型は2本で共通だが、境界を越える面はそれ以外に検証材料を載せない。

**ポートを跨ぐ2つの型をここに置く。** どちらもドメインは中身を解釈しない。

- `CredentialCoordinate` = `{ credentialId, kind, mapping }`。`mapping` は `CredentialLocator.mapping` と同じ不透明な写像材料である。`credentialId` を併せ持つのは、`cancelReservation` の束縛2条件と後始末のコーディネーター行の除外がこの値を要求するからである
- `SealedCanonical` = `{ ciphertext, encryptionGeneration, nonce }`。`credential_mappings` の PII 3列に1対1で対応し、**nonce は独立した項目であって暗号文に連結しない**
...
export interface AccountStore {
  ...
  /**
   * アカウントの初期化（新規登録 saga のユーザー単位設定側の初期化）で、
   * 呼び出し元束縛の不透明値を書く。書き手はここ1箇所だけで、消すのは退会の完走時だけである。
   */
  initializeCallerBinding(callerToken: string): void;

  /**
   * 退会の開始（新規登録 saga の終端によるアカウント候補の放棄を含む）。
   * status: "active" を条件に含む条件付き更新で、通れば "deleting" へ倒して sessionEpoch を進める。
   * 0 行（既に "deleting" / "deleted"）は冪等な成功として扱う。
   */
  beginDeletion(): void;
}
```
...
- **セッションの照合そのものはアダプター（認証ミドルウェア）の責務である。** ドメインが持つのは「世代を進める」という遷移と、その値が失効の唯一の権威であるという規則だけである。**照合が成立する形は3点で固定する** — (i) セッショントークンは**発行時点の `sessionEpoch` を署名対象の payload に載せて運ぶ**（載せないと突き合わせる相手が無い）、(ii) 認証ミドルウェアは署名と期限の検証を通したあとに `find()` の `sessionEpoch` を読み、**DO 側が大きければ未認証として扱う**、(iii) **セッションが拒否される理由を呼び出し元へ返さない**（署名不正・期限切れ・世代不一致・**トークンが指す DO が未初期化**（アカウントが無い）の4つはどれも同じ「未認証」で、区別するとアカウントの存在とその身に起きたことが漏れる。4つ目だけは基盤側のエラーとして届くので、**認証ミドルウェアは `NOT_INITIALIZED` に限って未認証へ畳む**。**セッションの発行側は非対称で畳まない** — 載せる世代が取れないことは失効ではなく壊れた不変条件なので `SystemError(DataIntegrityError)` で落ちる）。**この読みはリクエストごとに走り、キャッシュしない** — キャッシュは失効が効かない窓そのものである
- **`find` 以外の4メソッドは `ExpectedVersion` を取らず `version` も進めない。** 単調増加カウンタの前進であって設定の更新ではないので、OCC の条件を付けない単独文で書く（`ai_client_connections` の `recordUsage` と同じ扱い。`spec/database/index.md`）
- **`AccountState` は `version` を持たず、本ポートは `save` に相当するメソッドも持たない。** `account` テーブルは OCC の `version` 列を保持するが、**本 spec の範囲には `version` を条件に含む更新を発行する操作が無い**（`beginDeletion` は条件付き更新だが、条件は `version` ではなく `status` である）。`status` を型に残すのは、ログインとリセットのガードが `"active"` であることを読む側の権威だからである
- **`status` の `"deleting"` への遷移は `beginDeletion` が書く。** 呼び出す経路は2つで、これが全数である — **(1) 退会の起点**（退会 saga そのものは本 spec の範囲外である）と、**(2) 新規登録 saga が終端したときのアカウント候補の放棄**（効果と束縛の評価順序の正本は [recovery/index.md](../recovery/index.md) の `abandon-account`）。**`"deleted"` への遷移と完了処理は退会の完走が行い、そちらは本 spec の範囲外である**
- **呼び出し元束縛の不透明値（`account.caller_token`）の書き手も本ポートの射程にある。** **書くのはアカウントの初期化1箇所だけ**（`registerWithPassword` の手順5 / `registerOrLoginWithSso` の手順4。値は登録 saga の起点が採番してユーザー単位設定側へ渡す）、**消すのは退会の完走時だけ**である。**この値が書かれていないと、認証情報側からユーザー単位設定側へ発行する操作（放棄・写像の削除・逆引きの記録）の束縛がすべて成立しない** — 照合規則（`NULL`・空・規定長未満は照合の前に不一致）により常に拒否側へ倒れる。物理形は `spec/database/index.md` の `account` 表が持つ
...
export interface CredentialLocatorStore {
  ...
  /** その credentialId の credentialVersion を1つ進める。進めた後の値を返す */
...
- **`findByCredentialId` が複数世代の行に当たったときに返すのは最新世代の行である。** 全世代の行は下の単調非減少規則により `credentialVersion` が一致するので、この選択が決めるのは写像材料だけであり、**行から認証情報側へ辿り着く呼び出し元は現行の材料を要求する**
- **`credentialVersion` は `credentialId` 単位で単調非減少である。** `record` は**引数の値と、同じ `credentialId` の全行（対象行だけではない）の既存最大値のうち大きいほうを書く。この規則は既存行の更新だけでなく新規行の作成にも掛かる** — 掛けないと、別世代の行が先に前進した後の記録が古い値の行を作り、世代間で `credentialVersion` が分岐する（分岐すると `advanceCredentialVersion` の「進めた後の値」が一意でなくなり、到達性検査の版照合が実装裁量になる）。**既存行があれば何もしない no-op にしてはならない** — 記録が空振りすると到達性検査が利用者を締め出す
- **`advanceCredentialVersion` は進めた後の値を返し、認証情報側の昇格（`promoteVerifier`）はその戻り値へ揃える**（**冪等ではない** — 呼ぶたびに進むので、saga の再開時に再発行してはならない。`'advanced'` からの再開が版を `findByCredentialId` の読み直しで取る規則の正本は [recovery/index.md](../recovery/index.md)）。 戻り値が一意であることは、`record` の単調非減少が全行の最大に対して取られる（上の規則）ことで保たれる — 同じ `credentialId` の全行は常に同値なので、全行 +1 の後の値は1つである。 `AccountStore.advanceResetVersion` と同じ理由（読み直しと前進を分けると並行実行でずれる）に加えて、**版の採番がこの単一 DO のカウンタに線形化されていることが、同じ credential の2つの行が「同じ `credentialVersion` で違う検証材料を持つ」状態を作れないことの唯一の根拠である** — 認証情報側のローカル +1 で「揃え」を代用してはならない（鍵ローテーション中の並走で同版・異内容が生まれ、移送の正本判定が黙って新しい検証材料を捨てる。[rotation/index.md](../rotation/index.md)）
- **`usableForLogin` / `label` は引数の値でそのまま上書きする**（判定の権威は認証情報側にある）
- **`User.credentials` はこのストアの射影である。** `usableForLogin` が真である行の `credentialId` の異なり数が「ログイン手段の数」であり、`User` の不変条件が数えるのと同じ値である。**射影は `credentialId` で重複を畳んだ集合である** — 世代ごとの行は同じクレデンシャルの別の写像材料にすぎないので、要約は行数を数えない
- エラーケース: DB 例外 → `SystemError(DatabaseError)`
...
- **4ケース（登録済み / 未登録 / SSO 専用 / スロットル中）で `tokenId` の生成器が同一であることが契約である。** `issue` が返す `tokenId` と `mintDecoyTokenId` が返す値は、同じ CSPRNG・同じ長さ・同じ符号化で採る。**生成器が割れると、送らない側の payload だけが別の形になり、payload そのものが列挙オラクルになる**
...
- **戻り値の `boolean` が「イベント行を書くか」と「リセットトークンを発行するか」の両方を決める唯一の分岐である。** 発行判断と窓判定は同じ1つの分岐であって、2つの独立した条件ではない
...
- **導出主体は次のとおりである。** `windowKey` は **bucket 選択のために canonical の全長 HMAC を既に計算しているアダプター（Identity Directory の stub を選ぶ側）が、その同じ値を DO facade へプリミティブとして渡し**、ユースケースが窓と合成して組み立てる。**導出鍵はその stub 選択アダプターの中にあり、ユースケースにもポートにも渡らない。** facade が受け取る HMAC は server-side で導出された値であって外部入力ではないので、`CLAUDE.md`「Input validation」の第3の検証点にはならない（**クライアントからは受け取らない**）。合成は keyed な再導出を行わない（鍵付きの部分は HMAC 側で済んでいる）ので、`transactionSync` の中に暗号処理を持ち込まない
  ...
  - **「導出鍵が DO の中に無い」は「HMAC の値が DO の中に無い」ではない。** facade が受け取る全長 HMAC は、同じ DO の中で `CredentialMappingRepository.findByEmail` が `credential_mappings` の PK `(kind, hmac)` を引くのにも**同じ値**が使われる（正本は `spec/database/index.md`「窓キーの導出」）。`findByEmail` の signature は `email: Email` のまま動かず、**DO 側は canonical から HMAC を導出せず、facade から受け取った値を使う**
- **窓の長さは引数に現れないが、2層が同じ値を読む。** 呼び出し側は `windowKey` の合成（HMAC + 窓）のために、アダプターは窓行の `expires_at`（窓の終端 + 猶予）の算出のために、それぞれ窓の長さを知る必要がある。**正本は単一の設定値であり、`spec/database/index.md` の `reset_request_windows` の節が持つ**（実値の確定は運用設計が持つ）。**2箇所に別々の定数を置かない** — アダプター側が短いと、まだ有効な窓の行が `sweep-reset-tokens` に消され、`claimWindow` が同じ窓で2度目の `true` を返す（同節が写像鍵の世代跨ぎに限って明示的に許容している破れが、設定ミスで恒常化する）
...
- **呼ぶのは Alarm ジョブではなく、request Worker の `queue()` ハンドラ（mail consumer）である**。consumer は `identity.passwordResetRequested` のメッセージを受けて発行元 Identity Directory bucket へ**送信材料 RPC** を打ち、応答が `send` のときだけ本ポートを呼ぶ（`nothing-to-send` なら no-op して ack する）。**宛先の復号と生トークンの導出は DO の中に閉じたまま**であり、`to` / `resetToken` は RPC の応答としてのみ consumer に渡り、どこにも永続化されない（async/index.md「送信材料 RPC」）
...
- **リセット URL の base URL は Worker の設定値（環境変数）だけから取り、リクエスト由来のホスト情報（`Host` / `X-Forwarded-Host` / `Origin` など）からは導かない。** consumer は `queue()` ハンドラで動くので受信リクエストがそもそも存在しないが、規則として書く — 導くと攻撃者が制御するホストへ生トークンを載せたリンクをメールさせられる（リセットリンクのポイズニング）。**リセット画面からの Referer 送出の抑止と、リダイレクト時にトークンを引き継がないことは presentation 側の要件であり、実装が担う。**
- エラーケース:
