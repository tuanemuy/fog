...
| メソッド | 引数 | 戻り値 | 処理概要 |
|---|---|---|---|
| readAll | userId: UserId | Promise\<ExportSource\> | 対象ユーザーの全メモ・全トピック・全ドキュメントを読み出す。ソフトデリート済み（ゴミ箱内）の項目を除外し、各項目は最新リビジョンの内容で返す。`sourceMemoIds` にはハードデリート済みメモの ID を含めない |

- エラー: DB 障害は `SystemError(DatabaseError)`。範囲規則（ゴミ箱除外・最新リビジョンのみ）の充足はこのポート実装の契約
- 単一トランザクション（またはスナップショット読み）で読み出し、エクスポート内の相互参照が一時点で整合していることを保証する
...
| メソッド | 引数 | 戻り値 | 処理概要 |
|---|---|---|---|
| write | archive: ExportArchive | Promise\<ArchiveBinary\> | `rootDirName/` 配下に各 `ExportFile` を格納した zip を生成し、`{rootDirName}.zip` として返す |
