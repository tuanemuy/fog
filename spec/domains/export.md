...
**DO ローカル Outbox が加わった後も export は同期生成のままである**。export はイベントを定義せず、`outbox_events` にも `jobs` にも行を作らない — 生成の完了責任はリクエスト自身が持ち、委譲する consumer が無いからである（全数は [async/index.md](../async/index.md)）。
