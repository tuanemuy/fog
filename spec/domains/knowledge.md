...
- 関連 ADR: [ADR-001](../adr/001-restore-document-without-topic.md)（所属トピック消失時の復元）、[ADR-003](../adr/003-source-link-after-hard-delete.md)（ハードデリート後の出典リンク）、[ADR-004](../adr/004-domain-boundaries.md)（topic と document を同一ドメインに置く理由）。検索インデックスは同一トランザクションで更新する。
...
- 検索インデックスの更新は、本体を書くのと**同一のトランザクションの中の projection 処理**として行う（詳細は search.md「インデックスの維持」）。**検索インデックスについては配送しない。knowledge ドメインはイベントを定義しない** — consumer が無く、業務上の変更履歴は `document_revisions` が持つからである。**外部への配送の契約と全数は [async/index.md](../async/index.md)**
...
- バリデーション: 空文字を許容（書きかけのドキュメントは正当な状態）、**400,000 文字以内**（コードポイント数で数える）
- 補足: この値は DO SQLite の 1 行 2 MB から逆算した上限であり、**算出根拠は [database/index.md](../database/index.md)「### documents / 本文上限のバイト予算」が正本**
...
```ts
/**
 * トピック名の解決が描く分の Topic（読み取り射影。規約は domains/index.md「読み取り射影（サマリ）」）。
 * description も OCC トークンも持たないので、書き込みには接続できない。
 * `status` は直和の判別列として残す（落とすと trashed を含む広い射影が構造的に代入できてしまう）。
 */
...
export interface TopicRepository {
  ...
  /**
   * 指定 ID 群のトピックをサマリで一括取得する（表示用の名前解決。読み取り射影の規約は
   * domains/index.md「読み取り射影（サマリ）」）。
   * 検索結果（人間 UI）の「所属トピック名」表示と getTopicName（P-08 / P-09）が、
   * 結果件数分の N+1 照会にならず 1 クエリで成立する。
   * ゴミ箱内・存在しない（ハードデリート済み）ID は結果に含めない。
   * description も OCC トークンも読まないので、書き込みには接続できない。
   */
...
- `listSummariesByIds` は**射影読み**なので `Versioned<...>` を持たない（domains/index.md「読み取り射影（サマリ）」4 点目）。ここで読んだ行は `save` に渡せない
- `listSummariesByIds` の**並び順は契約に含めない**。呼び出し元は要求した ID を手元に持っており、欠落は `id` で突き合わせる（`DocumentRepository.listSummariesByIdsIncludingTrashed` と同じ整理）
...
```ts
/**
 * 一覧が描く分の Document（読み取り射影。規約は domains/index.md「読み取り射影（サマリ）」）。
 * 本文も OCC トークンも持たないので、書き込みには接続できない。
 */
type DocumentSummaryFields = "id" | "topicId" | "title" | "updatedAt" | "status";
...
export interface DocumentRepository {
  ...
  /**
   * 指定 ID 群のドキュメントを trashed 含め一括取得する（出典表示用の射影読み）。
  ...
  /** トピック配下の active なドキュメント一覧（セット削除の対象取得用） */
  listActiveByTopic(topicId: TopicId): readonly Versioned<ActiveDocument>[];

  /** トピック配下の active なドキュメント一覧の射影読み（トピック詳細画面用）。`updatedAt` 降順、同値は `id` 昇順で返す */
  listActiveSummariesByTopic(topicId: TopicId): readonly ActiveDocumentSummary[];

  /**
   * 複数トピック配下の active なドキュメントを射影で一括取得する（`listTopics` の「トピックと配下ドキュメントの一覧」用）。
   ...
   * （`listSourceLinksByMemos` / `listSummariesByIdsIncludingTrashed` と同水準の一括読み取り）。
   */
   ...
   * （memo の getTimeline が併用する。`listSummariesByIdsIncludingTrashed` と同水準の一括読み取り）。
   */
...
- 一覧系のうち書き込み（save / delete）に接続するもの（`listActiveByTopic` / `listTrashedByTopic` / `listTrashedByUser`）は `Versioned<...>` で OCC トークンを伴って返す（memo 側の `listTrashed` と同規約。セット削除・セット復元・トピックハードデリートのフローが読み直しなしで書き込める）。対して射影読み（`ActiveDocumentSummary` / `DocumentSummary` を返すもの）は書き込みに接続しないので `Versioned<...>` を持たない（domains/index.md「読み取り射影（サマリ）」4点目）
- **射影読みの並び順は契約である**: `listActiveSummariesByTopic` は `updatedAt` 降順、同値は `id` 昇順で返す（トピック詳細画面の表示順がこの契約に乗る）。**限界**: 並び順を宣言しているのはこの 1 本だけである — 複数トピックの一括射影読みは呼び出し元が `topicId` でグルーピングするため配列全体の並びを契約に含めず（トピック内の並びも宣言しない。契約にするならそれを固定するテストが要る）、ID 群で引く射影読み（`listSummariesByIdsIncludingTrashed`）は呼び出し元が要求した ID を持っているため宣言せず、OCC トークン付きの entity 一覧読み（セット削除の対象取得用）も未宣言のままで、その宣言は #7 の持ち分である
...
- `listTopics` — トピックと配下ドキュメントの一覧（UI / MCP `list_topics`。配下ドキュメントは `listActiveSummariesByTopics` で一括取得する）
...
- `getTopicName` ★ — トピック名 1 個の解決（P-08 のドキュメント画面ヘッダ・P-09 の新規作成画面の戻りリンク。`TopicRepository.listSummariesByIds` の射影読み 1 本で、description も配下ドキュメントも読まない。★ なのは AI 公開動詞を増やさないためであり、トピック名を秘匿するためではない — AI は `list_topics` で同じ名前を既に取れる）
