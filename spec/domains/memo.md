...
- 関連 ADR: [ADR-003](../adr/003-source-link-after-hard-delete.md)、[ADR-004](../adr/004-domain-boundaries.md)
...
- 検索インデックスの更新は、メモ本体を書くのと**同一のトランザクションの中の projection 処理**として行う（詳細は search.md「インデックスの維持」）。**検索インデックスについては配送しない。memo ドメインはイベントを定義しない** — consumer が無く、業務上の変更履歴は `memo_revisions` が持つからである。**外部への配送の契約と全数は [async/index.md](../async/index.md)**
...
- バリデーション:
  ...
  - **上限長**: **10,000 文字**（Unicode コードポイント数）を超えたら `BusinessRuleError(MemoErrorCode.BodyTooLong)`。メモは「アトム」であり長大な文書はドキュメントの領分、という位置づけを上限で表現する。この値の由来は製品上の位置づけであって行サイズではない。同じ逆算（[database/index.md](../database/index.md)「### documents / 本文上限のバイト予算」）を `memos` / `memo_revisions` に当てると、この上限ちょうどの本文を持つ行は 1 行の予算のごく一部にしか届かず、**行サイズはこの値の制約にならない**
...
```ts
/**
 * 履歴一覧が描く分の MemoRevision（読み取り射影。規約は domains/index.md「読み取り射影（サマリ）」）。
 * 本文を持たず、`memoId` は引数から決まるので含めない。
 */
