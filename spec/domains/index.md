...
- **ポートの同期契約**: 全ドメインポートは同期契約である（`Promise` を返さない）。書き込みは Durable Object 内の単一の同期トランザクションで完結し、その中では `await` を挟めないためである。**例外は `PasswordHasher` と `MailSender` の2つで、これは列挙であって導出規則ではない** — 残る理由は「暗号計算と外部 I/O であり、実装できる API が非同期しか無い」ことである。**トランザクションの外で動くことは `Promise` の根拠にならない**（`ArchiveWriter.write` は Durable Object の外で動くが同期契約である。domains/export.md）。新しいポートを足すときは、この2つに並べてよいかを「非同期 API しか無いか」で判定する
