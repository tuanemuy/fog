...
- **ドメインイベントの契約**: ドメインが返すのは **identity-less な draft** である（`{ type, payload, occurredAt, aggregateId }`）。**`EventId` はアプリケーション層が付ける** — UoW 実装が `IdGenerator` から採番するので、ドメインは id 生成に触らない。エンティティのファクトリ / 遷移は `{ entity, eventDrafts }` の形で draft を返し、ユースケースがそれを `enqueueEvent` へ渡す。**payload に PII と再利用可能な秘密を載せない**（衛生規則の正本は [async/index.md](../async/index.md)）。契約の詳細は identity.md「ドメインイベント」
