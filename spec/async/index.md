...
**「由来」欄の `—` は「対応する旧 `jobs.kind` が無い」を表す。** これに当たるのは**同期実行の4行と、上の段落が集合演算から除外する最終行の計5行**であり、**`—` の意味は2通りある** — 前者は「もともとジョブではないので書ける旧 `jobs.kind` を持たない」、後者は「イベント型が0件なので欄そのものを持てない」である。**旧 `jobs.kind` 12種との突き合わせは `—` を除いた由来欄の集合に対して行い**、そのとき集合はちょうど12個（`旧 send-mail` + local job 11種）になる。**由来欄が `—` の行を数えると5行になるのが正しい** — 4行だと読むと、最終行を二重に除外しているか表が壊れているかの判別がつかなくなる。
