...
グレーは青みをわずかに含む冷たいニュートラル（hue ≈ 280）で統一する。彩度を持つ色は Primary（紫）と Accent（橙）の2つだけ。橙はブランドの「点」（ロゴアイコンの点・モバイルヘッダーのドット）専用で、面には使わない（ロゴの詳細は [icons/logo.md](./icons/logo.md)）。
...
:root {
  ...
  --color-accent: oklch(0.62 0.19 40);           /* #e8590c ロゴアイコンの点・モバイルヘッダーのドットのみ */
