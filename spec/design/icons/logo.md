...
- ヘッダー表示規則: トップレベル5画面（タイムライン / トピック / 検索 / ゴミ箱 / 設定）はタイトルの代わりに lockup（h1 は sr-only で維持。lg 以上ではサイドバーの brand があるため非表示）。子ページ（トピック詳細 `topic`・メモ履歴 `history`）は短い英語タイトルを `--font-brand`（Avenir Next / Medium / lowercase）＋橙点で表示する。孫ページ（ドキュメント / 編集 / 履歴）は子の見出し＝トピック名を `--font-base` のまま表示（truncate 付き。ドキュメント / 編集では back-link としてトピックへ戻れる）。ページ種別の英語ラベルは子ページまでとし、孫では文脈（どのトピックの中か）を優先する。橙点は英語タイトル・トピック名の横に共通で付き、lg では非表示。長くなり得るドキュメントタイトルはヘッダーに置かず、シート内（`doc-title`）に置く
