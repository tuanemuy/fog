...
- **NotFound 変換**: `findById` 系の `null` を `NotFoundError` に変換するのはユースケースの責務。AI 向けユースケースは `findById` / `listActiveByIds` 系（active / Live のみ返す）だけを使い、ゴミ箱内は構造的に「存在しない」扱いになる（S-AI-04）。`findByIdIncludingTrashed` / `listByIdsIncludingTrashed`（およびその射影読み `listSummariesByIdsIncludingTrashed`）を使えるのは人間 UI・trash 系ユースケースのみ
...
- **書き込みは同期コールバックで行う**: 永続化は `UnitOfWorkProvider.run` に渡す**同期**のコールバックの中で完結する（`await` を挟めない）。**(1) 業務データの書き込み、(2) FTS5 projection の更新、(3) `enqueueEvent` によるイベント行の追加、の3つを同じ `transactionSync` の中で一度に確定できる**（rollback すると3つとも巻き戻る）。検索インデックスの更新は同じコールバックの中の projection 更新として行う（domains/search.md「インデックスの維持」）。**knowledge のユースケースはイベントを1つも発行しない** — イベントを発行するユースケースの全数は [async/index.md](../async/index.md) が持つ
...
| getTopicName | 人間 UI ★ | S-DT-04, S-DT-05 |
...
2. トピック ID 群を `DocumentRepository.listActiveSummariesByTopics(topicIds)` に渡し、配下 active ドキュメントの射影を 1 クエリで一括取得する（N+1 にしない）
...
3. `DocumentRepository.listActiveSummariesByTopic(topicId)` で配下 active ドキュメントの射影を取得する
...
| `body` | `string` | 必須 | `DocumentBody` の規則（空文字可、400,000 文字以内）はドメインで検証 |
...
6. **作成先トピックを touch する**: 同一 UoW 内で、手順 2 で読んだトピックを `TopicRepository.save(topic, expectedVersion)` で保存し `version` をインクリメントする（内容は変更しない）。「ソフトデリート済みトピック配下に active ドキュメントが生まれる」レースを排除しているのは、UoW コールバックが単一の `transactionSync` であること — 同一トピックへの `createDocument` と `trashTopic` / `updateTopic` は完全に直列化され、後から入った側は先行が進めた `version` の上に積まれるため、どちらも `ConflictError` にはならない（先に作成が入れば `listActiveByTopic` が新ドキュメントを拾い、先に trash が入れば手順 2 の `findById` が `null` を返す）。touch が消費する `version` が効くのは、トランザクション境界をまたいでトピックの OCC トークンを保持する書き手に対してだけである（現行のユースケースにその形は無く、`TopicRepository` を直接呼ぶ経路だけが取れる）
...
トピック名 1 個の解決。ドキュメント画面（P-08）のヘッダにある所属トピックへのリンクと、新規ドキュメント作成画面（P-09）の戻りリンクが描く唯一の値である。読みは `TopicRepository.listSummariesByIds` の射影 1 本で、`description` も配下ドキュメントも関連メモも読まない。P-09 では「トピックが実在すること」の確認そのものでもある（不在なら `NotFoundError` が返り、画面が「トピックが見つかりませんでした」になる）。

**★ の理由は AI 公開動詞の本数であって、トピック名の秘匿ではない。** AI は `list_topics`（AI 公開）で全トピックの名前を既に取得できるので、本ユースケースが隠している情報は 1 つも無い。AI 公開は 8 ユースケース固定で requirements 4.5 の 7 動詞と 1:1 に対応しており、9 本目を足すとその対応が壊れる — ★ はそれを避けるための決定である。

**★ を担保するのは AI 向け presentation が本ユースケースを配線しないことであり、`actor` を持たない入力型はそこまでを押さえない。** 入力に `actor` が無いことは「誰が呼んだかで分岐しない」ことしか言っておらず、呼べないことは言っていない（共通事項の二層のうち、配線分離のほうだけが効く）。
...
`description` / `status` / `version` / `createdAt` / `updatedAt` は含めない（射影読みが選択していない）。

### 処理フロー

1. `TopicId.create(input.topicId)` で検索キーを構築する
2. `TopicRepository.listSummariesByIds([topicId])`（ゴミ箱外のみ）。結果が空なら `NotFoundError`
3. `{ topicId, name }` を返す
...
ドキュメントのリビジョン履歴一覧（誰が・いつ・なぜ。S-DT-06）。AI クライアントの編集はクライアント名と変更理由で確認できる。**一覧はメタデータのみで全文スナップショットを含めない**（`DocumentBody` は最大 400,000 文字であり、リビジョン数 × 全文の一覧応答は現実的でない）。全文が要るのは差分表示（`diffDocumentRevisions`）とロールバック確認であり、それぞれのユースケースで取得する。memo 側の `listMemoRevisions` / `diffMemoRevisions` と同じ分割・同じ責務配置。
...
4. リンクの `documentId` 群を `DocumentRepository.listSummariesByIdsIncludingTrashed(documentIds)` に渡し、タイトル・削除状態を 1 クエリで取得する（S-TL-07 の表示が 1 クエリで成立する）。結果に含まれない ID（ハードデリート済み）は一覧に載せない（ADR-003）
