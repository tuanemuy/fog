...
- 各ユースケースは `ServiceArgs<TInput>` で `container`（`clock` / `idGenerator` / **認証情報側とユーザー単位設定側の Durable Object へ届くためのゲートウェイ** / 各ポート / **ポートでも時計でも ID 生成器でもない運用値**）と `input` を受け取る。`now` / 新規 `id` はユースケース冒頭で解決し、ドメイン内では生成しない
- **トランザクション境界は Durable Object の中にある。** 以降の処理フローに現れる `unitOfWorkProvider.run` は「**その DO の中で1つの `transactionSync` として確定する**」ことを表す記法であり、request Worker 側のユースケースがその Provider を握っていることを意味しない — ユースケースが持つのは上のゲートウェイ越しの**呼び出しの順序**であって、`run` を開くのは呼ばれた DO の RPC エントリの側である。**ゲートウェイの面はプリミティブだけを運ぶ**（ブランド型は structured clone を越えない。値オブジェクトの組み直しが入力検証の2点目であり、RPC hop は第3の点ではない）。**canonical から写像行の座標を導くのはこのゲートウェイの側である** — 写像鍵は request Worker にしか配布されないので、DO 側は導出済みの座標を受け取る（[domains/identity.md](../domains/identity.md)「契約を満たす実装の形」）
- **運用値をユースケースが読む経路は `container` だけであり、定数を直書きしない** — 同じ値をアダプターも読む場合、両者が同じ DI コンテナから受け取ることが「2箇所に別々の定数を置かない」の実体である。**identity がこの形で読む運用値は3群である** — (i) リセット依頼のスロットル窓の長さ（正本は `spec/database/index.md` の `reset_request_windows` の節）、(ii) クレデンシャル予約の TTL（**構築時に下限の不等式を検査する**。不等式の正本は [recovery/index.md](../recovery/index.md)）、(iii) ログインの濫用抑止の値（ロックアウトの閾値・初期の待ち幅・天井・減衰の刻み。規則の正本は [domains/identity.md](../domains/identity.md) の3規則）。**いずれも実走行の測定ではない** — (i) は正本の節が置く制約の内側で運用設計が確定させ、(ii) は構築時に検査する下限の不等式を満たすことだけが担保された選択（床は不等式の右辺に追随するが、宣言値は追随しない）、(iii) はドメインの3規則が要求する形からの選択である — **(iii) を測定で置き換える材料（実ログイン traffic の field data）は存在しない**
- 書き込みは `UnitOfWorkProvider.run` 内の**同期**コールバックで行う（`await` を挟めない）。**(1) 業務データの書き込み、(2) FTS5 projection の更新、(3) `enqueueEvent` によるイベント行の追加、の3つを同じ `transactionSync` の中で一度に確定できる**（rollback すると3つとも巻き戻る）。**イベントを発行するユースケースの全数は [async/index.md](../async/index.md) が持つ** — identity では `requestPasswordReset` の1つだけである。認証情報側（Identity Directory）とユーザー単位設定側（User Data DO）の両方に書く操作は、単一のトランザクションに収まらないので順序と再開の規則を持つ手続きとして各ユースケースに書く
...
4. **認証情報側**でメールの予約を取る（`reserveCredential`）。**一意性は予約の獲得で判定し、事前の読み取りを置かない** — 事前チェックは既に過ぎた瞬間について答えるので同時登録のレースを捕捉できず、行を返す読みは検証材料まで持ち出すことになる（持ち出してよいのはログインの行解決だけである。domains/identity.md）。既に使われていれば `ConflictError("EMAIL_ALREADY_REGISTERED")`
...
**手順4〜7は2つの物理境界をまたぐので、単一のトランザクションには収まらない。** 途中で落ちた場合は前進させる仕組みが引き取る。**利用者から観測できるのは次の3点だけである** — 中間状態のあいだはそのメールで登録もログインもできず、前進不能が確定した場合は一様な終端（記録を残して終端する。**運用へエスカレーションされるのは後始末が焼き切れるか材料を見失った場合だけである**）に落ち、**その終端では手続きが巻き戻される（アカウント候補が放棄され、予約と写像が解放される）ので、同じメールアドレスで登録をやり直せる**。**終端の具体的な手順の正本は [recovery/index.md](../recovery/index.md) である**（段 S1〜S4）。**巻き戻しそのものが前進不能に陥った場合は operator 経路が受ける** — そのときは同じメールアドレスがしばらく再登録不能なままになる
...
| メール登録済み（SSO ユーザーとの重複含む）・同時登録レース・手順6 の確定時に予約が自分のものでなくなっている | `ConflictError("EMAIL_ALREADY_REGISTERED")`（**3つは同じ1行である** — 判定はどれも予約の獲得と確定であり、利用者から見える事実は「そのメールは登録済み」である） |
...
4. 両方の予約に勝った場合だけ、**ユーザー単位設定側**の `unitOfWorkProvider.run` 内で初期化する:
   ...
   3. **同じトランザクションで `AccountStore.initializeCallerBinding` により `account.caller_token` を書き、`recordOperation` により手続きを記録する**（`kind: "signup"`。`registerWithPassword` の手順5-3 / 5-4 と同じ規則である）
5. 認証情報側の予約を確定させる（`activateReservation`）
6. **ユーザー単位設定側**で2件の逆引きを記録する（`CredentialLocatorStore.record`。`usableForLogin` は認証情報側の判定をそのまま写すので、メール側は偽になる）。**同じトランザクションで `updateOperation` により手続きの記録を完了にする**（`registerWithPassword` の手順7 と同じ規則である）
...
2. **認証情報側**で `CredentialMappingRepository.findByEmail(email)` の契約に従って対象クレデンシャルを解決する（canonical から座標を導き、その bucket の行を引く）。**応答には検証材料が載る** — 照合は Durable Object の外で行うためであり、**検証材料を返す読みはこの1本だけである**（domains/identity.md「契約を満たす実装の形」）
3. 次の5つはいずれも**実材料での照合に到達しない**ので `ValidationError("INVALID_CREDENTIALS")` に倒す — (i) 行が無い、(ii) 行が予約のままで `userId` が未確定、(iii) パスワードの検証材料を持たない（SSO 専用アカウントのメールクレデンシャル）、(iv) `changeState` が `null` でない（変更手続きの中間状態。旧新どちらの材料も通さない）、(v) `nextAttemptAllowedAt` が未到達（ロックアウト中）。**ここで応答を早めない** — ダミーの検証材料で同じ計算量を通し、登録有無を計算時間から推測できないようにする。**この5つでは照合結果を報告しない** — (v) を報告しないことが濫用抑止の規則 iii そのものであり、残る4つはそもそも数える相手の照合が起きていない
4. `container.passwordHasher.verify(plainPassword, verifier)` で照合する（タイミングセーフな照合はアダプター実装の責務。**計算は Durable Object の外で行う** — 単一スレッドの DO を長く占有させないため）。不一致は**失敗を報告してから** `ValidationError("INVALID_CREDENTIALS")`。報告する値（前進後の `failedAttempts` と `nextAttemptAllowedAt`）は規則 i（天井）と規則 ii（減衰）から本ユースケースが算出し、**算出の材料にした減衰前の `failedAttempts` を `observedFailedAttempts` として同行させる** — ストア側の書き戻しがこの観測値に対する CAS だからである（[domains/identity.md](../domains/identity.md)「照合結果の報告」）
...
6. **到達性検査を通ってから**成功を報告し（`failedAttempts` を 0 に戻し、ロックアウトを解除する）、`userId` を返す。**正しいパスワードでも到達不能なクレデンシャルではロックアウトを解除しない**

**応答時間の等価性には残余が1つある。** 誤ったパスワードは失敗を報告するので認証情報側への往復を1回多く払い、未登録アドレスには報告する相手が無いので等化できない。差は「1往復 対 1回の鍵導出」であり2桁離れているが、**報告を重くするかハッシュ計算を軽くすると等化が崩れる**。

### エラーケース

| 条件 | エラー |
|---|---|
| メール未登録 / 予約のまま / SSO ユーザー / 変更手続きの中間状態 / ロックアウト中 / パスワード不一致 / 到達不能 / 入力形式不正 | `ValidationError("INVALID_CREDENTIALS")`（すべて同一メッセージ。どれが原因かを明かさない） |
...
6. **ユーザー単位設定側**で、同じトランザクションの中で次を行う:
   ...
   2. `CredentialLocatorStore.advanceCredentialVersion(credentialId)` で対象クレデンシャルの世代を進める（**進めた後の値が返り、手順7の昇格（`promoteVerifier`）は認証情報側の `credentialVersion` をこの戻り値へ揃える** — 版の採番を単一カウンタに線形化するため。domains/identity.md。到達性検査が通り続けるようにする）
...
**手順5〜7は2つの物理境界をまたぐ。** 中間状態のあいだは旧新どちらのパスワードでもログインできず、前進不能が確定した場合は一様な終端（記録を残して終端する。**後始末が完走すればジョブは `done` に落ち、運用へエスカレーションされるのは後始末が焼き切れるか材料を見失った場合だけである**）に落ちる。**終端の具体的な手順の正本は [recovery/index.md](../recovery/index.md) である**（段 C1）— **手順6 がまだ適用されていなければ手続きは巻き戻り、リセットを依頼し直せる**（窓が明けていれば）。**手順6 が適用済みなら巻き戻さない**（ユーザー単位設定側が版を進めているため）ので、そのときは operator が原因を解消するまでログインできない（**解消後はリセットの再依頼1回で自力で回復し、ジョブの再投入は残渣の片付けを早めるだけである**）。**リセット完了に限り、前回のリセット完了以降に作られた AI クライアント接続が失効する**（通常のパスワード変更では `resetVersion` を進めないので対象が空になる。domains/identity.md）
...
6. **ユーザー単位設定側**で、同じトランザクションの中で `AccountStore.advanceSessionEpoch()` により**セッションの世代を進め**、`CredentialLocatorStore.advanceCredentialVersion(credentialId)` で対象クレデンシャルの世代を進める（**進めた後の値が返り、手順7の昇格はこの戻り値へ揃える** — domains/identity.md。既存セッションは次のリクエストで失効する）
7. 認証情報側で新しい検証材料を正本へ昇格させる（`promoteVerifier`）

**手順5〜7は2つの物理境界をまたぐ。** 中間状態のあいだは旧新どちらのパスワードでもログインできず、前進不能が確定した場合は一様な終端（記録を残して終端する。**後始末が完走すればジョブは `done` に落ち、運用へエスカレーションされるのは後始末が焼き切れるか材料を見失った場合だけである**）に落ちる。**終端の具体的な手順の正本は [recovery/index.md](../recovery/index.md) である**（段 C1）— **手順6 がまだ適用されていなければ手続きは巻き戻り、旧パスワードでログインして変更をやり直せる**（**手順6 のコミット後・記録前の窓に当たっていた場合だけは版照合で拒否されるので、そのときはリセットの依頼で回復する**。recovery/index.md「起点別の扱い」）。手順6 が適用済みなら巻き戻さないので、operator が原因を解消するまでログインできない（**解消後はリセットの依頼1回で自力で回復する**）。**`resetVersion` は進めず、AI クライアント接続も失効しない**（パスワードの変更が連携の取り消し意思を意味しないため。domains/identity.md）
...
2. **ユーザー単位設定側**の `unitOfWorkProvider.run` 内で手続きを開始する:
   ...
   2. `recordOperation` で手続きを記録し（**`kind: "link"`**）、連携先を特定する写像材料を控える。**同じトランザクションで `enqueueJob` により `resume-link` を投入する** — 途中で落ちたときに手続きの存在を知っているのはこの記録だけなので、**これが前進の唯一の投入点である**
...
**手順2〜5は2つの物理境界をまたぐので、単一のトランザクションには収まらない。** 途中で落ちた場合は前進させる仕組み（`resume-link`）が引き取る。**利用者から観測できるのは次の3点だけである** — 連携が完了するまでその SSO ではログインできず、前進不能が確定した場合は一様な終端（記録を残して終端する。**運用へエスカレーションされるのは後始末が焼き切れるか材料を見失った場合だけである**）に落ち、**その終端では手続きが巻き戻される（予約と写像が解放される）ので、同じ SSO 主体で連携をやり直せる**。**終端の具体的な手順の正本は [recovery/index.md](../recovery/index.md) である**（段 L1〜L3）。**巻き戻しそのものが前進不能に陥った場合は operator 経路が受ける** — そのときはその SSO 主体がしばらく連携不能なままになる
...
2. **ユーザー単位設定側**の `unitOfWorkProvider.run` 内で:
   ...
   4. `CredentialLocatorStore.deleteByCredentialId(credentialId)` で逆引きを消す。**消す前に、その `credentialId` の写像材料を全世代分、`recordOperation` が書く手続きの記録（**`kind: "unlink"`**）へ退避する**（消した後は認証情報側の行へ辿り着けなくなる。1世代分だけ控えると回収されない世代が残る）。**`kind` の値は後始末の L1 と退会側への要求がそのまま述語に使うので、値域トークン（`spec/database/index.md`）から外れた値を書かない**
...
**手順2〜3は2つの物理境界をまたぐ。** 順序はこの向きに固定する — 逆順にすると、途中で落ちたときに「ユーザー単位設定側には残っているが引けない」状態になり、次の連携で「既に使われている」と誤判定させる。**この向きなら片方向にしか壊れない** — 残った写像でログインしようとしても、手順2-4 で逆引きが消えているのでログインの到達性検査が拒否する（「解除したのにログインできる」は起きない）。**利用者から観測できるのは「解除後はその SSO でログインできない」ことだけで**、認証情報側に残った行の回収は前進させる仕組み（`sweep-orphan-mapping`）が引き取る。前進不能が確定した場合は一様な終端（記録を残して運用へエスカレーションする。**この `kind` は後始末を持たないので、前進不能の確定がそのまま operator の受け口へ渡る**）に落ちる。**この操作には巻き戻しが無い** — 解除はユーザー単位設定側で既に確定しており、前進（削除の再試行）そのものが回収だからである。**したがって終端後に残るのは認証情報側の写像行であり、その SSO 主体を再連携できないまま operator の受け口（ジョブの再投入）を待つ。** 正本は [recovery/index.md](../recovery/index.md) である
...
1. `UserId.create(input.userId)` で値オブジェクトを構築する
2. `UserSettingsRepository.find()` でクレデンシャル要約と設定を取得する（読み取りのみ。UoW 不要）。不在なら `NotFoundError("USER_NOT_FOUND")`。**未初期化の DO はこの読みに到達しない** — migration ゲートが1行も読まずに `SystemError(NOT_INITIALIZED)` で戻すので、`NotFoundError` の枝に落ちるのは「初期化済みの DO に設定行が無い」場合だけである
3. **認証情報側**からメールアドレスの原本を1件だけ復号して取得する（本人の自己参照であり、一覧のために複数件をまとめて復号する経路は開かない）。**bucket を選ぶ座標は逆引き（`CredentialLocatorStore.findByCredentialId`）から取る** — 手順2 が返すクレデンシャル要約は出力 DTO の4フィールドそのものであって写像材料を持たないからである。**復号は所有する `userId` で束縛し、一致しなければ何も返さない**
...
| 条件 | エラー |
|---|---|
| `userId` が未初期化の User Data DO を指す（そもそも作られていない） | `SystemError(NOT_INITIALIZED)`（migration ゲートが1行も読まずに戻す。認証ミドルウェアはこれを未認証へ畳むので、利用者にはログイン画面が出る） |
| 初期化済みの DO に設定行が無い | `NotFoundError("USER_NOT_FOUND")`（初期化は設定行とアカウント行を同じ `transactionSync` で書くので、この状態を作る経路は無い。不在は状態ではなくドリフトである） |
| メールのクレデンシャルが1件も無い / その逆引きが無い / 原本を復号できない | `SystemError(DataIntegrityError)`（**登録経路はどちらも1件書き、アカウントが生きているあいだ消す経路が無いので、不在は状態ではなくドリフトである**） |
| DB 例外 | `SystemError` |
