...
3. リンク先ドキュメント ID 群（重複除去）で knowledge の `DocumentRepository.listSummariesByIdsIncludingTrashed(documentIds)` を呼び、タイトルと trashed 状態を射影で取得する（「削除済みのドキュメント」表示のため trashed 込み。人間UI専用の読み取り経路）
...
2. `MemoRepository.listRevisionSummaries(memoId)` で全リビジョンの射影（誰が・いつ。本文なし）を revisionNumber 昇順で取得する（メモが存在すれば必ず 1 件以上）
