...
Failure is the third structural element of a streaming route, and it does not behave the way the skeleton suggests. A throw from the deferred fragment surfaces during the **leaf's** render, and the catch boundary is decided by **that route's own** `errorComponent` — `_app` having one does not make it a boundary for the leaf. Without one, a fragment failure therefore replaces the whole protected screen with `_app`'s error surface, nav and all. Any screen carrying an action that must survive its fragment failing (`/settings` carries sign-out, the only escape from a session the Durable Object has revoked) must put an `errorComponent` on that route and keep the action outside the `Suspense`, so the error surface and the action render together. The shared body of the in-shell surface is `apps/web/app/components/ui/ErrorSurface`; `_app` and `/settings` differ only in padding.
...
Operational guidance lives in `README.md`「Development commands」/「Deployment」と `apps/web/.dev.vars.example`。**`docs/runtime_cloudflare.md` は D1 + Queues 時代の運用手順のまま**であり、書き換えは #38 が担当する — 載っているコマンドの多くはもう存在しないので、現行の正本として読まないこと。`pnpm dev` / `pnpm build` / `pnpm start` are aliases of their `:cf` counterparts.
