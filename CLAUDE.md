...
Loading fallbacks come in two kinds, by scope. **Per-fragment RSC** is for content tied 1:1 to a URL (lists, details): call and `await renderServerComponent(...)` inside the server-function handler, then render the resolved value returned through the loader. Do not forward its unresolved promise through loader data. **Route-level pending** (`router.tsx`'s `defaultPendingComponent` + `defaultPendingMs`/`defaultPendingMinMs`) covers loaders that stay unresolved past the threshold. `/settings` is the shipped reference for a serializable DTO loader with a route-specific `pendingComponent`: it fetches only `email` and `authMethods`, renders `CurrentUserPanel` normally so its `LogoutButton` remains a client island, and uses `SettingsSkeleton` while navigation is pending. Skeletons live under `apps/web/app/components/ui/Skeleton` (generic) and `apps/web/app/components/settings/SettingsSkeleton` (shaped to the real DOM so it swaps in without layout shift). Keep loading fallbacks distinct from the optimistic primitives above, which cover post-mount mutations.
