...
- Avoid broad `try / catch` in ordinary application logic. Use it only at explicit boundaries (server-function serialization, the Durable Object's RPC entry points, per-job tolerance in the job runner, per-row tolerance in the Outbox relay, and the queue consumers — the mail consumer and the DLQ handler — which turn a failure into a queue retry rather than letting it escape the `queue()` handler).
...
- **worker → root**: the job runner wraps each job in `try / catch` so that one failing job neither aborts the rest of the queue nor escapes `alarm()`. **The relay pass sits under the same rule** — one row that cannot be published must not stop the other deliveries and must not escape `alarm()` either. On the far side of the queue the consumers hold the mirror-image boundary: a failure is caught and turned into a queue retry (and eventually the DLQ), never into an unhandled rejection out of the `queue()` handler. These are the only places a broad `catch` is expected outside the layer boundaries above.
