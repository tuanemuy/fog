...
async function mintToken(payload: object): Promise<string> {
...
describe("createHmacSessionCodec", () => {
  ...
  // The signature is genuine here, so this walks the one path the tampering
  // tests cannot reach: what the deploy that added the generation does to
  // the sessions minted before it. Refusing rather than defaulting means
  // every one of them lands on `/login`.
  it("rejects a correctly signed token minted before the generation joined the payload", async () => {
    const exp = NOW.getTime() + 60_000;

    await expect(
      codec.verify(await mintToken({ uid: USER_ID, exp }), NOW),
    ).resolves.toBeNull();
    await expect(
      codec.verify(await mintToken({ uid: USER_ID, exp, epoch: EPOCH }), NOW),
    ).resolves.toEqual({ userId: USER_ID, sessionEpoch: EPOCH });
  });
