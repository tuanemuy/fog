...
// Migrations run once per file (beforeAll), then each test gets a clean
// slate via TRUNCATE. D1 has no transaction-rollback escape hatch we can
// wrap a test in, so per-test isolation is achieved by explicit deletion
// of all rows.
beforeAll(async () => {
