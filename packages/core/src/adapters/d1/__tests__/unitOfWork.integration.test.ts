...
// Drives the deferred-batch contract end to end: writes do not materialize
// until `run()` returns, and an OCC failure from any participating write
...
describe("D1UnitOfWorkProvider (integration)", () => {
  ...
  it("reports a stale-version write as an OCC failure", async () => {
    const container = createTestContainer();
    const created = nextUser("occ-rollback");
    await container.unitOfWorkProvider.run(async ({ userRepository }) => {
      await userRepository.insert(created);
    });

    // Capture the v=0 token, advance the row to v=1, then re-use the stale
    // token. The OCC guard must abort the batch.
    ...
    const bumped = User.changeTrashRetentionDays(
      found.entity,
  ...
  it("returns the callback's value on successful commit", async () => {
    const container = createTestContainer();
    const user = nextUser("return-value");
