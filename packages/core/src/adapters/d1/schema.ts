...
// No outbox table here: an event row is written in the Durable Object
// that emitted it, inside the same `transactionSync` as the business
// data, because only the emitting DO's own tables can be written
// atomically with the update that produced the event. There is no
// `processed_events` table either — idempotency keys are declared per
// consumer, and no shared processed-events store exists.
...
// and the OCC handler in `PendingBatch` would translate the driver
// error into a `ConflictError("OPTIMISTIC_LOCK_FAILURE")` — `would`,
// because `pendingBatch.ts` has no caller left; what still exercises
// this table is `__tests__/occGuard.integration.test.ts`, which drives
// `db.batch()` directly. Because the success path never inserts, the
// table stays empty between batches without an explicit DELETE.
