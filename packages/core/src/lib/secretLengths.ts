 ...
 * adapter (the reversed dependency AC-25 forbids). Both readers must keep
 * reading it: restating `32` in either place lets the two checks split, and a
 * DI layer that brands a secret the codec then rejects throws a bare `Error`
 * outside the error middleware — a plain 500 on every request.
 *
