 ...
 * for first-time persistence, the lookup methods (`findById` /
 * `findByEmail`) as the issuers of the `ExpectedVersion<User>` token,
 * `save` consuming it — but deliberately does **not** extend it:
 * `TransactionalRepository` also mandates
