...
- **claim の `SELECT` は実行可能集合を `next_run_at` の昇順で読む。「索引順」で読まない。** 実行可能集合の索引は `(status, next_run_at)` なので索引順は `status` 主・`next_run_at` 従になり、`pending` が供給され続けるかぎり **lease 満了済みの leased 行が常に後ろへ回り、回収されずに滞留する**（DO reset のたびに起きる）。`status` が2値なのでこの順序は宣言済みの索引だけでは1本のシークにならず、**実行可能集合ぶんのソートが要る。そのコストを受け入れる。** 4本の min への分解と同じ種類の判断だが**結論は逆である** — あちらは分解しても値が一致するのでコストだけの問題だったが、こちらは順序が正しさ（飢餓の回避）に効くので避けられない。**`jobs` 側にも同じ形で掛かる。**
...
- **ファクトリ / 遷移が `{ entity, eventDrafts }` を返す形は「イベントを発行するファクトリ / 遷移」に限った契約であり、無条件の宣言にしない。** 現状そのような遷移は1つも存在せず（唯一のイベントは draft ファクトリ経由で出る）、`memo` / `knowledge` の「状態遷移は次状態のエンティティだけを返す」は動かない。無条件にすると、**イベントを1つも持たないドメインのファクトリまで戻り値の形を変える導線になる。** 将来イベントを発行する遷移が現れたときの形として残す。
...
- **呼び出しガードを置く。** 生トークンは行の `payload` が持つ `tokenId` から導かれる。**`event.id` は Queue メッセージと DLQ を通って DO の外へ出るので、この RPC が無条件だと「`event.id` を知る者 = リセットリンクを引ける者」になる。** 応答が `send` になるのは次の3条件がすべて成り立つときだけである。
  ...
  - **行の `owner_token` が `NULL` の呼び出しは、引数の値にかかわらず常に不一致として扱う。** 引数側も欠落・空文字・規定長未満は照合の前に不一致とする。**これは4つ目の条件ではなく条件 3. の照合の前提である** — 列は nullable で、`enqueueEvent` の INSERT 直後と、上限未到達の失敗で `pending` へ戻された後（上の 4.）の2状態が**正常系で頻出**し、`status` を照合しないので条件 1. と 2. はそのまま成立する。`null === null` を真と読む実装だと「`event.id` を知る者が送信材料を引ける」が実装裁量で成立してしまう。`password_reset_tokens.change_auth_token` に同型の明示規則の先例がある。
...
- **一様性は payload の外にも掛かるので、`identity.passwordResetRequested` の `aggregate_id` にはその `windowKey` を入れる。** 列は NOT NULL なので4ケースすべてで値が要るが、**素直な候補である `credentialId` は未登録の canonical に存在しない。** 窓キーは登録の有無・認証方式・宛先の存在を1つも参照せずに決まる唯一の識別子であり、「その窓のリセット依頼」という集約の読みも通る。**ダミー値で埋めない** — `tokenId` のダミーには使い道がある（送信材料 RPC が通常の `nothing-to-send` として扱う）が、`aggregate_id` のダミーには無く、同じ列に「実在の集約を指す値」と「どこも指さない値」が同居する。**この値は DO の外へ出ない**（Queue メッセージの項目に `aggregate_id` は無い。上の 6.）。
- **窓の判定（`claimWindow`）を宛先の解決（`CredentialMappingRepository.findByEmail`）より先に置き、`findByEmail` は `claimWindow` が `true` を返した場合にだけ走らせる。** 逆順だと、列挙オラクルの残差である `findByEmail` のヒット / ミスがスロットルの**外側**に置かれ、同一アドレスに対して回数無制限にサンプルできる。入れ替えが成立するのは **`claimWindow` が `windowKey` しか要らず `findByEmail` の結果に依存しない**からで、逆に `findByEmail` の戻り値が要るのはイベント行を書く側だけである。帰結として残差は**窓あたり1サンプル**に縛られる — 縛られるのはサンプルの回数であって差そのものではなく、窓は時間とともに明けるので**時間側の等価性は主張しない**（射程の正本は `spec/usecases/identity.md`）。
...
- **投入時の `next_run_at` は窓の終端（その窓の開始 + 窓の長さ）から導き、窓行の `expires_at` が持つ猶予は窓行を書くアダプター側の掃除条件に閉じる。** ユースケースが窓について得られるのは `claimWindow` の `boolean` だけなので（下の 9.）、猶予まで投入時刻の材料にすると「**ズレると静かに壊れる**」2層共有の運用値をもう1つ新設することになる。窓の終端に起きた掃除は窓行についてはまだ空振りしうるが、完了時の再武装が `min(expires_at)` を読み直して正しい時刻を張り直すので**単調に収束する**。リセットトークン行の `expires_at` も材料にしない。
...
- **合成に一方向性は要求しない。** したがって `reset_request_windows.window_key` と `outbox_events.aggregate_id`（上の 7.）は canonical の全長 HMAC を逐語で含みうる。受けられる根拠は、(i) 同じ DO の `credential_mappings.hmac` が登録済みクレデンシャルについて同じ値を既に恒久的に持っており **DO 内部の読み手にとって新しい相関材料ではない**こと、(ii) 窓行は `sweep-reset-tokens` が掃除すること、(iii) DO の外へ出る経路（Queue メッセージ）に `aggregate_id` を載せないこと、(iv) `list-quarantined-events` も `aggregate_id` を返さないことである。**受ける残差は、未登録アドレスについて掃除までのあいだ窓をまたいで相関できる仮名が DO 内に残ること**（登録済みは元から残っている）。**(iii) か (iv) のどちらかを緩めるなら、合成を一方向にする（全長 HMAC と窓を連結したうえで一方向ハッシュを1回通す）ところまで戻る** — 緩めた側では窓で切れない仮名が DO の外へ出る。
...
- **隔離行の再駆動が有効なリンクを届けられるのはリセットトークンが TTL 内のときだけである。** `quarantined` が恒久保持である以上、DLQ 側の運用値の制約（上の 6. の2本）に相当する束縛を隔離側には置けない。**この非対称は実体の非対称であり、消すために片方を歪めない** — DLQ 側が断定できるのは保持期間という上限が存在するからである。TTL を過ぎてからの再駆動は送信材料 RPC が `nothing-to-send` を返し、利用者側の復旧は窓が明けてからの再依頼になる。**これは 6. の「2本で全数」の外側の注記であって3本目ではない** — 縛る対象が operator の反応時間であって設定値ではないので、そもそも不等式として書けない。
...
- **`CLAUDE.md` の「retry は job runner の1箇所だけで、プラットフォームへは決して委譲しない」が2つに割れる。** DO の中では job runner と Outbox relay の**2箇所**で、どちらも従来どおり委譲しない。**委譲するのは Queue 境界の向こう側だけ**で、そこでは Queue の retry と DLQ が完了を管理する（上の 1. の規則2 と 12. の分界）。**数の権威は `CLAUDE.md` に残す** — 14. が落とすと決めたのは識別子の全数列挙であって、規約そのものの断言ではない。
