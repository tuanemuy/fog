...
- 参照: CLAUDE.md「Frontend」節 / `.issue/1/plan.md`（AC-9〜AC-15 / AC-18）/ `.issue/1/adr.md`（ADR-035〜043）/ `.issue/1/review/review-002-{frontend,security}.md` / `triage.md` / `spec/design/{index,tokens}.md` / `spec/design/pages/*.html` / `spec/pages/index.md`
...
- **[W-002]** `viewport-fit=cover` を採ったのに、画面下端に接するもう1箇所（シート本文の下端）だけ safe-area を含まない
  - 場所: `apps/web/app/components/layout/AppShell/index.tsx:166-172`（`<main>` の内側 `pb-2xl`）、`apps/web/app/styles/tokens.css:162-181`、`.issue/1/adr.md` ADR-041
