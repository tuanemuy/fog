...
参照: `.issue/1/plan.md`, `.issue/1/adr.md`（ADR-002 / 003 / 005 / 009 / 011 / 015）, `spec/usecases/identity.md`, `spec/domains/identity.md`, `spec/scenario/account.md`
...
- **[W-001]** ログイン失敗の**応答時間**でアカウントの存在が漏れる（ユーザー列挙オラクル）
  ...
  - 提案: 固定のダミーハッシュ（起動時に1回 `hash()` して保持、あるいは定数として埋め込む）に対する verify を「ユーザー不在 / SSO」の分岐でも実行して経路長を揃える。`.issue/1/progress.md` の項目1に記録済みなので、フォローアップ Issue として起票し ADR に「本 Issue では見送る」判断を昇格させる。
...
- **[W-008]** `SESSION_SECRET` の検証がリクエストごと（起動時ではない）
  ...
  - 理由: 4ランタイムとも `createXxxRequestContainer(config)` を**fetch ハンドラの中**で毎回呼ぶ。`requireSessionSecret` はその中にあるので、鍵が欠落・短すぎでも**プロセスは正常に起動し**、以後すべてのリクエストが 500 になる。`.issue/1/testing.md` の見出しは「起動時エラー」だが本文は「リクエスト処理時」で、本文が正しい。フェイルクローズなので安全側だが、デプロイのヘルスチェック次第では設定ミスがロールアウトを通過する。
