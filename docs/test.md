...
- **A test that drives a Durable Object's jobs must be the only thing driving them.** Every gated RPC entry ends in `armAfterRpc`, and a job that is already due is clamped to `setAlarm(now + 1000)` — so one second later workerd delivers a real `alarm()` into the middle of the test, running the queue with the *Durable Object's own* dependencies (the noop mail sender, since `MAIL_SENDER` is unbound here), settling the rows and deleting the alarm when nothing is left. Any assertion that counts job rows, recipients or `deleteAlarm` calls after that point is racing the wall clock: green while it lands inside the first second, red when the suite is slow. There is no way to suppress a delivery that is armed, so the fix is to leave none armed — call `disarm(stub)` (`adapters/cloudflare/__tests__/doHarness.ts`) immediately after the RPC. This is **not** an order or naming problem and neither uniquing nor the `afterEach` below can reach it; it is the one flakiness source in this suite that depends on elapsed time.
