...
- **Prefer a fresh Durable Object name per test.** Every suite but two derives one from a module-scope counter, and that — not the `afterEach` — is what actually makes them order-independent. Use a fixed name only when the test is *about* reuse; `packages/core/src/adapters/cloudflare/__tests__/cleanup.integration.test.ts` is the one that deliberately does, and it is what would go red if the cleanup ever stopped working.
