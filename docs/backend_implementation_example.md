...
Representative classes, one per layer role — **not the roster**. The complete list of error classes lives in the ban list of `lint/no-instanceof-error.grit`, which `lint/banList.test.ts` keeps in sync with the classes that actually exist (12 today), so there is one place to look and no second ledger to drift.
...
`serializedKind` is what the guards match on, and it cannot drift from what `toSerialized()` emits: the base declares it as `abstract readonly serializedKind: ReturnType<this["toSerialized"]>["kind"]`, so the type is derived from the subclass's own override and a disagreeing value fails to compile (TS2416). Writing `readonly serializedKind: SerializedConflictError["kind"] = "conflict"` on the subclass is optional — it documents the intent, the base is what binds it. Identity is tested structurally, never with `instanceof` (which is false across the SSR / RSC module-graph split and is rejected by `lint/no-instanceof-error.grit`): `isCodedError(error)` for the brand plus the shape the contract needs, `hasSerializedKind(error, kind)` for brand + discriminator, which is the one line every per-kind guard is. Pass that `kind` as `Serialized*Error["kind"]` rather than a bare literal — a typo would otherwise compile into a guard that is always `false`; the application layer's `kindGuard` factory enforces it by construction. Per-kind guards return the structural `CodedError & { serializedKind; toSerialized() }`, never a concrete class. `.adr/016-symbol-for-error-brands.md` records the reasoning, including why a `serializedKind` match narrows to the contract rather than to a class.
